*:***************************************************************************
*: Program file  : ARSJORER
*: Program desc. : Sales Journal Report For ERI02
*: Module        : Accounts receivable (AR)
*: Developer     : Ahmed Khalifa Mohamed (AKM)
*: Tracking Job Number: 200756
*: Date : 03/01/2007
*:***************************************************************************
*: Calls :
*:    Programs   : ....
...
*E037378,1                Aria27 and records belong to Aria4. 
*C122049,1 ASH 06/01/2004 Print the of shipped orders and change the reports heading to be 'Shipment Journal.
...
*C200756,1 AKM 03/01/2007 CUSTOM SALES/shipment JORNAL REPORT FOR ERI02  
*:**************************************************************************************************
*----------------------- Report Code Begin -----------------------------
...
*C122049 ASH 06/01/04 (Begin) Compute the number of shipped orders.
lcTmpCount = loOgscroll.gfTempname()
SELECT DISTINCT &lcWorkFile..Order FROM (lcWorkFile) WHERE &lcWorkFile..Status <> 'V' INTO  Cursor (lcTmpCount)
lnSumOrders = RECCOUNT(lcTmpCount)
USE IN (lcTmpCount)
*C122049 ASH 06/01/04 (End)
...
*!*************************************************************
*! Name      : lfCity
*! Developer : Ahmed Khailfa (AKM)
*! Date      : 02/28/2007
*! Purpose   : 1- Get cities from customer table.
*!           
*!*************************************************************
*! Passed Parameters : None
*!*************************************************************
...
lcWorkArea = SELECT()
lcCityFile = loOgscroll.gfTempname()

DIMENSION laCursStruct[1,4]
laCursStruct[1,1] = 'City'
laCursStruct[1,2] = 'C'
laCursStruct[1,3] = 30
laCursStruct[1,4] = ''

 =gfcrttmp(lcCityFile,@laCursStruct,'UPPER(CITY)',lcCityFile,.T.)

SELECT CUSTOMER
LOCATE 
SCAN
	IF gfSEEK(UPPER(CADDRESS3), (lcCityFile)) = .F. AND !EMPTY(ALLTRIM(CADDRESS3))
		SELECT (lcCityFile)
		APPEND BLANK 
		replace city WITH CUSTOMER.CADDRESS3
	ENDIF 
ENDSCAN
...
IF TYPE('lnVarbEnd') = 'C'
*!* C200756,1 AKM 03/01/2007 CUSTOM SALES/shipment JORNAL REPORT FOR ERI02 [START]  
...
*!*	   =gfOpenTable('SYCCOMP','CCOMP_ID','SH')
   ...
   =lfCity()
*!* C200756,1 AKM 03/01/2007 CUSTOM SALES/shipment JORNAL REPORT FOR ERI02  [END]
  PRIVATE lcThAlias
  lcThAlias = ALIAS()
  ...
  *lcSelectCommand2=[SELECT * FROM SYREPUVR WHERE Crep_id="ARSJOUR" AND cExpType="V" AND Nvarpos!=0] 
  ...
  lcSelectCommand2=[SELECT * FROM SYREPUVR WHERE Crep_id="ARSJORER" AND cExpType="V" AND nVarPos!=0 AND (EMPTY(CVER) OR CVER = "A40")] 
      ...
      IF lnoccurs > 0 
        ...
        *IF  (lnSeaOcurr > 0 OR lnDivOcurr > 0 OR lnTrmOcurr > 0 OR lnLocOcurr > 0)
        IF (lnSeaOcurr > 0 OR lnDivOcurr > 0 OR lnTrmOcurr > 0 OR lnLocOcurr > 0 OR;
            lnAccOcurr > 0 OR lnCurOcurr > 0 OR lnAdrOcurr > 0 OR lnRepOcurr > 0 OR lnCTYOcurr>0)
        *WSH [End]
...
IF !EMPTY(lcStatCursor) 
	IF !SEEK(ALLTRIM(Customer.CADDRESS4), lcStatCursor) 
		LOOP
	ELSE
		IF  (!EMPTY(lcRpCity ) AND !(ALLTRIM(lcRpCity) $ Customer.CADDRESS3 ))
			LOOP
		ENDIF 		
	ENDIF
ELSE
...
*! Developer : Ahmed Khalifa Mohamed(AKM)
*! Date      : 03/01/2007
*! Purpose   : Validate function for the City field.
*!*************************************************************
*! Calls     : 
*!             Procedures : ....
*!             Functions  : ....
...
*!*************************************************************
FUNCTION lfvCitys
PRIVATE lcVar , laTemp
IF !EMPTY(lcRpCity) AND (('?' $ lcRpCity) OR !SEEK(lcRpCity, UPPER(lcCityFile)))

  SELECT (lcCityFile)
  LOCATE
  IF EMPTY(ALLTRIM(lcRpCity)) OR ('?' $ lcRpCity) 
  	LOCATE
  ELSE
	  =GFSEEK(UPPER(lcRpCity))
  ENDIF   	
  
  DIMENSION laTemp[1]
  laTemp = ''      && Array to hold the Selected value

  lcBrFields = "City :R :H= 'Cities' "
  
  lcFile_Ttl = "Cities..."
  =gfBrows('','City','laTemp',lcFile_Ttl )
