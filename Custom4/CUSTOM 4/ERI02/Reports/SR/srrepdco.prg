...
*: Program desc. : Sales Representative commission statement Report For ERI02
*: System        : Aria4XP.
...
*:***************************************************************************
*: Modifications:
*: B611047,1 MMT 08/25/2015 Issue#52: Custom Sales rep. comm. report gives error while preview[P20150112.0001]
*: B611102,1 MMT 01/18/2016 Custom Commission Statement does not filter by Period correctly[T20160104.0179]
*: B611116,1 MMT 02/17/2016 Custom commission statement report for ERI02 payment date filter is not working correctly[T20160104.0179]
*: B611121,1 MMT 03/10/2016 Custom commission statement for ERI02 prints incorrect data[T20160104.0179]
*:***************************************************************************
*!*	  _screen.Visible = .T.
*!*	  ACTIVATE WINDOW trace
*!*	  SUSPEND
*: B611047,1 MMT 08/25/2015 Issue#52: Custom Sales rep. comm. report gives error while preview[P20150112.0001][Start]
llDonprnt=.F.
*: B611047,1 MMT 08/25/2015 Issue#52: Custom Sales rep. comm. report gives error while preview[P20150112.0001][End]
...
ELSE
  ...
  ENDIF
ENDIF
*: B611116,1 MMT 02/17/2016 Custom commission statement report for ERI02 payment date filter is not working correctly[T20160104.0179][Start]
*: B611121,1 MMT 03/10/2016 Custom commission statement for ERI02 prints incorrect data[T20160104.0179][Start]
*!*	IF !EMPTY(XLPAYDATE)
*!*	  lcexpR=lcexpR+" AND BETWEEN(PAYDATE,CTOD('"+XLPAYDATE+"'),CTOD('"+XHPAYDATE+"'))"
*!*	ELSE   
*!*	  IF  !EMPTY(XHPAYDATE)
*!*	    lcexpR=lcexpR+" AND PAYDATE<=CTOD('"+XHPAYDATE+"')"
*!*	  ENDIF
*!*	ENDIF
*: B611121,1 MMT 03/10/2016 Custom commission statement for ERI02 prints incorrect data[T20160104.0179][END]
*: B611116,1 MMT 02/17/2016 Custom commission statement report for ERI02 payment date filter is not working correctly[T20160104.0179][End]
...
ELSE   
  ...
  ENDIF
ENDIF
*: B611102,1 MMT 01/18/2016 Custom Commission Statement does not filter by Period correctly[T20160104.0179][Start]
*: B611121,1 MMT 03/10/2016 Custom commission statement for ERI02 prints incorrect data[T20160104.0179][Start]
*!*	IF !EMPTY(LDATE)
*!*	  lcexpM = lcexpM + " AND BETWEEN(DATE,CTOD('"+LDATE +"'),CTOD('"+HDATE+"'))"
*!*	ELSE
*!*	  IF  !EMPTY(HDATE)
*!*	    lcexpM = lcexpM + " AND DATE<=CTOD('"+HDATE+"')"
*!*	  ENDIF
*!*	ENDIF
*: B611121,1 MMT 03/10/2016 Custom commission statement for ERI02 prints incorrect data[T20160104.0179][End]
*: B611102,1 MMT 01/18/2016 Custom Commission Statement does not filter by Period correctly[T20160104.0179][End]
...
SCAN FOR pay_type="R"  
  ...
  WAIT WINDOW 'Collecting statements for sales representative ' + XSALESREP NOWAIT
    ...
    IF llMultCurr AND lcRpCurr = "F"
      ...
      lnBgbal=IIF(gfSEEK(repcode+lcCurrtCur ,lcbgbal),&lcbgbal..amnts,0)
      RUNBAL=lnBgbal
    ELSE
      lnBgbal=IIF(gfSEEK(repcode,lcbgbal),&lcbgbal..amnts,0)
      RUNBAL=lnBgbal
    ENDIF
...
=gfOpenTable('REPCOMM','REPCOMM','SH')
...
loOgScroll.lcOGLastForm ='SRREPDCO'
...
DIMENSION laTempStru[28,18] ,laTempREP[1,18],laTempCOM[1,18]
...
IF !EMPTY(ACCOUNT) .AND. gfSEEK('M'+REPCOMM.ACCOUNT,'CUSTOMER')
