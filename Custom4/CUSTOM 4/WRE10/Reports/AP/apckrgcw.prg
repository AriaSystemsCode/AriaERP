*:***************************************************************************
*: Program file  : APCKRGCW.prg
*: Program desc. : Custom AP Vendor Union report For Carole Wren
*: System        : Aria Advantage Series.(Aria4XP)
*: Module        : Accounts Payable (AP)
*: Developer     : Mariam Mazhar(MMT)
*! Date          : 03/16/2011
*! Entry No.     : c201315.122,c201319.EXE- [T20101213.0029]
*!***************************************************************************
*: Modifications:
*********************************************************************************************
#INCLUDE R:\Aria4xp\reports\ap\APCKRGCW.H
...
IF lnPosPyDate> 0 
  ...
  ENDIF  
ENDIF
lcOrderApp  = "CPAYTYPE+CVENDCLSS+CVENDTYPE+CPAYCLNO+DTOS(DPAYDATE)"
lcRPForm= 'APCKCWVS'  
  IF llOGFltCh
  IF !USED('APPAYMNT')
  ...
  DIMENSION laAppStru[lnAppCnt +11,18]
  laAppStru[lnAppCnt +1,1]= 'capsessno'
	...
	laAppStru[lnAppCnt +9,1]= 'CVENDTYPE'
	laAppStru[lnAppCnt +9,2]= 'C'
	laAppStru[lnAppCnt +9,3]= 3
	laAppStru[lnAppCnt +9,4]= 0
	
	laAppStru[lnAppCnt +10,1]= 'CVENDCLSS'
	laAppStru[lnAppCnt +10,2]= 'C'
	laAppStru[lnAppCnt +10,3]= 1
	laAppStru[lnAppCnt +10,4]= 0
	
	laAppStru[lnAppCnt +11,1]= 'cvencomp'
	laAppStru[lnAppCnt +11,2]= 'C'
	laAppStru[lnAppCnt +11,3]= 30
	laAppStru[lnAppCnt +11,4]= 0
  ...
  IF !USED('APVENDOR')
	  =gfOPenTable('APVENDOR','CVENDCODE')
  ENDIF
  ...
  WAIT WINDOW LANG_apckreg_COLLECT NOWAIT 
	SCAN REST WHILE CPAYTYPE+CPAYCLNO = "P" FOR IIF(!EMPTY(lcRpBnkCod),cbnkcode = lcRpBnkCod ,.T.) AND;
		...
		=gfSeek(PADR(appaymnt.cpayclno,8) ,'APVENDOR','VENCODE')
		IF !INLIST(APVENDOR.CVENDCLSS,'N','U') OR !INLIST(APVENDOR.CVENDTYPE,'CLS','CLC')
		  LOOP
		ENDIF
		SELECT APDIST
	    ...
	    IF IIF(APPAYMNT.cPayStat = 'V' , IIF(APINVHDR.nInvAmnt > 0 .OR. APPAYMNT.lPayAdvan , APDIST.nApdAmnt , -APDIST.nApdAmnt) < 0 , .T.)
	      ...
	      SCATTER MEMO MEMVAR 
	      *MT
	      m.CVENDTYPE  = APVENDOR.CVENDTYPE
	      m.CVENDCLSS = APVENDOR.CVENDCLSS
