*!--------------------------------------------------------------------------
*! Program file        : SORDLBU.PRG
*! Program description : CUSTOM ORDER DETAIL REPORT FOR LBU.
*! For System          : Aria Advantage Series - Version oldAriaSql
*! For Module          : SO
*! Developer Name      : BASSEM RAAFAT ERNEST(BWA)
*! Tracking Job Number : CP131746
*! Date                : 04/09/2006
*!--------------------------------------------------------------------------
*! Calls               : SORDLBUA.FRX , SORDLBUB.FRX , SORDLBUG.FRX
*!--------------------------------------------------------------------------
...
*!--------------------------------------------------------------------------
*! Modificatoins       :
*! T20060808.0287, C132460,1 AYM 01/09/2007 :Add Style Pic to each order line 
*!--------------------------------------------------------------------------
...
*--Open the Invline file to get from it the shipped values.
IF !USED('INVLINE.DBF')
  llInvline = gfOpenFile(gcDataDir+'INVLINE',gcDataDir+'INVLINEO','SH')
ENDIF
...
IF llFrTime
  ...
  *-- Create temporary file that holding order line data. [begin]
  ...
  =lfAddField("laTempStru", "cTempKey" ,"C",16,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cCurrCode" ,"C",3,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "Dist_ctr" ,"C",8,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "CType" ,"C",1,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cpBokQty" ,"N",7,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cpBokAmt" ,"N",14,2,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cpShpQty" ,"N",7,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cpShpAmt" ,"N",14,2,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cpOpnQty" ,"N",7,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "cpOpnAmt" ,"N",14,2,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "LastInv" ,"C",6,0,.T.,.F.,"","","","","","","","","","",0,0)
  =lfAddField("laTempStru", "InvDatLb" ,"D",8,0,.T.,.F.,"","","","","","","","","","",0,0)
...
IF llClearFn OR llOGFltCh
  ...
  *--Merge Custom SO GMA to Standard SO [START]
  STORE '' TO lcPackExp
...
*! T20060808.0287, C132460,1 AYM 01/09/2007 :Add Style Pic to each order line ..Begin
=gfOpenFile(oAriaApplication.DataDir+'OBJECTS',oAriaApplication.DataDir+'objectid ','SH')
=gfOpenFile(oAriaApplication.DataDir+'OBJLINK',oAriaApplication.DataDir+'objlnkty','EX')
lcTempObj=  loOgscroll.gfTempName()

SELECT * FROM OBJLINK INTO dbf (lcTempObj)
SELECT (lcTempObj)
INDEX ON cobjlnktyp +LEFT(cobjlink,6) +cobj_desc TAG (lcTempObj)
SELECT (lcMastFile)
SET RELATION TO 'O'+ORDER+STYLE INTO (lcTempObj) ADDITIVE
SELECT (lcTempObj)
...
*!*	SELECT (lcMastFile)
*!*	SET RELATION TO 'O'+SUBSTR(Style,1,lnMajorLen) INTO OBJLINK ADDITIVE
*!*	SELECT OBJLINK
*!*	SET RELATION TO cobject_id INTO OBJECTS ADDITIVE
*! T20060808.0287, C132460,1 AYM 01/09/2007 :Add Style Pic to each order line ..End

*-- Select Master report file.
SELECT (lcMastFile)

*--Add the customized filters in the filter expression.
...
*--Clear SELECT BY if user change only SELECT BY option from 
*one type to another or user presses RESET button [Begin]
...
*--Add Sort by DC/Style to Account.
...
*--Check the pre-saved filter setting.
lcRpStatus = ' '
...
*--New in version 4 instead of old lfOGShowGet
      ...
      SCAN REST WHILE cOrdType+Order+STR(LineNo,6) = cOrdType+&lcTOrdFile..Order
        ...
        *-- insert this data into workfile.
        IF IIF(llRpCorrGp ,!EMPTY(Group),.T.) AND ;
          ...
          *--Bas
          m.cpBokQty = ORDLINE.TOTBOOK
          m.cpBokAmt = ORDLINE.TOTBOOK * ORDLINE.GROS_PRICE
          m.cpOpnQty = ORDLINE.TOTQTY
          m.cpOpnAmt = ORDLINE.TOTQTY * ORDLINE.GROS_PRICE

		  DIMENSION lcValues[4]
		  lcValues = SPACE(0)
		  =lfShipQty(@lcValues)
          m.cpShpQty = lcValues[1]
          m.cpShpAmt = lcValues[2]
          m.LastInv  = lcValues[3]
  		  m.InvDatLb = lcValues[4]
  		  lcValues = SPACE(0)
        ...
        *-- scan ordline file for full index expression (rushmore)
        SCAN FOR CORDTYPE + ORDER + STR(LINENO,6) = lcRpOrdTyp
          IF EVALUATE(lcStatusEx) AND ;
            ...
            *--if the order is completed get the Book Qty insted of the Tot Qty.
            IF ORDHDR.STATUS = "C"
            *-- Get qty from book qty becauce qty =0
              FOR I = 1 To 8
                lcCount = ALLTRIM(STR(I))
...
IF !llRpPrTPak
  lcRpForm = "SORDLBUG"
ELSE
  lcRpForm = IIF(llRpSizes,'SORDLBUA','SORDLBUB')
  IF lcRpForm = 'SORDLBUB'
    lcRpForm = IIF(lcRpKind = 'D' AND lcRpSortBy = "O" AND lcRpSrt2 = 'S', 'SORDLBUV' , 'SORDLBUB')
  ENDIF
ENDIF
= lfRepPltFr(lcRpForm)
llRpScale = llRpSizes
...
*!*************************************************************
*! Called from : SORDLBUA.FRX, OR SORDLBUB.FRX [Variable lcDum in the report]
*!*************************************************************
*! Calls       : 
*!              Procedures : ....
*!              Functions  : ....
...
*!*************************************************************
*! Called from : [SORDLBUA.FRX OR SORDLBUB.FRX, ORDER GROUP HEADER BAND] 
*!*************************************************************
*! Calls       : ....
*!*************************************************************
*! Passed Parameters : None
...
*! Developer : BASSEM RAAFAT ERNEST(BWA)
*! Date      : 06/04/2006
*! Purpose   : Function to get shipped quantity from Invline.
*!*************************************************************
*! Calls     : 
*!*************************************************************
*! Passed Parameters  : NONE
...
IF SEEK(ORDER + STR(lineno,6) , 'INVLINE')
  ...
  STORE SPACE(0) TO lcLstInv , lcInvDat

  SELECT INVLINE
  SCAN REST WHILE ORDER + STR(lineno,6) + INVOICE = M.ORDER + STR(M.lineno,6)
    lnTotQty = lnTotQty + (INVLINE.TOTQTY)
    lnTotVal = lnTotVal + (INVLINE.TOTQTY * INVLINE.PRICE)
    lcLstInv = INVLINE.INVOICE
    lcInvDat = INVLINE.INVDATE
  ENDSCAN
  SELECT (lcInvDB)
