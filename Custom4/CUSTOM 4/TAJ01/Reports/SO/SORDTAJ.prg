*!*****************************************************************************************
*! Name      : SORDTAJ.Prg  (SO Available to Allocate Report)
*! Developer : MMT - Mariam Mazhar
*! Date      : 11/23/2022 08:16:22 PM
*! Entry no. : C202449 (T20220518.0001)
*!*****************************************************************************************
*!Modifications :
*******************************************************************************************
...
SELECT (lcMastFile)
SELECT 0 
SELECT Account As 'Account_Code' , btname As 'Account_Name', CustPO as 'Cust_PO',;
ORDER as 'SO_No', Start as 'SO_Start_Date', Complete as 'SO_Cancel_Date',;
STYLE as 'STYLE_COLOR',SPACE(5) AS 'SIZE',0000000 as  'ORIG_SO_QTY_Booked_Qty',;
0000000 as 'Order_Open_Qty', 0000000 as 'Stock_UnAllocated_QTY' from (lcWorkFile) where .F. into CURSOR 'TmpExcel' readwrite
...
lcOutputExcel = oAriaApplication.WorkDir+gfTempName()+'.CSV'
lcHeader = '"Account Code","Account Name","Cust PO","SO#","SO Start Date","SO Cancel Date","Style Color","Size","Orig. SO Qty(Booked Qty)",'+;
'"Order Open Qty","Stock Unallocated Qty"'+CHR(13)+CHR(10)
llBytesWrote = STRTOFILE(lcHeader ,lcOutputExcel ,1)    
SCAN
  FOR lnZ =1 TO 8
    lcZ = STR(lnZ,1)
    IF !EMPTY(&lcMastFile..sz&lcZ.) AND &lcMastFile..Qty&lcZ. > 0
      WAIT WINDOW 'Exporting Data of Style: '+&lcMastFile..STYLE  NOWAIT 
      lcDetLine = ["]+ &lcMastFile..Account+[","]+ &lcMastFile..btname+[","]+&lcMastFile..CustPO +[","=TEXT(""]+ &lcMastFile..ORDER +["",""000000"")","]+;
          DTOC(&lcMastFile..Start )+[","]+DTOC( &lcMastFile..Complete )+;
           [","]+&lcMastFile..STYLE +[","]+ &lcMastFile..sz&lcZ.+[","]+STR(&lcMastFile..Book&lcZ.,7)+[","]+STR( &lcMastFile..Qty&lcZ.,7)+[","]+;
          STR(&lcMastFile..Pik&lcZ.,7)+["]+CHR(13)+CHR(10)
      llBytesWrote = STRTOFILE(lcDetLine ,lcOutputExcel ,1)    
    ENDIF  
  ENDFOR
  
*!*	  m.Account_Code = &lcMastFile..Account
*!*	  m.Account_Name = &lcMastFile..btname
*!*	  m.Cust_PO =   &lcMastFile..CustPO 
*!*	  m.SO_No =  &lcMastFile..ORDER 
*!*	  m.SO_Start_Date =  &lcMastFile..Start 
*!*	  m.SO_Cancel_Date = &lcMastFile..Complete 
*!*	  m.STYLE_COLOR =  &lcMastFile..STYLE 
*!*	  FOR lnZ =1 TO 8
*!*	    lcZ = STR(lnZ,1)
*!*	    IF !EMPTY(&lcMastFile..sz&lcZ.) AND &lcMastFile..Qty&lcZ. > 0
*!*	      m.size = &lcMastFile..sz&lcZ.
*!*	      M.ORIG_SO_QTY_Booked_Qty = &lcMastFile..Book&lcZ.
*!*	      m.Order_Open_Qty = &lcMastFile..Qty&lcZ.
*!*	      m.Stock_UnAllocated_QTY = &lcMastFile..Pik&lcZ.
*!*	      INSERT INTO 'TmpExcel' FROM  MEMVAR 
*!*	    ENDIF
*!*	  ENDFOR 
ENDSCAN
WAIT CLEAR
...
*!*	LOCATE
*!*	SCAN
*!*	   lcDetLine = '"'+Account_Code+'","'+Account_Name +'","'+Cust_PO+'","'+SO_No +'","'+DTOC(SO_Start_Date)+'","'+DTOC(SO_Cancel_Date)+;
*!*	               '","'+STYLE_COLOR +'","'+size +'","'+STR(ORIG_SO_QTY_Booked_Qty ,7)+'","'+STR(Order_Open_Qty ,7)+'","'+;
*!*	              STR(Stock_UnAllocated_QTY ,7)+'"'+CHR(13)+CHR(10)
*!*	  llBytesWrote = STRTOFILE(lcDetLine ,lcOutputExcel ,1)              
*!*	ENDSCAN
OEXCEL = NEWOBJECT("Table2Excel",ADDBS(oAriaApplication.Ref4.ClassDir)+"OptionGrid.VCX","",lcOutputExcel )
IF VARTYPE(OEXCEL) != "O" OR ISNULL(OEXCEL)
  WAIT CLEAR
  =GFMODALGEN("INM00406B00000","DIALOGE")

  *B609671,1 WAM 09/19/2011 Can export from any file other than the currently selected alias
  SELECT (LNALIAS)
...
OEXCEL = .NULL.
lcOutputExcel = FORCEEXT(lcOutputExcel,'XLSX')
*COPY TO (lcOutputExcel) TYPE XLS
IF file(lcOutputExcel)
  loRun = CreateObject("WScript.Shell")
  loRun.Run(lcOutputExcel, 3)
  loRun = NULL
ENDIF
RETURN 
...
ENDIF
llAvailable  = gfGetMemVar("M_TOTAVLBL",oAriaApplication.ActiveCompanyId)
...
*!*	          ENDIF
          ...
          *IF lcRpBkOp = 'A'
            =gfSeek(m.Style+Ordhdr.cWareCode,'STYDYE_A','STYDYE')
            FOR I = 1 To 8
              lcCount = ALLTRIM(STR(I))
              m.PIK&lcCount =  MAX(IIF(llAvailable ,STYDYE_A.WIP&lcCount ,0)+STYDYE_A.STK&lcCount- STYDYE_A.ALO&lcCount ,0)
            ENDFOR
...
llMultLoc = ALLTRIM(UPPER(gfGetMemVar('M_WAREHOUS')))  = 'Y'
IF llMultLoc
