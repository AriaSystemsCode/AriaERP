...
*: T20060804.0034,1 MMT Convert Custom Report RED07 09/20/2006
*:**************************************************************************
*----------------------- Report Code Begin -----------------------------
  ...
  IF LEN(loOGScroll.lcRpExp) > 1
    ...
    IF lnOccur > 0
      FOR lnCount = 1 TO lnOccur + 1
        ...
        *T20060804.0034,1 MMT Convert Custom Report RED07 [Start]
        lnClrOcurr=ATC('SUBSTR(STYLE.STYLE,lnNonMajSt,lnColorLen)',lcTake)
        *T20060804.0034,1 MMT Convert Custom Report RED07 [End]
...
*!*	          IF  (lnSeaOcurr > 0 OR lnDivOcurr > 0 OR lnGrpOcurr > 0 OR lnLocOcurr > 0 OR lnCurOcurr > 0)
          ...
          *IF  (lnSeaOcurr > 0 OR lnDivOcurr > 0 OR lnGrpOcurr > 0 OR lnLocOcurr > 0 OR lnCurOcurr > 0 OR lnStatOcurr>0 )
          IF  (lnSeaOcurr > 0 OR lnDivOcurr > 0 OR lnGrpOcurr > 0 OR lnLocOcurr > 0 OR lnCurOcurr > 0 OR lnStatOcurr>0 OR lnClrOcurr > 0)
          *T20060804.0034,1 MMT Convert Custom Report RED07 [End]
 ...
 *E999999,3,4 AYM Make State mover     ....[End]
  *T20060804.0034,1 MMT Convert Custom Report RED07 [Start]
  lcClrCursor =""
  lnInvClr = ASCAN(loOgScroll.laOgFxFlt,"SUBSTR(STYLE.STYLE,lnNonMajSt,lnColorLen)")
  lnInvClr = ASUBSCRIPT(loOGScroll.laOgFxFlt,lnInvClr,1)
  lcClrs = loOgScroll.laOgFxFlt[lnInvClr,6]
  IF !EMPTY(lcClrs)
    IF lnInvClr > 0 
      lcClrCursor = loOgScroll.gfTempName() &&Cursor Hold Selected Seasons
      DIMENSION laTempacstru[1,4]
      laTempacstru[1,1]='CColor'
      laTempacstru[1,2]='C'
      laTempacstru[1,3]= 6
      laTempacstru[1,4]= 0
      gfCrtTmp(lcClrCursor ,@laTempacstru,"CColor",lcClrCursor ,.T.)
      IF !EMPTY(lcClrs)
        lnStart=1
        lnEnd=AT('|',lcClrs)
        DO WHILE lnEnd <> 0
          SELECT(lcClrCursor) 
          APPEND BLANK 
          REPLACE CColor WITH SUBSTR(lcClrs,lnStart,lnEnd-1)
          lcClrs = STUFF(lcClrs ,lnStart,lnEnd,"") 
          lnEnd=AT('|',lcClrs)
        ENDDO 
        IF lnEnd = 0
          SELECT(lcClrCursor) 
          APPEND BLANK 
          REPLACE CColor  WITH lcClrs
        ENDIF 
      ENDIF 
    ENDIF 
  ENDIF 
  IF !EMPTY(lcClrCursor)
    SET ORDER TO TAG (lcClrCursor)
  ENDIF 
  *T20060804.0034,1 MMT Convert Custom Report RED07 [End]
...
*!*	  SCAN FOR &lcRpExp
  ...
  *-- Scan to fill Temp. File with filter data.
   ...
   *E999999,3,5 AYM Make State mover     ....[Begin]
	...
	*T20060804.0034,1 MMT Convert Custom Report RED07 [Start]
    IF !EMPTY(lcClrCursor)
      IF !SEEK(SUBSTR(STYLE.STYLE,lnNonMajSt,lnColorLen),lcClrCursor)
        LOOP 
      ENDIF 
    ENDIF                 
	*T20060804.0034,1 MMT Convert Custom Report RED07 [End]
...
*!*	  ENDFOR
  DIMENSION laTempStru[lnTempStru+5, 18] 
  laTempStru[lnTempStru + 01  ,1] = 'RepCode'
  laTempStru[lnTempStru + 01  ,2] = 'C'
  laTempStru[lnTempStru + 01  ,3] = 3
  laTempStru[lnTempStru + 01  ,4] = 0
  
  laTempStru[lnTempStru + 02  ,1] = 'RepName'
  laTempStru[lnTempStru + 02  ,2] = 'C'
  laTempStru[lnTempStru + 02  ,3] = 24
  laTempStru[lnTempStru + 02  ,4] = 0

  laTempStru[lnTempStru + 03 ,1] = 'cColorDesc'
  laTempStru[lnTempStru + 03 ,2] = 'C'
  laTempStru[lnTempStru + 03 ,3] = 30
  laTempStru[lnTempStru + 03 ,4] = 0

  laTempStru[lnTempStru+4,1] = 'cTempKey'
  laTempStru[lnTempStru+4,2] = 'C'
  laTempStru[lnTempStru+4,3] = 55
  laTempStru[lnTempStru+4,4] = 0

  laTempStru[lnTempStru+5,1] = 'cState'
  laTempStru[lnTempStru+5,2] = 'C'
  laTempStru[lnTempStru+5,3] = 30
  laTempStru[lnTempStru+5,4] = 0
  
  FOR  lnInc=7 TO 18 
    STORE SPACE(1) TO laTempStru[lnTempStru+1,lnInc], laTempStru[lnTempStru+2,lnInc], laTempStru[lnTempStru+3,lnInc],;
     laTempStru[lnTempStru+4,lnInc], laTempStru[lnTempStru+5,lnInc]
  ENDFOR 
  *T20060804.0034,1 MMT Convert Custom Report RED07 [End]
...
*: T20060804.0034,1 MMT Convert Custom Report RED07 09/20/2006 [Start]
*lcRpForm = IIF(llRpShowSz,'ARSHLOGS','ARSHLOGN')
lcRpForm = IIF(llRpShowSz,'ARSHLORS','ARSHLORN')
*: T20060804.0034,1 MMT Convert Custom Report RED07 09/20/2006 [End]
