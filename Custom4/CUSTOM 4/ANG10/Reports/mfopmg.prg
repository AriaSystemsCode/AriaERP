...
LOCATE
DO WHILE !EOF()
  lcOpr = CTKTNO+COPRCODE
  REPLACE COWNER WITH '' REST WHILE CTKTNO+COPRCODE+CTARGET+CLOTNO+ITEM+TRANCD+CTRGOPR+CTRGLOT = lcOpr 
  SKIP -1 
  REPLACE COWNER WITH 'N' 
  SKIP
ENDDO
LOCATE
*- tmi
...
*:* Date        : 05/20/2009
*:* Purpose     : update the record # in the cowner field
*:***************************************************************************
*:* Called from : frx
*:***************************************************************************
*:* Parameters : None
...
*:***************************************************************************
FUNCTION lfUpdRecno
*!*	an idea to hide the summary field in the bottom of the page when there is no 
*!*	styles in the page

*!*	you can base on the _pageno field


REPLACE COWNER WITH 'N'+STR(_PAGENO,3)
*-- end of lfUpdRecno.