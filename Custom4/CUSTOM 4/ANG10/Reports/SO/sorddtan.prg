...
*! Date      : 11/10/2010 
*! Entry no. :   *! C201284,1 TMI 11/08/2010 Custom Open Order Report for Angle [T20100827.0014]
                 *! C201284,1 TMI I copied the SORDDET.PRG as it is and I commented the changes I did with the fix # C201284
                 * I preferred not to mess the original code as it is a complicated report build elegantly
*!*****************************************************************************************
...
*! C201284,1 TMI 11/08/2010 Custom Open Order Report for Angle [T20100827.0014]
*! C201284,1 TMI I copied the SORDDET.PRG as it is and I commented the changes I did with the fix # C201284
*******************************************************************************************
...
*C201284,1 TMI 11/07/10 [Start] 
*loOGScroll.cCROrientation = 'P'
*C201284,1 TMI 11/07/10 [End  ] 

*C201284,3 TMI 11/11/2010 [Start] define the value of lcRpEdiPrn
lcRpEdiPrn = 'O'
llRpSizes = .T.
*C201284,3 TMI 11/11/2010 [End  ] 
...
*C201284,3 TMI 11/11/2010 [Start] 
SELECT &lcMastFile
LOCATE
IF EOF()
...
*C201284,1 TMI 11/07/10 [Start] collect data for C/Ts
IF llClearFn OR llOGFltCh
  
  *- Create a new temp file that will holds the only temp fields needed for this custom report
  =lfCrTmpOpnOrd()
  
  *- Collect data
  =lfCollCTData()
  
  *- Define report parameters and tables
  =lfRptParams()
...
*      lcOutFootR = [PADR(ALLTRIM(CUSTOMER.BtName),25) +"/"+EVALUATE(lcLineCurr)+ ", " + TRANSFORM(CUSTOMER.Phone1,lcPhonPict)]  && Right title of outer group footer.
  ...
  *C201284,1 TMI 11/09/2010 [Start] 
  *CASE lcRpSortBy = 'O'    && Sort by order
  CASE lcRpSortBy $ 'OT'    && Sort by order
    *C201284,1 TMI 11/09/2010 [End  ] 
    lcSubTitle = 'Order'
    lcInnGrp   = ['']
    lcInnFootL = [''] 
...
*C201284,1 TMI 11/08/2010 [Start] 
*laSortDesc[6,1] = 'Complete Date'
laSortDesc[6,1] = 'Sales Order Complete Date'
*C201284,1 TMI 11/08/2010 [End  ] 
...
*C201284,1 TMI 11/03/10 [Start] add the sort by cutting ticket
lnSrtElms = lnSrtElms + 1
DIMENSION laSortDesc[lnSrtElms,1],laSortVal[lnSrtElms,1]
laSortDesc[ALEN(laSortDesc,1),1] = "Cutting Ticket#"
laSortVal[ALEN(laSortDesc,1),1]  = "T"
*C201284,1 TMI 11/03/10 [End  ] 
...
***************************************************************************
***** The following are the functions related to C/T data collections *****
***************************************************************************
...
*! Developer : TMI - TAREK MOHAMMED IBRAHIM
*! Date      : 11/07/10 
*! Purpose   : Collect related c/t data
*!**************************************************************************
FUNCTION lfCollCTData

*- Define an array to hold the costing types and descriptions, call a function to update the array
DIMENSION laCost[14,2]
=lfUpdaCost()

*- Get the orders selected in one variable
lcOrders = lfGetList(lcMastFile,'ORDER')  

*- Get the related C/Ts from the cutpick file
lcSelFld  = [CTKTNO,"ORDER",CORDLINE,CTKTLINENO,QTY1,QTY2,QTY3,QTY4,QTY5,QTY6,QTY7,QTY8]
lcTable   = "CUTPICK"
lcSelCond = ["ORDER" IN (]+lcOrders+[)]
lfOpenSql(lcSelFld,lcTable,'TCUTPICK',lcSelCond)

SELECT TCUTPICK
INDEX ON ORDER+CORDLINE TAG ORDER

*- Get C/T #'s in a string variable
lcCuttkts = lfGetList('TCUTPICK','CTKTNO')

*- Get the related lines from MFGOPRHD
lcSelFld  = "CTKTNO,CCONTCODE,COPERSEQ"
lcTable   = "MFGOPRHD"
lcSelCond = "CIMTYP='M' AND CTKTNO IN ("+lcCuttkts+")"
lfOpenSql(lcSelFld,lcTable,'MFGOPRHD',lcSelCond)

SELECT MFGOPRHD
INDEX ON CTKTNO+COPERSEQ TAG MFGOPRHD

*- Get the related lines form POSHDR & POSLN
lcSelFld  = "POSHDR.PO,POSHDR.ENTERED,POSHDR.ACT_DATE,POSHDR.STATUS,POSLN.*"
lcTable   = "POSHDR,POSLN"
lcSelCond = " POSHDR.CBUSDOCU ='P' AND POSHDR.CSTYTYPE ='U' AND POSHDR.PO=POSLN.PO AND POSHDR.PO IN ("+lcCuttkts+")"
lfOpenSql(lcSelFld,lcTable,'TPOSLN',lcSelCond)

SELECT TPOSLN
INDEX ON PO+STR(LINENO,6) TAG TPOSLN

*C201284,1 TMI 11/08/2010 [Start] as per Wael, no need to get the costing of the style if there is no c/t related
*!*	*- Get selected styles
*!*	lcStyles = lfGetList(lcMaster,'PADR(substr(style,1,lnMajorLen),19)')

*!*	*- Get BOM lines for selected styles
*!*	lcSelFld  = "BOM.*"
*!*	lcTable   = "BOM"
*!*	lcSelCond = "CINVTYPE = '0001' AND CITMMAJOR IN ("+lcStyles+")"
*!*	lfOpenSql(lcSelFld,lcTable,'TBOM',lcSelCond)
*C201284,1 TMI 11/08/2010 [End  ] 

*- Set relations 
SELECT (lcMastFile)
SET RELATION TO ORDER+STR(LINENO,6) INTO TCUTPICK
SET RELATION TO 'O'+ORDER INTO ORDHDR ADDITIVE
SET RELATION TO 'S'+SCALE INTO SCALE ADDITIVE
SET RELATION TO STYLE INTO STYLE ADDITIVE

SELECT TCUTPICK
SET RELATION TO CTKTNO+CTKTLINENO INTO TPOSLN

*- The main loop to collect data
SELECT (lcMastFile)
LOCATE
SCAN
  *- Initialize the temp file variables by blank 
  SELECT &lcRpTmpOpnOrd
  SCATTER MEMVAR MEMO BLANK
  
  *- populate variables from master file
  SELECT (lcMastFile)
  SCATTER MEMVAR MEMO
  ...
  m.REP1      = ORDHDR.REP1
  m.CCURRCODE = ORDHDR.CCURRCODE
  m.ORDHDCMDT = DTOS(&lcMastFile..COMPLETE)  && used in the group by in thr rpt file
  m.LHASNOTES = !EMPTY(&lcMastFile..NOTE_MEM)

  *- From here listing all fields needed in the reprt, not listed fields are populated from the SCATTER command    
  *C201284,3 TMI 11/11/2010 [Start] 
  *m.CTKTNO    = IIF(!EOF('TCUTPICK'),TCUTPICK.CTKTNO,'**N/A**')
  m.CTKTNO    = '**N/A**'
  *C201284,3 TMI 11/11/2010 [End  ] 
  m.ENTERED   = ORDHDR.ENTERED
  m.CDIVISION = ORDHDR.CDIVISION  
  m.STYLE     = SUBSTR(&lcMastFile..STYLE,1,lnMajorLen)
  m.COLOR     = SUBSTR(&lcMastFile..STYLE,lnNonMajSt,lnColorLen)
  
  *- Get sizes
  SELECT SCALE
  m.Sz1 = ALLTRIM(SCALE.SZ1)
  m.Sz2 = ALLTRIM(SCALE.SZ2)
  m.Sz3 = ALLTRIM(SCALE.SZ3)
  m.Sz4 = ALLTRIM(SCALE.SZ4)
  m.Sz5 = ALLTRIM(SCALE.SZ5)
  m.Sz6 = ALLTRIM(SCALE.SZ6)
  m.Sz7 = ALLTRIM(SCALE.SZ7)
  m.Sz8 = ALLTRIM(SCALE.SZ8) 
  
  m.Status = '' && must be empties first 
  ...
  *C201284,3 TMI 11/11/2010 [Start] 
  SELECT TCUTPICK
  SCAN REST WHILE ORDER+CORDLINE = &lcMastFile..ORDER+STR(&lcMastFile..LINENO,6)
  *C201284,3 TMI 11/11/2010 [End  ] 
  
    *C201284,3 TMI 11/11/2010 [Start] 
    m.CTKTNO    = TCUTPICK.CTKTNO
    *C201284,3 TMI 11/11/2010 [End  ] 
  
    m.CTEntered = TPOSLN.ENTERED
    
    m.CTReceived = TPOSLN.DATE
    SELECT TPOSLN  
    SCAN REST WHILE PO+STR(LINENO,6) = TCUTPICK.CTKTNO+TCUTPICK.CTKTLINENO
      && loop to get latest receive data if receive on intervals
      IF m.CTReceived < TPOSLN.DATE
        m.CTReceived = TPOSLN.DATE
      ENDIF
    ENDSCAN
    
    SELECT TCUTPICK
    GOTO RECNO() && this line to refresh the pointer in TPOSLN based on the relation declared above  
    m.CTComplete = TPOSLN.COMPLETE
    m.ACT_DATE   = TPOSLN.ACT_DATE
    
    *- Get contractors
    =lfGetCont()
    
    m.Status = TPOSLN.STATUS
    
    *- Get qty's
    m.Book1 = TCUTPICK.QTY1
    m.Book2 = TCUTPICK.QTY2
    m.Book3 = TCUTPICK.QTY3
    m.Book4 = TCUTPICK.QTY4
    m.Book5 = TCUTPICK.QTY5
    m.Book6 = TCUTPICK.QTY6
    m.Book7 = TCUTPICK.QTY7
    m.Book8 = TCUTPICK.QTY8
    m.TOTBOOK = m.Book1+m.Book2+m.Book3+m.Book4+m.Book5+m.Book6+m.Book7+m.Book8
  
    SELECT TPOSLN
    lcKey = EVAL(KEY())
    *- if the actual cost is needed then check if the c/t cost sheet is closed and locate the same lineno where trancd is '2'
    IF lcRpPrnCst ='A' AND STATUS = 'S'
      LOCATE REST WHILE PO+STR(LINENO,6) = lcKey FOR TRANCD = '2'
    ENDIF
    
    STORE 0 TO m.nMatCst,m.nMFGCst
    lcCstFld = ''
    FOR lnI=1 TO 7
      lcZ = STR(lnI,1)
      lcCstFld = IIF(lcRpPrnCst = 'E','NICOST','nAct_Cost')
      
      IF laCost[lnI,2] $ 'F,T'
        *- material cost ( fabric & trim )
        m.nMatCst = m.nMatCst + &lcCstFld.&lcZ
      ELSE
        *- other costs
        m.nMFGCst = m.nMFGCst + &lcCstFld.&lcZ
      ENDIF
    ENDFOR
    =SEEK(lcKey,'TPOSLN')
    
    *- unit cost
    m.UnitCost = m.nMatCst + m.nMFGCst
    
    *- total cost
    m.TotCost   = m.TOTBOOK*m.UnitCost
    
    *C201284,3 TMI 11/11/2010 [Start] 
    INSERT INTO &lcRpTmpOpnOrd FROM MEMVAR
...
*- close temp table to be used in the rpt ( crystal report file)
USE IN (lcRpTmpOpnOrd)
...
*- end of FUNCTION lfCollCTData

*!**************************************************************************
*! Name      : lfUpdaCost
*! Developer : TMI - TAREK MOHAMMED IBRAHIM
*! Date      : 11/07/10 
*! Purpose   : To update the array laCost
*!**************************************************************************
*C201284,1 TMI 11/07/10 
FUNCTION lfUpdaCost
...
*- end of FUNCTION lfUpdaCost

*!**************************************************************************
*! Name      : lfRptParams
*! Developer : TMI - TAREK MOHAMMED IBRAHIM
*! Date      : 11/07/10 
*! Purpose   : Define report parameters and tables
*!**************************************************************************
FUNCTION lfRptParams
...
loOGScroll.laCRParams[lnI,2]  = lcRpMsg1
...
lnI = lnI + 1
loOGScroll.laCRParams[lnI,1]  = 'SortName'
loOGScroll.laCRParams[lnI,2]  = laSortDesc[ASCAN(laSortVal,lcRpSortBy)]

lnI = lnI + 1
loOGScroll.laCRParams[lnI,1]  = 'ReportName'
loOGScroll.laCRParams[lnI,2]  = 'Open Orders Detail Report'

lnI = lnI + 1
loOGScroll.laCRParams[lnI,1]  = 'llRpOrdLnt'
loOGScroll.laCRParams[lnI,2]  = llRpOrdLnt


lcTmpOrdDet = loOgScroll.gfTempName()

DIMENSION loOgScroll.lacrTABLES[1]  && array For Temp Table & pathes 
loOgScroll.lacrTABLES[1] = oAriaApplication.WorkDir+lcRpTmpOpnOrd+'.DBF'

loOgScroll.lcOGLastForm = "SORDDTAN"
loogScroll.cCROrientation = 'L'

*- end of FUNCTION lfRptParams

*!**************************************************************************
*! Name      : lfGetCont
*! Developer : TMI - TAREK MOHAMMED IBRAHIM
*! Date      : 11/08/10
*! Purpose   : Get contractors
*!**************************************************************************
*C201284,1 TMI 11/08/2010 
 
FUNCTION lfGetCont
LOCAL lnSlct,lnCount,lcCount
lnSlct = SELECT(0)
SELECT MFGOPRHD
IF SEEK(TPOSLN.PO,'MFGOPRHD')
  lnCount = 1
  SCAN REST WHILE CTKTNO = TPOSLN.PO 
...
*! Developer : TMI - TAREK MOHAMMED IBRAHIM
*! Date      : 11/07/10 
*! Purpose   : to get a field values from one field into a string variable
*!**************************************************************************
*C201284,1 TMI 11/07/10 
FUNCTION lfGetList
LPARAMETERS lcTable,lcFld
LOCAL lcVar
lcVar = "'XX',"   && just add this so that the sql statement does not crash
SELECT &lcTable
LOCATE
SCAN
  IF !EVAL(lcFld) $ lcVar
    lcVar = lcVar + "'" + EVAL(lcFld) + "'" + "," 
  ENDIF
ENDSCAN
lcVar = LEFT(lcVar,LEN(lcVar)-1)
RETURN lcVar

*- end of FUNCTION lfGetList
...
*! Developer : TMI - TAREK MOHAMMED IBRAHIM
*! Date      : 11/07/10 
*! Purpose   : to create a temp file for the Open Orders with C/T
*!**************************************************************************
*C201284,1 TMI 11/07/10 
FUNCTION lfCrTmpOpnOrd

DIMENSION laOpnOrdStru[46,4]

LOCAL lnI 
lnI = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CTKTNO'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 7
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'ORDER'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'ENTERED'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'START'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'COMPLETE'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'ACCOUNT'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CDIVISION'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'STYLE'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 19
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CSTYGROUP'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'COLOR'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CTEntered'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CTReceived'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CTComplete'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'ACT_DATE'
laOpnOrdStru[lnI,2] = 'D'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CONTR1'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CONTR2'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CONTR3'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 8
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Status'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 1
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book1'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book2'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book3'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book4'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book5'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book6'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book7'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'Book8'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'TOTBOOK'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 7
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'nMatCst'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 15
laOpnOrdStru[lnI,4] = 3

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'nMFGCst'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 15
laOpnOrdStru[lnI,4] = 3

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'UnitCost'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 15
laOpnOrdStru[lnI,4] = 3

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'TotCost'
laOpnOrdStru[lnI,2] = 'N'
laOpnOrdStru[lnI,3] = 15
laOpnOrdStru[lnI,4] = 3

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'NOTE_MEM'
laOpnOrdStru[lnI,2] = 'M'
laOpnOrdStru[lnI,3] = 10
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'LHASNOTES'
laOpnOrdStru[lnI,2] = 'L'
laOpnOrdStru[lnI,3] = 1
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SCALE'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 3
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ1'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ2'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ3'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ4'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ5'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ6'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ7'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'SZ8'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 5
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CWARECODE'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 6
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'REP1'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 3
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'CCURRCODE'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 3
laOpnOrdStru[lnI,4] = 0

lnI = lnI + 1
laOpnOrdStru[lnI,1] = 'ORDHDCMDT'
laOpnOrdStru[lnI,2] = 'C'
laOpnOrdStru[lnI,3] = 10
laOpnOrdStru[lnI,4] = 0

gfCrtTmp(lcRpTmpOpnOrd,@laOpnOrdStru,"CTKTNO",lcRpTmpOpnOrd)

Return
*- End of FUNCTION lfCrTmpOpnOrd
