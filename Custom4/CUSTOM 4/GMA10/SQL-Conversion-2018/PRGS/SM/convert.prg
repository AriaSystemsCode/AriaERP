...
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047]
*:************************************************************************
...
*:*************************************************************
FUNCTION lfConvFile
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
*LPARAMETERS lcRemt_Name,lcFile_Name,lcSydIndex,oFormSet
PARAMETERS lcRemt_Name,lcFile_Name,lcSydIndex,oFormSet
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
...
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
IF llSqlFile 
  lnFLFLDTable =oAriaApplication.RemoteCompanyData.execute ("Select * from SYDFLFLD WHERE cfile_nam ='"+lcFile_Name+"' and cadd_user ='MMTGMA'",;
                                '',"TMPFLFLD","",oAriaApplication.cAria4SysFiles,;
                                3,"",oFormSet.DataSessionID)
  IF lnFLFLDTable > 0  AND USED("TMPFLFLD")                                
     SELECT "TMPFLFLD"
     LOCATE
     IF !EOF()
 
       lnResultCur = oAriaApplication.remotecompanydata.sqlrun("Select * From "+lcFile_Name,lcFile_Name,;
       lcFile_Name,oAriaApplication.ActiveCompanyConStr,3,'SAVE',oFormSet.DataSessionID) 
       IF lnResultCur > 0 AND USED(lcFile_Name)     
         oFormSet.lnRecCnt = RECCOUNT(lcFile_Name)
        ...
        *=gfOpenTable(lcFile_Name,SYCNVLIB.cFile_tag,'SH')
     ENDIF
  ENDIF
 RETURN
ENDIF
...
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
*IF SYCNVLIB.lExt_call AND !EMPTY(ALLTRIM(cFile_a26))
IF SYCNVLIB.lExt_call AND !EMPTY(ALLTRIM(SYCNVLIB.cFile_a26))
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
  ...
  *IF !gfOpenFile(oAriaApplication.DataDir+ALLTRIM(cFile_a26),'','SH')
  IF !gfOpenFile(oAriaApplication.DataDir+ALLTRIM(SYCNVLIB.cFile_a26),'','SH')
  *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
    *N000682,1 11/20/2012 MMT Globlization changes[Start]
...
IF lnSYDINDEXTable > 0
  ...
  *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
  =SEEK("STYLE")
  LOCATE REST WHILE CFILE_NAM = "STYLE" FOR ALLTRIM(cfile_tag) = 'CSTYLE'
  IF FOUND()
     REPLACE lunique WITH .F.
  ENDIF
  *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
ELSE
  RETURN .F.
...
*N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    *--Create Table.
    IF llGetStructure AND llGetIndeces
      ...
      *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
      llSqlFile = .F.
      * Add Code to handle adding fields to SQL table without overwriting data from VFP9
      lnSYDFileTable =oAriaApplication.RemoteCompanyData.execute ("Select * from SYDFILES WHERE cfile_nam ='"+lcFile_Name+"' and CVER = 'A40'",;
                                          '',"TMPFILES","",oAriaApplication.cAria4SysFiles,;
                                          3,"",oFormSet.DataSessionID)
      IF lnSYDFileTable > 0 AND USED("TMPFILES")  
        SELECT "TMPFILES"
        LOCATE
        IF !EOF()
          llSqlFile = .T.
          REPLACE SYCNVFLS.NFLAG WITH 2 IN SYCNVFLS
        ENDIF
      ENDIF                        
      *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
...
ENDIF
*X
IF !llNewRec AND SYDFILES.CVER <> 'A40'
  lnArrStr = AFIELDS(laFileStru)
  IF lnArrStr > 0
    FOR lnS = 1 TO lnArrStr
      IF EVALUATE(lcRemt_Name+'.'+laFileStru[lnS,1]) <> EVALUATE("m."+laFileStru[lnS,1])
        llNewRec = .T.
        SELECT(lcRemt_Name)
        APPEND BLANK
        EXIT
...
SCAN
  ...
  *IF UPPER(ALLTRIM(name))=='REC_NO' 
  IF UPPER(ALLTRIM(name))=='REC_NO' OR UPPER(ALLTRIM(name))=='OID'
  *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
    LOOP
  ENDIF
  *AMH [End]
...
SCAN
  ...
  *AMH [End]
  *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
  IF  UPPER(ALLTRIM(name))=='OID'
   lcFields = lcFields + IIF(llFirstField,"",",") + "[" + ALLTRIM(name) + "]= NewID()" 
  ELSE
  *! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][End]
...
*: B608646,1 MMT 08/07/2008 Convert Fill Currency fields in BOM and Bomline Tables[End]
*! E303912,1 MMT 01/16/2018 Add Modifications for GMA SQL conversion[Task:T20171029.0047][Start]
FUNCTION lfGetBillToAdd
LPARAMETERS lcAccount,lcStore,lcRetVal,lcMulti
lcReturnValue = ''
LOCAL lnOldAlias 
lnOldAlias = SELECT()
IF !USED('Customer')
...
IF lcMulti = 'Y'
  DO CASE 
    CASE lcRetVal = 'name'
      lcReturnValue = 'At Store Level'
    OTHERWISE   
      lcReturnValue = ''
  ENDCASE  
ELSE
  IF gfSeek(IIF(!EMPTY(lcStore),'S','M')+lcAccount+lcStore,'Customer','Customer')
    DECLARE laAddress[6,3]
    *-- Get customer Default billing name & address
    STORE '' TO lcblladd1,lcblladd2,lcblladd3,lcblladd4,lcblladd5
    =gfGetAdr('Customer','','','',1,'2')
    FOR lnCount = 1 TO ALEN(laAddress,1)
      lcCount = STR(laAddress[lnCount,1],1)
      lcBllAdd&lcCount = ALLTRIM(lcBllAdd&lcCount) + IIF(EMPTY(lcBllAdd&lcCount),'',',')+;
      SUBSTR(laAddress[lnCount,2],1,laAddress[lnCount,3])
    ENDFOR
    DO CASE 
      CASE lcRetVal = 'name'
        lcReturnValue = Customer.BtName
      CASE lcRetVal = '1'
        lcReturnValue = lcblladd1
      CASE lcRetVal = '2'
        lcReturnValue = lcblladd2    
      CASE lcRetVal = '3'
        lcReturnValue = lcblladd3    
      CASE lcRetVal = '4'
        lcReturnValue = lcblladd4    
      CASE lcRetVal = '5'
        lcReturnValue = lcblladd5    
    ENDCASE
  ENDIF
ENDIF
SELECT(lnOldAlias)
...
FUNCTION lfGetWarehouseAdd
LPARAMETERS lcWareCode,lcRetVal   
LOCAL lnOldAlias 
lnOldAlias = SELECT()
lcReturnValue = ''
IF !USED('WareHous')
...
ENDIF
IF gfSeek(lcWareCode,'WareHous','WareHous')
  DECLARE laAddress[6,3]
  *-- Get customer Default billing name & address
  STORE '' TO lcblladd1,lcblladd2,lcblladd3,lcblladd4,lcblladd5
  =gfGetAdr('WareHous','','','',1)
...
FUNCTION lfCdRlFld
LPARAMETERS lcFieldName,lcRelFldName,lcFieldVal
lcRetValue = ''
LOCAL lnOldAlias 
lnOldAlias = SELECT()
DIMENSION laRelFldArr[1,2]
laRelFldArr[1,1]  = lcRelFldName
laRelFldArr[1,2]  = 'lcRetValue'
=gfRltFld(lcFieldVal,@laRelFldArr,lcFieldName)
SELECT(lnOldAlias)
RETURN lcRetValue

FUNCTION lfGetRepName
LPARAMETERS lcRepCode
IF EMPTY(lcRepCode)
  RETURN ''
ENDIF
LOCAL lnOldAlias 
...
ENDIF
lcRetValue = ''
IF gfSeek(lcRepCode,'Salesrep','SALESREP')                                        
  lcRetValue = Salesrep.name
ENDIF
SELECT(lnOldAlias)
RETURN lcRetValue 

FUNCTION lfShpToAdd
LPARAMETERS lcAccount,lcStore,lcRetVal,llAlt_ShpTo,llStrDirct
lcReturnValue = ''
IF llAlt_ShpTo
  IF lcRetVal = 'name'
    RETURN m.stname
  ELSE
    RETURN m.CADDRESS&lcRetVal.    
  ENDIF  
ELSE
  LOCAL lnOldAlias 
  lnOldAlias = SELECT() 
  IF !USED('Customer')
    =gfOpenTable('Customer','Customer','SH')
  ENDIF
  IF gfSEEK(IIF(EMPTY(lcStore) , 'M' , 'S') + lcAccount + lcStore,'CUSTOMER','CUSTOMER')
*!*      lcDistCntr = CUSTOMER.Dist_Ctr
*!*      IF !EMPTY(lcDistCntr) AND !llStrDirct
*!*        gfSEEK('S' + lcAccount + lcDistCntr,'CUSTOMER','CUSTOMER')
*!*      ENDIF
    IF lcRetVal = 'name'
      lcReturnValue = IIF(EMPTY(CUSTOMER.DBA) , CUSTOMER.STName , CUSTOMER.DBA)
    Else
      DECLARE laAddress[6,3]
      *-- Get customer Default shipping name & address
      STORE '' TO lcadd1,lcadd2,lcadd3,lcadd4,lcadd5
      =gfGetAdr('Customer','','','',1)
      FOR lnCount = 1 TO ALEN(laAddress,1)
        lcCount = STR(laAddress[lnCount,1],1)
        lcAdd&lcCount = ALLTRIM(lcAdd&lcCount) + IIF(EMPTY(lcAdd&lcCount),'',',')+;
        SUBSTR(laAddress[lnCount,2],1,laAddress[lnCount,3])
      ENDFOR
      DO CASE 
        CASE lcRetVal = '1'
          lcReturnValue = lcadd1
        CASE lcRetVal = '2'
          lcReturnValue = lcadd2    
        CASE lcRetVal = '3'
          lcReturnValue = lcadd3    
        CASE lcRetVal = '4'
          lcReturnValue = lcadd4    
        CASE lcRetVal = '5'
          lcReturnValue = lcadd5    
      ENDCASE
    ENDIF
  ENDIF
ENDIF
...
FUNCTION lfPikShpToAdd
LPARAMETERS lcAccount,lcStore,lcRetVal,lcOrder
lcReturnValue = ''
LOCAL lnOldAlias 
lnOldAlias = SELECT() 
IF !USED('ORDHDR')
...
ENDIF
=gfSeek('O'+lcOrder,'ORDHDR','ORDHDR')
IF !EMPTY(lcOrder) AND ORDHDR.Alt_ShpTo
  IF lcRetVal = 'name'
    SELECT(lnOldAlias)
    RETURN ORDHDR.stname
  ELSE
    SELECT(lnOldAlias)
    RETURN ORDHDR.CADDRESS&lcRetVal.    
  ENDIF  
ELSE
  IF !USED('Customer')
    =gfOpenTable('Customer','Customer','SH')
...
FUNCTION lfStoreShpAdd
LPARAMETERS lcAccount,lcStore,lcRetVal
IF EMPTY(lcStore)
  RETURN ''
ENDIF
LOCAL lnOldAlias 
...
IF !USED('Customer')
  =gfOpenTable('Customer','Customer','SH')
ENDIF
IF gfSEEK('S' + lcAccount + lcStore,'CUSTOMER','CUSTOMER')
*!*    lcDistCntr = CUSTOMER.Dist_Ctr
*!*    IF !EMPTY(lcDistCntr) AND !ORDHDR.lStrDirct
*!*      gfSEEK('S' + lcAccount + lcDistCntr,'CUSTOMER','CUSTOMER')
*!*    ENDIF
...
FUNCTION lfGetScaleInfo
LPARAMETERS lcScale,lcRetVal
LOCAL lnOldAlias 
lnOldAlias = SELECT() 
IF !USED('Scale')
...
ENDIF
lcReturnValue = ''
IF gfSeek('S'+lcScale,'Scale','Scale')
  DO CASE
  CASE lcRetVal = 'desc'
    lcReturnValue = Scale.cscl_desc
  CASE lcRetVal = 'cnt'
    lcReturnValue = Scale.cnt  
  OTHERWISE 
    lcReturnValue = Scale.Sz&lcRetVal
  ENDCASE
ENDIF
SELECT(lnOldAlias)
RETURN lcReturnValue

FUNCTION lfGetStyScaleInfo
LPARAMETERS lcStyle,lcRetVal
SET STEP ON
lcReturnValue =''
LOCAL lnOldAlias 
lnOldAlias = SELECT()
IF TYPE('m.cInvType') <> 'U' AND m.cInvType ='0002'
  IF !USED('ITEM')
  ...
  IF !USED('Scale')
    =gfOpenTable('Scale','Scale','SH')
  ENDIF
  IF gfSeek('0002'+lcStyle,'ITEM','Style')  
    IF gfSeek('S'+ITEM.Scale,'Scale','Scale')  
      DO CASE
        CASE lcRetVal = 'scale'
          lcReturnValue = ITEM.Scale
        CASE lcRetVal = 'stydesc'
          lcReturnValue = ITEM.Desc
        CASE lcRetVal = 'stydesc1'    
          lcReturnValue = ITEM.Desc1
        CASE lcRetVal = 'desc'
          lcReturnValue = Scale.cscl_desc
        CASE lcRetVal = 'cnt'
          lcReturnValue = Scale.cnt  
...
ELSE
  ...
  IF gfSeek(lcStyle,'Style','Style')  
    IF gfSeek('S'+Style.Scale,'Scale','Scale')  
      DO CASE
        CASE lcRetVal = 'scale'
          lcReturnValue = Style.Scale
        CASE lcRetVal = 'stydesc'
          lcReturnValue = Style.Desc
        CASE lcRetVal = 'stydesc1'    
          lcReturnValue = Style.Desc1
        CASE lcRetVal = 'desc'
          lcReturnValue = Scale.cscl_desc
        CASE lcRetVal = 'cnt'
          lcReturnValue = Scale.cnt  
...
FUNCTION lfFactrAddress
LPARAMETERS lcFactor,lcRetVal
IF EMPTY(lcFactor)
  RETURN ''
ENDIF
LOCAL lnOldAlias 
...
lcRetValue = ''
IF !USED('SycFact')
  =gfOpenTable('SycFact','CFACCODE','SH')
ENDIF
IF gfSeek(lcFactor,'SycFact','CFACCODE')
  DO CASE
    CASE  lcRetVal ='comp'
      lcRetValue = SycFact.CFACCOMP
    OTHERWISE
      lcRetValue = SycFact.caddress&lcRetVal.
  ENDCASE
ENDIF
SELECT(lnOldAlias)
RETURN lcRetValue

FUNCTION lfGetPrePackInfo
LPARAMETERS lcScale,lcPrePak,lcRetVal
IF EMPTY(lcPrePak)
  RETURN 0
ENDIF
LOCAL lnOldAlias 
...
IF !USED('Scale')
  =gfOpenTable('Scale','Scale','SH')
ENDIF
IF gfSeek('P'+lcScale+lcPrePak,'Scale','Scale')  
  lcReturnValue = Scale.PP&lcRetVal
ENDIF
SELECT(lnOldAlias)
RETURN lcReturnValue


FUNCTION lfGetPIKWarehouse
LPARAMETERS lcPIKTKT,lcRetVal 
LOCAL lnOldAlias 
lnOldAlias = SELECT()
IF !USED('PIKTKT')
  =gfOpenTable('PIKTKT','PIKTKT','SH')
ENDIF
=gfSeek(lcPIKTKT,'PIKTKT','PIKTKT')
lcWareCode = PIKTKT.cWareCode
IF lcRetVal ='warehouse'  
  SELECT(lnOldAlias)
  RETURN PIKTKT.cWareCode
ENDIF
IF !USED('Warehous')
  =gfOpenTable('Warehous','WAREHOUS','SH')
ENDIF
...
FUNCTION lfGetVenInf
LPARAMETERS lcVendor,lcRetVal
IF m.cStyType ="N" AND m.cBusDocu = 'N'
  RETURN lfGetWarehouseAdd(ALLTRIM(lcVendor),lcRetVal)
ENDIF
LOCAL lnOldAlias 
lnOldAlias = SELECT()
lcReturnValue =''
...
ENDIF
IF gfSeek(lcVendor,'APVENDOR','VENCODE')
  DECLARE laAddress[6,3]
  *-- Get customer Default billing name & address
  STORE '' TO lcblladd1,lcblladd2,lcblladd3,lcblladd4,lcblladd5
  =gfGetAdr('APVENDOR','','','',1)
  ...
  DO CASE
    CASE lcRetVal= 'desc'
      lcReturnValue = APVENDOR.cvencomp
    CASE lcRetVal= 'fax'
      lcReturnValue = APVENDOR.cfaxno
    CASE lcRetVal= '1'
      lcReturnValue = lcblladd1
    CASE lcRetVal= '2'
      lcReturnValue = lcblladd2    
