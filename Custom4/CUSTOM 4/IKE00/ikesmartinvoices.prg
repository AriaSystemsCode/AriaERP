*! B611150,1 MMT 06/05/2016 Issue#8 - 852 Apply the style discount on style price[T20151014.0017 - Issue#8]
*------------------------------------------------------------------------------------------------------------
...
Select PRODACTI
=Seek(lcAccount)
Select Distinct PRODACTI.dprd_bgn,PRODACTI.Store From PRODACTI Where Invoiced And cFileCode = lcFileCode And Account = lcAccount Into Cursor 'InvProdStore'
Select 'InvProdStore'
Go Top
If !Eof()
	lcConsInvoicList= ""
...
Endif
**Start Update
Select Distinct PRODACTI.dprd_bgn,PRODACTI.Store From PRODACTI Where  cFileCode = lcFileCode And Account = lcAccount Into Cursor 'InvProdStore'
Select 'InvProdStore'
Locate
If !Eof()
...
***MM
	...
	Scan
		...
		Scan Rest While Account+Dtos(dprd_bgn)+Dtos(DPRD_END)+DEPT+Store+Style+TRAN_TYPE = lcAccount+Dtos(InvProdStore.dprd_bgn) FOR;
		    cFileCode = lcFileCode And Store =InvProdStore.Store
...
*MT Get Related Multi Store order list
        ...
        INSERT INTO (lcLineCursor) FROM MEMVAR
	      llGotOrders = .T.
    	ELSE
    	  llGotOrders = lfAddLinesFromOrder(lcAccount,m.Style,m.Store,@laQtyArr)
	    ENDIF  
			If !llGotOrders
				llLineIssue = .T.
				Exit
...
Lparameters lcAccountID,lcStyle,lcStore,laQtyArray
llReturnValue = .T.
...
*Select SUM(Qty1) As QTY1,SUM(Qty2) As QTY2,SUM(Qty3) As QTY3,SUM(Qty4) As QTY4,SUM(Qty5) As QTY5,SUM(Qty6) As QTY6,;
                      SUM(Qty7) As QTY7,SUM(Qty8) As QTY8 from (lcLineCursor);
                      Where [Order]= lcOrderNo AND nordlineno = ORDLINE.LINENO AND STYLE = Ordline.Style INTO CURSOR 'TempQty'
					Select (lcLineCursor)
					Set Order To (lcLineCursor)
					...
					If Seek(m.Style,'STYLE','STYLE')
						m.Price = Style.PriceA
            *! B611150,1 MMT 06/05/2016 Issue#8 - 852 Apply the style discount on style price[T20151014.0017 - Issue#8][Start]
            IF Seek(m.Style,'STYLE','STYLE') AND !EMPTY(Style.cdisccode)
              ...
              ENDIF
            ENDIF
            *! B611150,1 MMT 06/05/2016 Issue#8 - 852 Apply the style discount on style price[T20151014.0017 - Issue#8][End]
