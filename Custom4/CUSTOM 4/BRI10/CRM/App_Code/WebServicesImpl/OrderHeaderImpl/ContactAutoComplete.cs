using System;
using System.Web;
using System.Collections;
using System.Web.Services;
using System.Web.Services.Protocols;
using System.Collections.Generic;
using Aria.Data.BusinessObject;
using Aria.DataTypes;
using System.Data;


/// <summary>
/// Summary description for SalesRepAutoComplete
/// </summary>
[WebService(Namespace = "http://tempuri.org/")]
[WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
[System.Web.Script.Services.ScriptService]
public class ContactAutoComplete : System.Web.Services.WebService
{

    public ContactAutoComplete()
    {

        //Uncomment the following line if using designed components 
        //InitializeComponent(); 
    }

    [WebMethod(EnableSession = true)]
    public string[] GetCompletionList(string prefixText, int count)
    {
        string AccountID = Session["AccountID"].ToString().PadRight(8);

        if (Session["StoreID"].ToString().TrimEnd() != "Main" && Session["StoreID"].ToString().TrimEnd() != "Multi")
        {
            AccountID += Session["StoreID"].ToString().PadRight(8);
        }
        else
        {
            AccountID += "".PadRight(8);

        }

        AriaBusinessObjectAdapter businessObject = new AriaBusinessObjectAdapter();
        AriaConditionList conditionList = new AriaConditionList();
        AriaCondition condition = new AriaCondition();
        condition.RightHandSide = new AriaStandardDataType();
        condition.LeftHandSide = new AriaStandardDataType();
        condition.Operator = AriaConditionOperators.GreaterOrEqual;
        if (prefixText.TrimEnd() == "*")
        {
            ((AriaStandardDataType)condition.RightHandSide).Value = AccountID;
        }
        else
        {
            ((AriaStandardDataType)condition.RightHandSide).Value = AccountID + prefixText.ToUpper(); // User Input
        }
        ((AriaStandardDataType)condition.LeftHandSide).PropertyDataPathDictionary.Add("Value", "Customer"); // Field

        conditionList.Items.Add(condition);

        DataTable xssss = businessObject.GetBusinessObjects("", "", "99", "CRM.Contact", conditionList);

        int size = xssss.Rows.Count;

        List<string> r = new List<string>(size);

        for (int i = 0; i < size && i < 256; i++)
        {

            string s = xssss.Rows[i]["Contact"].ToString().Trim();
            r.Add(s);
        }

        if (size == 1 && xssss.Rows[0]["Contact"].ToString().TrimEnd() == prefixText.TrimEnd())
        {
            return null;
        }

        if (size > 256)
        {
            r.Add("The number of matched records excced than maximum, please type more characters");
        }

        //string[] array = r.ToArray();
        //System.Windows.Forms.MessageBox.Show("finish "+array.Length);
        return r.ToArray();
    }

}

