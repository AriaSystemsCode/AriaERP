*!*****************************************************************************************
*! Name      : SOORDDBR.Prg  (Custom Order detail report For BRI10)
*! Developer : MMT - Mariam Mazher
*! Date      : 12/14/2008 
*! Entry no. : C201088(A4),C201087(27) T20081111.0024 
*!*****************************************************************************************
*!Modifications :
*B611074,1 MMT 11/05/2015 Error while exporting custom order detail report to Excel[T20151030.0008]
*B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016]
*B611929,1 MMT 06/07/2022 Error while Preview/Export if status complete is selected only[T20220606.0002]
*******************************************************************************************
...
IF llFrTime
  ...
  *E302321,1 MMT 11/02/2006 Enhancement on Summerize multiStore option appearence [End]
  ...
  DIMENSION laTempStru[ALEN(laTempStru,1) + 5, 18]

  *-- cTempKey :  field used in most sort by case as the master key ,
  *--          :  and in case of summarize multi store as the total amount.

  
  laTempStru[ALEN(laTempStru,1)-4 ,1] = 'State'
  laTempStru[ALEN(laTempStru,1)-4 ,2] = 'C'
  laTempStru[ALEN(laTempStru,1)-4,3]  = 3
  laTempStru[ALEN(laTempStru,1)-4,4]  = 0
  laTempStru[ALEN(laTempStru,1)-4,17] = 0
  laTempStru[ALEN(laTempStru,1)-4,18] = 0
...
IF llClearFn OR llOGFltCh
  ...
  llUseStat  = .F.
  lnStatPos = ASCAN(loOgScroll.laOgFXFlt,"CUSTOMER.CADDRESS4")
  IF lnStatPos  > 0 
    lnStatPos  = ASUBSCRIPT(loOGScroll.laOgFxFlt,lnStatPos ,1)
    lcStateSel =IIF(!EMPTY(laOgFxFlt[lnStatPos ,6]),laOgFxFlt[lnStatPos ,6],'')
    IF !EMPTY(lcStateSel) 
      lcStatFile = loOGScroll.gfTempName()
      llUseStat  = IIF(LEN(lcStateSel)>0,.T.,.F.) AND lfConvertToCursor(lcStateSel,'STATE',lcStatFile)
      IF llUseStat  
        ...
        IF lnStatStart  > 0
           ...
           lcRpExp= STUFF(lcRpExp,lnStatStart  ,lnNumChar,".T.")
        ENDIF 
      ENDIF     
    ENDIF   
  ENDIF   
  ...
  llUseClass  = .F.
  lnClssPos = ASCAN(loOgScroll.laOgFXFlt,"CUSTOMER.CLASS")
  IF lnClssPos> 0 
    lnClssPos= ASUBSCRIPT(loOGScroll.laOgFxFlt,lnClssPos,1)
    lcClassSel =IIF(!EMPTY(laOgFxFlt[lnClssPos,6]),laOgFxFlt[lnClssPos,6],'')
    IF !EMPTY(lcClassSel) 
      lcClassFile= loOGScroll.gfTempName()
      llUseClass = IIF(LEN(lcClassSel)>0,.T.,.F.) AND lfConvertToCursor(lcClassSel,'CLASS',lcClassFile)
      IF llUseClass
        lnClassStart = AT('INLIST(CUSTOMER.CLASS',lcRpExp)
        IF lnClassStart > 0
           lnEndPos = AT(")",SUBSTR(lcRpExp,lnClassStart))+lnClassStart-1
           lnNumChar = lnEndPos -lnClassStart +1
           lcRpExp= STUFF(lcRpExp,lnClassStart  ,lnNumChar,".T.")
        ENDIF 
      ENDIF     
    ENDIF   
  ENDIF   
  ...
  llUseRep1  = .F.
  lnRep1Pos = ASCAN(loOgScroll.laOgFXFlt,"ORDHDR.REP1")
  IF lnRep1Pos > 0 
    lnRep1Pos = ASUBSCRIPT(loOGScroll.laOgFxFlt,lnRep1Pos ,1)
    lcRep1Sel =IIF(!EMPTY(laOgFxFlt[lnRep1Pos,6]),laOgFxFlt[lnRep1Pos,6],'')
    IF !EMPTY(lcRep1Sel)  AND USED(lcRep1Sel)
      SELECT (lcRep1Sel)
      LOCATE 
      IF !EOF()
        llUseRep1  = .T.
  ...
  llUseRep2  = .F.
  lnRep2Pos = ASCAN(loOgScroll.laOgFXFlt,"ORDHDR.REP2")
  IF lnRep2Pos > 0 
    lnRep2Pos = ASUBSCRIPT(loOGScroll.laOgFxFlt,lnRep2Pos ,1)
    lcRep2Sel =IIF(!EMPTY(laOgFxFlt[lnRep2Pos,6]),laOgFxFlt[lnRep2Pos,6],'')
    IF !EMPTY(lcRep2Sel)  AND USED(lcRep2Sel)
      SELECT (lcRep2Sel)
      LOCATE 
      IF !EOF()
        llUseRep2  = .T.
  ...
  ldSEntered = {}
  ldEEntered = {}
  lnEnteredPos = ASCAN(loOgScroll.laOgFXFlt,"ORDHDR.ENTERED")
  IF lnEnteredPos > 0 
    lnEnteredPos = ASUBSCRIPT(loOGScroll.laOgFxFlt,lnEnteredPos,1)
    lcEnteredSel =IIF(!EMPTY(laOgFxFlt[lnEnteredPos,6]),laOgFxFlt[lnEnteredPos,6],'')
    IF !EMPTY(lcEnteredSel)  
      ldSEntered = CTOD(SUBSTR(laOGFxFlt[lnEnteredPos ,6],1,10))
      ldEEntered = CTOD(SUBSTR(laOGFxFlt[lnEnteredPos ,6],12,21))
      IF !EMPTY(SUBSTR(laOGFxFlt[lnEnteredPos ,6],1,10)) AND !EMPTY(SUBSTR(laOGFxFlt[lnEnteredPos ,6],12,21))
        llEntred = .T.
      ENDIF 
    ENDIF   
  ENDIF   
  ...
  *B802444,1 Depend on both sides Flag when collecting data [End  ]
  IF 'K' $ lcRpStatus
    lcStatusEx = [ORDHDR.STATUS $ lcRpStatus Or (ORDHDR.STATUS $ "OH" AND OrdHdr.Ship > 0)]
  ELSE
    IF 'O' $ lcRpStatus OR 'H' $ lcRpStatus
      lcStatusEx = [ORDHDR.STATUS $ lcRpStatus AND IIF(ORDHDR.STATUS $ "OH",OrdHdr.Ship = 0,.T.)]
    *B611929,1 MMT 06/07/2022 Error while Preview/Export if status complete is selected only[T20220606.0002][Start]
    ELSE
        lcStatusEx = [ORDHDR.STATUS $ lcRpStatus]
    *B611929,1 MMT 06/07/2022 Error while Preview/Export if status complete is selected only[T20220606.0002][End]  
    ENDIF   
  ENDIF   
...
*!*	  SET FILTER TO (OrdHdr.cOrdType = 'T') ;
...
IF TYPE('lcNewExp')='U' .OR. EMPTY(lcNewExp)
  SET FILTER TO &&(OrdHdr.cOrdType = 'T') .AND.  OrdHdr.cOrdType = 'T' 
ELSE
  SET FILTER TO &lcNewExp
ENDIF 
*B606069,1 RAE [End]
*B803742,1 (End)
...
IF lcRpSortBy = 'T'    
  lcOrderName = ORDER()
  lcIndexFld = KEY()
  lcNewIndFld = "State+"+ lcIndexFld
  INDEX ON &lcNewIndFld TAG &lcOrderName 
ENDIF 
...
ELSE
  ...
  **Export to Excel
  IF !USED("invline")
    =gfOPenTable('Invline',"INVLINEO",'Sh')
  ENDIF 
  ...
  DIMENSION laHeadStr[7,4]
  laHeadStr[1,1] = 'cOrdType'
  laHeadStr[1,2] = 'C'
  laHeadStr[1,3] = 1
  laHeadStr[1,4] = 0
  ...
  laHeadStr[2,3] = 6
  laHeadStr[2,4] = 0
  *B611074,1 MMT 11/05/2015 Error while exporting custom order detail report to Excel[T20151030.0008][Start]
*!*	  laHeadStr[3,1] = 'ShipQty'
*!*	  laHeadStr[3,2] = 'N'
*!*	  laHeadStr[3,3] = 9
*!*	  laHeadStr[3,4] = 0
*!*	  
...
*!*	  laHeadStr[4,4] = 0
*!*	    
*!*	  laHeadStr[5,1] = 'OPenAmt'
*!*	  laHeadStr[5,2] = 'N'
*!*	  laHeadStr[5,3] = 9
*!*	  laHeadStr[5,4] = 2
*!*	  
*!*	  laHeadStr[6,1] = 'BOOKQTY'
*!*	  laHeadStr[6,2] = 'N'
*!*	  laHeadStr[6,3] = 9
*!*	  laHeadStr[6,4] = 0
  laHeadStr[3,1] = 'ShipQty'
  ...
  laHeadStr[6,3] = 12
  laHeadStr[6,4] = 0
  *B611074,1 MMT 11/05/2015 Error while exporting custom order detail report to Excel[T20151030.0008][End]  
  
  laHeadStr[7,1] = 'nLINES'
  laHeadStr[7,2] = 'N'
  laHeadStr[7,3] = 5
  laHeadStr[7,4] = 0
  
  = gfCrtTmp(lcHeadTemp ,@laHeadStr,"cOrdType+Order",lcHeadTemp ,.T.)
  

  x= createobject('Excel.Application') 
  z=x.Workbooks.Add ()
  DO WHILE  z.Sheets.Count > 2
    z.Sheets(z.Sheets.Count).DELETE()
  ENDDO 
  ...
  z.Sheets(1).sELECT() 
  z.Sheets(1).nAME = 'Summary'
  
  z.Sheets(2).sELECT() 
  z.Sheets(2).nAME = 'Details'
  
  SELECT (lcMastFile)
  LOCATE 
  z.Sheets(2).sELECT() 
  z.Sheets(2).Cells(2,1).Value = 'Start'
  z.Sheets(2).Cells(2,1).Font.Bold = .T.
  z.Sheets(2).Cells(2,2).Value = 'Cancel'
  z.Sheets(2).Cells(2,3).Value = 'Customer Name'
  z.Sheets(2).Cells(2,4).Value = 'Account'
  z.Sheets(2).Cells(2,5).Value = 'Store #'
  z.Sheets(2).Cells(2,6).Value = 'Order#'
  z.Sheets(2).Cells(2,7).Value = 'Type'
  z.Sheets(2).Cells(2,8).Value = 'Status'
  z.Sheets(2).Cells(2,9).Value = 'L'
  lnCont = 10
  IF llRpSizes 
    z.Sheets(2).Cells(2,lnCont ).Value = 'Size'
    lnCont = lnCont +  1
  ENDIF 
  z.Sheets(2).Cells(2,lnCont ).Value = 'Booked'
  z.Sheets(2).Cells(2,lnCont +1).Value = 'Shipped'
  z.Sheets(2).Cells(2,lnCont +2).Value = 'Open'
  z.Sheets(2).Cells(2,lnCont +3).Value = 'Cat'
  z.Sheets(2).Cells(2,lnCont +4).Value = 'Item#'
  z.Sheets(2).Cells(2,lnCont +5).Value = 'Item Description'
  z.Sheets(2).Cells(2,lnCont +6).Value = 'Cost'
  z.Sheets(2).Cells(2,lnCont +7).Value = 'Total $'
  z.Sheets(2).Cells(2,lnCont +8).Value = 'Notes'
  z.Sheets(2).Cells(2,lnCont +9).Value = 'PO#'
  z.Sheets(2).Cells(2,lnCont +10).Value = 'Entered'
  z.Sheets(2).Cells(2,lnCont +11).Value = 'Rep1'
  z.Sheets(2).Cells(2,lnCont +12).Value = 'Rep2'
  z.Sheets(2).Cells(2,lnCont +13).Value = 'Reason'
  z.Sheets(2).Cells(2,lnCont +14).Value = 'Customer Class'
  z.Sheets(2).Cells(2,lnCont +15).Value = 'State'
  
  z.Sheets(2).Cells(2,2).Font.Bold = .T.
  z.Sheets(2).Cells(2,3).Font.Bold = .T.
  z.Sheets(2).Cells(2,4).Font.Bold = .T.
  z.Sheets(2).Cells(2,5).Font.Bold = .T.
  z.Sheets(2).Cells(2,6).Font.Bold = .T.
  z.Sheets(2).Cells(2,7).Font.Bold = .T.
  z.Sheets(2).Cells(2,8).Font.Bold = .T.
  z.Sheets(2).Cells(2,9).Font.Bold = .T.
  z.Sheets(2).Cells(2,10).Font.Bold = .T.
  z.Sheets(2).Cells(2,11).Font.Bold = .T.
  z.Sheets(2).Cells(2,12).Font.Bold = .T.
  z.Sheets(2).Cells(2,13).Font.Bold = .T.
  z.Sheets(2).Cells(2,14).Font.Bold = .T.
  z.Sheets(2).Cells(2,15).Font.Bold = .T.
  z.Sheets(2).Cells(2,16).Font.Bold = .T.
  z.Sheets(2).Cells(2,17).Font.Bold = .T.
  z.Sheets(2).Cells(2,18).Font.Bold = .T.
  z.Sheets(2).Cells(2,19).Font.Bold = .T.
  z.Sheets(2).Cells(2,20).Font.Bold = .T.
  z.Sheets(2).Cells(2,21).Font.Bold = .T.
  z.Sheets(2).Cells(2,22).Font.Bold = .T.
  z.Sheets(2).Cells(2,23).Font.Bold = .T.
  z.Sheets(2).Cells(2,24).Font.Bold = .T.
  z.Sheets(2).Cells(2,25).Font.Bold = .T.
  z.Sheets(2).Cells(2,26).Font.Bold = .T.
  lnStartRow = 3
  *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][Start]
  oPross = CREATEOBJECT('ariaprogressbar')
  oPross.TotalProgress = RECCOUNT()
  oPross.AUTOCENTER = .T.
  oPross.Show()
  lnCntRec = 0
  *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][End]
  
  
  SCAN 
    *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][Start]
    *WAIT WINDOW 'Exporting Order Details to Excel' NOWAIT 
    lnCntRec = lnCntRec + 1
    oPross.CurrentProgress(lnCntRec)
    oPross.lblFirstLabel.CAPTION = 'Exporting Order Details: Order# '+order +" Style# "+ Style 
    oPross.Show()
    *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][End]
    IF !SEEK(cordtype+order,lcHeadTemp)
      m.cOrdType = cordtype
      m.Order = order
      INSERT INTO (lcHeadTemp) FROM MEMVAR 
    ENDIF 
   	IF llRpSizes 
      ...
      FOR lnY = 1 TO Scale.cnt
        ...
        lcStyle = Style 
        lnLineNum = LineNo
        lnShipQty = 0 &&m.ShipQty = 
        STORE 0 TO m.Shp&lcY.
        IF gfSeek(lcOrder,'invline')
          SELECT invline
          SCAN REST WHILE ORDER+STR(LINENO,6)+INVOICE = lcOrder FOR;
          				  Style = lcStyle And LineNo = lnLineNum AND gfSeek(Invline.Invoice,'InvHdr') AND InvHdr.Status <> 'V'
            lnShipQty = lnShipQty +  Qty&lcY.
            m.Shp&lcY. = m.Shp&lcY. + Qty&lcY.
          ENDSCAN 
          SELECT (lcMastFile)
          =SEEK(cordtype+order,lcHeadTemp)
          REPLACE ShipQty WITH ShipQty +lnShipQty  IN (lcHeadTemp)
        ENDIF 
        SELECT (lcMastFile)
        =SEEK(cordtype+order,lcHeadTemp)
        lnOPen = Qty&lcY.
        ...
        REPLACE OPen WITH OPen +lnOPen ,;
          BOOKQTY WITH BOOKQTY   + lnBook ,;
          OPenAmt WITH OPenAmt +  lnOPen * lnPrice ,;
          nLINES WITH nLINES+1 IN (lcHeadTemp)
          
        =SEEK(cordtype+order,'Ordhdr','Ordhdr')
        =SEEK("M"+Ordhdr.Account,'Customer','Customer')
        z.Sheets(2).Cells(lnStartRow ,1).Value = DTOC(Ordhdr.Start)
        z.Sheets(2).Cells(lnStartRow ,2).Value = DTOC(Ordhdr.Complete)
        
        IF !EMPTY(Store)
          =SEEK("S"+Account+Store,'Customer','Customer')
        ENDIF 
        z.Sheets(2).Cells(lnStartRow ,3).Value = Customer.StnAme
        z.Sheets(2).Cells(lnStartRow ,4).Value = Customer.Account
        z.Sheets(2).Cells(lnStartRow ,5).Value = Store
        z.Sheets(2).Cells(lnStartRow ,6).Value = Ordhdr.order
        z.Sheets(2).Cells(lnStartRow ,7).Value = IIF(Ordhdr.Multi = 'N',"Single",'Multi')
        =SEEK("M"+Ordhdr.Account,'Customer','Customer')
        IF Ordhdr.SHip >0 and ordhdr.Status $ "OH"
          z.Sheets(2).Cells(lnStartRow ,8).Value ='Backorder'
        ELSE
          DO CASE 
            CASE ordhdr.Status = "O"
              z.Sheets(2).Cells(lnStartRow ,8).Value ='Open'
            CASE ordhdr.Status = "H"
              z.Sheets(2).Cells(lnStartRow ,8).Value ='Hold'
            CASE ordhdr.Status = "C"
              z.Sheets(2).Cells(lnStartRow ,8).Value ='Complete'
            CASE ordhdr.Status = "X"
              z.Sheets(2).Cells(lnStartRow ,8).Value ='Canceled'
            CASE ordhdr.Status = "B"
              z.Sheets(2).Cells(lnStartRow ,8).Value ='Bid'
          ENDCASE     
        ENDIF 
        z.Sheets(2).Cells(lnStartRow ,9).Value =STR(LineNo)
        
        
        ****
        lnCont = 10
        IF llRpSizes 
          z.Sheets(2).Cells(lnStartRow ,lnCont ).Value = Scale.Sz&lcY.
          lnCont = lnCont +  1
        ENDIF 
        *****
        
        z.Sheets(2).Cells(lnStartRow ,lnCont ).Value =STR(BOOK&lcY.)
        z.Sheets(2).Cells(lnStartRow ,lnCont +1).Value =STR(m.Shp&lcY.)
        
        IF ordhdr.Status = "X"
          z.Sheets(2).Cells(lnStartRow ,lnCont +2).Value =STR(0)  
        ELSE
          z.Sheets(2).Cells(lnStartRow ,lnCont +2).Value =STR(Qty&lcY.)  
        ENDIF   
        
        =SEEK(Style,"Style","Style")
        z.Sheets(2).Cells(lnStartRow ,lnCont +3).Value =STyle.cStyGroup
        z.Sheets(2).Cells(lnStartRow ,lnCont +4).Value =STyle
        z.Sheets(2).Cells(lnStartRow ,lnCont +5).Value =desc1 
        z.Sheets(2).Cells(lnStartRow ,lnCont +6).Value =STR(Price,8,2)
        IF ordhdr.Status = "X"
          z.Sheets(2).Cells(lnStartRow ,lnCont +7).Value =STR(0,9,2)
        ELSE
          z.Sheets(2).Cells(lnStartRow ,lnCont +7).Value =STR(Qty&lcY. * Price,9,2)
        ENDIF   
        z.Sheets(2).Cells(lnStartRow ,lnCont +8).Value =Ordhdr.Note1
        z.Sheets(2).Cells(lnStartRow ,lnCont +9).Value =Ordhdr.CustPO
        z.Sheets(2).Cells(lnStartRow ,lnCont +10).Value =DTOC(Ordhdr.Entered)
        z.Sheets(2).Cells(lnStartRow ,lnCont +11).Value =Ordhdr.Rep1
        z.Sheets(2).Cells(lnStartRow ,lnCont +12).Value =Ordhdr.Rep2    
        z.Sheets(2).Cells(lnStartRow ,lnCont +13).Value =gfCodDes(Ordhdr.DECL_CODE,'DECL_CODE')
        z.Sheets(2).Cells(lnStartRow ,lnCont +14).Value =gfCodDes(Customer.Class,'CLASS')    
        z.Sheets(2).Cells(lnStartRow ,lnCont +15).Value = State  
      ...
      IF gfSeek(lcOrder,'invline')
        SELECT invline
        SCAN REST WHILE ORDER+STR(LINENO,6)+INVOICE = lcOrder FOR Style = lcStyle And LineNo = lnLineNum AND gfSeek(Invline.Invoice,'InvHdr') AND InvHdr.Status <> 'V'
      ...
      *****
      ...
      z.Sheets(2).Cells(lnStartRow ,lnCont ).Value =STR(TOTBOOK)
      z.Sheets(2).Cells(lnStartRow ,lnCont +1).Value =STR(lnShipQty)
      
      IF ordhdr.Status = "X"
        z.Sheets(2).Cells(lnStartRow ,lnCont +2).Value =STR(0)  
      ELSE
        z.Sheets(2).Cells(lnStartRow ,lnCont +2).Value =STR(totQty)  
      ENDIF   
      ...
      IF ordhdr.Status = "X"
        z.Sheets(2).Cells(lnStartRow ,lnCont +7).Value =STR(0,9,2)
      ELSE
        z.Sheets(2).Cells(lnStartRow ,lnCont +7).Value =STR(totQty*Price,9,2)
      ENDIF   
      z.Sheets(2).Cells(lnStartRow ,lnCont +8).Value =Ordhdr.Note1
      z.Sheets(2).Cells(lnStartRow ,lnCont +9).Value =Ordhdr.CustPO
      z.Sheets(2).Cells(lnStartRow ,lnCont +10).Value =DTOC(Ordhdr.Entered)
  ...
  z.Sheets(1).Cells(2,1).Value = 'Start'
  z.Sheets(1).Cells(2,1).Font.Bold = .T.
  z.Sheets(1).Cells(2,2).Value = 'Cancel'
  z.Sheets(1).Cells(2,3).Value = 'Customer Name'
  z.Sheets(1).Cells(2,4).Value = 'Account'
  z.Sheets(1).Cells(2,5).Value = 'Order#'
  z.Sheets(1).Cells(2,6).Value = 'Type'
  z.Sheets(1).Cells(2,7).Value = 'Status'
  z.Sheets(1).Cells(2,8).Value = 'L'
  z.Sheets(1).Cells(2,9).Value = 'Booked'
  z.Sheets(1).Cells(2,10).Value = 'Shipped'
  z.Sheets(1).Cells(2,11).Value = 'Open'
  z.Sheets(1).Cells(2,12).Value = 'Total $'
  z.Sheets(1).Cells(2,13).Value = 'Notes'
  z.Sheets(1).Cells(2,14).Value = 'Entered'
  z.Sheets(1).Cells(2,15).Value = 'PO#'
  z.Sheets(1).Cells(2,16).Value = 'Rep1'
  z.Sheets(1).Cells(2,17).Value = 'Rep2'
  z.Sheets(1).Cells(2,18).Value = 'Hold Reason'
  z.Sheets(1).Cells(2,19).Value = 'Customer Class'
  
  z.Sheets(1).Cells(2,2).Font.Bold = .T.
  z.Sheets(1).Cells(2,3).Font.Bold = .T.
  z.Sheets(1).Cells(2,4).Font.Bold = .T.
  z.Sheets(1).Cells(2,5).Font.Bold = .T.
  z.Sheets(1).Cells(2,6).Font.Bold = .T.
  z.Sheets(1).Cells(2,7).Font.Bold = .T.
  z.Sheets(1).Cells(2,8).Font.Bold = .T.
  z.Sheets(1).Cells(2,9).Font.Bold = .T.
  z.Sheets(1).Cells(2,10).Font.Bold = .T.
  z.Sheets(1).Cells(2,11).Font.Bold = .T.
  z.Sheets(1).Cells(2,12).Font.Bold = .T.
  z.Sheets(1).Cells(2,13).Font.Bold = .T.
  z.Sheets(1).Cells(2,14).Font.Bold = .T.
  z.Sheets(1).Cells(2,15).Font.Bold = .T.
  z.Sheets(1).Cells(2,16).Font.Bold = .T.
  z.Sheets(1).Cells(2,17).Font.Bold = .T.
  z.Sheets(1).Cells(2,18).Font.Bold = .T.
  z.Sheets(1).Cells(2,19).Font.Bold = .T.
  *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][Start]
  lnCntRec = 0
  oPross.CurrentProgress(0)
  oPross.lblFirstLabel.CAPTION = ''  
  *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][End]
  lnStartRow = 3
  SELECT (lcHeadTemp)
  SCAN 
    *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][Start]
    *WAIT WINDOW 'Exporting Order Summary to Excel' NOWAIT 
    lnCntRec = lnCntRec + 1
    oPross.CurrentProgress(lnCntRec)
    oPross.lblFirstLabel.CAPTION = 'Exporting Order Summary: Order# '+order 
    oPross.Show()
    *B611097,1 MMT 12/21/2015 Custom Order detail report hangs with back order filter[T20151207.0016][End]
    =SEEK(cordtype+order,'Ordhdr','Ordhdr')
    =SEEK("M"+Ordhdr.Account,'Customer','Customer')
    z.Sheets(1).Cells(lnStartRow ,1).Value = DTOC(Ordhdr.Start)
    z.Sheets(1).Cells(lnStartRow ,2).Value = DTOC(Ordhdr.Complete)
    z.Sheets(1).Cells(lnStartRow ,3).Value = Customer.stnAme
    z.Sheets(1).Cells(lnStartRow ,4).Value = Customer.Account
    z.Sheets(1).Cells(lnStartRow ,5).Value = Ordhdr.order
    z.Sheets(1).Cells(lnStartRow ,6).Value = IIF(Ordhdr.Multi = 'N',"Single",'Multi')
    IF Ordhdr.SHip >0 and ordhdr.Status $ "OH"
      z.Sheets(1).Cells(lnStartRow ,7).Value ='Backorder'
    ELSE
      DO CASE 
        CASE ordhdr.Status = "O"
          z.Sheets(1).Cells(lnStartRow ,7).Value ='Open'
        CASE ordhdr.Status = "H"
          z.Sheets(1).Cells(lnStartRow ,7).Value ='Hold'
        CASE ordhdr.Status = "C"
          z.Sheets(1).Cells(lnStartRow ,7).Value ='Complete'
        CASE ordhdr.Status = "X"
          z.Sheets(1).Cells(lnStartRow ,7).Value ='Canceled'
        CASE ordhdr.Status = "B"
          z.Sheets(1).Cells(lnStartRow ,7).Value ='Bid'
      ENDCASE     
    ENDIF 
    z.Sheets(1).Cells(lnStartRow ,8).Value = STR(nLINES)
    z.Sheets(1).Cells(lnStartRow ,9).Value = STR(BOOKQTY)
    z.Sheets(1).Cells(lnStartRow ,10).Value = STR(ShipQty)
    IF ordhdr.Status = "X"
      z.Sheets(1).Cells(lnStartRow ,11).Value = STR(0)
      z.Sheets(1).Cells(lnStartRow ,12).Value = STR(0,9,2)
    ELSE
      z.Sheets(1).Cells(lnStartRow ,11).Value = STR(OPen)
      z.Sheets(1).Cells(lnStartRow ,12).Value = STR(OPenAmt,9,2)
    ENDIF   
    z.Sheets(1).Cells(lnStartRow ,13).Value = Ordhdr.Note1
    z.Sheets(1).Cells(lnStartRow ,14).Value = DTOC(Ordhdr.entered)
    z.Sheets(1).Cells(lnStartRow ,15).Value = Ordhdr.CustPO
    z.Sheets(1).Cells(lnStartRow ,16).Value = Ordhdr.Rep1
    z.Sheets(1).Cells(lnStartRow ,17).Value = Ordhdr.Rep2
    z.Sheets(1).Cells(lnStartRow ,18).Value = gfCodDes(Ordhdr.DECL_CODE,'DECL_CODE')
    z.Sheets(1).Cells(lnStartRow ,19).Value = gfCodDes(Customer.Class,'CLASS') 
    lnStartRow = lnStartRow +  1
  ENDSCAN 
*  z.SaveAs (oAriaApplication.WorkDir+lcDetailTemp+'.XLS')
  x.Visible = .t.
*!*	  laHeadStr[3,1] = 'Start'
*!*	  laHeadStr[3,2] = 'D'
*!*	  laHeadStr[3,3] = 8
...
*!*	  laHeadStr[4,1] = 'Cancel'
*!*	  laHeadStr[4,2] = 'D'
*!*	  laHeadStr[4,3] = 8
*!*	  laHeadStr[4,4] = 0
*!*	  
*!*	  laHeadStr[5,1] = 'BtName'
*!*	  laHeadStr[5,2] = 'C'
*!*	  laHeadStr[5,3] = 30
*!*	  laHeadStr[5,4] = 0
*!*	  
*!*	  laHeadStr[6,1] = 'Account'
*!*	  laHeadStr[6,2] = 'C'
*!*	  laHeadStr[6,3] = 6
*!*	  laHeadStr[6,4] = 0
*!*	  
...
*!*	  laHeadStr[8,1] = 'Status'
*!*	  laHeadStr[8,2] = 'C'
*!*	  laHeadStr[8,3] = 15
*!*	  laHeadStr[8,4] = 0
*!*	  
*!*	  laHeadStr[9,1] = 'ItmPOrd'
*!*	  laHeadStr[9,2] = 'N'
*!*	  laHeadStr[9,3] = 6
*!*	  laHeadStr[9,4] = 0

*!*	  laHeadStr[10,1] = 'TOTBOOK'
*!*	  laHeadStr[10,2] = 'N'
*!*	  laHeadStr[10,3] = 7
*!*	  laHeadStr[10,4] = 0
*!*	  
*!*	  laHeadStr[11,1] = 'Shipped'
*!*	  laHeadStr[11,2] = 'N'
*!*	  laHeadStr[11,3] = 7
*!*	  laHeadStr[11,4] = 0
*!*	  
...
*!*	  laHeadStr[12,4] = 0
*!*	  
*!*	  laHeadStr[13,1] = 'StyGrp'
*!*	  laHeadStr[13,2] = 'C'
*!*	  laHeadStr[13,3] = 6
*!*	  laHeadStr[13,4] = 0
*!*	  
*!*	  laHeadStr[14,1] = 'CStyMaj'
*!*	  laHeadStr[14,2] = 'C'
*!*	  laHeadStr[14,3] = 19
*!*	  laHeadStr[14,4] = 0
*!*	  
*!*	  laHeadStr[15,1] = 'StyDesc'
*!*	  laHeadStr[15,2] = 'C'
*!*	  laHeadStr[15,3] = 30
*!*	  laHeadStr[15,4] = 0
*!*	  
*!*	  laHeadStr[16,1] = 'Price'
*!*	  laHeadStr[16,2] = 'N'
*!*	  laHeadStr[16,3] = 7
*!*	  laHeadStr[16,4] = 2
*!*	  
*!*	  laHeadStr[17,1] = 'TOTAMT'
*!*	  laHeadStr[17,2] = 'N'
*!*	  laHeadStr[17,3] = 9
*!*	  laHeadStr[17,4] = 2
*!*	  
*!*	  laHeadStr[18,1] = 'NOTES'
*!*	  laHeadStr[18,2] = 'M'
*!*	  laHeadStr[18,3] = 10
*!*	  laHeadStr[18,4] = 0
*!*	  
...
*!*	  laHeadStr[23,1] = 'REASON'
*!*	  laHeadStr[23,2] = 'C'
*!*	  laHeadStr[23,3] = 30
*!*	  laHeadStr[23,4] = 0
*!*	  
*!*	  laHeadStr[24,1] = 'Class'
*!*	  laHeadStr[24,2] = 'C'
*!*	  laHeadStr[24,3] = 6
*!*	  laHeadStr[24,4] = 0
*!*	  
*!*	  laHeadStr[25,1] = 'State'
*!*	  laHeadStr[25,2] = 'C'
*!*	  laHeadStr[25,3] = 3
*!*	  laHeadStr[25,4] = 0
*!*	  
...
IF TYPE('lnVarbEnd') = 'C'
  ...
  STORE 'Backorder' TO laRpSource[6]
  lcRpStatus = 'BOHCXK'
...
IF !EMPTY(laRpTarget[1])
  FOR lnI = 1 TO ALEN(laRpTarget,1)
    lcRpStatus = lcRpStatus + IIF(laRpTarget[lnI] = 'Bid','B',;
                              ...
                              IIF(laRpTarget[lnI] = 'Backorder','K',''))))))
  ENDFOR
ENDIF
...
PROCEDURE lpChkStat
PRIVATE llStatus
llStatus = IIF('O' $ lcRpStatus OR 'H' $ lcRpStatus OR 'K' $ lcRpStatus,.T.,.F.)
IF !llStatus
  llRpPrtShp = .F.
  llRpAloOrd = .T.
ENDIF
      ...
      SCAN REST WHILE cOrdType+Order+STR(LineNo,6) = cOrdType+&lcTOrdFile..Order
        ...
        *-- insert this data into workfile.
        IF IIF(llRpCorrGp ,!EMPTY(Group),.T.) AND ;
           ...
           IIF(llRpPrtShp,IIF(OrdHdr.Status $ 'BCX',.F.,.T.),.T.) AND ;
          ...
          IF ordhdr.multi = 'N'
            IF ordhdr.alt_shpto 
              lnFrstComm = ATc(',',ordhdr.caddress3,1)
              lnSecondComm =ATc(',',ordhdr.caddress3,2)
              m.State = SUBSTR(ordhdr.caddress3,lnFrstComm +1,lnSecondComm -lnFrstComm-1)
            ELSE
              IF !EMPTY(Ordhdr.Store)
                ...
                ELSE
                  ...
                  IF !EMPTY(Customer.Dist_ctr)
                    lcDcCentr = Customer.Dist_ctr
                    =SEEK('S'+Ordhdr.Account+lcDcCentr,'CUSTOMER')
                    m.State = Customer.Caddress4
                  ELSE
                    m.State = Customer.Caddress4
                  ENDIF 
          ...
          IF llUseStat AND (!SEEK(ALLTRIM(m.State),lcStatFile) OR EMPTY(m.State))
            LOOP 
          ENDIF 
          
          
          IF llUseClass AND SEEK("M"+Ordhdr.Account ,'Customer') AND !SEEK(Customer.Class,lcClassFile)
            LOOP 
          ENDIF 
          
          
          IF (llUseRep2 AND llUseRep1) 
            IF !SEEK(ORDHDR.REP1,lcRep1Sel) AND !SEEK(ORDHDR.REP2,lcRep2Sel)
              LOOP
            ENDIF 
          ELSE   
            IF llUseRep1 AND !SEEK(ORDHDR.REP1,lcRep1Sel)
              LOOP
            ENDIF 

            IF llUseRep2 AND !SEEK(ORDHDR.REP2,lcRep2Sel)
              LOOP
            ENDIF 
          ENDIF 
          
          IF llEntred AND !BETWEEN(ORDHDR.eNTERED,ldSEntered,ldEEntered)
            LOOP 
          ENDIF 
...
IF !llRpPrTPak
  lcRpForm = "SORDDGMP"
ELSE
  lcRpForm = IIF(llRpSizes,'sorddbra','sorddbrb')
ENDIF
= lfRepPltFr(lcRpForm)
llRpScale = llRpSizes
ClearRead()
 ...
 ENDIF
    ...
    *C102262,1 (End)
    lcOutGrp  = [CCURRCODE]
     ...
     CASE lcRpSortBy = 'T'    && Sort by State
       lcInnFootL = ['']
       lcSubTitle = 'State Code'
       lcInnGrp   = ['']
        DO CASE
      CASE lcRpSrt2 = 'L'    && Sort by Line#
        IF llCurInGrp
...
*!*          CASE lcRpSrt2 = 'L'    && Sort by Line#
*!*             IF llCurInGrp
*!*               lcIndexTg = 'CCURRCODE+State+CORDTYPE+ORDER+STR(LINENO,6)+STYLE'
*!*  	         ELSE
*!*      	      lcIndexTg = 'State+CORDTYPE+ORDER+STR(LINENO,6)+STYLE'
*!*             ENDIF
*!*          CASE lcRpSrt2 = 'S'    && Sort by Style
*!*       	    IF llCurInGrp
*!*           	  lcIndexTg = 'CCURRCODE+State+CORDTYPE+ORDER+STYLE+STR(LINENO,6)'
*!*  	        ELSE
*!*              lcIndexTg = 'State+CORDTYPE+ORDER+STYLE+STR(LINENO,6)'
*!*  	        ENDIF  

*!*        	CASE lcRpSrt2 $ 'TY'    
*!*          
*!*            IF llCurInGrp
*!*              lcIndexTg = 'CCURRCODE+State+CORDTYPE+ORDER+STORE'
*!*            ELSE
*!*              lcIndexTg = 'State+CORDTYPE+ORDER+STORE'
*!*            ENDIF
*!*          
*!*          *-- Sort by Store/Line#
*!*            IF lcRpSrt2="T"
...
*!*            IF llCurInGrp
*!*              lcIndexTg = 'CCURRCODE+State+CORDTYPE+ORDER+DIST_CTR+STORE+STYLE'
*!*            ELSE
*!*              lcIndexTg = 'State+CORDTYPE+ORDER+DIST_CTR+STORE+STYLE'
*!*            ENDIF
*!*            
*!*            lcStorCond = IIF(llTextMode,[.T.],[ORDHDR.MULTI="Y"])
*!*            lcInnGrp   = [IIF(EVALUATE(lcStorCond),STORE,"")]
...
*!*      ENDIF  
*!*      IF llCurInGrp
*!*        lcSeekVal = [cCurrCode+State+cOrdType+ORDER]
*!*        lcOutGrp  = [CCURRCODE+State+ORDER]
*!*      ELSE
*!*        lcSeekVal = [State+cOrdType+ORDER]
*!*        lcOutGrp  = [State+ORDER]
*!*      ENDIF  
...
IF llCurInGrp AND !(lcRpSortBy $ 'TU')
...
IF !EMPTY(laRpTarget[1])
  FOR lnI = 1 TO ALEN(laRpTarget,1)
    lcRpStatus = lcRpStatus + IIF(laRpTarget[lnI] = 'Bid','B',;
                              ...
                              IIF(laRpTarget[lnI] = 'Hold','H',;
                              IIF(laRpTarget[lnI] = 'Complete','C',;                                                            
                              IIF(laRpTarget[lnI] = 'Cancelled','X',IIF(laRpTarget[lnI] = 'Backorder','K',''))))))
  ENDFOR  && end Loop to make Status expression.
ENDIF

lcRpStatus = IIF(EMPTY(lcRpStatus),'BOHCXK',ALLTRIM(lcRpStatus))
...
DO CASE
  *-- Contracts 
  CASE lcRpEdiPrn="C"
    lcRpEdiFlt = [OrdHdr.cOrdType = 'C' AND !OrdHdr.lEdiOrder]
    lcRpOrdTyp = 'C'
  * -- Orders
  CASE lcRpEdiPrn="O"
