...
public class CustomerStoreAutoComplete : System.Web.Services.WebService
{

    public CustomerStoreAutoComplete()
    {
    ...
    public string[] GetCompletionList(string prefixText, int count)
    {
        ...
        ((AriaStandardDataType)condition.LeftHandSide).PropertyDataPathDictionary.Add("Value", "Store"); // Field
        ...
        DataTable xssss = businessObject.GetBusinessObjects("", "", "99", "CRM.Customer.Store", conditionList);
        ...
        for (int i = 0; i < size && i < 256; i++)
        {
            ...
            string s = Formatter.FormateString(0, rowItems[1].ToString().Trim());

            for (int j = 2; j < rowItems.GetLength(0); j++)
            {
        ...
        if (size == 1 && xssss.Rows[0]["Store"].ToString().TrimEnd() == prefixText.TrimEnd())
        {
            return null;
        }
