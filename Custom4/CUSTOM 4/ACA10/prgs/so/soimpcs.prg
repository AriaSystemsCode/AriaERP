...
CREATE CURSOR (lcSOCSVTmp) ;
 (order C(6), season C(6), cdivision C(6), cwarecode C(6), ;
  ...
  entered D(8), start D(8), complete D(8), StName C(30),  caddress1 C(30), ;
  caddress2 C(30), caddress3 C(30), caddress4 C(30), caddress5 C(30), ;
  shipvia C(30), disc N(5), ctermcode C(30), lineno N(6), style C(19), ;
  color C(6),qty1 N(6),qty2 N(6),qty3 N(6),qty4 N(6),qty5 N(6),qty6 N(6),qty7 N(6),qty8 N(6),;
  totqty C(7), price N(12,2))
...
SELECT DISTINCT Order, style,color FROM (lcSOCSVTmp) WHERE !DELETED() INTO CURSOR 'StyleTmp'
...
LOCATE
SCAN
  lcStyle = PADR(StyleTmp.style,lnMajLen)+lcClrSpr +PADR(StyleTmp.Color,lnClrLen )
  IF !gfSeek(lcStyle,'style','style')
   ...
   IF EOF()
     APPEND BLANK
   ENDIF
   REPLACE mStrRep WITH mStrRep +"Order#:"+ALLTRIM(StyleTmp.Order)+", Style: "+lcStyle  +" is not found in Style file"+CHR(13)+CHR(10)
  ENDIF
ENDSCAN
...
SELECT Codes
=gfSetOrder('cCode_NO')
if !gfSeek("N"+PADR(lcCodeName,10)+UPPER(ALLTRIM(lcCodeNum)))
  *XXX
  =gfSeek("N"+PADR(lcCodeName,10))
  LOCATE REST WHILE CDEFCODE+CFLD_NAME+CCODE_NO+CDISCREP+CRLTD_NAM = "N"+PADR(lcCodeName,10) FOR UPPER(ALLTRIM(CDISCREP)) = UPPER(ALLTRIM(lcCodeNum))
  IF FOUND()
  *XXX
...
*!*	  STORE 0 TO m.comm1,m.comm2,m.Nexrate,m.Disc,m.Appramt
  ...
  *C201898,1 Sara.o 11/20/2016 Custom Import SO program does not work if there is empty lines in the CSV file[T20161019.0045][End]                
  ...
  m.shipvia = lfGeCodeNO(m.ShipVia,'SHIPVIA')  
  m.SEASON= lfGeCodeNO(m.Season ,'SEASON')  
 m.CDIVISION= lfGeCodeNO(m.CDIVISION ,'CDIVISION')  
 m.CORDERCAT= lfGeCodeNO(m.Cordercat,'CORDERCAT')  
 m.CTERMCODE= lfGeCodeNO(m.ctermcode ,'CTERMCODE') 
...
*!*	       
   ...
   SUM Qty1, Qty2,Qty3,Qty4,Qty5,Qty6,Qty7,Qty8,;
       ...
       FOR Order = TmpOrder.Order  AND Account = m.Account TO M.qty1,M.qty2,M.qty3,M.qty4,M.qty5,M.qty6,M.qty7,M.qty8,m.TotAmnt 
...
lnCompDays=gfGetMemVar('M_COMPDATE')
...
*xxxxxx
   ...
   SCAN FOR ORDER = TmpOrder.Order AND Account = m.Account 
     ...
     m.Style = PADR(m.style,lnMajLen)+lcClrSpr +PADR(m.Color,lnClrLen)
     *m.Order   = lcOrder         
     *m.CwareCode = WAREHOUS.CwareCode
