...
*!**************************************************************************
*! Modifications :
*! B122012,1 HFK ,03/07/2004  Fix bug of saving number of additional users always 1.
...
*!*	ENDIF
*!*	RETURN lcConnStr
RETURN oAriaApplication.ActiveCompanyConStr
    ...
    CASE lnBarSel=6   &&Connection Information
      ...
      *DO FORM (oAriaApplication.ScreenHome+"sucsconn.SCX") WITH lcScrMode
      IF oAriaApplication.MULTIINST 
        =gfCallForm('sucsconn',.F.,"'"+lcScrMode+"'")
      ELSE
        DO FORM (oAriaApplication.ScreenHome+"sucsconn.SCX") WITH lcScrMode
      ENDIF
...
IF oAriaApplication.MULTIINST 
  lcPara = "'"+lcScrMode+"','"+lcConnString+"'"
  =gfCallForm('SUCsPro',.F.,lcPara)
ELSE
  DO FORM (oAriaApplication.ScreenHome+"SUCsPro.SCX") WITH lcScrMode,lcConnString
ENDIF
*B609488,1 WAM 01/09/2011  (End)
...
IF oAriaApplication.MULTIINST 
  =gfCallForm('SUActKey',.F.,"'"+lcScrMode+"'")
ELSE
  DO FORM (oAriaApplication.ScreenHome+"SUActKey.SCX") WITH lcScrMode
ENDIF
*B609488,1 WAM 01/09/2011 (End)
...
SCAN
  ...
  *! B037319,1 SSH Fix the bug of not save the modifications
  ENDIF
  GATHER MEMVAR
*--B122012,1 HFK - Replace the nQty field with the correct value   [Start]
**-  REPLACE Ccust_id   WITH CUSTPROF.cCust_id,;
  ...
  REPLACE Ccust_id   WITH CUSTPROF.cCust_id,;
      ...
      NEDIPUse   WITH 0,;
      LEDIMpPk   WITH .F.
*--B122012,1 HFK - Replace the nQty field with the correct value   [End]
ENDSCAN
...
*!*  ************************************************************************
*!*  Descrption    : Schedule Activity in the customer screen.
...
PRIVATE lnAlias, lnResult, lcCustFile, lcLastAct, ldLastDate, lcNextAct, ldNextDate,;
        llPending, llCompleted
        
lnAlias    = SELECT()
STORE '' TO lcCustFile, lcLastAct, lcNextAct
STORE {} TO ldLastDate, ldNextDate

*-- Open the customer file in another alias to be able to update the following fields only
*-- cLast_Actt, dLast_ActD, cLast_Actt, dLast_ActD in all modes instead of using tableupdate
*-- in the customer file. Because the schedule screen is used in all modes, so we can not
*-- use tableupdate in case of edit or add modes.
=gfOpenFile(oAriaApplication.DataDir+"Customer",oAriaApplication.DataDir+"Customer","SH",@lcCustFile,.T.)

*-- Open the Syschdul file 
lnResult = oAriaApplication.RemoteSystemData.Execute("Select * from syschdul WHERE cconttype+ccont_id+store+ccompleted+DTOS(dtrandate)+ctrantime+cseqnumber LIKE '"+;
'C'+PADR(Customer.Account,8)+Customer.Store+ "%' AND cComp_Id ='"+oAriaApplication.ActiveCompanyID+;
"'ORDER BY cContType,cCont_Id,Store,cCompleted,dTranDate,cTranTime,cSeqNumber",;
'',"SyschdulFl","",oAriaApplication.SystemConnectionString,3,'',SET("DATASESSION"))

IF lnResult = 1
  lnCurBuff = CURSORGETPROP("Buffering")
  =CURSORSETPROP("Buffering" ,3,"SyschdulFl")
  INDEX ON cContType+cCont_Id+Store+cCompleted+DTOS(dCmpltDate)+cTranTime+cSeqNumber TAG 'SCHCOMDT' DESCENDING
  INDEX ON cContType+cCont_Id+Store+cCompleted+DTOS(dTranDate)+cTranTime+cSeqNumber TAG 'SCHDATE' 
  =CURSORSETPROP("Buffering" ,lnCurBuff ,"SyschdulFl")

  SELECT SyschdulFl
  *-- To indicate if there is a next action
  llPending   = SEEK('C'+PADR(Customer.Account,8)+Customer.Store+'N')
  *-- To indicate if there is a last action
  llCompleted = SEEK('C'+PADR(Customer.Account,8)+Customer.Store+'Y')

  DO CASE
    *-- Case there is a pending activity (Next Action) and history activity (Last Action)
    CASE llPending AND llCompleted
      *-- Last Action
      SET ORDER TO TAG SCHCOMDT
      IF SEEK('C'+PADR(Customer.Account,8)+Customer.Store+'Y')
        lcLastAct  = ALLTRIM(SyschdulFl.cSubject)+"/"+ALLTRIM(gfCodDes(SyschdulFl.cTranReson,'CTRANRESON'))
        ldLastDate = SyschdulFl.dCmpltDate
      ENDIF  

      *-- Next Action
      SET ORDER TO TAG SCHDATE
      IF SEEK('C'+PADR(Customer.Account,8)+Customer.Store+'N')
        lcNextAct  = ALLTRIM(SyschdulFl.cSubject)+"/"+ALLTRIM(gfCodDes(SyschdulFl.cTranReson,'CTRANRESON'))
        ldNextDate = SyschdulFl.dTranDate
      ENDIF                  
    
    *-- Case there is a pending activity (Next Action) and NO history activity (Last Action)
    CASE llPending AND !llCompleted
      lcLastAct  = ""
      ldLastDate = {}
      SET ORDER TO TAG SCHDATE
    ...
    *-- Case there is No pending activity (Next Action) and there is a history activity (Last Action)
    CASE llCompleted AND !llPending    
      lcNextAct  = "Complete"
      ldNextDate = oAriaApplication.SystemDate
      SET ORDER TO TAG SCHCOMDT
      IF SEEK('C'+PADR(Customer.Account,8)+Customer.Store+'Y')
        lcLastAct  = ALLTRIM(SyschdulFl.cSubject)+"/"+ALLTRIM(gfCodDes(SyschdulFl.cTranReson,'CTRANRESON'))
        ldLastDate = SyschdulFl.dCmpltDate
  ...
  *-- Update the customer file
  SELECT (lcCustFile)
  IF SEEK (Customer.Type+Customer.Account+Customer.Store)
    *!E128241,1 MMT 06/02/2005 change fields where next action data saved[START]
    *  REPLACE cLast_Actt WITH lcLastAct ,;
            dLast_ActD WITH ldLastDate,;
            cNext_Actt WITH lcNextAct ,;
            dNext_Actd WITH ldNextDate
    REPLACE cLast_Actt WITH lcLastAct ,;
            dLast_ActD WITH ldLastDate,;
            usr_dfnd9 WITH lcNextAct ,;
            usr_dfnd10 WITH DTOS(ldNextDate)
    *!E128241,1 MMT 06/02/2005 change fields where next action data saved[END]                
  ENDIF
