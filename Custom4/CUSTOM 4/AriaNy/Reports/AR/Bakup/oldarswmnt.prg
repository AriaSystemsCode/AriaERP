...
IF lnResult >= 1
  ...
  REPLACE &lcHdrFile..dDateFrom  WITH &lcHdrFile..dDateFrom + 1 ,;
      		&lcHdrFile..dDateTo    WITH &lcHdrFile..dDateTo + IIF(MONTH(&lcHdrFile..dDateTo)>2,IIF(MOD(YEAR(&lcHdrFile..dDateTo)+1, 4) = 0 , 365 ,  364 ),IIF(MOD(YEAR(&lcHdrFile..dDateTo), 4) = 0 , 365 ,  364 ))
...
IF lnResult >=1
  ...
  SELECT cStSoftW
	...
	SCAN FOR cModType = 'E' AND lMaintain = .T. 
	 IF lnEDITP  > 0
	   ...
	   INSERT INTO &lcDtlFile (cSortKey ,Account            , cVer            , cModName  , nQty    , nPrice         , cModId          , nMtnc              , cModType);
	                   VALUES ('D',cStSoftW.cCust_Id,cStSoftW.capp_id, lcModName , lnEDITP ,  lnEDITP * 975 , cStSoftW.cmod_id, lnEDITP * 97.5 , cStSoftW.cModType) 
	   EXIT
   ENDIF 
  ENDSCAN 
  *--#037909, HFK(Heba Fathi),03/09/2004
  ...
  SCAN FOR cModType = 'U'  && additional users
    INSERT INTO &lcDtlFile (cSortKey ,nQty ,Account          , cVer            , cModName                    ,nPrice                                           ,nMtnc                                   ,cModId                 ,cModType);
              VALUES       ('B'      ,cStSoftW.nqty ,cStSoftW.cCust_Id, cStSoftW.capp_id, cStSoftW.csoft_modu,cStSoftW.nqty*IIF(cStSoftw.cmod_id='US5',500,400),cStSoftW.nqty*IIF(cStSoftw.cmod_id='US5',50,40),cStSoftW.cmod_id,cStSoftW.cModType) 
  ENDSCAN 
  *--#037909, HFK(Heba Fathi),03/09/2004  
ENDIF 
SELECT (lcWorkArea)   
