...
Const TRAKING_ENTRIES_FOLDER   As String = "FIXES"     'E302042,1 YMA 10/21/2002 The folder in which the tracking entries will be downloaded.
' FAT MAN
Const Free_Files_Folder        As String = "Other"
Const UPDATE_SP_PROGRAM_FOLDER As String = "UPDATESP"  'E000000,1 YMA 01/15/2003 The folder from which the updates for the Update Service Packs program will be downloaded.
Const UPDATE_Free_Files_FOLDER As String = "Free"
' FAT MAN
...
' FAT MAN
Private mstrUpdateSPFilesList As String
Private mstrUpdateFreeFilesList As String
' FAT MAN
...
Private mevlDownloadType      As enmDownLoadType
' Fat Man
...
Private Sub ftpConnector_OnDirList(ByVal DirLine As String, ByVal Complete As Boolean, ByVal retCode As Long)
  ...
  If Len(Trim(DirLine)) Then
    Select Case mevlDownloadType
      Case evlDTStandard
        If mobjModulesRecSet.EOF Then
          strDirName = Space(0)
        Else
          strDirName = mobjModulesRecSet!cApp_ID
        End If
        
      Case evlDTCustom
        strDirName = Trim(txtUserID)
      
      Case evlDTUServiceP
        strDirName = "/" & DOWNLOAD_DIR & "/" & UPDATE_SP_PROGRAM_FOLDER & "/"
        
      Case evlDTUFree
        strDirName = "/" & DOWNLOAD_DIR & "/" & Trim(txtUserID) & "/" & UPDATE_Free_Files_FOLDER & "/"
      
    End Select

    Select Case mevlDownloadType
      Case evlDTStandard
        strFileName = Replace(DirLine, "/" & DOWNLOAD_DIR & "/" & strDirName & "/", "")
      ...
      Case evlDTUServiceP
        strFileName = Trim(Replace(DirLine, strDirName, ""))
        mstrUpdateSPFilesList = mstrUpdateSPFilesList & strFileName
        If Len(Trim(strFileName)) < 12 Then
          mstrUpdateSPFilesList = mstrUpdateSPFilesList & Space(12 - Len(strFileName))
        End If
      Case evlDTUFree
        strFileName = Trim(Replace(DirLine, strDirName, ""))
        mstrUpdateFreeFilesList = mstrUpdateFreeFilesList & strFileName
        If Len(Trim(strFileName)) < 12 Then
          mstrUpdateFreeFilesList = mstrUpdateFreeFilesList & Space(12 - Len(strFileName))
        End If
    End Select
  End If
...
Private Function ProgressGetInfo() As Boolean
  ...
  prgInfoTherm.Max = mobjModulesRecSet.RecordCount + 2
  ...
  mevlDownloadType = evlDTStandard
  ' Fat MAn
  ...
  prgInfoTherm.Value = prgInfoTherm.Max - 1
  'Fat Man
  Call SetCaption(mintCurStep)
  
  
  'Fat Man
  mevlDownloadType = evlDTUServiceP
  lblIfoStatus(1) = "Update servicee packs utility."
  Call DownloadList(UPDATE_SP_PROGRAM_FOLDER)
  prgInfoTherm.Value = prgInfoTherm.Max
  'Fat Man
  ...
  'Fat Man
  mevlDownloadType = evlDTUFree
  lblIfoStatus(1) = "Update Free files."
  Call DownloadList(Trim(txtUserID) & "/" & UPDATE_Free_Files_FOLDER)
  prgInfoTherm.Value = prgInfoTherm.Max
  'Fat Man
...
Private Function ProgressDownload() As Boolean
  ...
  ' Fat Man
  If Len(mstrUpdateSPFilesList) > 0 Then
    Call DownLoadSP
    Call ShowList(mstrFailedFiles, True)
  End If
  If Len(mstrUpdateFreeFilesList) > 0 Then
    Call DownLoadFree
    Call ShowList(mstrFailedFiles, True)
  End If
  
  ' Fat Man
  If Len(Trim(mstrFilesList)) > 0 Then
    Screen.MousePointer = vbHourglass
    ...
    'E302042,1 YMA 10/21/2002 Start - Add the custom tracking entries to the lis of files
    '                                 to be updated.
    intNoOfFiles = intNoOfFiles + GetTheTrackingFiles(strFilesToUpdate)
    'E302042,1 YMA 10/21/2002 End.
...
End Function
'Fat man
Private Sub DownLoadSP()
  Dim intNoOfFiles As Integer
  intNoOfFiles = Len(mstrUpdateSPFilesList) / 12
  ...
  For intCount = 0 To intNoOfFiles - 1
    Dim strFileName As String
    strFileName = Mid(mstrUpdateSPFilesList, (intCount * 12) + 1, 12)
    
    ftpConnector.RemoteFile = "/" & DOWNLOAD_DIR & _
                              "/" & UPDATE_SP_PROGRAM_FOLDER & _
                              "/" & Trim(strFileName)
    Call WaitUntilFTPFinishsExecuting
    ftpConnector.LocalFile = App.Path & "\" & Trim(strFileName)
    Call WaitUntilFTPFinishsExecuting
    
    mstrCurFileName = Trim(strFileName)
...
'Fat Man
Private Sub DownLoadFree()
  Dim intNoOfFiles As Integer
  Dim strLocalFile As String
  
  intNoOfFiles = Len(mstrUpdateFreeFilesList) / 12
  
  prgDownTherm.Min = 0
  prgDownTherm.Max = intNoOfFiles
  prgDownTherm.Value = prgDownTherm.Min
  Call SetCaption(mintCurStep, ConvertPrecent(prgDownTherm))
  strLocalFile = LoadSetting(DOWNLOAD_TO) & "\" & Free_Files_Folder
  Dim objFileSystem As Scripting.FileSystemObject
  Set objFileSystem = New Scripting.FileSystemObject
  If Not objFileSystem.FolderExists(strLocalFile) Then
    Call objFileSystem.CreateFolder(strLocalFile)
  End If
  ...
  For intCount = 0 To intNoOfFiles - 1
    Dim strFileName As String
    strFileName = Mid(mstrUpdateFreeFilesList, (intCount * 12) + 1, 12)
    
    ftpConnector.RemoteFile = "/" & DOWNLOAD_DIR & _
                              "/" & Trim(txtUserID) & _
                              "/" & UPDATE_Free_Files_FOLDER & _
                              "/" & Trim(strFileName)
    Call WaitUntilFTPFinishsExecuting
    ftpConnector.LocalFile = strLocalFile & "\" & Trim(strFileName)
    Call WaitUntilFTPFinishsExecuting
    ...
    ' Delete the Transfered file badman
    ' ftpConnector.DELE (ftpConnector.RemoteFile)
...
'E302042,1 YMA 10/21/2002 Start
Private Function GetTheTrackingFiles(ByRef strFilesToUpdate As String) As Integer
  Const cnsTrackingEntriesInitals As String = "BCEN"
  Dim intNoOfFiles As Integer
  
  Dim strBatchNumber As String
  strBatchNumber = BatchNumberGet()
  
  If Len(Trim(strBatchNumber)) > 0 Then
    Dim intCurTrackType As Integer
    For intCurTrackType = 1 To Len(cnsTrackingEntriesInitals)
      Dim strCurTrackType As String
      strCurTrackType = Mid(cnsTrackingEntriesInitals, intCurTrackType, 1) & _
                        Mid(cnsTrackingEntriesInitals, intCurTrackType, 1)
      
      Dim intCount As Integer
      
      
      Dim intPos As Integer
      intPos = InStr(1, mstrFilesList, strCurTrackType, vbTextCompare)
      Do While intPos > 0
        Dim strFileName As String
        strFileName = Mid(mstrFilesList, intPos, 12)
        
        Dim strExtension As String
        strExtension = Mid(strFileName, 10, 3)
        
        If strExtension = strBatchNumber Then
          strFilesToUpdate = strFilesToUpdate & strFileName
          intNoOfFiles = intNoOfFiles + 1
        End If
        
        intPos = InStr(intPos + 12, mstrFilesList, strCurTrackType, vbTextCompare)
      Loop
    Next intCurTrackType
  End If
  
  GetTheTrackingFiles = intNoOfFiles
End Function
'E302042,1 YMA 10/21/2002 End.

'E302042,1 YMA 10/21/2002 Start
Private Function BatchNumberGet() As String
  Dim strtRetValue As String
  strtRetValue = Space(0)
  ...
  On Error GoTo ErrHand
  Dim rsBatchNumber As ADODB.Recordset
  Set rsBatchNumber = New ADODB.Recordset
  ...
  With rsBatchNumber
    ...
    .CursorType = adOpenStatic
    .LockType = adLockOptimistic
    .Open "SELECT max(nBatchNo) AS nBatch FROM sydTrans", objCon
  End With
  On Error GoTo 0
  
  Dim intBatchNumber As Integer
  intBatchNumber = 0
  If Not (rsBatchNumber.BOF() Or rsBatchNumber.EOF()) Then
    intBatchNumber = rsBatchNumber.Fields("nBatch").Value
  End If
  
  rsBatchNumber.Close
  Set rsBatchNumber = Nothing
  
  strtRetValue = Trim(Str(intBatchNumber))
  strtRetValue = String(3 - Len(strtRetValue), "0") & strtRetValue
  
  BatchNumberGet = strtRetValue
  Exit Function

ErrHand:
  MsgBox "The following Error occurred while reading the " & _
         "batch number: " & vbCrLf & vbCrLf & _
         "Error: " & Err.Description & vbCrLf & vbCrLf & _
         "Cannot download the custom tracking entries.", vbCritical + vbOKOnly
  BatchNumberGet = Space(0)
  Exit Function
End Function
'E302042,1 YMA 10/21/2002 End
...
Private Sub DownLoad(ByVal intNoOfFiles As Integer, ByVal strFilesToUpdate As String)
  ...
  For intCount = 0 To intNoOfFiles - 1
    ...
    If IsThisMyServicePackFile(strFileName) Then
      strLocalFile = App.Path
    Else
      strLocalFile = strDownTo
      If InStr(1, "BBEECCNN", Left(strFileName, 2), vbTextCompare) > 0 Then
        strLocalFile = strLocalFile & "\" & TRAKING_ENTRIES_FOLDER
      ...
      End If
    End If
    strLocalFile = strLocalFile & "\" & strFileName
    ftpConnector.LocalFile = strLocalFile
...
Private Function GetTheFilesForThisModule(ByRef strFilesToUpdate As String) As Integer
  Dim intNoOfFiles As Integer
  Dim intCount As Integer
