...
'bad man  Handle Command Line Parameters ..... BEGIN
...
Public cnSQLConnectionStr As String
'bad man  Handle Command Line Parameters ..... END
...
Public strParameters As String

Public Sub Main()
  'bad man  Handle Command Line Parameters ..... BEGIN
  cnSQLConnectionStr = "Provider=MSDATASHAPE;Data Provider=SQLOLEDB;Initial Catalog=OpenIssues;"
  strParameters = Command
  If Len(strParameters) = 0 Then
    'cnSQLConnectionStr = cnSQLConnectionStr & "Data Source=AriaWeb;uid=web;pwd=ariaweb"
    cnSQLConnectionStr = cnSQLConnectionStr & "Data Source=AriaWebsite;uid=web;pwd=webaria"
  Else
    Dim aParameters() As String
    aParameters = Split(strParameters, ",")
    cnSQLConnectionStr = cnSQLConnectionStr & _
      "Data Source=" & aParameters(0) & ";uid=" & aParameters(1) & ";pwd=" & aParameters(2)
  End If
  'bad man  Handle Command Line Parameters ..... END
