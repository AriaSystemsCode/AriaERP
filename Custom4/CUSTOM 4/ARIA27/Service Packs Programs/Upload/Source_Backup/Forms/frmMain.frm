...
Private rsIssueLin As ADODB.Recordset
...
Private Sub cmdAttach_Click()
  ...
  If Len(Trim(cntDialog.FileName)) > 0 Then
    ...
    strOpenIssue = GetOpenIssueHeader(strTrackingEntry)
...
Private Function UpLoad(ByVal strFullFile As String, _
  ...
  strRemoteFile = strRemoteFolder & strFileName
  ...
  If ftpConnector.Connected Then
    ...
    'badran Create the folder if not exists.
...
Private Function GetOpenIssueHeader(ByVal strTrackID As String) As String
  ...
  Dim strSQL As String
  strSQL = "SELECT cIssueNo, cCust_ID, dRespDate, lstARespDate, cRespType FROM SuIssHdr " & _
  ...
  If rsIssueHdr.EOF Then
    GetOpenIssueHeader = ""
  Else
    GetOpenIssueHeader = rsIssueHdr.Fields(0).Value & "-" & rsIssueHdr.Fields(1).Value
  End If
End Function
...
Private Sub txtResponse_Validate(Cancel As Boolean)
  If mResponseText = Trim(txtResponse.Text) Then
    Exit Sub
  End If
...
Private Function RegisterEntry(ByVal strFileName As String, _
  ...
  If Len(Trim(strIssue)) = 0 Then
    ...
    Dim bolIsValidMail As Boolean
    
    Do While True
      strMail = InputBox("Send mail To:", "Send To " & strCustomer)
      bolIsValidMail = IsValidMail(strMail)
      If bolIsValidMail Then
        Exit Do
      Else
...
Private Sub SendIssue(ByVal strExe As String, ByVal strResponse As String, strIssue As String)
  ...
  strSQLCommand = "INSERT INTO SuIssDt " & _
      "(CIssueNo, cRespType, cRespBy, mRespAct, DRespDate, tRespTime, intLineNo, NotificationSent, Hidden)" & _
      "VALUES ('" & strIssue & "','Y','ARIA','" & _
      strResponse & "','" & Date2String(Date, True) & "','" & FormatDateTime(Now, vbShortTime) & "'," & rsMaxLine.Fields(0).Value & ",1,0)"
