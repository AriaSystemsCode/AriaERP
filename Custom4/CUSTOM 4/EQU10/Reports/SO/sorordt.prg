*!*****************************************************************************************
*! Name      : SORORDT.Prg  (Order detail report)
*! Developer : Ahmed Moustafa (AHS)
*! Note : this prg is a typical copy of the SORDDET.prg file
...
STORE '' TO lcGrpExp,lcSeaExp,lcDivExp,lcStatusEx,lcCatExp,lcRegExp
*AHS
...
IF llClearFn OR llOGFltCh
  ...
  *AHS , Add Customer region to the filter [Start]
  IF !EMPTY(laOGFxFlt[lnRegPos,6])
    lcRegExp  = "&laOGFxFlt[lnRegPos,1]." + ' $ laOGFxFlt[lnRegPos,6]'
  ENDIF
  *AHS , Add Customer region to the filter [End  ]
...
IF TYPE('lnVarbEnd') = 'C'
  ...
  *AHS
  *- Add customer region to the filter
  lnRegPos = lfItmPos('ORDHDR.CSOREGION')
  *AHS
...
*!*	       AND IIF(llRpWebOrd,.T.,IIF(OrdHdr.lFromWeb,.F.,.T.))
    ...
    IF IIF(EMPTY(lcRpOrdTyp),SEEK('O'+&lcTOrdFile..ORDER) OR SEEK('T'+&lcTOrdFile..ORDER),SEEK(lcRpOrdTyp+&lcTOrdFile..ORDER)) AND;    
       ...
       (EMPTY(lcRpEdiFlt) OR EVALUATE(lcRpEdiFlt)) AND  ;
       IIF(EMPTY(lcCatExp),.T., EVALUATE(lcCatExp)) AND ;
       IIF(EMPTY(lcRegExp),.T., EVALUATE(lcRegExp)) AND ;
       IIF(EMPTY(laOGFxFlt[lnPriPos,6]),.T.,ORDHDR.PRIORITY = laOGFxFlt[lnPriPos,6]) AND ;
       IIF(llMultCurr AND !EMPTY(laOGFxFlt[lnCurrPos,6]),ORDHDR.CCURRCODE$laOGFxFlt[lnCurrPos,6],.T.) AND ;
       IIF(llRpPrtShp,IIF(OrdHdr.Status $ 'BCX',.F.,OrdHdr.Open > 0 AND OrdHdr.Ship > 0),.T.) ;
       AND IIF(llRpWebOrd,.T.,IIF(OrdHdr.lFromWeb,.F.,.T.)) 
...
DO CASE
  CASE  OpGrdParm='S'           
    ...
    *SELECT  Pack_Id,cPkColor,cPckSize,cPkVersion FROM SPCK_HDR INTO DBF (gcWorkDir + lcTmpIndex)
     SELECT  Pack_Id,cPkColor,cPckSize,cPkVersion FROM SPCK_HDR INTO DBF (oAriaApplication.WorkDir + lcTmpIndex)
    USE IN SPCK_HDR
    use in (lcTmpIndex)
   * USE (gcWorkDir + lcTmpIndex) IN 0 ALIAS SPCK_HDR
     USE (oAriaApplication.WorkDir+ lcTmpIndex) IN 0 ALIAS SPCK_HDR
    SELECT SPCK_HDR
   ...
   * =gfOpenFile(gcDataDir+'SPCK_HDR',gcDataDir+'SPCK_HDRVR','SH')
    =gfOpenFile(oAriaApplication.DateDir+'SPCK_HDR',gcDataDir+'SPCK_HDRVR','SH')
ENDCASE
