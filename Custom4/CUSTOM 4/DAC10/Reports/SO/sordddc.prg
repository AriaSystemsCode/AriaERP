*!*****************************************************************************************
*! Name      : Sordddc.Prg  (Order detail report)
*! Developer : MAB - Mohamed Atia Badran
*! Date      : 10/15/2002 08:16:22 PM
*! Entry no. : C202463, Custom Order Detail report for Dash Clothing[P-ERP-20240327.0003]
*!*****************************************************************************************
*!Modifications :
...
*! B611638,1 HMS 08/05/2018 - Aria 5 - Sales Order Detail Report - Customer PO# [T20180719.0020 ]
*! B611633,1 HMS 08/07/2018 - Aria 5 - Issue with order detail report [T20180723.0170 ]
*! B612396,1 MMT 05/23/2021 SO Order detail report calls gfMover instead of lfOgMover
*! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005]
*******************************************************************************************
...
*!*    laTempStru[ALEN(laTempStru,1)-10 ,18] = 0
  ...
  *E303591,1 MMT 06/29/2015 Add notes and contract ref. fields to order detail report excel[T20150518.0001][Start]
  ...
  SET STEP ON 
  lnColumnsq = ALEN(laTempStru,1) + 1
  DIMENSION laTempStru[lnColumnsq , 18]
  laTempStru[lnColumnsq  ,1] = 'QtyType'
  laTempStru[lnColumnsq  ,2] = 'C'
  laTempStru[lnColumnsq  ,3] = 30
  laTempStru[lnColumnsq  ,4] = 0
  laTempStru[lnColumnsq  ,17] = 0
  laTempStru[lnColumnsq  ,18] = 0
  *E303591,1 MMT 06/29/2015 Add notes and contract ref. fields to order detail report excel[T20150518.0001][End]  
...
IF EMPTY(laRpTypSrc)
  DECLARE laRpTypSrc[5],laRpTypDst[1]  && Redeclare the source and target arrays.
  STORE "Open"               TO laRpTypSrc[1],laRpTypDst[1]
  STORE "Booked" TO laRpTypSrc[2]
  STORE "Allocated"  TO laRpTypSrc[3]
  STORE "UnAllocated"  TO laRpTypSrc[4]
  STORE "Shipped"  TO laRpTypSrc[5]
ENDIF
...
IF !llStatus
  llRpPrtShp = .F.
  *! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005][Start]
  *llRpAloOrd = .T.
  llRpAloOrd = 'Y'
  *! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005][End]
ENDIF
...
ELSE
  ...
  *IF !llRpAloOrd AND ('B' $ lcRpStatus OR 'C' $ lcRpStatus OR 'X' $ lcRpStatus)
  IF llRpAloOrd='N' AND ('B' $ lcRpStatus OR 'C' $ lcRpStatus OR 'X' $ lcRpStatus)
  *! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005][End]
    *-- Message < This option will be applied only on Hold and Open Orders, it will >
    *--         < ignore other orders status.                                       >
    *-- Buttons <                                OK                                 >
...
IF llWorkDeal
  ...
  *-- Scan order cursor.
  SCAN
    ...
    *! B611633,1 HMS 08/07/2018 - Aria 5 - Issue with order detail report [T20180723.0170 ][Begin]
    ...
    *! B611633,1 HMS 08/07/2018 - Aria 5 - Issue with order detail report [T20180723.0170 ][End]
        *C102262,1 (End)
...
*!*	           IIF(llRpAloOrd,.T.,IIF(OrdHdr.Status $ 'BCX',.F.,(EMPTY(PIKTKT) .OR. TotQty<>TotPik)))
        IF IIF(llRpCorrGp ,!EMPTY(Group),.T.) AND ;
           ...
           IIF(EMPTY(lcCrFrExp) ,.T.,EVALUATE(lcCrFrExp)) AND ;
           IIF(llRpPrtShp,IIF(OrdHdr.Status $ 'BCX',.F.,TotQty > 0),.T.) AND ;
           IIF(llRpAloOrd='Y',.T.,IIF(llRpAloOrd='O',!EMPTY(PIKTKT),IIF(OrdHdr.Status $ 'BCX',.F.,(EMPTY(PIKTKT) .OR. TotQty<>TotPik))))
        *! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005][End]  
        *! B607998,1 SSH [END]
		  ...
		  *! B611638,1 HMS 08/05/2018 - Aria 5 - Sales Order Detail Report - Customer PO# [T20180719.0020 ][begin]
		   m.custpo = IIF(EMPTY(m.custpo),ordhdr.custpo,m.custpo)
           *! B611638,1 HMS 08/05/2018 - Aria 5 - Sales Order Detail Report - Customer PO# [T20180719.0020 ][END]
          *E303591,1 MMT 06/29/2015 Add notes and contract ref. fields to order detail report excel[T20150518.0001][End]
          IF 'O' $ lcRpBkOp 
          ...
          *! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005][Start]
          ...
          IF 'S' $ lcRpBkOp 
		    ...
		    ENDFOR
		     m.TOTQTY = 0
            IF SEEK(m.Order,'INVLINE')
              SELECT INVLINE
              SCAN REST WHILE ORDER+STR(LINENO,6)+INVOICE= m.Order FOR Style =m.Style AND nordline = m.LineNo 
                IF SEEK(INVLINE.INVOICE,'INVHDR')
                  ...
                  FOR I = 1 To 8
		            lcCount = ALLTRIM(STR(I))
		            m.QTY&lcCount = m.QTY&lcCount + INVLINE.Qty&lcCount
		          ENDFOR
		          m.TOTQTY = m.QTY1+m.QTY2+m.QTY3+m.QTY4+m.QTY5+m.QTY6+m.QTY7+m.QTY8
                ENDIF
              ENDSCAN
...
*!*		            ENDIF
                ...
                *E303591,1 MMT 06/29/2015 Add notes and contract ref. fields to order detail report excel[T20150518.0001][End]
                *C202463,1 MMT 07/07/2024 Custom Order Detail report for Dash Clothing[P-ERP-20240327.0003][Start]
   	            *INSERT INTO (lcWorkFile) FROM MEMVAR
  		          IF 'O' $ lcRpBkOp 
  		            INSERT INTO (lcWorkFile) FROM MEMVAR
  		          ENDIF
                ...
                *! E612695,1 MMT 09/26/2023 Order Detail Report enhancement to filter on Allocated orders[T-ERP-20230614.0005][Start]
                ...
                IF 'S' $ lcRpBkOp 
                  ...
                  INSERT INTO (lcWorkFile) FROM MEMVAR
                ENDIF
                *C202463,1 MMT 07/07/2024 Custom Order Detail report for Dash Clothing[P-ERP-20240327.0003][End]
...
*!*	             IIF(llRpWebOrd,.T.,IIF(OrdHdr.lFromWeb,.F.,.T.))
          IF !SEEK(lcSeekExp,lcTempLine,lcTempLineTag) AND IIF(EMPTY(lcRpOrdTyp),CORDTYPE <> 'C',CORDTYPE= lcRpOrdTyp) AND ;
             ...
             IIF(llRpPrtShp,IIF(OrdHdr.Status $ 'BCX',.F.,OrdHdr.Open > 0 AND OrdHdr.Ship > 0),.T.) AND ;
             IIF(llRpPrtShp,IIF(OrdHdr.Status $ 'BCX',.F.,TotQty > 0),.T.) AND ;
             IIF(llRpAloOrd='Y',.T.,IIF(llRpAloOrd='O',!EMPTY(PIKTKT),IIF(OrdHdr.Status $ 'BCX',.F.,(EMPTY(PIKTKT) .OR. TotQty<>TotPik)))) AND ;
             IIF(llRpWebOrd,.T.,IIF(OrdHdr.lFromWeb,.F.,.T.))
...
IF !llRpPrTPak
  lcRpForm = "SORDDGMP"
ELSE
  lcRpForm = IIF(llRpSizes,'SORDDDCA','SORDDDCB')
ENDIF
= lfRepPltFr(lcRpForm)
llRpScale = llRpSizes
ClearRead()
...
*! B612396,1 MMT 05/23/2021 SO Order detail report calls gfMover instead of lfOgMover[Start]
...
*! B612396,1 MMT 05/23/2021 SO Order detail report calls gfMover instead of lfOgMover[End]
* N000682 ,1 Thabet Handle globalization issues [Start]
...
IF !EMPTY(laStorScr)
  ...
  *= gfMover(@laStorScr,@laStorTar,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SELSTORE,oAriaApplication.GetHeaderText("LANG_SELSTORE",AHEADERFILE)),.T.)
  = lfOgMover(@laStorScr,@laStorTar,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SELSTORE,oAriaApplication.GetHeaderText("LANG_SELSTORE",AHEADERFILE)),.T.)
  *! B612396,1 MMT 05/23/2021 SO Order detail report calls gfMover instead of lfOgMover[End]
  * N000682 ,1 Thabet Handle globalization issues [End]
  laStores = ""
...
*!*************************************************************
FUNCTION lfvQtyTypes
PRIVATE lcOldTypes,lcCurrChr

lcOldTypes = lcRpType  && Save old status value.
= lfOGMover(@laRpTypSrc,@laRpTypDst,"Select Quantity Type",.T.,'')  && call mover function.

lcRpBkOp= ' '
*-- Loop to make Status expression.
IF !EMPTY(laRpTypDst[1])
  FOR lnI = 1 TO ALEN(laRpTypDst,1)
    lcRpBkOp= lcRpBkOp+ IIF(laRpTypDst[lnI] = "Open",'O',;
                          IIF(laRpTypDst[lnI] = "Booked",'B',;
                          IIF(laRpTypDst[lnI] = "Allocated",'P',;
                          IIF(laRpTypDst[lnI] = "UnAllocated",'A',IIF(laRpTypDst[lnI] = "Shipped",'S','')))))
  ENDFOR
ENDIF
lcRpBkOp= IIF(EMPTY(lcRpBkOp),'O',ALLTRIM(lcRpBkOp))
*!*************************************************************
*! Name      : RefreshTypes
