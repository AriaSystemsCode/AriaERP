...
*! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009]
*! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003]
*! B610882,1 TMI 10/14/2014 fix problem : file access is denied when RB are created [T20140226.0012] 
*******************************************************************************************
*WAIT WINDOW "Order Detail Report................."
*-- Main program code                        BEGIN

*! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003][Start]
PARAMETERS lcRequestID, lcXMLFileName, ClientId
*! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003][End]
...
IF TYPE('lcXMLFileName') = 'C'
  ...
  *B610882,1 TMI 10/14/2014 13:37 [Start] define gcRequestID,gcClientID in case that they are not defined in the requesthandler calling class, 
  *                                       this change has bee employed at the customer's site since 6/21/2014
  TRY 
  gcRequestID = lcRequestID 
  gcClientID = ClientID
  CATCH
  ENDTRY 
  *B610882,1 TMI 10/14/2014 13:37 [End  ] 
  ...
  *B610882,1 TMI 10/14/2014 13:41 [Start] assign to gcWorkDir the client's work path instead of the shared path
  gcWorkDir = STRTRAN(UPPER(oAriaEnvironment.systemfilespath),'\SYSFILES\','\WORK\')
  *B610882,1 TMI 10/14/2014 13:41 [End  ] 
ELSE 
  loOGScroll.cCROrientation = 'P'
ENDIF
*! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003][End]
...
*! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003][Start]
IF TYPE('lcXMLFileName') = 'C'  
  gcCmpModules = oAriaApplication.CompanySetupModules
  gcBaseCurr   = oAriaApplication.BaseCurrency  
  
  =gfOpenTable('ORDHDR','ORDHDR','SH','ORDHDR')
  =gfOpenTable('ORDLINE','','SH','ORDLINE')
  ...
  =gfOpenTable('SALESREP','SALESREP','SH','SALESREP')
  =gfOpenTable('WAREHOUS','WAREHOUS','SH','WAREHOUS')
  =gfOpenTable('STYDYE','STYDYE','SH','STYDYE')
  =gfOpenTable('SYCCURR','CCURRCODE','SH','SYCCURR')
  =gfOpenTable('SPCK_HDR','SPCK_HDR','SH','SPCK_HDR')
  =gfOpenTable('INVLINE','INVLINEO','SH','INVLINE')
  =gfOpenTable('INVHDR','INVHDR','SH','INVHDR')
  
  IF !USED('FABRIC')
    USE (oAriaEnvironment.DataDir+'FABRIC') IN 0 SHARED ALIAS 'FABRIC' ORDER TAG FABRIC
  ENDIF
  IF !USED('SYCCURR')
...
*!*    laTempStru[ALEN(laTempStru,1)-10 ,18] = 0
  ...
  *! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009][Begin]
  *-- WH and stock fields
  lnArrLen = ALEN(laTempStru,1) 
  DIMENSION laTempStru[lnArrLen+11, 18]
  
  lnArrLen = lnArrLen + 1
  laTempStru[lnArrLen,1] = 'WHCODE'
  laTempStru[lnArrLen,2] = 'C'
  laTempStru[lnArrLen,3] = 6
  laTempStru[lnArrLen,4] = 0
  laTempStru[lnArrLen,17] = 0
  laTempStru[lnArrLen,18] = 0  

  lnArrLen = lnArrLen + 1
  laTempStru[lnArrLen,1] = 'WHCODE2'
  laTempStru[lnArrLen,2] = 'C'
  laTempStru[lnArrLen,3] = 6
  laTempStru[lnArrLen,4] = 0
  laTempStru[lnArrLen,17] = 0
  ...
  *! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009][End  ]
...
*! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009][Begin]
*- Add a new line for each WH from stydye file 

IF lcRpForm = 'SORDDNIN' .AND. ;
  lcRpSortBy = 'S' .AND. !llRpAloOrd .AND. llRpShwAvl 
  lcSvTag = ''
  =lfShwAvl()
ENDIF
*! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009][End  ]
  ...
  *WAIT WINDOW 'Selected ' + ALLTRIM(STR(RECCOUNT(lcMastFile))) + ' Records in ' + ALLTRIM(STR(lnInterval,6,2)) + ' Seconds...' NOWAIT
  ...
  ELSE
    *loProgress.Percent = Percent of processing done
    loProgress.DESCRIPTION = 'Selected ' + ALLTRIM(STR(RECCOUNT(lcMastFile))) + ' Records in ' + ALLTRIM(STR(lnInterval,6,2)) + ' Seconds...'
    loAgent.UpdateObjectProgress(lcRequestID, loProgress, ClientID)
  ENDIF
  *! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003][End]
...
IF lcRpForm = 'SORDDNIN' .AND. ;
  lcRpSortBy = 'S' .AND. !llRpAloOrd .AND. llRpShwAvl
  SELECT &lcMastFile
  SET ORDER TO &lcSvTag
  LOCATE 
  DELETE FOR !EMPTY(WHCODE)
ENDIF
*! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009][End  ]
...
*  
*ENDIF
lcRpForm = IIF(llRpSizes,'SORDDNIN','SORDDNIB')
  ...
  IF (lnClrStart == 0)
    ...
    *lnCount =gfItemMask(@laItemSeg)
    ...
    IF TYPE('lcXMLFileName') = 'C'
      LOCAL loItemMask
      loItemMask = CREATEOBJECT("GetItemMask")
      lnCount = loItemMask.Do(@laItemSeg)
    ELSE
      lnCount = gfItemMask(@laItemSeg)
    ENDIF
    *! C201599,1 SAB 10/24/2012 Convert Custom Order Details for NIN24 [T20130904.0003][End]
...
*:* Developer   : TMI - TAREK MOHAMED IBRAHIM
*:* Date        : 08/28/2007
*:* Purpose     : *- Add a new line for each WH from stydye file 
*:***************************************************************************
FUNCTION lfShwAvl

PRIVATE lnSlct,lcSty,lcKey
...
lcKey = KEY()
DO WHILE !EOF(lcMastFile)
  lcSty = &lcMastFile..STYLE
  SCATTER MEMVAR MEMO
  
  =SEEK(lcSty,'STYDYE')
  SELECT STYDYE
  SCAN REST WHILE STYLE+CWARECODE+DYELOT = lcSty FOR EMPTY(DYELOT)
    STORE 0 TO m.Stk1,m.Stk2,m.Stk3,m.Stk4,m.Stk5,m.Stk6,m.Stk7,m.Stk8
    *C200843,1 TMI [Start] 
    *m.Stk1 = MAX(STYDYE.STK1,0)
    *m.Stk2 = MAX(STYDYE.STK2,0)
    *m.Stk3 = MAX(STYDYE.STK3,0)
    *m.Stk4 = MAX(STYDYE.STK4,0)
    *m.Stk5 = MAX(STYDYE.STK5,0)
    *m.Stk6 = MAX(STYDYE.STK6,0)
    *m.Stk7 = MAX(STYDYE.STK7,0)
    *m.Stk8 = MAX(STYDYE.STK8,0)

    *C200843,5 TMI [Start] the customer needs the IOTS
    *m.Stk1 = MAX(STYDYE.STK1-STYDYE.ALO1,0)
    *m.Stk2 = MAX(STYDYE.STK2-STYDYE.ALO2,0)
    *m.Stk3 = MAX(STYDYE.STK3-STYDYE.ALO3,0)
    *m.Stk4 = MAX(STYDYE.STK4-STYDYE.ALO4,0)
    *m.Stk5 = MAX(STYDYE.STK5-STYDYE.ALO5,0)
    *m.Stk6 = MAX(STYDYE.STK6-STYDYE.ALO6,0)
    *m.Stk7 = MAX(STYDYE.STK7-STYDYE.ALO7,0)
    *m.Stk8 = MAX(STYDYE.STK8-STYDYE.ALO8,0)
    m.Stk1 = MAX(STYDYE.STK1-STYDYE.ORD1,0)
    m.Stk2 = MAX(STYDYE.STK2-STYDYE.ORD2,0)
    m.Stk3 = MAX(STYDYE.STK3-STYDYE.ORD3,0)
    m.Stk4 = MAX(STYDYE.STK4-STYDYE.ORD4,0)
    m.Stk5 = MAX(STYDYE.STK5-STYDYE.ORD5,0)
    m.Stk6 = MAX(STYDYE.STK6-STYDYE.ORD6,0)
    m.Stk7 = MAX(STYDYE.STK7-STYDYE.ORD7,0)
    m.Stk8 = MAX(STYDYE.STK8-STYDYE.ORD8,0)
    *C200843,5 TMI [END  ] the customer needs the IOTS
    ...
    *C200843,1 TMI [End  ] 
    ...
    IF m.Totstk > 0
      ...
      STORE '' TO m.ORDER,m.Account,m.Piktkt,m.Scale
      STORE {} TO m.Start,m.Complete      
      
      m.WHCode = STYDYE.CWARECODE 
      INSERT INTO &lcMastFile FROM MEMVAR
    ENDIF
  ENDSCAN  
  
  SELECT &lcMastFile
  SCAN REST WHILE STYLE+DTOS(COMPLETE)+CORDTYPE+ORDER+STR(LINENO,6) = lcSty
  ENDSCAN
ENDDO
...
lcKey = "STYLE+STRTRAN(WHCODE2,'      ','Z' )+DTOS(COMPLETE)+CORDTYPE+ORDER+STR(LINENO,6)"

INDEX ON &lcKey TAG STY_WHCODE
SET FILTER TO TOTQTY <> 0 .OR. OrdHdr.cOrdType = 'T' .OR. TOTSTK>0
LOCATE
SELECT (lnSlct)
*-- end of lfShwAvl.
*! C201518,1 HIA 09/17/2012 Convert custom C200843 to A4XP [T20120803.0009][End  ]
...
PRIVATE lcStatus
lcStatus = ''
IF !EMPTY(&lcMastFile..ORDER)
  *C200843,4 TMI [Start] if ORDER is Hold , show H instead of O
  *lcStatus = 'O'  
  lcStatus = ORDHDR.STATUS
  *C200843,4 TMI [End  ] 
ENDIF
IF SEEK(ORDER+STR(LINENO,6) , 'INVLINE') AND ;
  INVHDR.STATUS<>'V' AND ;
  INVLINE.STYLE = &lcMastFile..STYLE
  lcStatus = 'C'   
ENDIF
RETURN lcStatus
*-- end of lfGetLnSts.
