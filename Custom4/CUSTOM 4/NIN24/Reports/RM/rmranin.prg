...
*: Developer     : Mariam Mazhar[T20121211.0003] C201547.EXE
*:***************************************************************************
*: Calls : 
*:    Procedures : ....
*:    Functions  : ....
...
STORE '' TO lcActFltr, lcWarFltr, lcResFltr,lcSttFltr
...
IF !EMPTY(lcActFltr) AND USED(lcActFltr)
  ...
  SCAN
    ...
    SCAN REST WHILE ACCOUNT+RANO = EVALUATE(lcActFltr + '.Account') FOR &lcRpExp
      ...
      IF !EMPTY(lcSttFltr) AND !SEEK(PADR(ALLTRIM(CUSTOMER.CADDRESS4),6),lcSttFltr)
        LOOP
      ENDIF
      SELECT RETAUTH
      SCATTER MEMVAR
...
ENDIF
*-- State Filter
lcCond = lfCheckFilter(1, 'CUSTOMER.CADDRESS4')
IF !EMPTY(lcCond)
  lcSttFltr = loOgScroll.gfTempName()
  CREATE CURSOR (lcSttFltr) (State C(6))
  DIMENSION laValues[1]
  =gfSubStr(lcCond, @laValues, '|')
  SELECT (lcSttFltr)
  INDEX ON State TAG (lcSttFltr)
  FOR lnI = 1 TO ALEN(laValues,1)
    APPEND BLANK
    REPLACE State WITH laValues[lnI]
  ENDFOR
ELSE
  IF TYPE("lcSttFltr") = "C" AND USED(lcSttFltr)
    USE IN (lcSttFltr)
  ENDIF
  lcResFltr = ''
ENDIF
...
laFileStru[lnFileStru+3,1] = 'STATE'
