...
if isobject(session("rsSty")) then
	...
	do while not session("rsSty").EOF 
		...
		Else
			...
			Session("RSLine").Fields("cordtype").Value = "H"
			Session("RSLine").Fields("account").Value = Session("RSCust").Fields("account").value
			Session("RSLine").Fields("style").value = session("rsSty")("lstStyle")
			...
			'get qty entered
			for i= 1 to 8
				IF Len(request("txtord"&trim(RSScale("sz"&i))))=0 Then
				'Session("RSLine").fields("qty1") = 0
				Else
					Session("RSLine").fields("qty"&i) = request("txtord"&trim(RSScale("sz"&i)))
				END IF
