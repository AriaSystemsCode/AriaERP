...
Dim intUpLimit
Dim intLowLimit
'Response.Write "<font size=5>M_STYINFO===="&Session("M_STYINFO")&"</font>"
...
if application("styProfile")<> "" then
	...
	if ubound(arrStyle) > 0 then
		strsql = "select distinct cstyGroup from style where "
	...
	if trim(application("StyGroup")) = "" then
		...
		IF arrFound = "" Then  arrFound = split(trim(UCASE(application("StyGroup"))),",")
		Group =  trim(UCASE(application("StyGroup")))
		'check to apply search 
		if Search= "" then
			Search = "Y"
		end if
...
if Trim(Group)="ALL" Or Trim(Group)="" then
	'strsql = "select DISTINCT style.cstymajor,Style.desc,Style.CSTYGROUP, codes.cdiscrep from style,codes where cstymajor like '" & UCASE(Trim(strRequiredStyle)) & "' and " &makeQuery("CSTYgroup", session("showcatalogval"), 10, false) & " And cdefcode+cfld_name+ccode_no+cdiscrep+crltd_nam like 'NCSTYGROUP%'  and style.cstygroup=codes.ccode_no And style.status='A' " & strSeasDivWhere &strCond
	...
	'strsql = "select DISTINCT style.cstymajor,Style.desc,Style.CSTYGROUP, codes.cdiscrep from style,codes where cstymajor like '" & UCASE(Trim(strStyle)) & "%' and " &makeQuery("CSTYgroup", session("showcatalogval"), 10, false) & " And cdefcode+cfld_name+ccode_no+cdiscrep+crltd_nam like 'NCSTYGROUP%'  and style.cstygroup=codes.ccode_no And style.status='A' " & strSeasDivWhere
...
'	strsql = "select DISTINCT style.cstymajor,Style.desc,Style.CSTYGROUP, codes.cdiscrep from style,codes where status+cstygroup like 'A" & Group & "%' and  cstymajor Like '" &UCASE(Trim(strRequiredStyle))& "' And cdefcode+cfld_name+ccode_no+cdiscrep+crltd_nam like 'NCSTYGROUP%' And style.cstygroup=codes.ccode_no "	&strSeasDivWhere &strCond
	...
	'strsql = "select DISTINCT style.cstymajor,Style.desc,Style.CSTYGROUP, codes.cdiscrep from style,codes where status+cstygroup like 'A" & Group & "%' and  cstymajor Like '" &UCASE(Trim(strStyle))& "%' And cdefcode+cfld_name+ccode_no+cdiscrep+crltd_nam like 'NCSTYGROUP%' And style.cstygroup=codes.ccode_no "	&strSeasDivWhere
	'strSQL = strSQL& " order by desc"
