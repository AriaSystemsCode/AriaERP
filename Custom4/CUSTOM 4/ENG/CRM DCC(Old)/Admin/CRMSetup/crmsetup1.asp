...
<table bordercolor="#111111" border="0" align=center width=95% style="border-collapse: collapse" cellpadding="0" cellspacing="0">
  ...
  <P> 
   ...
   <%
      ...
      <TR> 
        <TD width="171">Show Immediate OTS</TD>
        <TD width="563"> 
			<input type=radio name=radImmOTS value="T" <%if session("ImmOTS") = "T" then%>checked<%end if%>>Yes
			<input type=radio name=radImmOTS value="F" <%if session("ImmOTS") = "F" or session("ImmOTS") = "" then %>checked<%end if%>>No
        </TD>
      </TR>
      <%%>
       <TR> 
