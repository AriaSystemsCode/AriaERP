...
function do_move()
{
	document.frm.action = "mover2.asp?type=M"
	document.frm.submit();
}
function do_moveall()
{
	document.frm.action = "mover2.asp?type=MA"
	document.frm.submit();
}
function do_remove()
{
	document.frm.action = "mover2.asp?type=R"
	document.frm.submit();
}

function do_removeall()
{
	document.frm.action = "mover2.asp?type=RA"
	document.frm.submit();
}
...
Else
	set Session("rs3") = server.CreateObject("ADODB.RecordSet")
	
	IF Session("StyleColor") = "" Then
		strsql = "select * from codes where cdefcode+crltfield+cfld_name='ZZZZZZZZ'"
	Else
		strarGroup = split(Session("StyleColor"),",")
		strGroups = ""
		for inttemp = 0 To Ubound(strarGroup)
			...
			End IF
		next
		strsql = "select * from codes where cdefcode+crltfield+cfld_name='NNSEASON' and cdiscrep <> '' and " & MakeQuery("ccode_no", strGroups, 10, false)
	End IF
	Session("rs3").Open strsql,con,2,4
    'response.write("<BR>" & Session("rs3").recordcount)
	Session("rs3").ActiveConnection = nothing
End IF

			strsql = "select * from codes where cdefcode+crltfield+cfld_name='NNSEASON' and cdiscrep <> '' "
			IF Not(Session("rs3").Eof and Session("rs3").Bof) Then
				Session("rs3").movefirst
				strcode_no = ""
				Do while not Session("rs3").eof
					IF strcode_no = "" Then
						strcode_no = "'" & Session("rs3").fields("ccode_no").value & "'"
					Else
						strcode_no = strcode_no & ",'" & Session("rs3").fields("ccode_no").value & "'"
					End IF	
...
Select case request("type")
	case "M"
		IF request("lstsource") <> "" Then
			...
			Session("rs3").Filter = "ccode_no='" & request("lstsource") & "'" 
			IF (Session("rs3").EOF and Session("rs3").BOF) Then
				'Add to the target recordset
				Session("rs3").addnew
				for inti=0 to rs1.Fields.count - 1
					Session("rs3").fields(rs1.Fields(inti).name).value = rs1.Fields(inti).value
				next
			End IF
			Session("rs3").Filter = "" 
			rs1.filter = "" 
	...
	case "R"
		IF request("lsttarget") <> "" Then
			Session("rs3").Filter = "ccode_no='" & request("lstTarget") & "'" 
			IF Not(Session("rs3").EOF and Session("rs3").BOF) Then								
				Session("rs3").movefirst
				Session("rs3").delete
...
End Select
RS = Move_IT(Session("rs3"),rs1,"Define Styles' Seasons","ccode_no","cdiscrep")
%>
