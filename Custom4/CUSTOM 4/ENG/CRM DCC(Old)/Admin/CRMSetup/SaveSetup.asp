...
if rscheck.EOF and rscheck.BOF then
	...
	") ON [PRIMARY]")
	'wal_add customer warecode table[start]
	conn.Execute ("CREATE TABLE [dbo].[CustomerWareCode] (" &_
	"[cWareCode] [char] (6) COLLATE SQL_Latin1_General_CP1_CI_AI NULL ," &_
	"[cCustID] [char] (5) COLLATE SQL_Latin1_General_CP1_CI_AI NULL " &_
	"[cUserID] [char] (10) COLLATE SQL_Latin1_General_CP1_CI_AI NULL " &_
	"[cStyleGroup] [char] (6) COLLATE SQL_Latin1_General_CP1_CI_AI NULL " &_
	") ON [PRIMARY]")
	'wal_add customer warecode table[end]
	conn.Execute ("CREATE TABLE [dbo].[Email] (" &_
	"[email] [varchar] (50) NULL ," &_
	"[cissueno] [varchar] (50) NULL " &_
	") ON [PRIMARY]")
	'wal_131300 add new tables for style profile[start]
	conn.Execute ("CREATE TABLE [dbo].[StyleProfileHeader] (" &_
	"[cStyleProfileCode] [char] (10) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ," &_
	"[cStyleProfileName] [char] (30) COLLATE SQL_Latin1_General_CP1_CI_AS NULL " &_
	") ON [PRIMARY]")
	
	conn.Execute ("CREATE TABLE [dbo].[StyleProfileDetail] (" &_
	"[cStyleProfileCode] [char] (10) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ," &_
	"[cStyle] [char] (19) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ," &_
	"[cStyleCroup] [char] (6) COLLATE SQL_Latin1_General_CP1_CI_AS NULL " &_
	") ON [PRIMARY]")
	
	conn.Execute ("ALTER TABLE [dbo].[StyleProfileDetail] ADD " &_
	"CONSTRAINT [PK_StyleProfileDetail] PRIMARY KEY  CLUSTERED " &_
	"(" &_
	"	[cStyleProfileCode]," &_
	"	[cStyle]" &_
	")  ON [PRIMARY] ")

	conn.Execute ("ALTER TABLE [dbo].[StyleProfileHeader] ADD " &_
	"CONSTRAINT [PK_StyleProfileHeader] PRIMARY KEY  CLUSTERED " &_
	"(" &_
	"	[cStyleProfileCode]" &_
	")  ON [PRIMARY] ")
	'wal_131300 add new tables for style profile[end]
	...
	"[profile] [char] (5) NULL  " &_
	"[cStyleProfileCode] [char] (10) NULL  " &_
	"[priccode] [char] (6) NULL  " &_
	") ON [PRIMARY]")
