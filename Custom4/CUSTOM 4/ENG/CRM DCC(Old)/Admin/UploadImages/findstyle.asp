...
case "R"'R/A
	...
	if Request("Group")="" or Request("Group")="ALL" then
		...
		strSQL = "SELECT * FROM Style where  style like '"&strRequiredStyle&"' and status+cstygroup = 'A" & Request("Group")&"'"
		strSQL = strSQL& strSeasDivWhere & " order by style"
	end if
Case "O"
	...
	IF Session("M_STYVIEW") = "G" Then
		...
		'ARD - [End]
		IF request("Group") = "ALL" or request("Group") = "" then
			if trim(Session("ShowCatalogVal"))  = "" then
		...
		Else
			strsql = "select * from style where status+cstygroup = 'A" & request("Group") & "' and style like '" & strRequiredStyle & "' "
			strSQL = strSQL& strSeasDivWhere & " order by style"
		End IF
	Else
		'Case Of Packs
...
'Response.Write "strsql =="&strsql&"<Br><br>" & RSSTyResult.RecordCount 
'Response.End 
	...
	if RSSTyResult.RecordCount  = 1 then
		select case Request("logintype")
			case "T"
				...
				'Response.Redirect "Default.asp?sty="&trim(RSSTyResult("style"))&"&desc1="&trim(RSSTyResult("desc1"))
'				Response.Redirect "Default.asp?sty="&trim(RSSTyResult("Cstymajor"))&"&desc1="&trim(RSSTyResult("desc1"))
...
<%
	...
	Do While Not RSSTyResult.EOF And Count < NumPerPage
					...
					select case Request("logintype")
						case "O"%>
							<!--a href="Default.asp?sty=<%=trim(RSSTyResult("style"))%>&desc1=<%=trim(RSSTyResult("desc1"))%>">&nbsp;<%=trim(RSSTyResult("style"))%></a-->
							<a href="Default.asp?sty=<%=trim(RSSTyResult("Cstymajor"))%>&desc1=<%=trim(RSSTyResult("desc1"))%>">&nbsp;<%=trim(RSSTyResult("Cstymajor"))%></a>										
					...
					'if Session("M_STYVIEW") = "P" then
						'Response.Write("&nbsp;"&trim(rsTemSty("desc1")))
					'Else
						Response.Write("&nbsp;"&trim(RSSTyResult("desc1")))
					'End IF
					Response.Write("</td>")
					...
					'if Session("M_STYVIEW") = "P" and Request("logintype")="O" then
						'Response.Write("<a href=""javascript:openwindow('ordshwimg.asp?name=" & trim(rsTemSty("cstymajor")) & "')""> "  & "&nbsp;Show Image</a>") 
					'Else
						Response.Write("<a href=""javascript:openwindow('ordshwimg.asp?name=" & trim(RSSTyResult("cstymajor")) & "')""> "  & "&nbsp;Show Image</a>") 
					'End IF
...
<%
    ...
    if CurPage > 1 then
			...
			<a href="findstyle.asp?firsttime=&Logintype=<%=Request("LoginType")%>&curpage=<%=curpage - 1%>&Group=<%=Request("Group")%>&Type=<%=Request.QueryString("Type")%>"><img border=0 src="../../images/<%=Session("theme")%>/back.gif"></a>
    <%End If

		if CInt(CurPage) <> CInt(TotalPages) then
        'We are not at the end, show a next button%>
			<a href="findstyle.asp?firsttime=&Logintype=<%=Request("LoginType")%>&curpage=<%=curpage+ 1%>&Group=<%=Request("Group")%>&Type=<%=Request.QueryString("Type")%>"><img border=0 src="../../images/<%=Session("theme")%>/next.gif"></a>
    <%End If
		End IF
		%>
</TD></TR></Table>
