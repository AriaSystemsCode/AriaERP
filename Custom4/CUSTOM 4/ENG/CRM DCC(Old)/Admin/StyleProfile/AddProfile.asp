	...
	if session("strStyles") = "" then
		...
		do while not rsprofiledt.EOF
			if session("strStyles") = "" then
				...
				' HHM Aug. 01 2007 Remove sapce which corrupt split in another work add space in begin of style Begin
...
<table width="80%" border="0" align="center">
  ...
  if trim(session("strStyles")) <> "" then
		...
		strSQL = "SELECT * FROM Style where " & MakeQuery("status+cstygroup", Session("ShowCatalogVal"), 10, false) 
		...
		rsResult.Open strsql,cnndb,1,3
		set session("rsResult") = server.CreateObject("ADODB.RecordSet")
		call session("rsResult").fields.append("style",129,19)
		call session("rsResult").fields.append("cstygroup",129,6)
		Session("rsResult").CursorLocation = 3
		Session("rsResult").CursorType = 2
		Session("rsResult").LockType = 3
		Session("rsResult").open
  %>
  <tr>       
  <td >
	...
	if session("strRmvStr") = "" then'check first if there are removed records
		...
		if ubound(arrStyle) > 0 then
			...
			for i=0 to ubound(arrStyle)
				...
				rsResult.Filter = "style =" & arrStyle(i)
if rsResult.EOF  OR rsResult.BOF  then
' response.write arrStyle(i) & " - " 
else

					session("rsResult").addnew
						session("rsResult").fields("style") = rsResult("style")
						session("rsResult").fields("cstygroup") = rsResult("cstygroup")
					session("rsResult").update
			%>
				<TR>
					<TD class="light_cell" ><%=trim(rsResult("style"))%></TD>
					<TD class="light_cell" >&nbsp<%=trim(rsResult("desc1"))%></TD>
					<%
						RSCodes.Filter = "ccode_no='"& Trim(rsResult.Fields("cstygroup").Value)& space(6-(len(Trim(rsResult.Fields("cstygroup").Value)))) &"'"
			...
			rsResult.Filter = "style =" & arrStyle(0)
				session("rsResult").addnew
					session("rsResult").fields("style") = rsResult("style")
					session("rsResult").fields("cstygroup") = rsResult("cstygroup")
				session("rsResult").update
		...
		if ubound(arrStyle) > 0 then
			...
			for i=0 to ubound(arrStyle)
				'strSql = strSql & "style =" & arrStyle(i)
				' HHM Aug. 01, 2007 There is error in paramteters oorder Begin
				' if instr(1,arrStyle(i),session("strRmvStr")) <= 0 then
				if instr(1,session("strRmvStr"), arrStyle(i)) <= 0 then
				' HHM End
					rsResult.Filter = "style =" & arrStyle(i)
if not rsResult.eof then
			...
			if instr(1,arrStyle(0),session("strRmvStr")) <= 0 then
				rsResult.Filter = "style =" & arrStyle(0)
				session("rsResult").addnew
					session("rsResult").fields("style") = rsResult("style")
					session("rsResult").fields("cstygroup") = rsResult("cstygroup")
