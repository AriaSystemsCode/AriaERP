...
if session("Notes") = "" then
	session("Notes") = request("txtNotes")
end if
...
if trim(rsLoc("Ups")) = "" then
	strShipFrom = mid(Session("RSCust")("Caddress5"),1,3)
else
...
'get shipType code
strShipType = Session("RSCust").fields("shipvia")
'get def values for other charges[start]
Dim rsShipCode
set rsShipCode = server.CreateObject ("ADODB.Recordset")
strSQL = "select crltd_vlu ,crltd_nam from codes where cfld_name='SHIPVIA' and ccode_no='"& strShipType &"' and (crltd_nam = 'NCODCHARGE' or crltd_nam = 'NINSCHARGE' or crltd_nam = 'CUPS')"
'Response.Write strSQl
'Response.End 
rsShipCode.Open strSQL,conn
...
if trim(Request.QueryString ("type")) = "" then
	...
	'wma use deposit
	'ntDep = round(intTotChg,2)* 0.5
	intDep = round(intTotChg,2)* (Session("DepositPercent")/100)
	'create new charges record
	'check if record already exist then update values
	if isobject(Session("rsCharges")) then
		if  not Session("rsCharges") is nothing then
			...
			Session("rsCharges").Open "Select * from ordCharg where 1=0",conn,1,3
			Session("rsCharges").AddNew
			Session("rsCharges")("cOrder") = strOrder
			Session("rsCharges")("nShipAmt") = Session("ordAmount")
			Session("rsCharges")("nCOD") = round(intCODAmt,2) 'Request.Form ("txtCOD")
			Session("rsCharges")("nFreight") = round(intFreightAmt,2)'Request.Form ("txtFreight")
	...
	if Request.QueryString ("Type") = "D" and trim(Request.Form ("txtDisc")) <> "" then'calc disc
...
%>
		<script language="javascript">
			window.location.href = 'custorder.asp?From=Cat';
		</script>
<%
end if
%>
...
function doSubmit(strSubmitTo,nValue,type)
{
	...
	{	<%if session("UseOnlinePayment") = "T" then %>  //use online payment
			...
			<%if trim(session("GateWay")) = "PlugNPay.com (MemberShip Managment)" then %> //use PNP Membership
				...
				if (document.all("card-amount").value != 0)
				{
					...
					document.frmChg.action="../PnpCom/paymentform.asp?PNPCase=Deposit";														
				}
				else
				{
					document.frmChg.action="saveord.asp";
	...
	else
	{
		...
		document.frmChg.action = 'ordCharge.asp?save=<%=trim(Request.QueryString ("Save"))%>&type='+type+'&val='+nValue+'&taxAmt=<%=intTaxAmt%>&totTax=<%=intTotTax%>&totChg=<%=intTotChg%>&netChg=<%=intNetAmt%>&InsChg=<%=intInsurAmt%>&CODChg=<%=intCODAmt%>&FreightChg=<%=intFreightAmt%>&wght=<%=intTotWght%>&noCtn=<%=intTotalCtn%>';
		document.frmChg.submit ();
	}
}
...
//		alert(document.all("card-amount").value)
	...
	{	
		/*wma use deposit
		document.frmChg.txtDep.value = <%=round(intTotChg*0.5,2)%>;
		document.frmChg.hidDep.value = <%=round(intTotChg*0.5,2)%>;
		document.all("card-amount").value = <%=round(intTotChg*0.5,2)%>;
		document.frmChg.txtDep.disabled = false;
		*/
		document.frmChg.txtDep.value = <%=round(intTotChg * Session("DepositPercent")/100,2)%>;
		document.frmChg.hidDep.value = <%=round(intTotChg * Session("DepositPercent")/100,2)%>;
		document.all("card-amount").value = <%=round(intTotChg * Session("DepositPercent")/100,2)%>;
		document.frmChg.txtDep.disabled = false;
	}
}
function saveord()
...
function back()
{
	document.frmChg.action = 'ordCharge.asp?add=T'
	document.frmChg.submit ();
}
//-->
</SCRIPT>
...
<%
'WMA Hide menus when open this page in window
if trim(Request.QueryString ("Save")) <> "" then 
%>
...
<input type="hidden" name="card-amount" value=<%=round(intDep,2)%><%'=round(intTotChg*0.5,2)%>>
...
<TR>
	...
	<TD width=25% class="dark_cell"><%=trim(Session("AmountField"))%></TD>
...
<%'wal_127343 check the chargs fileds to display case custmer not like sales rep
if strFile = "cust" then%>
	<TR>
		<%if session("Weight") = "T" then%>
		...
		<%end if%>
		<TD class="dark_cell"><%=trim(Session("FreightField"))%></TD>
		<TD class="light_cell" align=right>
			<%if Trim(Session("CurrencyAlign")) = "LEFT" then%>
	...
	<%if lTax = "Y" and Session("Taxable") = "T" and session("UseTaxes") = "T" then'and cdbl(intTaxAmt) > 0 then%>
		...
		<TD class="light_cell" align=right>
			...
			<input type=text name=txtTax size=4  <%if strFile = "cust" then%> readonly <%end if%>
	...
	<%end if%>
	<%if session("ShowExpected") = "T" then%>
		<TR>
...
<%If Session("PriChg")>0 then'wma%>
	<TR>
		...
		<TD class="light_cell" align=right>
			<input type=checkbox name=chkPri 
			<%if trim(Request.QueryString ("Save")) = "" then%> disabled <%end if%>
			<%if Session("rsCharges")("lPrichg")= true then%>checked<%end if%> onclick="addChg(this,'P');">
			<%if Trim(Session("CurrencyAlign")) = "LEFT" then%>
...
<%If Session("DropChg")>0 then'wma%>
	<TR>
		...
		<TD class="light_cell" align=right>
			<input type=checkbox name=chkDrop 
			<%if trim(Request.QueryString ("Save")) = "" then%> disabled <%end if%>
			<%if Session("rsCharges")("lDropchg")= true then%>checked<%end if%> onclick="addChg(this,'S');">
			<%if Trim(Session("CurrencyAlign")) = "LEFT" then%>
...
<TR>
	...
	<TD class="dark_cell"><%=trim(Session("BalanceField"))%></TD>
	<TD class="light_cell" align=right>
...
<%'wal_check if there'll be deposit
   if session("UseDeposit") = "T" then
%>
<TR>
	<TD colspan=2 class="dark_cell" align=right></TD>
	...
	<TD class="light_cell" align=right>
		<input type=checkbox name=chkFull 
		<%if trim(Request.QueryString ("Save")) = "" then%> disabled <%end if%>
		<%if Request.Form ("chkFull") <> "" then%>checked<%end if%> onclick="payFull(this,'<%=round(intTotChg,2)%>');">Pay in Full
		<%if Trim(Session("CurrencyAlign")) = "LEFT" then%>
			<%=Session("Currency")%><input type=text name=txtDep size=5 <%if trim(Request.QueryString ("Save")) = "" then%> readonly <%end if%> <%if Request.Form ("chkFull") <> "" then%>value="<%=formatnumber(round(intTotChg,2),2)%>" disabled <%else%>value="<%=formatnumber(round(intDep,2),2)%>"<%end if%> onchange="document.all('card-amount').value = document.all('txtDep').value" <%if session("AllowEditDeposit") = "F" then Response.Write "readonly" %>>
		<%else%>
			<input type=text name=txtDep size=5 <%if trim(Request.QueryString ("Save")) = "" then%> readonly <%end if%> <%if Request.Form ("chkFull") <> "" then%>value="<%=formatnumber(round(intTotChg,2),2)%>" disabled <%else%>value="<%=formatnumber(round(intDep,2),2)%>"<%end if%> onchange="document.all('card-amount').value = document.all('txtDep').value" <%if session("AllowEditDeposit") = "F" then Response.Write "readonly" %> ><%=Session("Currency")%> 
		<%end if %>
		<input type=hidden name=hidDep size=5 value="<%=formatnumber(round(intTotChg,2),2)%>">
	</TD>
</TR>
<%end if%>
...
%>
		...
		<TR>
			...
			<TD class="light_cell" align=right>
				<select name="paymethod" <%if trim(Request.QueryString ("Save")) = "" then%> disabled <%end if%>>
...
%>
	...
	<TABLE  border=1  width="85%" bordercolor="#111111" style="border-collapse: collapse" cellpadding="0" cellspacing="0">
		<tr>
			<td class="dark_cell" width=100%>Notes:
<textarea name=txtNotes rows=8 align=left width=100% cols=104>
<%if session("Notes") = "" then%>Purchase Card No:
Expiry Date:
