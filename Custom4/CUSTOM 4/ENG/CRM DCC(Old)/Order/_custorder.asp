...
if trim(application("WareCode")) = "" then    
	...
	if rsOtsChk.EOF then'no orders for this style then get value in stock
		...
		strSQl = "Select stydye.stk1 as sum1, stydye.stk2 as sum2, "& _
				 ...
				 "from stydye where style = '" &Session("getstyle")& "' and styDye.CWARECODE = '"& trim(application("WareCode")) &"'"
...
IF Not Session("RSLine").EOF OR Not Session("RSLine").BOF   Then
   ...
   Session("RSLine").movefirst      
	...
	Do While Not Session("RSLine").EOF
		...
		'Response.Write "<font size=5>" & Session("RSLine").Fields("gros_price").value&"<BR>"	
		'Response.Write  Session("RSLine").Fields("price").value&"<BR>"	
		'Response.End 
