...
*! C202424,1 MMT 06/15/2021 Add new format for IRELAND[T20210330.0001]
***************************************************************************
  ...
  IF INVHDR.CONSOL = 'Y'
    ...
    *--Case consolidated invoice.
    ...
    SCAN REST WHILE Invoice+Store+Order+Style+STR(lineno,6) = lcInvcee
      ...
      *WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_SELCTDATA,oAriaApplication.GetHeaderText("LANG_ARPINV_SELCTDATA",AHEADERFILE)) + lcInvcee NOWAIT
      IF TYPE('oAriaEnvironment') <> 'O'
        WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_SELCTDATA,oAriaApplication.GetHeaderText("LANG_ARPINV_SELCTDATA",AHEADERFILE)) + lcInvcee NOWAIT
      ENDIF
...
IF TYPE('llRpIREFRM') ='L' AND INLIST(lcFormName , 'ARPINVIR','ARPINVOR')
  IF !USED('pack_hdr')
  ...
  DIMENSION laBank[4]
  laBank = ''
	IF !"gforbitinv" $ LOWER(SET("Procedure"))
	  SET PROCEDURE TO (oAriaApplication.DefaultPath+"gforbitinv.fxp") ADDITIVE
	ENDIF

  IF llRpIREFRM
    lcFormName = 'ARPINVIR'
    lcRpForm = "ARPINVIR"
    loOgScroll.lcOGLastForm = "ARPINVIR"
    loOgScroll.lcRpFrxMod = "Graphics"
    lcOGFormArr = 'laRpObj'+SUBSTR(loogscroll.lcOGPlatForm,1,3)
    loogscroll.lcOGLastForm = "ARPINVIR"
    *-- Create temporary .FRX or .LBX
    = gfCrtFrm("ARPINVIR",lcOGFormArr,.T.)
  ELSE
    lcFormName = 'ARPINVOR'
    lcRpForm = "ARPINVOR"
    loOgScroll.lcOGLastForm = "ARPINVOR"
    loOgScroll.lcRpFrxMod = "Graphics"
    lcOGFormArr = 'laRpObj'+SUBSTR(loogscroll.lcOGPlatForm,1,3)
    loogscroll.lcOGLastForm = "ARPINVOR"
    *-- Create temporary .FRX or .LBX
    = gfCrtFrm("ARPINVOR",lcOGFormArr,.T.)
  ENDIF
  ...
  *B608727,1 WAM 10/23/2008 (End)
  ...
  IF lnTerDiscD <> 0 .AND. lnTerDiscR <> 0
    ...
    *laString[1] = "If paid by " + DTOC(ldInvDate+lnTerDiscD) + " you may deduct " +  ALLT(STR( (lnTotalChg-lnTax_Amt)*lnTerDiscR/100 , 10,2))    
    laString[1] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_IFPAID,oAriaApplication.GetHeaderText("LANG_ARPINV_IFPAID",AHEADERFILE))  +;
                 DTOC(ldInvDate+lnTerDiscD) +;
                 IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_YOUMAYDEDUCT,oAriaApplication.GetHeaderText("LANG_ARPINV_YOUMAYDEDUCT",AHEADERFILE))   +  ALLT(STR( (lnTotalChg-lnTax_Amt)*lnTerDiscR/100 , 10,2))
    *N000682,1 MMT 02/25/2013 Globalization Changes[End]
    *B608727,1 WAM 10/23/2008 (End)
  ENDIF
  ...
  IF lnTerDscD1 <> 0 .AND. lnTerDscR1 <> 0
    ...
    *laString[2] = "If paid by " + DTOC(ldInvDate+lnTerDscD1) + " you may deduct " +  ALLT(STR( (lnTotalChg-lnTax_Amt)*lnTerDscR1/100 , 10,2))    
    laString[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_IFPAID,oAriaApplication.GetHeaderText("LANG_ARPINV_IFPAID",AHEADERFILE)) + DTOC(ldInvDate+lnTerDscD1) +;
                  IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_YOUMAYDEDUCT,oAriaApplication.GetHeaderText("LANG_ARPINV_YOUMAYDEDUCT",AHEADERFILE)) +  ALLT(STR( (lnTotalChg-lnTax_Amt)*lnTerDscR1/100 , 10,2))
    *N000682,1 MMT 02/25/2013 Globalization Changes[End]
    *B608727,1 WAM 10/23/2008 (End)
  ENDIF
