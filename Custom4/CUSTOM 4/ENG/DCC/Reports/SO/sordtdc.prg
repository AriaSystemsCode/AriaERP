*!*****************************************************************************************
*! Name      : Sordtdc.Prg  (Order detail report) custom for DCC
*! Developer : Mos - Mostafa eid 
*! Date      : 02/03/2009
*! Entry no. : T20080815.0003  
*! Tracking# : C201100 A27 - C201101 Aria4xp  
*!*****************************************************************************************
...
IF !llRpPrTPak
  DO CASE 
    CASE lcRpSortBy = 'A'    && Sort by Account#
     IF llRpPrStore 
      INDEX ON  ACCOUNT+Store+CORDTYPE+ORDER+Dist_ctr+Pack_Id+cPkColor+cPckSize+cPkVersion TAG lcPackINDX
        ELSE 
...
lcRpForm = IIF(llRpSizes,'SORDTDCA','SORDTDCB')
      ...
      *-- Scan ordline file for rest order data.
      SCAN REST WHILE cOrdType+Order+STR(LineNo,6) = cOrdType+&lcTOrdFile..Order
        ...
        *! B607998,1 SSH [END]
          ...
          ** Print un allocated Qty Only - Mos - 02/04/2009 [start]   
           IF !llRpAloOrd
             m.TOTQTY = 0
             FOR I = 1 To 8
               lcCount = ALLTRIM(STR(I))
               m.QTY&lcCount = m.QTY&lcCount - m.pik&lcCount 
               m.TOTQTY = m.QTY&lcCount + m.TOTQTY 
             ENDFOR    
           ENDIF  
          ** Print un allocated Qty Only - Mos - 02/04/2009 [end]   
...
IF llRpPrStore 
    IF lcRpSrt2 = 'L'    && Sort by Line#
      IF llCurInGrp
        lcIndexTg = 'ACCOUNT+STORE+CCURRCODE+CORDTYPE+ORDER+STR(LINENO,6)+STYLE'
      ELSE
        lcIndexTg = 'ACCOUNT+STORE+CORDTYPE+ORDER+STR(LINENO,6)+STYLE'
      ENDIF  
    ELSE      && Else Sort by Style
      IF lcRpSrt2 = 'I'    && Sort by DC/Style
        IF llCurInGrp
          lcIndexTg = 'ACCOUNT+STORE+CCURRCODE+CORDTYPE+ORDER+DIST_CTR+STYLE'
        ELSE
          lcIndexTg = 'ACCOUNT+STORE+CORDTYPE+ORDER+DIST_CTR+STYLE'
        ENDIF  
      ELSE  && Else Sort by Style  
        IF llCurInGrp
          lcIndexTg = 'ACCOUNT+STORE+CCURRCODE+CORDTYPE+ORDER+STYLE+STR(LINENO,6)'
        ELSE
          lcIndexTg = 'ACCOUNT+STORE+CORDTYPE+ORDER+STYLE+STR(LINENO,6)'
        ENDIF  
      ENDIF
   ENDIF  
ELSE     
