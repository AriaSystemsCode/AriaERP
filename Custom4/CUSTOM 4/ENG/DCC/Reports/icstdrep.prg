*:***************************************************************************
*: Program file  : ICSTDREP
*: Program desc. : Custom Style Summary report for DCC to sort by Vendor
*: For Report    : ICSTDBYS.FRX,ICSTDBYO.FRX,ICSTDBYL.FRX,ICSTDBYW.FRX
*: System        : Aria Advantage Series.
*: Module        : Inventory Control (IC)
*: Developer     : Mariam Mazhar[MMT] [T20150910.0001] C201718.exe 
*:***************************************************************************
*: Calls :
*:    Procedures :
*:    Functions  :
...
IF lcRpSortBy $ 'VSW' AND ALEN(laRPRepTar,1) = 1
...
ELSE
  ...
  IF lcRpSortBy == 'V' 
    GO BOTTOM
    STORE SPACE(6) TO lcVendor
    lcVendor = Vendor
    LOCATE FOR CENDREP = "A"
    REPLACE Vendor WITH lcVendor
    LOCATE
  ENDIF
...
ELSE  && Sort by anything rather than location.
  ...
  *-- if have one transaction only.
  ...
  ELSE
    lcRPFormNa = "ICSTDBYO"
  ENDIF
  IF (lcRpSortBy == "V" ) AND ALEN(laRPRepTar,1) = 1 AND !EMPTY(laRPRepTar)
    lcRPFormNa = "ICSTDBYV"
  ENDIF
...
IF FILE(oAriaApplication.WorkDir +RevStyle+".DBF")
  ...
  IF lcRpSortBy $ 'V'
    INDEX ON VENDOR + Style TAG VndrSty ADDITIVE 
  ENDIF  
...
laIndx[2,2] = 'StySort'
laIndx[3,1] = "Vendor+StyCode+WareCode+Dyelot+NotScale"
laIndx[3,2] = 'VndrSort'
FOR lnCnt=7 TO 16
  FOR lnInc=1 TO 183
...
CASE lcRpSortBy == 'V'   && Sort by Vendor
  laIndx[1,1] = "Vendor+StyCode+WareCode+Dyelot+cEndMaj+cEndSort+cEndRep+NotScale"
  laIndx[1,2] = 'VndrSort'
...
DO CASE
CASE lcRpSortBy == 'V'   && Sort by Location
  laIndx[1,1] = "cEndMaj+cEndSort+cEndRep+Vendor+cStyMajor+NotScale"
  laIndx[1,2] = 'VndrSort'
...
SCAN
  ...
  *--Collect data From StyDye in case of (dyelots for Stock)=Yes ,or (print configuration)=Yes
  ...
  ELSE
    ...
    ELSE
      SELECT (lcTrns)
      SCATTER FIELDS cStyMajor,StyCode,WareCode,Dyelot,SCALE,Price,TotCost,Ave_Cost,COLOR,Vendor MEMVAR
...
REPLACE Vendor WITH &lcStyle..vendor 
...
SCAN
  ...
  ELSE
    ...
    ELSE
      ...
      *------------------------------------------------------*
      SELECT (lcStyTmp)
      SCATTER FIELDS cStyMajor,StyCode,WareCode,Dyelot,Season,Division,FGroup,SGroup,MatType,Dept,SCALE,Price,TotCost,Ave_Cost,Vendor MEMVAR
      SELECT (lcTotTmp)
...
CASE lcRpSortBy == 'V'
  lcSortFld  = '.Vendor'
  lcSortTtl  = "Vendor"
  lcGroupExp = lcStyTmp+'.Vendor'
  lcSortExp  = ["Vendor"+SPACE(1)+PADL(": ",2+MAX(LEN(lcMajTtl)-LEN("Vendor"),0))]
  lcMajExp   = [lcMajTtl + SPACE(1) + PADL(': ',2+MAX(LEN("Vendor")-LEN(lcMajTtl),0))]
  lcNMajExp  = [ALLTRIM(lcRepNMTtl)+SPACE(1)+ PADL(": ",2+MAX(LEN(LANG_ICSTYREP_Description)-LEN(ALLTRIM(lcRepNMTtl)),0))]
  lcDescExp  = [LANG_ICSTYREP_Description+SPACE(1)+PADL(': ',2+MAX(LEN(ALLTRIM(lcRepNMTtl))-LEN(LANG_ICSTYREP_Description),0))]  
ENDCASE
    ...
    *-- if not print sizes "lcPrintHdr" will not include scales
    IF !llRpPrtSiz
      lcPrintHdr = PADR(ALLTRIM(lcMajTtl) + ' :' +ALLTRIM(EVALUATE(lcStyGroup)) + ' ' + PADR(&lcStyle..DESC,15) +;
        lfEndOfGrp(@lnEndOfSty,IIF(lcRpSortby ='V',&lcStyle..Vendor,'')+PADR(&lcStyle..STYLE,lnMajorLen)),35) + SPACE(3)
    ELSE
      IF llTextMode
        lcPrintHdr = PADR(ALLTRIM(lcMajTtl) + ' :' +ALLTRIM(EVALUATE(lcStyGroup)) + ' ' + PADR(&lcStyle..DESC,12) +;
          lfEndOfGrp(@lnEndOfSty,IIF(lcRpSortby ='V',&lcStyle..Vendor,'')+PADR(&lcStyle..STYLE,lnMajorLen)),28) + SPACE(0) +;
      ...
      ELSE
        lcPrintHdr = PADR(ALLTRIM(EVALUATE(lcStyGroup)) + ' ' + &lcStyle..DESC +;
          lfEndOfGrp(@lnEndOfSty,IIF(lcRpSortby ='V',&lcStyle..Vendor,'')+lcPrntSty),35)
...
lcTagName = IIF(lcMastFile = '&lcStyle' and lcrpSortby='V', 'VndrSty',lcTagName )
SELECT (lcRevFile)
lcFilter = SET("Filter")
SET FILTER TO
...
IF ALEN(laRpRepTar,1)=1 AND lcRpFormNa $ 'ICSTDBYS|ICSTDBYL|ICSTDBYV'
