*** 
*** ReFox XI+  #BD405411  Mariam  Mariam [VFP90]
***
 llalpktk = .F.
 ...
 *XX
 ...
 SCAN
    ...
    ENDIF
 ENDSCAN
 STORE SPACE(0) TO lcpktkts, lcfrmpktk, lctopktk
 STORE 0 TO lntotpktk
 SELECT (lctmpgroup)
 LOCATE
 IF FOUND()
    lcfrmpktk = piktkt
 ENDIF
 SCAN
    lntotpktk = lntotpktk+1
    lctopktk = piktkt
    lcpktkts = lcpktkts+' '+piktkt
 ENDSCAN
 ...
 IF llrpblksum
    ...
    IF  .NOT. EOF()
       ...
       IF llrpbatch
          ...
          SELECT lcbthpik
          SCAN
             IF gfseek(lcbthpik.piktkt, 'pickpack') .AND.  .NOT. pickpack.lcomppick
                SELECT pickpack
 ...
 *X
 ...
 IF SEEK("S" + SUBSTR(&lctmpordl..STYLE , lnscaposgl , 2))
    ...
    SCAN FOR type+scale+prepak="S"+lckey
       ...
       *SELECT (lctmpsizes)
       *SET ORDER TO (lctmpsizes)
       IF SEEK(lckey+m.cdim1, lctmpsizes)
