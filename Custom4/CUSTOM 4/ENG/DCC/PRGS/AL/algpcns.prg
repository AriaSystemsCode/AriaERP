...
*: B609814,1 MMT 02/01/2012 Fix bug of wrong totpick qty in the custom scan pick tkt prg[T20120111.0024]
*: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[T20110401.0003]
*: B609977,1 SAB 06/25/2012 CRM/AR StyHist.nUsed field value doubling on invoicing [T20120606.0013]
*: B610007,1 SAB 07/16/2012 Manual Packing List update book figure incorectly [T20120227.0014]
*: B610217,1 MMT 01/27/2013 Call INVCHARG trigger after saving invoice from scan picking ticket [T20130121.0002]
*: B610244,1 HIA 02/14/2013 AR-Invoice VAT(Tax) Calculation incorrect with Auto carriage charges [T20130212.0001]
*: B610371,1 HIA 06/010/2013 T20130530.0019	AL - spelling mistake in the Scan Pick Ticket Program
*: B611174,1 MMT 08/02/2016 Custom Scan Picking ticket program gives error INVCHRG table is not found[T20160725.0004]
*:*********************************************************************************
...
loFormSet.llchkpkpack= gfGetMemVar('M_CHKPKPAK',oAriaApplication.ActiveCompanyID)
  ...
  ELSE
    ...
    IF EOF()
      ...
      *IF EMPTY(ALLTRIM(PICKPACK.CPICKBY)) OR EMPTY(ALLTRIM(PICKPACK.CHKEDBY))
  	  IF loFormset.llchkpkpack AND EMPTY(ALLTRIM(PICKPACK.CPICKBY)) &&OR EMPTY(ALLTRIM(PICKPACK.CHKEDBY))
  	  *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[End]
        ...
        *= GFMODALGEN('INM00000B00000',.F.,.F.,.F.,'You cannot scan Pick Ticket that has an empty Picked by or Checked by value')
        = GFMODALGEN('INM00000B00000',.F.,.F.,.F.,'You cannot scan Pick Ticket that has an empty Picked by')
        *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[END]
	    loFormSet.AriaForm1.txtPiktkt.Value = ''
	    RETURN 0
 	  ENDIF	 
 	  *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[Start]
 	  IF loFormset.llchkpkpack AND !PICKPACK.LCOMPPICK
  	    loFormSet.ariaform1.cboPick.Value = ""
	    loFormSet.ariaform1.cboPack.Value = ""
        = GFMODALGEN('INM00000B00000',.F.,.F.,.F.,'You cannot scan Pick Ticket that is not completely picked')
	    loFormSet.AriaForm1.txtPiktkt.Value = ''
	    RETURN 0
 	  ENDIF
 	  *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[END]
    ...
    ELSE
       ...
       *IF loFormSet.ariaform1.cboPick.Value <> ALLTRIM(PICKPACK.CPICKBY) OR loFormSet.ariaform1.cboPack.Value <> ALLTRIM(PICKPACK.CHKEDBY)
       IF loFormset.llchkpkpack AND  loFormSet.ariaform1.cboPick.Value <> ALLTRIM(PICKPACK.CPICKBY) &&OR loFormSet.ariaform1.cboPack.Value <> ALLTRIM(PICKPACK.CHKEDBY)
       *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[End]
         llScanOk = .F.
    ...
    IF LLSCANOK
      ...
      *: C201334,1 MMT 05/11/2011 Custom Pick and Pack monitor screen[T20110401.0003][End]
      ...
      SELECT ORDLINE
      *: B609814,1 MMT 02/01/2012 Fix bug of wrong totpick qty in the custom scan pick tkt prg[T20120111.0024][Start]
      *SUM TOTQTY TO lnTotPik  REST WHILE CORDTYPE+ORDER+STR(LINENO,6) = 'O'+ &lcTmpPkTk..ORDER  FOR PIKTKT = &lcTmpPkTk..PIKTKT
      SUM TOTPIK TO lnTotPik  REST WHILE CORDTYPE+ORDER+STR(LINENO,6) = 'O'+ &lcTmpPkTk..ORDER  FOR PIKTKT = &lcTmpPkTk..PIKTKT
      *: B609814,1 MMT 02/01/2012 Fix bug of wrong totpick qty in the custom scan pick tkt prg[T20120111.0024][END]
      SELECT &lcTmpPkTk 
...
SCAN
   ...
   *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[Start]
   	=gfReplace("CHKEDBY WIth '"+loFormset.ariaform1.cboPack.Value+"'") 
   	=gfReplace("dchkby WIth DATE()")
   	=gfReplace("cchkbytime WIth '"+TIME()+"'")
   ENDIF
   *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[ENd]
   *: C201334,1 MMT 05/11/2011 Custom Pick and Pack monitor screen[T20110401.0003][End]
ENDSCAN
...
= LFCRTPACK(loFormset)
*: B609977,1 SAB 06/25/2012 CRM/AR StyHist.nUsed field value doubling on invoicing [Start]
* Don't update StyHist as it is updated in SOUPDATE.PRG
*= LFUPDENT()
*: B609977,1 SAB 06/25/2012 CRM/AR StyHist.nUsed field value doubling on invoicing [End]
...
IF LLEDISYS .AND. LLEDIACC
   *: C201465,1 MMT 03/14/2012 Some changes in custom Pick Pack programs for DCC[T20110401.0003][Start]
   lnDrctTo = IIF(Pack_Hdr.CToStorCn='C',2,1)
   *: C201465,1 MMT 03/14/2012 Some changes in custom Pick Pack programs for DCC[T20110401.0003][END]
...
ENDIF
*: B611174,1 MMT 08/02/2016 Custom Scan Picking ticket program gives error INVCHRG table is not found[T20160725.0004][Start]
*LLINVCHRG = LLISENGLAN .AND.  GFOPENTabLE('InvChrg','InvChrg','SH')
LLINVCHRG = LLISENGLAN 
IF !USED('InvChrg')
  =GFOPENFILE(oAriaApplication.DataDir+'InvChrg',oAriaApplication.DataDir+'InvChrg','SH')
ENDIF
LLINVCHRG = LLINVCHRG AND USED('InvChrg')
*: B611174,1 MMT 08/02/2016 Custom Scan Picking ticket program gives error INVCHRG table is not found[T20160725.0004][End]
...
IF ASCAN(loFORMSET.laEvntTrig,'INVCHARG')=0
   ...
   *: B610217,1 MMT 01/27/2013 Call INVCHARG trigger after saving invoice from scan picking ticket [Start]
   *loFORMSET.LAEVNTTRIG[ ALEN(loFORMSET.laEvntTrig,1),2] = "DIRMAIN"
   loFORMSET.LAEVNTTRIG[ ALEN(loFORMSET.laEvntTrig,1),2] = "PANMAIN"
   *: B610217,1 MMT 01/27/2013 Call INVCHARG trigger after saving invoice from scan picking ticket [END]
...
SCAN
   ...
   LADATA[ 5] = STORE
   M.NCHARGES = 0
   *: B610244,1 HIA 02/14/2013 AR-Invoice VAT(Tax) Calculation incorrect with Auto carriage charges [T20130212.0001][Start]
   m.TRDE_DISC = &lcInvHdr..TRDE_DISC
   *: B610244,1 HIA 02/14/2013 AR-Invoice VAT(Tax) Calculation incorrect with Auto carriage charges [T20130212.0001][End]
   = LFUPDIRCHG()
ENDSCAN
SELECT &lcInvHdr
SET DELETED ON
...
*: B610007,1 SAB 07/16/2012 Manual Packing List update book figure incorectly [Start]
IF USED('PACK_HDR')
  SELECT PACK_HDR
  =gfTableUpdate()
ENDIF
*: B610007,1 SAB 07/16/2012 Manual Packing List update book figure incorectly [End]	
...
*! B610371,1 HIA 06/010/2013 T20130530.0019	AL - spelling mistake in the Scan Pick Ticket Program [Begin]
lclok_user = ''
*! B610371,1 HIA 06/010/2013 T20130530.0019	AL - spelling mistake in the Scan Pick Ticket Program [End]

IF LFBEFORSAV(loFormSet)
   IF GFMODALGEN('INM00000B00006',.F.,.F.,.F.,'Are you sure you want to Save')=1
     *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[Start]
     loFormSet.ariaform1.cmdFinish.Click()
     *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[End]
	 ...
	 *: C201334,1 MMT 05/11/2011 Custom Pick and Pack monitor screen[T20110401.0003][Start]
	   ...
	   *IF EMPTY(loFormSet.ariaform1.cboPick.Value) OR EMPTY(loFormSet.ariaform1.cboPack.Value)
       IF loFormset.llchkpkpack AND EMPTY(loFormSet.ariaform1.cboPick.Value) 
       *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[END]
       *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[Start]
       *= GFMODALGEN('INM00000B00000',.F.,.F.,.F.,'Cannot save with empty Picked by or Empty Checked by')
	   = GFMODALGEN('INM00000B00000',.F.,.F.,.F.,'Cannot save with empty Picked by')
       *: C201465,1 MMT 03/04/2012 Some changes in custom Pick Pack programs for DCC[END]
	   RETURN .F.
	 ENDIF	 
	 *: C201334,1 MMT 05/11/2011 Custom Pick and Pack monitor screen[T20110401.0003][End]
