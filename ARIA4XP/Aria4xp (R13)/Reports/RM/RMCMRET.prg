...
*: Program desc. : CREDIT MEMO FOR RETURNS.
*: Date          : 05/17/2006
*: System        : Aria 4XP
...
*: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [T20091009.0002]
*! B609356,1 SMA 07/21/2010 Fix bug of creating empty *.cdx files [T20091027.0093]
*! B609384,1 SMA 08/17/2010 Totals in CM header journal report(summary,details)are not the same [T20100803.0014]
*! B609929,1 SAB 05/20/2012 Fix Problem in CR Header Jornal When Select 25 Division [T20120507.0010]
*! B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038]
*****************************************************************************
#Include r:\aria4xp\reports\rm\RMCMRET.h
...
*! B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
STORE .F. TO llStateInFilter
*! B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][End]
...
IF llOGFltCh
  ...
  *-- if you have previous data clear workfile then recreate it.
  ...
  IF !EMPTY(laOGVrFlt[1,1])
    ...
    *! B609929,1 SAB 05/20/2012 Fix Problem in CR Header Jornal When Select 25 Division [T20120507.0010][Start]
    *- DIVISION
    lnDivPos = AT("INLIST(RETHDR.CDIVISION", lcVarbFilt)
    IF lnDivPos > 0
      lcDivSel = SUBSTR(lcVarbFilt, lnDivPos)
      lnDivLen = AT(")", lcDivSel)
      lcDivSel = SUBSTR(lcVarbFilt, lnDivPos, lnDivLen)
      ...
      laTempacstru[1,3] = 6
      laTempacstru[1,4] = 0
      =gfCrtTmp(lcDivFile, @laTempacstru, 'CDIVISION', lcDivFile, .T.)

      FOR lnItm = 1 TO OCCURS(",", lcDivSel) + 1
        lnStart = AT(",'", lcDivSel, lnItm) + 2
        lnEnd   = AT("'", SUBSTR(lcDivSel, lnStart)) - 1
        SELECT(lcDivFile)
        APPEND BLANK
        REPLACE CDIVISION WITH SUBSTR(lcDivSel, lnStart, lnEnd)
      ENDFOR

      lcVarbFilt = STUFF(lcVarbFilt, lnDivPos, lnDivLen, "Seek(RETHDR.CDIVISION,'&lcDivFile')")
    ENDIF
    *! B609929,1 SAB 05/20/2012 Fix Problem in CR Header Jornal When Select 25 Division [T20120507.0010][End]
    *! B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Start]
    *- State
    STORE .F. TO llStateInFilter
    lnStatePos = AT("INLIST(CUSTOMER.CADDRESS4", lcVarbFilt)
    IF lnStatePos > 0
      STORE .T. TO llStateInFilter
      lcStateSel = SUBSTR(lcVarbFilt, lnStatePos)
      lnStateLen = AT(")", lcStateSel)
      lcStateSel = SUBSTR(lcVarbFilt, lnStatePos, lnStateLen)

      lcStateFile = loOGScroll.gfTempName()
      DIMENSION laTempacstru[1,4]
      laTempacstru[1,1] = 'STATE'
      laTempacstru[1,2] = 'C'
      laTempacstru[1,3] = 6
      laTempacstru[1,4] = 0
      =gfCrtTmp(lcStateFile, @laTempacstru, 'STATE', lcStateFile, .T.)

      FOR lnItm = 1 TO OCCURS(",", lcStateSel) + 1
        lnStart = AT(",'", lcStateSel, lnItm) + 2
        lnEnd   = AT("'", SUBSTR(lcStateSel, lnStart)) - 1
        SELECT(lcStateFile)
        APPEND BLANK
        REPLACE STATE WITH SUBSTR(lcStateSel, lnStart, lnEnd)
      ENDFOR

      lcVarbFilt = STUFF(lcVarbFilt, lnStatePos, lnStateLen, " .T. ")
...
*sales rep selection
 ...
 ELSE
    *-- SQL connection error. can't open the report
    ...
    RETURN .F.
  ENDIF
  *B609384,1 SMA 08/17/2010 Totals in CM header journal report(summary,details)are not the same....[BEGIN]
  *DO CASE
    *CASE lcRpFormat = "D"
     *B609384,1 SMA 08/17/2010 Totals in CM header journal report(summary,details)are not the same....[END]
      =lfGetDetal()
      *B609384,1 SMA 08/17/2010 Totals in CM header journal report(summary,details)are not the same....[BEGIN]
    *CASE lcRpFormat = "S"
...
IF TYPE('lcLastTag') = 'N'
  ...
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
  lnLen = ALEN(laTempStru,1) + 1
  *DIMENSION laTempStru[lnLen,4]
  DIMENSION laTempStru[lnLen,18]
  laTempStru[lnLen,1] = 'STATE'
...
*B609356,1 SMA 07/21/2010 remove of clause to prevent empty *.cdx files from creation.....[END]
  ...
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
  CASE lcRpSortBy = "T"
    INDEX ON STATE+cCurrcode TAG "TempIndx" OF (lcWorkFile)
  CASE lcRpSortBy = "D"
    INDEX ON DTOS(CRDATE)+cCurrcode TAG "TempIndx" OF (lcWorkFile)
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][End]
 ...
 *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
  CASE lcRpSortBy = 'T'		&& Sort by State
    lcRpIndTag = [STATE + CRMEMO]
    IF llMultCurr .OR. lcRpCurr<> "F"
      lcRpIndTag = [STATE +ccurrcode+ CRMEMO]
    ENDIF

  CASE lcRpSortBy = 'D'		&& Sort by Transaction Date
    lcRpIndTag = [DTOS(CRDATE) + CRMEMO]
    IF llMultCurr .OR. lcRpCurr<> "F"
      lcRpIndTag = [DTOS(CRDATE) +ccurrcode+ CRMEMO]
    ENDIF
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][End]
ENDCASE
*B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
...
*: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [Start]
IF M.nHstAmt !=0
  M.nHstAmt = gfAmntDisp(M.nHstAmt,lcRpCurr,ldRpExDate,lcRpTmpNam,.F.,"RETHDR")
ENDIF
IF M.nPstAmt !=0
  M.nPstAmt = gfAmntDisp(M.nPstAmt,lcRpCurr,ldRpExDate,lcRpTmpNam,.F.,"RETHDR")
ENDIF
*: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [End]
...
DO CASE
  ...
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
  CASE lcRpSortBy = 'T'		&& Sort by State case
    lcGrpBy    ="STATE"
    lcGrpFoot  =" 'State: '+STATE+' '+ SUBSTR(gfCodDes(STATE,'STATE'),1,10) "
    IF llMultCurr .OR. lcRpCurr<> "F"
      lcGrpBy="STATE+ccurrcode"
      lcGrpFoot  =" 'State: '+SUBSTR(STATE,1,3)+ SUBSTR(gfCodDes(STATE,'STATE'),1,10) +'('+ ccurrcode+ ')' "
    ENDIF
    IF llMultCurr AND lcRpCurr<>"F" AND  lcRpFormat="S"
      STORE '' TO lcGrpFoot
    ENDIF
  CASE lcRpSortBy = 'D'		&& Sort by Primary Sales Rep. Case
    lcGrpBy    ="DTOS(CRDATE)"
    lcGrpFoot  =" 'Tran. Date: '+DTOC(CRDATE) "
    IF llMultCurr .OR. lcRpCurr<> "F"
      lcGrpBy="DTOS(CRDATE)+ccurrcode"
      lcGrpFoot  =" 'Tran.Date: '+DTOC(CRDATE) +' ('+ ccurrcode+ ')' "
    ENDIF
    IF llMultCurr AND lcRpCurr<>"F" AND  lcRpFormat="S"
      STORE '' TO lcGrpFoot
    ENDIF
 ...
 *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
 =SEEK('M'+RETHDR.ACCOUNT,'CUSTOMER') && GFsEEK in case of table is opened with gfopentable
 M.STATE = SUBSTR(CUSTOMER.CADDRESS4,1,6)
 *IF AT("INLIST(CUSTOMER.CADDRESS4", lcVarbFilt) > 0
 IF llStateInFilter
    IF !Seek(M.STATE,lcStateFile)
      Loop
    ENDIF
  ENDIF
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][End]
...
*!*	PRIVATE lnTax_Amt
*!*	lnTax_Amt = 0
PRIVATE lnTax_Amt, lnPstAmt
STORE 0 TO lnTax_Amt, lnPstAmt
*: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [End]
 ...
 *IF AT("INLIST(CUSTOMER.CADDRESS4", lcVarbFilt) > 0
  ...
  *!*	  lnTax_Amt  = lnTax_Amt +M.Tax_Amt
  lnTax_Amt  = lnTax_Amt + M.Tax_Amt + M.nHstAmt
  lnPstAmt   = lnPstAmt  + M.nPstAmt
  *: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [End]
ENDSCAN
 ...
 *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
  CASE lcRpSortBy = "T"
    lcExp = "M.STATE+RETHDR.ccurrcode"
  CASE lcRpSortBy = "D"
    lcExp = "DTOS(RETHDR.CRDATE)+RETHDR.ccurrcode"
  *B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][End  ]
ENDCASE
 ...
 *IF AT("INLIST(CUSTOMER.CADDRESS4", lcVarbFilt) > 0
  ...
  *: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [Start]
  && Including the HST tax amount with the GST
  m.Tax_Amt = m.Tax_Amt + m.nHstAmt
  *: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [End]
  ...
  *: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [Start]
    REPLACE nPstAmt    WITH nPstAmt + M.nPstAmt
  *: B609076,1 HES 10/29/2009 HST amount is not printing on credit memo header journal [End]
  ENDIF
...
*--End of lfvAcc.*B609940,1 HIA 05/27/2012 Fix Problem in CR Header Missing [T20120514.0038][Begin]
