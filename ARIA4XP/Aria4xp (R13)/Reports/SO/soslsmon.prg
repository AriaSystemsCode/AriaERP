...
*B612656,1 MMT 02/07/2023 Order/Sales Summary Report by Month report export to Excel does not export the shipped columns[T20230202.0001]
*B612659,1 MMT 02/20/2023 Export Reports data with valid columns names[T20230202.0001]
*:***************************************************************************
*N000682,1 MMT 02/12/2013 Globalization changes[Start]
#include r:\aria4xp\reports\so\soslsmon.h
*N000682,1 MMT 02/12/2013 Globalization changes[End]
...
*=gfModalGen("TRM000000B00000","DIALOG",'','',LANG_SoSlsmon_MsgMonth)
=gfModalGen("TRM000000B00000","DIALOG",'','',IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SoSlsmon_MsgMonth,oAriaApplication.GetHeaderText("LANG_SoSlsmon_MsgMonth",AHEADERFILE)))
...
*N037459,1 MMT 05/25/2006 Invalid date format [End]
  ...
  * N000682 ,1 Thabet Handle globalization issues [Start]
  *N000682,1 11/20/2012 MMT Globlization changes[Start]
*=gfModalGen("TRM000000B00000","DIALOG",'','',LANG_SoSlsmon_MsgDate)
=gfModalGen("TRM000000B00000","DIALOG",'','',IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SoSlsmon_MsgDate,oAriaApplication.GetHeaderText("LANG_SoSlsmon_MsgDate",AHEADERFILE)))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
ELSE
  ...
  *B612656,1 MMT 02/07/2023 Order/Sales Summary Report by Month report export to Excel does not export the shipped columns[T20230202.0001][Start]
  *B612659,1 MMT 02/20/2023 Export Reports data with valid columns names[T20230202.0001][Start]
*!*	  IF (oAriaApplication.gcDevice = "FILE" .And. loogScroll.cTextRepType = "EXCEL")
*!*	    SELECT &lcWorkFile..*,&lcTmpDet..nShipQty,&lcTmpDet..nShipAmt,&lcTmpDet..cYear as 'Ship_Year',&lcTmpDet..cMonth as 'Ship_Month' from (lcWorkFile) ,(lcTmpDet) WHERE .F. INTO CURSOR 'ExcelData' READWRITE 
*!*	    SELECT (lcWorkFile)
*!*	    SCAN
*!*	      SCATTER MEMO MEMVAR 
*!*	      llAdded =.F.
...
*!*	      
*!*	      
*!*	      IF SEEK(IIF(lcRpGroup="S",&lcWorkFile..cStyMajor,IIF(lcRpGroup="Z",&lcWorkFile..cSeason,IIF(lcRpGroup="D",&lcWorkFile..cDivision,"")))+&lcWorkFile..cYear+&lcWorkFile..cMonth,(lcTmpDet)) 
*!*	        m.nShipQty = &lcTmpDet..nShipQty 
*!*	        m.nShipAmt = &lcTmpDet..nShipAmt
*!*	        m.Ship_Year = &lcTmpDet..cYear 
*!*	        m.Ship_Month = &lcTmpDet..cMonth 
*!*	        INSERT INTO 'ExcelData' FROM MEMVAR 
*!*	        llAdded =.T.
*!*	      ENDIF
*!*	      IF SEEK(IIF(lcRpGroup="S",&lcWorkFile..cStyMajor,IIF(lcRpGroup="Z",&lcWorkFile..cSeason,IIF(lcRpGroup="D",&lcWorkFile..cDivision,"")))+ALLTRIM(STR(VAL(&lcWorkFile..cYear)-1))+&lcWorkFile..cMonth,(lcTmpDet)) 
*!*	        m.nShipQty = &lcTmpDet..nShipQty 
*!*	        m.nShipAmt = &lcTmpDet..nShipAmt
*!*	        m.Ship_Year = &lcTmpDet..cYear 
*!*	        m.Ship_Month = &lcTmpDet..cMonth 
...
*!*	        llAdded =.T.
*!*	      ENDIF
*!*	      IF SEEK(IIF(lcRpGroup="S",&lcWorkFile..cStyMajor,IIF(lcRpGroup="Z",&lcWorkFile..cSeason,IIF(lcRpGroup="D",&lcWorkFile..cDivision,"")))+ALLTRIM(STR(VAL(&lcWorkFile..cYear)-2))+&lcWorkFile..cMonth,(lcTmpDet))
*!*	        m.nShipQty = &lcTmpDet..nShipQty 
*!*	        m.nShipAmt = &lcTmpDet..nShipAmt
*!*	        m.Ship_Year = &lcTmpDet..cYear 
*!*	        m.Ship_Month = &lcTmpDet..cMonth 
...
*!*	    ENDSCAN
 IF (oAriaApplication.gcDevice = "FILE" .And. loogScroll.cTextRepType = "EXCEL")
    SELECT &lcWorkFile..cDivision,&lcWorkFile..cSeason,&lcWorkFile..cStyGroup,;
    &lcWorkFile..cMonth AS Month, &lcWorkFile..cYear AS Year,&lcWorkFile..cDesc,;
    &lcWorkFile..nStartQty AS 'Open_Order_Qty_By_Start'  , &lcWorkFile..nStartAmt AS 'Open_Order_Amount_By_Start' ,;
    &lcWorkFile..nComplQty AS 'Open_Order_Qty_By_Complete' , &lcWorkFile..nComplAmt AS 'Open_Order_Amount_By_Complete',  &lcWorkFile..cStyMajor,;
    &lcWorkFile..DCurrDate AS DTRDATE ,&lcTmpDet..nShipQty As Ship,&lcTmpDet..nShipAmt,&lcTmpDet..cYear as 'Ship_Year',;
    &lcTmpDet..cMonth as 'Ship_Month' from (lcWorkFile) ,(lcTmpDet) WHERE .F. INTO CURSOR 'ExcelData' READWRITE 
    SELECT (lcWorkFile)
    SCAN
      SCATTER MEMO MEMVAR 
      ...
      m.Month = m.CMONTH
      m.Year = m.CYEAR
      m.Open_Order_Qty_By_Start= m.nStartQty 
      m.Open_Order_Amount_By_Start= m.nStartAmt 
      m.Open_Order_Qty_By_Complete= m.nComplQty 
      m.Open_Order_Amount_By_Complete= m.nComplAmt 
      m.DTRDATE = m.DCurrDate
      ...
      ENDIF 
    ENDSCAN
    *B612659,1 MMT 02/20/2023 Export Reports data with valid columns names[T20230202.0001][End]
    IF USED('ExcelData')
      SELECT 'ExcelData'
    ENDIF
  ENDIF  
  *B612656,1 MMT 02/07/2023 Order/Sales Summary Report by Month report export to Excel does not export the shipped columns[T20230202.0001][End]
...
*          WAIT WINDOW "Collecting data for order# " + OrdLine.Order NOWAIT
          *N000682,1 11/20/2012 MMT Globlization changes[Start]
*WAIT WINDOW LANG_SoSlsmon_MsgCollect + OrdLine.Order NOWAIT
WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SoSlsmon_MsgCollect,oAriaApplication.GetHeaderText("LANG_SoSlsmon_MsgCollect",AHEADERFILE)) + OrdLine.Order NOWAIT
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
*!*	laSlctDesc[4,1] = 'None'
*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laSlctDesc[2,1] = LANG_SEASON
laSlctDesc[2,1] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SEASON,oAriaApplication.GetHeaderText("LANG_SEASON",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]

*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laSlctDesc[3,1] = LANG_DIVISION
laSlctDesc[3,1] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_DIVISION,oAriaApplication.GetHeaderText("LANG_DIVISION",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]

*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laSlctDesc[4,1] = LANG_NONE
laSlctDesc[4,1] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_NONE,oAriaApplication.GetHeaderText("LANG_NONE",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
*  lcMessage = "Number of lines:" + ALLTRIM(STR(RECCOUNT()))+"   will be printed in :  "+ALLTRIM(STR(_PAGETOTAL))+"  Pages"
  *N000682,1 11/20/2012 MMT Globlization changes[Start]
*lcMessage = LANG_Number_of_lines + ALLTRIM(STR(RECCOUNT()))+ LANG_print +ALLTRIM(STR(_PAGETOTAL))+ LANG_Pages
lcMessage = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Number_of_lines,oAriaApplication.GetHeaderText("LANG_Number_of_lines",AHEADERFILE)) + ALLTRIM(STR(RECCOUNT()))+ IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_print,oAriaApplication.GetHeaderText("LANG_print",AHEADERFILE)) +ALLTRIM(STR(_PAGETOTAL))+ IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Pages,oAriaApplication.GetHeaderText("LANG_Pages",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]
