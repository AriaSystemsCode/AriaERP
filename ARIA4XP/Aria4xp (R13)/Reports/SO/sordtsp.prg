*:**********************************************************************************
*: Program file  : SORDTSP.PRG
*: Program desc. : Custom Order Detail Report For Speed Sourcing
*: For Report    : (SORDTSPA.FRX,SORDTSPB.FRX)
...
*:**********************************************************************************
*: Example : DO SORDTSP.PRG
*:**********************************************************************************
*: This Program is due to CP131124 ...
*:**********************************************************************************
*:Modifications
*:**********************************************************************************
#Include r:\aria4xp\reports\so\sordts.h
...
*--Adding a variable to detect the changes of SCALE in FRX so as not to print it except when it changes.
lcOldScale = SPACE(3)
...
PRIVATE lnItmPos
lnItmPos = ASCAN(laOGObjType,lcItmInFlt,-1,-1,-1,5)
IF lnItmPos > 0
  ...
  *-- Different sort by cases.
  DO CASE
    ...
    CASE lcRpSortBy = 'S'      && Sort by style
      IF lcRpKind = 'D'
		*--The old code commented all of it.
      ENDIF
      *-- Enable/disable variable objects due to sort case. [begin]
...
lcRpForm = IIF(llRpSizes,'SORDTSPA','SORDTSPB')
= lfRepPltFr(lcRpForm)
...
*!*************************************************************
*! Called from : SORDTSPA.FRX, OR SORDTSPB.FRX [Variable lcDum in the report]
*!*************************************************************
*! Calls       : 
*!              Procedures : ....
*!              Functions  : ....
...
*!*************************************************************
*! Called from : [SORDTSPA.FRX OR SORDTSPB.FRX, ORDER GROUP HEADER BAND] 
*!*************************************************************
*! Calls       : ....
*!*************************************************************
*! Passed Parameters : None
...
IF 'MA' $ gcCmpModules
  ...
  lnInsFabIn = ASCAN(laSlctDesc,'All',-1,-1,-1,5) &&ASCAN(laSlctDesc,'All',1)
...
lnItmPos = ASCAN(laOGFxFlt,lcItmInFlt,-1,-1,-1,5)
IF lnItmPos > 0
...
lnCurVlPos = ASCAN(laCurrVal,lcCurrVal,-1,-1,-1,5)
IF lnCurVlPos > 0
