...
*B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008]
*:***************************************************************************
...
*!*	                  Pack_No INTO (lcRevTemp) ADDITIVE
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][Start]
*!*	  SET RELATION TO "O" + ORDER +  STR(nordlineno,6) INTO &lcTempordLn,;
*!*	                  "O" + ORDER INTO &lcTempordhdr,;
*!*	                  IIF(EMPTY(Store),'M','S') + Account + Store INTO &lcTempCustomer ,;
*!*	                  IIF(lcRpSort='P',PACK_NO,bill_ladg+ STYLE + Pack_No ) INTO (lcRevTemp) ADDITIVE
  SET RELATION TO "O" + ORDER +  STR(nordlineno,6) INTO &lcTempordLn,;
                  "O" + ORDER INTO &lcTempordhdr,;
                  IIF(EMPTY(Store),'M','S') + Account + Store INTO &lcTempCustomer ,;
                  IIF(lcRpSort='P',PACK_NO,IIF(lcRpFormat = 'D',bill_ladg+ STYLE + Pack_No,bill_ladg+ Pack_No+ STYLE)) INTO (lcRevTemp) ADDITIVE
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]                  
...
IF llMultCurr AND (lcRpCurr = "F") 
  ...
  *=gfCrtTmp(lcPackTmp,@laTempStru,IIF(lcRpSort='P',"cCurrCode + Pack_No","cCurrCode +bill_ladg+ STYLE + Pack_No"),lcPackTmp,.T.)
  =gfCrtTmp(lcPackTmp,@laTempStru,IIF(lcRpSort='P',"cCurrCode + Pack_No",IIF(lcRpFormat = 'D',"cCurrCode +bill_ladg+ STYLE + Pack_No","cCurrCode +bill_ladg+ Pack_No+ STYLE")),lcPackTmp,.T.)
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]  
  *E302884,1 MMT 03/31/2011 Add Sort by Option to Sort by BOL[End]
  SELECT(lcPackTmp)
  ZAP
  ...
  *E302884,1 MMT 03/31/2011 Add Sort by Option to Sort by BOL[Start]
  ELSE
    ...
    *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][Start]
    ELSE
      INDEX ON bill_ladg+ Pack_No+ STYLE  TAG (lcRevTemp)  DESCENDING
    ENDIF
    *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]
  ENDIF
  *E302884,1 MMT 03/31/2011 Add Sort by Option to Sort by BOL[End]
ELSE
  ...
  *=gfCrtTmp(lcPackTmp,@laTempStru,IIF(lcRpSort='P'," Pack_No","bill_ladg+ STYLE + Pack_No"),lcPackTmp,.T.)  
  =gfCrtTmp(lcPackTmp,@laTempStru,IIF(lcRpSort='P'," Pack_No",IIF(lcRpFormat = 'D',"bill_ladg+ STYLE + Pack_No","bill_ladg+ Pack_No+ STYLE")),lcPackTmp,.T.)
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]
  *E302884,1 MMT 03/31/2011 Add Sort by Option to Sort by BOL[End]
  SELECT(lcPackTmp)
  ZAP
...
IF llMultCurr AND (lcRpCurr = "F")
  ...
  *SCAN REST WHILE cCurrCode +bill_ladg+ STYLE + Pack_No = lcCurr+lcBolNum  
  SCAN REST WHILE IIF(lcRpFormat = 'D',cCurrCode +bill_ladg+ STYLE+Pack_No,cCurrCode +bill_ladg+Pack_No+ STYLE) = lcCurr+lcBolNum
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]
...
ELSE
  ...
  *SCAN REST WHILE bill_ladg+ STYLE + Pack_No = lcBolNum  
  SCAN REST WHILE IIF(lcRpFormat = 'D',bill_ladg+STYLE+Pack_No,bill_ladg+Pack_No+STYLE) = lcBolNum
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]
    IF ASCAN(laPackArr,&lcPackTmp..Pack_No) = 0
      lnTotCrtn = lnTotCrtn + &lcPackTmp..Tot_CART 
      IF !EMPTY(laPackArr[1])
...
IF llMultCurr AND (lcRpCurr = "F")
  ...
  *SCAN REST WHILE cCurrCode +bill_ladg+ STYLE + Pack_No = lcCurr
  SCAN REST WHILE IIF(lcRpFormat = 'D',cCurrCode+bill_ladg+STYLE+Pack_No,cCurrCode+bill_ladg+Pack_No+STYLE) = lcCurr
  *B611210,1 MMT 10/19/2016 Packing list log duplicates Packing list in summary format if Packing list has more than one style[T20160926.0008][End]
    IF ASCAN(laPackArr,&lcPackTmp..Pack_No) = 0
      lnTotCrtn = lnTotCrtn + &lcPackTmp..Tot_CART 
      IF !EMPTY(laPackArr[1])
