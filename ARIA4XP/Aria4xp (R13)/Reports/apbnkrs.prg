...
*:****************************************************************
*:Modifications  :
*:B610898,1 MMT 10/28/2014 Bank reconciliation option grid does not save bank code and checking acct.[T20141027.0039]
*:****************************************************************
#INCLUDE R:\aria4xp\reports\apbnkrs.h
...
*=gfModalGen("TRM04028B00000","DIALOG",LANG_TO+"|"+LANG_FROM)
=gfModalGen("TRM04028B00000","DIALOG",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_TO,oAriaApplication.GetHeaderText("LANG_TO",AHEADERFILE))+"|"+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_FROM,oAriaApplication.GetHeaderText("LANG_FROM",AHEADERFILE)))
...
*B610898,1 MMT 10/28/2014 Bank reconciliation option grid does not save bank code and checking acct.[T20141027.0039][Start]
IF loOgScroll.lnOGSeting =1
*B610898,1 MMT 10/28/2014 Bank reconciliation option grid does not save bank code and checking acct.[T20141027.0039][End]
  ...
  *** Search for the current Group code
  ...
  IF !EMPTY(loFld.Value) AND ('?' $ loFld.Value .OR.!SEEK(loFld.Value))
    ...
    *=gfBrows([],'CBnkCode',"laRpRetFld",'Bank Codes ',.F.)    
    =gfBrows([],'CBnkCode',"laRpRetFld",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_TTL_BANK_CODE,oAriaApplication.GetHeaderText("LANG_TTL_BANK_CODE",AHEADERFILE)),.F.)
    *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    *** Search for the current Group code
    IF !SEEK(laOGVrFlt[1,6]+loOgScroll.ActiveControl.Value)
      ...
      *=gfBrows('','CChkAcct',"laRpRetFld",'Bank & Check Accounts ',.F.)      
      =gfBrows('','CChkAcct',"laRpRetFld",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_TTL_CHKACC,oAriaApplication.GetHeaderText("LANG_TTL_CHKACC",AHEADERFILE)),.F.)
      *N000682,1 11/20/2012 MMT Globlization changes[END]
      IF EMPTY(laRpRetFld[1])
...
IF EMPTY(loOgScroll.laOgVrFlt[1,6])
  ...
  *N000682,1 11/20/2012 MMT Globlization changes[Start]
*gfModalGen('INM00000B00000',.F.,.F.,.F.,LANG_SELECTBANK)
gfModalGen('INM00000B00000',.F.,.F.,.F.,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SELECTBANK,oAriaApplication.GetHeaderText("LANG_SELECTBANK",AHEADERFILE)))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
IF EMPTY(loOgScroll.laOgVrFlt[2,6])
  ...
  *N000682,1 11/20/2012 MMT Globlization changes[Start]
*gfModalGen('INM00000B00000',.F.,.F.,.F.,LANG_SELECTCHECKACC)
gfModalGen('INM00000B00000',.F.,.F.,.F.,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SELECTCHECKACC,oAriaApplication.GetHeaderText("LANG_SELECTCHECKACC",AHEADERFILE)))
*N000682,1 11/20/2012 MMT Globlization changes[End]
