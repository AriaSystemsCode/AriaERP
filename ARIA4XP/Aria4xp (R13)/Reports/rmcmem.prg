...
*! B611020,1 SAR 22/06/2015 Agent Request Not generated PDF 
*  B611024,1 7-5-2015 ,Derby , Lucy Missing items T20150422.0007 
*:***************************************************************************

*! E303310,1 SAB 11/30/2012 Modify the Credit Memo Report to Run from Request Builder [Start]
#Include r:\aria4xp\reports\rmcmem.h
...
IF TYPE('lcRequestID') = 'C' .AND. 'TEMP.TXT' $ UPPER(lcRequestID)
  ...
  *STRTOFILE("RM 1000 ","C:\Test RMCMEM",1)
...
IF TYPE('lcXMLFileName') = 'C'
  ...
  *! E303361,1 SAB 02/28/2013 Merge RB R13 modification with R12 and update R13 VSS [Aria5.2 R13 Media][End]
  ...
  LOCAL loEnvironment
  ON ERROR DO =errHandler1(ERROR( ), MESSAGE( ), MESSAGE(1), PROGRAM( ), LINENO( ))
  *! E303361,1 SAB 02/28/2013 Merge RB R13 modification with R12 and update R13 VSS [Aria5.2 R13 Media][Start]
...
*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laMemoData[1,1] = IIF(!EMPTY(RetHdr.RaNo),LANG_RANO,'')

*B611024, 7-5-2015 ,Derby , Lucy Missing items T20150422.0007 [Start]

 IF TYPE('lcXMLFileName') = 'C'
*laMemoData[1,1] = IIF(!EMPTY(RetHdr.RaNo),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_RANO,oAriaApplication.GetHeaderText("LANG_RANO",AHEADERFILE)),'')
 *N000682,1 11/20/2012 MMT Globlization changes[End]
 laMemoData[1,1] = IIF(!EMPTY(RetHdr.RaNo),LANG_RANO,'')
 laMemoData[1,2] = RetHdr.RaNo
*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laMemoData[2,1] = IIF(!EMPTY(RetHdr.Order),LANG_ORDERNO,'')

laMemoData[2,1] = IIF(!EMPTY(RetHdr.Order),LANG_ORDERNO,'')
*laMemoData[2,1] = IIF(!EMPTY(RetHdr.Order),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ORDERNO,oAriaApplication.GetHeaderText("LANG_ORDERNO",AHEADERFILE)),'')
*N000682,1 11/20/2012 MMT Globlization changes[End]

laMemoData[2,2] = RetHdr.Order
*N000682,1 11/20/2012 MMT Globlization changes[Start]
laMemoData[3,1] = IIF(!EMPTY(RetHdr.Invoice),LANG_INVOICENO,'')
*laMemoData[3,1] = IIF(!EMPTY(RetHdr.Invoice),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_INVOICENO,oAriaApplication.GetHeaderText("LANG_INVOICENO",AHEADERFILE)),'')
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
*B611024, 7-5-2015 ,Derby , Lucy Missing items T20150422.0007 [End]
 ...
 *! B611020,1 SAR 06/18/2015 Request builder items missing T20150422.0007 [START]
      ...
      IF (ALLTRIM(UPPER(lcFilter)) = 'RETHDR.CRMEMO' AND ALLTRIM(UPPER(laOGvrFlt[lnPOS ,5])) = 'IN LIST') AND !EMPTY(laOGvrFlt[lnPOS,6])
        lcidpos = AT("INLIST(RETHDR.CRMEMO, '", lcrpExp)
           ...
           *! B611020,1 SAR 06/18/2015 Request builder items missing T20150422.0007 [END]
        ...
        lcidpos = AT("INLIST(RETHDR.CRMEMO,[", lcrpExp)
          ...
          lnpracdpos =  AT(')',SUBSTR(lcrpExp, lcidpos + LEN("INLIST(RETHDR.CRMEMO,[")))
           *! B611020,1 SAR 06/18/2015 Request builder items missing T20150422.0007 [END]
          ...
          FOR lcIndex = 0 TO lnpracdpos STEP 9
            APPEND BLANK
             *! B611020,1 SAR 06/18/2015 Request builder items missing T20150422.0007 [START]
            *REPLACE KEYEXP  WITH laOGvrFlt[lnPOS,6],;
	  	   *     CRMEMO  WITH laOGvrFlt[lnPOS,6]
  		    	 REPLACE KEYEXP  WITH SUBSTR(lcrpExp, lcidpos + LEN("INLIST(RETHDR.CRMEMO,[") + lcIndex,6)  ,;
	  	        CRMEMO  WITH  SUBSTR(lcrpExp, lcidpos + LEN("INLIST(RETHDR.CRMEMO,[") + lcIndex,6)  
            *! B611020,1 SAR 06/18/2015 Request builder items missing T20150422.0007 [END]
          ENDFOR
   ...
   OTHERWISE
      ...
      SCAN
        ...
        *WAIT WINDOW "Collecting data for Credit Memo: " + ALLTRIM(CRMEMO) NOWAIT
         ...
         * WAIT WINDOW "Collecting data for Credit Memo: " + ALLTRIM(CRMEMO) NOWAIT
                    *N000682,1 11/20/2012 MMT Globlization changes[Start]
*WAIT WINDOW LANG_Collecting_data_for + ALLTRIM(CRMEMO) NOWAIT
WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Collecting_data_for,oAriaApplication.GetHeaderText("LANG_Collecting_data_for",AHEADERFILE)) + ALLTRIM(CRMEMO) NOWAIT
*N000682,1 11/20/2012 MMT Globlization changes[End]
