...
*! E611825,1 MMT 11/13/2019 Call the new Global function to update GLTRNHD,GLTRNDT[GL Enhancement]
*! E612200,1 MMT 08/06/2020 Update EDITRANS with 812 [Send 812 Enhancement]
*!******************************************************************
FUNCTION lfSavDCAdj
...
*! B607995,1 SSH 03/05/2007 do not Round when update APPAYMNT
  ...
  *! E612200,1 MMT 08/06/2020 Update EDITRANS with 812 [Send 812 Enhancement][Start]
	IF !llEdiAccount AND 'EB' $ oAriaApplication.CompanyInstalledModules
	  IF !USED('EDIACPRT')
	     =gfOpenTable(oAriaApplication.DataDir+'EDIACPRT',oAriaApplication.DataDir+'ACCFACT','SH')
	  ENDIF
	  IF !USED('EDIPD')
	  ...
	  IF !USED('EDITRANS')  
	    =gfOpenTable(oAriaApplication.DataDir+'EDITRANS',oAriaApplication.DataDir+'TYPEKEY','SH')
	  ENDIF
	  IF  gfSEEK('A'+&lcAdjTemp..Account,'EDIACPRT') AND gfSEEK(EDIACPRT.cPartCode+'812','EDIPD') AND edipd.ctrantype ='S'
	    INSERT INTO 'EDITRANS' (cEdiTrnTyp,Key,Type,cPartner,cStatus,lInterComp) VALUES ;
	               ('812',&lcAdjTemp..TranType+&lcAdjTemp..Tran,'A',EdiAcPrt.cPartner,'N',EdiAcPrt.lInterComp)  && Warning 
	    =gfAdd_Info('EDITRANS') 
	    SELECT EDITRANS
	  ...
	  ENDIF
	ENDIF
  *! E612200,1 MMT 08/06/2020 Update EDITRANS with 812 [Send 812 Enhancement][End]
...
IF llLink_Gl
  ...
  *E611825,1 MMT 11/13/2019 Call the new Global function to update GLTRNHD,GLTRNDT[GL Enhancement][Start]
  lcTmpGlDist = gfTempName()
  SELECT(lcAdjTemp)
  SCAN 
    SELECT *  FROM (lcGLTemp) WHERE Tran_no = &lcAdjTemp..Tran INTO CURSOR (lcTmpGlDist)
    =gfCreateGLEntries(lcTmpGlDist,&lcAdjTemp..Account)
  ENDSCAN 
  IF USED(lcTmpGlDist)
    USE IN (lcTmpGlDist)
  ENDIF
  SELECT (lcGLTemp)
  *E611825,1 MMT 11/13/2019 Call the new Global function to update GLTRNHD,GLTRNDT[GL Enhancement][End]
  USE
