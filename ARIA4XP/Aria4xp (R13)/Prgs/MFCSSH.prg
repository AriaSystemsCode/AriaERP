...
*! B612711,1 MMT 04/02/2024  Error in Style Po shipment #838 (in transit)[T-ERP-20240321.0002]
*:************************************************************************
*--- Doc. [Begin]
...
IF !FOUND()
  *! B612711,1 MMT 04/02/2024  Error in Style Po shipment #838 (in transit)[T-ERP-20240321.0002][Start]
  llCompShip = .T.
  IF !USED('POSLN_SHP')
    =gfOpenTable("POSLN","POSLNSH",'SH','POSLN_SHP')
  ENDIF
  IF gfSeek(lcShpCode+EVALUATE(loFormSet.lcPosHdr+'.CBUSDOCU')+EVALUATE(loFormSet.lcPosHdr+'.CSTYTYPE'),'POSLN_SHP')
    SELECT POSLN_SHP
    LOCATE REST WHILE SHIPNO+CBUSDOCU+CSTYTYPE+PO+CINVTYPE+STYLE+STR(LINENO,6)+TRANCD = ;
                      lcShpCode+EVALUATE(loFormSet.lcPosHdr+'.CBUSDOCU')+EVALUATE(loFormSet.lcPosHdr+'.CSTYTYPE')  for PO <>EVALUATE(loFormSet.lcPosHdr+'.PO') and ;
                      TRANCD ='3'
    IF FOUND()
      llCompShip = .F.
    ENDIF                  
  ENDIF
  IF llCompShip 
  *! B612711,1 MMT 04/02/2024  Error in Style Po shipment #838 (in transit)[T-ERP-20240321.0002][End]
    SELECT SHPMTHDR
