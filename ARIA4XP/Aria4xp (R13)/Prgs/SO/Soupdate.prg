...
*! B610877,1 MMT 10/12/2014 Fix the bug of wrong value of nbulk field in customer table due to wrong updates from the sales order screen[T20141003.0007]
*! B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017]
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024]
*! B611692,1 HMS 11/14/2018 -EDI - Errors in EDI Transaction Burlington/Modecra [T20180911.0009]
*! B611702,1 Es, 12/06/2018 "Modify issue 'Error While editing sales order created from bulk order'" [T20181127.0007]
*! B611704,1 HMS 12/17/2018 -Needs to reopen so 246189 [T20181213.0001]
*! B611739,1 ES 3/10/2019 User cannot cancel SO has entered date falls in invalid period. [T20190307.0003]
*! B611783,1 Es 06/09/2019 Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [T20190515.0005]
*! B611789 ,1 ES 06/19/2019 Auto Allocate crashing with duplicate SO order header entry [T20190509.0001]
*! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [T20190808.0003]
*! B611990,1 MMT 12/18/2019 different location in sales order header and lines[T20191014.0005]
*! E612003,1 MMT 01/16/2020 Re-open Bulk order if depleted order is modified based on new setup[T20190319.0005]
*!*****************************************************************************
...
*! E612003,1 MMT 01/16/2020 Re-open Bulk order if depleted order is modified based on new setup[T20190319.0005][Start]
lcReOpenBulk = gfGetMemVar("M_REOPBLK ",oAriaApplication.ActiveCompanyID)
*! E612003,1 MMT 01/16/2020 Re-open Bulk order if depleted order is modified based on new setup[T20190319.0005][End]
*B608262,1 WAM 09/09/2007 Store header information in a temporary file instead of buffer mode
...
*! B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017][Start]
IF !USED('Scale_PP')
	=gfOpenTable('Scale','Scale','SH','Scale_PP')
ENDIF
*! B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017][End]
...
*IF gfModalGen("QRM32085B32008","DIALOG",IIF(&lcOrdHDr..STATUS="O","Open","Hold")) = 1
	IF gfModalGen("QRM32085B32008","DIALOG",IIF(&lcOrdHDr..STATUS="O",IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_StatusOpen,LOFORMSET.GetHeaderText("LANG_StatusOpen",LOFORMSET.HeaderAlias)),;
			IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_StatusHold,LOFORMSET.GetHeaderText("LANG_StatusHold",LOFORMSET.HeaderAlias)))) = 1
...
*=gfModalGen("INM32087B00000","DIALOG",'account')
			=gfModalGen("INM32087B00000","DIALOG",IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_SOORD_MSGACCOUNT,LOFORMSET.GetHeaderText("LANG_SOORD_MSGACCOUNT",LOFORMSET.HeaderAlias)))
*N000682,1 MMT 12/09/2012 Globalization changes[enD]
...
*WAIT 'Updating alocated quantity...' WINDOW NOWAIT
		WAIT IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('loFormSet')<>'O',LANG_SOUPDATE_UPDATEALLOCQTY,loformset.GetHeaderText("LANG_SOUPDATE_UPDATEALLOCQTY",loformset.HeaderAlias)) WINDOW NOWAIT
*N000682,1 MMT 12/09/2012 Globalization changes[ENd]
...
*lcSelString = "UPDATE PosHdr SET TotOrd=TotOrd+"+ALLTRIM(STR(lnOrdQty))+",nStyOrder = nStyOrder + "+ALLTRIM(STR(lnOrdQty))+" ,[Open] = [Open] + "+ALLTRIM(STR(lnOrdQty))+" WHERE cBusDocu+cstyt
...
WAIT IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('loFormSet')<>'O',LANG_SOUPDATE_UPDATINGORDERLINE,loformset.GetHeaderText("LANG_SOUPDATE_UPDATINGORDERLINE",loformset.HeaderAlias)) WINDOW NOWAIT
*N000682,1 MMT 12/09/2012 Globalization changes[end]
...
*! B611990,1 MMT 12/18/2019 different location in sales order header and lines[T20191014.0005][Start]
...
llModifyAllLines = &lcOrdHDr..cWareCode <> lcWareCode OR ;
	(&lcOrdHDr..MULTI <> 'Y' AND STORE <> &lcOrdHDr..STORE) OR ;
	(!&lcOrdHDr..MultiPo AND CustPo <> &lcOrdHDr..CustPo) OR ;
	(!llLineComission AND (Comm1 <> &lcOrdHDr..Comm1 OR Comm2 <> &lcOrdHDr..Comm2)) OR ;
	(!llCompleteDate  AND COMPLETE <> &lcOrdHDr..COMPLETE)
*! B611990,1 MMT 12/18/2019 different location in sales order header and lines[T20191014.0005][End]	
*B128682,1 [End]
...
*B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017][Start]
		IF !EMPTY(ORDLINE.Prepak) AND gfSeek('P'+ORDLINE.Scale+ORDLINE.Prepak,'Scale_PP')
			REPLACE PPQTY WITH Ordline.TotQty/Scale_PP.PPTOT IN ORDLINE
		ENDIF
*B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017][End]
		UNLOCK IN ORDLINE
	ENDIF
...
&& Adjust Style And StyDye Record Pointers.
		= SEEK(ORDLINE.STYLE,'Style')
		= SEEK(ORDLINE.STYLE+ORDLINE.cWareCode+SPACE(10),'StyDye')
*! B611704,1 HMS 12/17/2018 -Needs to reopen so 246189 [T20181213.0001][Start]
		FOR innerstr = 1 TO 8
			cstr = ALLTRIM(STR(innerstr))
			OldQTY&cstr.  = ORDLINE.Qty&cstr.
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [Start]
*OldBook&cstr. = ORDLINE.Book&cstr.
 OldBook&cstr. = ORDLINE.Qty&cstr.
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [End]

		ENDFOR
*! B611704,1 HMS 12/17/2018 -Needs to reopen so 246189 [T20181213.0001][End]
...
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
				IF llShowMessage AND xDiff < 0
					...
					*IF  gfModalGen('QRM52026B32000','DIALOG',ordhdr.cfromorder) =1			        
					IF  lcReOpenBulk = 'N' OR (lcReOpenBulk = 'I' AND  gfModalGen('QRM52026B32000','DIALOG',ordhdr.cfromorder) =1)
					*! E612003,1 MMT 01/16/2020 Re-open Bulk order if depleted order is modified based on new setup[T20190319.0005][End]
						llCancelBulk = .T.

*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [Start]
					ELSE
						IF BlkOrdHd.Status ='X'
							lfClearBulk(lcFromOrder,'OrdLine')
						ENDIF
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [End]
					ENDIF
					llShowMessage = .F.
...
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[END]
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024][Start]
*!*	          REPLACE Qty&cstr.   WITH MAX((Qty&cstr. - xDiff),0) IN ORDLINE
*!*	          REPLACE TotQty      WITH MAX((TotQty - xDiff),0)    IN ORDLINE
					lnExactDiff = IIF(xDiff > 0 ,MIN(ORDLINE.Qty&cstr.,xDiff),xDiff)
					REPLACE Qty&cstr.   WITH MAX((Qty&cstr. - lnExactDiff),0) IN ORDLINE
					REPLACE TotQty      WITH MAX((TotQty - lnExactDiff),0)    IN ORDLINE
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024][End]
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024][Start]
*!*	          lnOpen    = lnOpen + xDiff
*!*	          lnOpenAmt = lnOpenAmt + (xDiff * ORDLINE.Price)
					lnOpen    = lnOpen + lnExactDiff
					lnOpenAmt = lnOpenAmt + (lnExactDiff * ORDLINE.Price)
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024][End]
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
					lnQtyB&cstr = 0
				ELSE
...
*!*	        REPLACE TotBook     WITH MAX((TotBook - xDiff),0)    IN ORDLINE
				lnExactDiff = IIF(xDiff > 0 ,MIN(ORDLINE.Book&cstr.,xDiff),xDiff)
				REPLACE Book&cstr.  WITH MAX((Book&cstr. - lnExactDiff),0) IN ORDLINE
				REPLACE TotBook     WITH MAX((TotBook - lnExactDiff),0)    IN ORDLINE
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024][End]
...
*!*	        lnCancelAmt = lnCancelAmt + (xDiff * ORDLINE.Price)
				lnCancel = lnCancel + lnExactDiff
				lnCancelAmt = lnCancelAmt + (lnExactDiff * ORDLINE.Price)
*! B611220,1 MMT 10/24/2016 Depleted order updates Bulk order booked qty incorrectly[T20160615.0024][End]
...
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
			IF !llCancelBulk
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[end]
*! B611702,1 Es, 12/06/2018 "Modify issue 'Error While editing sales order created from bulk order'" [T20181127.0007][Start]
...
*! B611702,1 Es, 12/06/2018 "Modify issue 'Error While editing sales order created from bulk order'" [T20181127.0007][End]
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
			ELSE
*! B610206,1 HIA 01/21/2013 WHEN CANCELLING AN EDI CONFIRMATION ORDER WE FIND THAT THE BULK ORDER FOR THAT EDI CONFIRMATION WAS REVERSED [T20120711.0017][Start]
...
*B610877,1 MMT 10/12/2014 Fix the bug of wrong value of nbulk field in customer table due to wrong updates from the sales order screen[T20141003.0007][Start]
...
*B610877,1 MMT 10/12/2014 Fix the bug of wrong value of nbulk field in customer table due to wrong updates from the sales order screen[T20141003.0007][End]
				=RLOCK('Customer')
				REPLACE nBulk WITH nBulk - 1 IN Customer
				UNLOCK IN 'Customer'
			ENDIF
...
*=gfModalGen('INM32045B00000','DIALOG',IIF(OrdHdr.cOrdType='C','Contract','Order')+'|'+lcOrderNo)
			=gfModalGen('INM32045B00000','DIALOG',IIF(OrdHdr.cOrdType='C',;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_CONTRACTLABEL,LOFORMSET.GetHeaderText("LANG_CONTRACTLABEL",LOFORMSET.HeaderAlias)),;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_LabelOrder,LOFORMSET.GetHeaderText("LANG_LabelOrder",LOFORMSET.HeaderAlias)))+'|'+lcOrderNo)
*N000682,1 MMT 12/09/2012 Globalization changes[END]
*!--C201131 - MOS - SAVING SPREADSHEET 04/23/2009 [START]
		ENDIF
	ENDIF
...
*!B611739,1 ES 3/10/2019 User cannot cancel SO has entered date falls in invalid period. [Start]
*!*	IF OrdHdr.cOrdType='O' .AND. !CHECKPRD(OrdHdr.Entered,'lcGlYear','lcGlPeriod','',.T.)
*!*	  RETURN
*!*	ENDIF
IF OrdHdr.cOrdType='O'
...
ENDIF
*!B611739,1 ES 3/10/2019 User cannot cancel SO has entered date falls in invalid period. [End]
...
*IF gfModalGen('QRM32002B32000','ALERT',IIF(OrdHdr.cOrdType='C','Contract','Order')) =2
		IF gfModalGen('QRM32002B32000','ALERT',IIF(OrdHdr.cOrdType='C',;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_CONTRACTLABEL,LOFORMSET.GetHeaderText("LANG_CONTRACTLABEL",LOFORMSET.HeaderAlias)),;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('LOFORMSET')<>'O',LANG_LabelOrder,LOFORMSET.GetHeaderText("LANG_LabelOrder",LOFORMSET.HeaderAlias)))) =2
*N000682,1 MMT 12/09/2012 Globalization changes[END]
*B128066,1 HBG Fix bug of file is used by another user [Start]
			SET REPROCESS TO lnRePro
*B128066,1 HBG [End]
...
*WAIT 'Uncancelling and updating Cut & Sold...' WINDOW NOWAIT
		WAIT IIF(oAriaApplication.oActivelang.cLang_ID = "EN" OR TYPE('loFormSet')<>'O',;
			LANG_SOUPDATE_UNCANCELANDUPDATE,loformset.GetHeaderText("LANG_SOUPDATE_UNCANCELANDUPDATE",loformset.HeaderAlias)) WINDOW NOWAIT
*N000682,1 MMT 12/09/2012 Globalization changes[end]
*! E303218,1 HIA 08/22/2012 Convert ICSTYHST to SQL [T20120813.0035][Begin]
*=gfOpenFile(oAriaApplication.DataDir+'icStyHst',oAriaApplication.DataDir+'Styhst','SH')
		=gfOpenTable('ICSTYHST','STYHST','SH')
...
*B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017][Start]
				IF !EMPTY(BlkOrdLine.Prepak) AND gfSeek('P'+BlkOrdLine.Scale+BlkOrdLine.Prepak,'Scale_PP')
					REPLACE PPQTY WITH BlkOrdLine.TotQty/Scale_PP.PPTOT IN BlkOrdLine
				ENDIF
*B611119,1 MMT 02/29/2016 Update PPQTY in Bulk order after releasing orders from it[T20151014.0017][End]
				lnCancel = lnCancel + xDiff
				lnCancelAmt = lnCancelAmt + (xDiff * ORDLINE.Price)
...
*-- Update order header cancel and open quantity and amount
	SELECT OrdHdr
	*! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [Begin]
    ...
    cCancReson WITH SPACE(6) ,;
    Cancelled  WITH {} ,;
    CANCEL     WITH CANCEL-lnNewOpen ,;
    Cancelamt  WITH Cancelamt-lnNewOAmt ,;
    OPEN       WITH lnNewOpen ,;
    OpenAmt    WITH lnNewOAmt ,;
    FLAG       WITH SPACE(1)
	*! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [End]
...
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
	...
	*IF !EMPTY(ordhdr.cfromorder) AND  gfModalGen('QRM52026B32000','DIALOG',ordhdr.cfromorder) =1
	IF !EMPTY(ordhdr.cfromorder) AND  (lcReOpenBulk = 'N' OR (lcReOpenBulk = 'I' AND  gfModalGen('QRM52026B32000','DIALOG',ordhdr.cfromorder) =1))
	*! E612003,1 MMT 01/16/2020 Re-open Bulk order if depleted order is modified based on new setup[T20190319.0005][End]
		llCancelBulk = .T.
	ENDIF
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[END]
...
*! B611692,1 HMS 11/14/2018 -EDI - Errors in EDI Transaction Burlington/Modecra [T20180911.0009][Start]
		FOR innerstr = 1 TO 8
			cstr = ALLTRIM(STR(innerstr))
			OldQTY&cstr.  = ORDLINE.Qty&cstr.
			OldBook&cstr. = ORDLINE.Qty&cstr.
		ENDFOR
*! B611692,1 HMS 11/14/2018 -EDI - Errors in EDI Transaction Burlington/Modecra [T20180911.0009][End]
...
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [Start]
			IF BlkOrdHd.Status ='X'
				lfClearBulk(lcFromOrder,'BlkOrdLine')
			ENDIF
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [End]
...
*!*	          lnOpen    = lnOpen + xDiff
*!*	          lnOpenAmt = lnOpenAmt + (xDiff * ORDLINE.Price)
					lnBlkOpen      = lnBlkOpen      + xDiff
					lnOBlkpenAmt   = lnOBlkpenAmt   + (xDiff * ORDLINE.Price)
*! B611692,1 HMS 11/14/2018 -EDI - Errors in EDI Transaction Burlington/Modecra [T20180911.0009][End]
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
				ELSE
*! B610206,1 HIA 01/21/2013 WHEN CANCELLING AN EDI CONFIRMATION ORDER WE FIND THAT THE BULK ORDER FOR THAT EDI CONFIRMATION WAS REVERSED [T20120711.0017][Start]
...
*!*	        APPRAMT   WITH MAX(0,APPRAMT + lnOpenAmt) IN BlkOrdHd
				REPLACE Book      WITH (Book  + lnCancel)   ,;
					BookAmt   WITH (BookAmt + lnCancelAmt),;
					OPEN      WITH (OPEN    + lnBlkOpen)     ,;
					OpenAmt   WITH (OpenAmt + lnOBlkpenAmt)  ,;
					STATUS    WITH IIF(OPEN = 0 ,'X',STATUS) ,;
					APPRAMT   WITH MAX(0,APPRAMT + lnOpenAmt) IN BlkOrdHd
*! B611692,1 HMS 11/14/2018 -EDI - Errors in EDI Transaction Burlington/Modecra [T20180911.0009][End]
*! B610053,1 MMT 08/22/2012 When cancel release order from bulk order, reopen bulk order if user confirmed to reopen it[Start]
...
*!*	    FLAG       WITH SPACE(1)
	...
	*! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [Begin]
    ...
    REPLACE STATUS     WITH IIF(Ship > 0,'C','X') ,;
      cCancReson WITH lcCanReason ,;
      Cancelled  WITH oAriaApplication.SystemDate  ,;
      CANCEL     WITH CANCEL+lnOpen,;
      Cancelamt  WITH Cancelamt+lnOpenAmt,;
      OPEN       WITH 0,;
      OpenAmt    WITH 0,;
      TotCut     WITH 0,;
      Book       WITH lnBook    ,;
...
*lcMessage = IIF(OrdHdr.STATUS="O","Open","Hold")+LANG_Message1+IIF(OrdHdr.STATUS="O","Open","Hold")+LANG_Message2
			lcMessage = IIF(OrdHdr.STATUS="O",;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_StatusOpen,loFormSet.GetHeaderText("LANG_StatusOpen",loFormSet.HeaderAlias)),;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_StatusHold,loFormSet.GetHeaderText("LANG_StatusHold",loFormSet.HeaderAlias)))+;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Message1,loFormSet.GetHeaderText("LANG_Message1",loFormSet.HeaderAlias))+;
				IIF(OrdHdr.STATUS="O",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_StatusOpen,loFormSet.GetHeaderText("LANG_StatusOpen",loFormSet.HeaderAlias)),;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_StatusHold,loFormSet.GetHeaderText("LANG_StatusHold",loFormSet.HeaderAlias)))+;
				IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Message2,loFormSet.GetHeaderText("LANG_Message2",loFormSet.HeaderAlias))
...
*N000682,1 MMT 12/09/2012 Globalization changes[Start]
*=gfModalGen("INM32087B00000","DIALOG",'account')
					=gfModalGen("INM32087B00000","DIALOG",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SOORD_MSGACCOUNT,loFormSet.GetHeaderText("LANG_SOORD_MSGACCOUNT",loFormSet.HeaderAlias)))
*N000682,1 MMT 12/09/2012 Globalization changes[End]
					REPLACE STATUS WITH 'B' IN OrdHdr
				ENDIF
			ELSE
...
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [Start]
FUNCTION lfClearBulk
LPARAMETERS lcBulkOrder,lcOrdlineFileName
SET STEP ON 
lnSeleOld = ALIAS()
lcOrdType = EVALUATE(lcOrdlineFileName+'.cordType')
SELECT (lcOrdlineFileName)
lcOldOrdKey = cOrdType+Order+STR(LineNo,6)
IF SEEK(lcOrdType +lcBulkOrder,lcOrdlineFileName)
  SELECT (lcOrdlineFileName)
  SCAN REST WHILE CORDTYPE+ORDER+STR(LINENO,6) =lcOrdType +lcBulkOrder
    FOR lnQ=1 TO  8 
      lcQ = STR(lnQ,1)
*! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [Begin]     
*!*	      REPLACE Book&lcQ. WITH Book&lcQ.-Qty&lcQ.,;
*!*	              Qty&lcQ. WITH 0
           REPLACE Qty&lcQ. WITH 0
*! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [End]              
    ENDFOR 
 *! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [Begin] 
*!*	    REPLACE  TotBook  WITH TotBook - TOtQty,;
*!*	 		    TOtQty WITH 0 
	 REPLACE TOtQty WITH 0
 *! B611919 ,1 HMS 10/08/2019 - T20190808.0003 - Aria 5 - Bulk Order Problem [End]      		
  ENDSCAN 
ENDIF
=SEEK(lcOldOrdKey,lcOrdlineFileName)
SELECT (lnSeleOld)
*B611783,1 Es Canceled Bulk order original qty is returned back if used canceled released order from this bulk order then canceled the bulk order. [End]