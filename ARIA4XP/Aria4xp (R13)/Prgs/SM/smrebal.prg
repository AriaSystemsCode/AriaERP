...
*: B608487,1 WAM 03/19/2008 Enhance performanGO TOP IN (lcStyAls)ce of rebalance                     T20080229.0009
...
*! B610402,1 HIA 06/25/13 T20130613.0003 - Function name is missing error message when trying to re-balance Inventory, Sales Order & Sales Order
...
*! B610767,1 TMI 07/08/2014 change the loAgent to be called remotely [T20140529.0002]
...
*! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033]
*! B611091,1 MMT 12/10/2015 Packing List browse in rebalance displayes wrong order CUSTPO#[T20151119.0008]
*! B611096,1 MMT 12/15/2015 Error while rebalancing AP module with update option[T20151210.0043]
*! E303873,1 MMT 09/28/2017 Adding GL Module to Rebalance program[P20170905.0001]
*! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001]
*! B611463,1 AHH 15/11/2017 modify the rebalance program to set nstkval=0 if totstk=0 in style and stydye tables [T20171106.0028]
*! E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL
*! E303990,1 MMT 05/28/2018 Add AR History rebalance option to AR module[T20180511.0001]
*! B611616,1 Es  07/12/2018  Modify this issue-> "Error while running rebalance program on AR module - AR history balance option is Verify or update" [T20180703.0006]
*! E304029,1 SAH 07/3/2018  CONVERT STYINVJL TO SQL 
*! B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [T20190215.0004]
*! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover
*! B612455,1 MMT 08/25/2021 Switching between Aria4XP option grid and Aria5 option grid gives error with Rebalance program[T20210713.0002]
*! B612688,1 MMT 09/03/2023 Can not run the Rebalance program from the Request builder[T-ERP-20230821.0001]
*:***************************************************************************
...
#INCLUDE d:\shared\aria4xp\prgs\SM\SMREBAL.H

SET STEP ON 
*! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
IF TYPE('lcXMLFileName') = 'C'
   lcRequestID=UPPER(lcRequestID)
  ...
  *B610767,1 TMI 07/08/2014 11:17 [Start]
  ...
  *B610767,1 TMI 07/08/2014 11:17 [End  ]
  ...
  *B610750,1  test code TMI 06/18/2014 14:58 [End  ]
  ...
  oAriaApplication.syspath =oAriaApplication.systemfilespath
   =lfSTRTOFILE(  oAriaApplication.syspath )
      =lfSTRTOFILE(  oAriaApplication.workdir )
ENDIF
*! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][End]
...
IF oAriaApplication.oActivelang.cLang_ID <> "EN"
  lcHeaderFile = ADDBS(UPPER(ALLTRIM(oAriaApplication.LangPath))) + "PRGS\SM\SMREBAL_H.XML"
...
lcWinTitl = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_REBLOG,loFormSet.GetHeaderText("LANG_SMREBAL_REBLOG",loFormSet.HeaderAlias))
...
lcAria4Modules = lcAria4Modules + ",GL"
=lfSTRTOFILE(lcAria4Modules )
*E303873,1 MMT 09/26/2017 Adding GL Module to Rebalance program[P20170905.0001][End]
IF !llAutoBal
=lfSTRTOFILE('llAutoBal' )
  llOpenRep  = .F.
  lcFilHandl = ''
ELSE
  *N000682,1 11/20/2012 MMT Globlization changes[Start]
  *=FPUTS(lcFilHandl, LANG_SMREBAL_REB)
  =FPUTS(lcFilHandl, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_REB,loFormSet.GetHeaderText("LANG_SMREBAL_REB",loFormSet.HeaderAlias)))
  *N000682,1 11/20/2012 MMT Globlization changes[End]

  *N000682,1 11/20/2012 MMT Globlization changes[Start]
  *=FPUTS(lcFilHandl, LANG_SMREBAL_PROCESS + DTOC(DATE()) + SPACE(5) + TIME())
  =FPUTS(lcFilHandl, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_PROCESS,loFormSet.GetHeaderText("LANG_SMREBAL_PROCESS",loFormSet.HeaderAlias)) + DTOC(DATE()) + SPACE(5) + TIME())
  *N000682,1 11/20/2012 MMT Globlization changes[End]

ENDIF
=lfSTRTOFILE('llSaveAut' )
...
lcGLBtTrn = gfTempName()
*E303873,1 MMT 09/26/2017 Adding GL Module to Rebalance program[P20170905.0001][End]

lcCurrComp_ID = ''

lcFilePath  = oAriaApplication.DataDir
*lcFilePath = STRTRAN(UPPER(lcFilePath),"X:\","\\10.0.1.8\TAJ01SH\")
...
ELSE
  ...
  loProgress.DESCRIPTION = LANG_SMREBAL_COMP+ lcCurrComp_ID +' '
  loAgent.UpdateObjectProgress(lcRequestID, loProgress, CLIENTID)
  =lfSTRTOFILE('loProgress' )
    ...
    *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][End]
    FOR lnAllComp = 1 TO ALEN(laRpCmpCod,1)
      ...
      IF lcCurrComp_ID $ lcRpCmpExp
        ...
        *! B612688,1 MMT 09/03/2023 Can not run the Rebalance program from the Request builder[T-ERP-20230821.0001][Start]
        IF TYPE('lcXMLFileName') = 'C'
         lcFilePath= loEnvironment.ResolveMappedDrive( lcFilePath)
        ENDIF 
        *! B612688,1 MMT 09/03/2023 Can not run the Rebalance program from the Request builder[T-ERP-20230821.0001][End]        
        ...
        *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
        IF TYPE('lcXMLFileName') = 'C'
          ...
          oAriaEnvironment.GetCompanyInformation(lcCurrComp_ID)
           oAriaEnvironment.DataDir= loEnvironment.ResolveMappedDrive( oAriaEnvironment.DataDir)
...
=lfSTRTOFILE(STR(lnWip + lnWo + lnIntrans + lnOrd + lnAlo + lnShp + lnRet + lnRa + lnStock + lnPackReb) )
  *: B608518,1 MMT 04/17/2008 Add Allocation Module To Reblanace [Start]
...
FUNCTION lfwRepWhen
  ...
  *E303873,1 MMT 09/26/2017 Adding GL Module to Rebalance program[P20170905.0001][Start]
  ...
  IF FOUND()
    lcRpFiscYr = &lcSYCCOMP..cCurr_Yer
  ELSE
    lcRpFiscYr = ''
  ENDIF
...
FUNCTION lfFillRepVars
  *! B612455,1 MMT 08/25/2021 Switching between Aria4XP option grid and Aria5 option grid gives error with Rebalance program[T20210713.0002][Start]
  *IF EMPTY(laRpCmpCod[1,1]) OR EMPTY(laRpSorCmp[1,1])
  IF EMPTY(laRpCmpCod[1,1]) OR TYPE('laRpSorCmp[1,1]')='U' OR EMPTY(laRpSorCmp[1,1])
  *! B612455,1 MMT 08/25/2021 Switching between Aria4XP option grid and Aria5 option grid gives error with Rebalance program[T20210713.0002][End]
    LOCAL lnDataSess
    lnDataSess = SET("Datasession")
    SET DATASESSION TO (lnPrgSession)
...
FUNCTION lfvCompany
  ...
  *= gfMover(@laRpSorCmp, @laRpTarCmp, 'Select Company', .T., '')  && call mover function.
  lcSelect_Company = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Select_Company,loFormSet.GetHeaderText("LANG_Select_Company",loFormSet.HeaderAlias))
  *! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover[Start]
*  = gfMover(@laRpSorCmp, @laRpTarCmp, lcSelect_Company, .T., '')  && call mover function.
  = lfOgMover(@laRpSorCmp, @laRpTarCmp, lcSelect_Company, .T., '')  && call mover function.
  *! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover[End]
...
FUNCTION lfvModule
  ...
  *= gfMover(@laRpSorMod,@laRpTarMod,'Select Module',.T.,'')  && call mover function.
  LOCAL lcSlMod
  lcSlMod = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Select_Module,loFormSet.GetHeaderText("LANG_Select_Module",loFormSet.HeaderAlias))
    *! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover[Start]
*  = gfMover(@laRpSorMod,@laRpTarMod,lcSlMod,.T.,'')  && call mover function.
  = lfOgMover(@laRpSorMod,@laRpTarMod,lcSlMod,.T.,'')  && call mover function.
    *! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover[End]
...
FUNCTION lfvDfinMod
  ...
  *IF EMPTY(laRpCodMod[1,1]) OR EMPTY(laRpSorMod[1,1])
  IF EMPTY(laRpCodMod[1,1]) OR TYPE('laRpSorMod[1,1]') = 'U' OR EMPTY(laRpSorMod[1,1])  
  *! B612455,1 MMT 08/25/2021 Switching between Aria4XP option grid and Aria5 option grid gives error with Rebalance program[T20210713.0002][End]
...
FUNCTION lfInfoHdr
  ...
  *! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033][Start]
  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPINLK' ,;
    cUpdFunNam WITH 'UpdName'   ,;
    cVryFunNam WITH 'VryName'   ,;
    cIgnFunNam WITH 'IgnName'   ,;
    cLogFile   WITH  'Y'
  *! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033][End]
  ...
  *E303873,1 MMT 09/26/2017 Adding GL Module to Rebalance program[P20170905.0001][Start]
  SELECT (lcRebalHdr)
  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPITEM25'  ,;
    cUpdFunNam WITH 'lfUpdItm25' ,;
    cVryFunNam WITH 'lfVryItm25' ,;
    cIgnFunNam WITH 'IgnName'    ,;
    cUpdVryIgn WITH  lcUpVrIg     ,;
    cAddSetup  WITH 'lfStpItm25' ,;
    cValidFun  WITH 'lfVldItm25',;
    cModule WITH "GL"
  ...
  SELECT (lcRebalHdr)
  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPITEM26'  ,;
    cUpdFunNam WITH 'lfUpdItm26' ,;
    cVryFunNam WITH 'lfVryItm26' ,;
    cIgnFunNam WITH 'IgnName'    ,;
    cUpdVryIgn WITH  lcUpVrIg     ,;
    cAddSetup  WITH '' ,;
    cValidFun  WITH 'lfVldItm26',;
    cModule WITH "GL"

  SELECT (lcRebalDtl)
  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPITEM26' ,;
    cFileName  WITH 'GLDIST'    ,;
    cNdxName   WITH 'GLDISTNO'   ,;
    cAliasName WITH 'GLDIST_G'    ,;
    lOpenBefor WITH .F.
  ...
  REPLACE cItemName  WITH 'LCRPITEM26' ,;
    cFileName  WITH 'FSPRD'    ,;
    cNdxName   WITH 'COMFYRPRDI'   ,;
    cAliasName WITH 'FSPRD_G'    ,;
    lOpenBefor WITH .F.

  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPITEM26' ,;
    cFileName  WITH 'GLBATCH'    ,;
    cNdxName   WITH 'BATCHNO'   ,;
    cAliasName WITH 'GLBATCH_G'    ,;
    lOpenBefor WITH .F.

  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPITEM26' ,;
    cFileName  WITH 'GLTRNSHD'    ,;
    cNdxName   WITH 'BATCHTRN'   ,;
    cAliasName WITH 'GLTRNSHD_G'    ,;
    lOpenBefor WITH .F.

  APPEND BLANK
  REPLACE cItemName  WITH 'LCRPITEM26' ,;
    cFileName  WITH 'GLTRNSDT'    ,;
    cNdxName   WITH 'BATCHTRN'   ,;
    cAliasName WITH 'GLTRNSDT_G'    ,;
    lOpenBefor WITH .F.
  *E303873,1 MMT 09/26/2017 Adding GL Module to Rebalance program[P20170905.0001][End]
...
APPEND BLANK
REPLACE cItemName  WITH 'LCRPITEM27' ,;
		cUpdFunNam WITH 'lfUpdCsHst' ,;
		cVryFunNam WITH 'lfVryCsHst' ,; 
		cIgnFunNam WITH 'IgnName'    ,; 		
		cUpdVryIgn WITH lcUpVrIg     ,;
		...
		cMFiles    WITH lcMFiles ,;
		cModule    WITH "AR"
* Detail info. of AR Customer History (lcRpItem27).
* --------------------------------------------------------
SELECT (lcRebalDtl)
APPEND BLANK
REPLACE cItemName  WITH 'LCRPITEM27' ,;
...
*B611616,1 Es  07/12/2018  Modify this issue-> "Error while running rebalance program on AR module - AR history balance option is Verify or update" [Start]
*REPLACE cItemName  WITH 'LCRPITEM27' ,;
	    cFileName  WITH 'RETHDR'     ,;
   	    cNdxName   WITH ''           ,;
        cAliasName WITH 'RETHDR'     ,;	  
        ...
        cAliasName WITH 'RETHDR'     ,;	  
	    lOpenBefor WITH .F.
*B611616,1 Es  07/12/2018  Modify this issue-> "Error while running rebalance program on AR module - AR history balance option is Verify or update" [End]
...
FUNCTION lfOpnFiles
  ...
  *!*        * Message : "Some of the ð were not succesfully reposted to the "
  *!*        *           "balances file , Unable to proceed with rebalancing."
  *!*        * Buttons :           <Ok>
  *!*        DECLARE laRebMsg[5]
  *!*        laRebMsg[1] = " "
...
FUNCTION lfVryRport
  ...
  *B610767,1 TMI 07/08/2014 11:14 [Start]
  IF TYPE('lcFilHandl') = 'C'
    RETURN
  ENDIF
  *B610767,1 TMI 07/08/2014 11:14 [End  ]
  ...
  IF llViewLog OR llOpenRep
    IF !llOpenRep
      ...
      *laRebMsg[2] = LANG_SMREBAL_NOERR
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_NOERR,loFormSet.GetHeaderText("LANG_SMREBAL_NOERR",loFormSet.HeaderAlias))
      *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    *B610750,3 TMI Closing the file [start]
    =lfSTRTOFILE("start closing the file")
    *B610750,3 TMI Closing the file [end  ]

    *B610767,1 TMI 07/08/2014 11:14 [Start]
    *IF lcFilHandl>0
    IF TYPE('lcFilHandl')='N' AND lcFilHandl>0
      *B610767,1 TMI 07/08/2014 11:14 [End  ]
    ...
    IF TYPE('lcXMLFileName') <> 'C'
      ...
      *-- Close the text file
      ...
      REPLACE mStrRep WITH REPLICATE('*',68) + CHR(13) +;
        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_REBVERI,loFormSet.GetHeaderText("LANG_SMREBAL_REBVERI",loFormSet.HeaderAlias)) + CHR(13) +;
...
FUNCTION lfVryHdDtl
  ...
  *=lfInitThermo(lnUpThermo, LANG_SMREBAL_RECALC)
  =lfInitThermo(lnUpThermo, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_RECALC,loFormSet.GetHeaderText("LANG_SMREBAL_RECALC",loFormSet.HeaderAlias)))
  *N000682,1 11/20/2012 MMT Globlization changes[End]
      ...
      *E303410,1 TMI 09/01/2013 [Start] if called from style screen do not show progress bar
      IF EMPTY(lcStyMaj)
        *E303410,1 TMI 09/01/2013 [End  ]
        ...
        oProgress.lblSecondLabel.CAPTION = IIF(UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcMfCt,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CTNO,loFormSet.GetHeaderText("LANG_SMREBAL_CTNO",loFormSet.HeaderAlias)),lcKeyVal)+ ' : ' + ALLTRIM(&lcKeyVal)
      ...
      *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
      IF TYPE('lcXMLFileName') <>'C'
        ...
        *WAIT WINDOW LANG_SMREBAL_SHPCOLL1 + Order + LANG_SMREBAL_COLL NOWAIT
        WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_SHPCOLL1,loFormSet.GetHeaderText("LANG_SMREBAL_SHPCOLL1",loFormSet.HeaderAlias)) + ORDER + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COLL,loFormSet.GetHeaderText("LANG_SMREBAL_COLL",loFormSet.HeaderAlias)) NOWAIT
        *N000682,1 11/20/2012 MMT Globlization changes[End]
        *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
      ENDIF
      *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][End]
    ...
    IF &lcIfExp
      ...
      *IF UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = LCSPO
      ...
      IF llViewLog
        ...
        *! B610402,1 HIA 06/25/13 T20130613.0003 - Function name is missing error message when trying to re-balance Inventory, Sales Order & Sales Order [Begin]
        *laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID + ": " + ;
        *  IIF(UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = LCMFCT,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CTNO,loFormSet.GetHeaderText("LANG_SMREBAL_CTNO",loFormSet.HeaderAlias)),;
        *  IIF((UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = LCALPACK),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_PACKNO,loFormSet.GetHeaderText("LANG_SMREBAL_PACKNO",loFormSet.HeaderAlias)),lcKeyVal)) +;
        *  IIF((UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = LCALPACK),"",' # ') + &lcHdrFile..&lcKeyVal + IIF((UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = LCALPACK),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_PACKDIFF,loFormSet.GetHeaderText("LANG_SMREBAL_PACKDIFF",loFormSet.HeaderAlias)),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREB,loFormSet.GetHeaderText("LANG_SMREB",loFormSet.HeaderAlias))

        lcLANG_SMREBAL_COMP     = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))   &&1
        lcLANG_SMREBAL_CTNO     = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_CTNO     ,loFormSet.GetHeaderText("LANG_SMREBAL_CTNO"    , loFormSet.HeaderAlias))   &&2
        lcLANG_SMREBAL_PACKNO   = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_PACKNO   ,loFormSet.GetHeaderText("LANG_SMREBAL_PACKNO"  , loFormSet.HeaderAlias))   &&3
        lcLANG_SMREBAL_PACKDIFF = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_PACKDIFF ,loFormSet.GetHeaderText("LANG_SMREBAL_PACKDIFF", loFormSet.HeaderAlias))   &&4
        lcLANG_SMREBAL_DIFFAMT  = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_DIFFAMT  ,loFormSet.GetHeaderText("LANG_SMREBAL_DIFFAMT" , loFormSet.HeaderAlias))   &&5

        laRebMsg[2] = lcLANG_SMREBAL_COMP + lcCurrComp_ID + ": " + ;
          IIF(UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcMfCt,lcLANG_SMREBAL_CTNO,;
          IIF((UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcALPack),lcLANG_SMREBAL_PACKNO,lcKeyVal)) +;
          IIF((UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcALPack),"",' # ') + &lcHdrFile..&lcKeyVal + IIF((UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcALPack), lcLANG_SMREBAL_PACKDIFF, lcLANG_SMREBAL_DIFFAMT)
        *! B610402,1 HIA 06/25/13 T20130613.0003 - Function name is missing error message when trying to re-balance Inventory, Sales Order & Sales Order [End]
  ...
  *B128464,2 WSH 08/21/2005 [End]
  ...
  IF FILE(oAriaApplication.WorkDir + ALLTRIM(&lcRebalHdr..cTempName) + '.DBF')
    ...
    *=lfInitThermo(lnUpThermo, LANG_SMREBAL_UPDBAL)
    =lfInitThermo(lnUpThermo, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_UPDBAL,loFormSet.GetHeaderText("LANG_SMREBAL_UPDBAL",loFormSet.HeaderAlias)))
    *N000682,1 11/20/2012 MMT Globlization changes[End]
      ...
      *B128464,2 WSH 08/21/2005 [End]
      ...
      IF DTOC(DADD_DATE)+CADD_TIME <> DTOC(&lcHdrFile..DADD_DATE)+&lcHdrFile..CADD_TIME
        ...
        DO CASE
          CASE UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcArInv
            ...
            *lcExpr = LANG_SMREBAL_INVNO
            lcExpr = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_INVNO,loFormSet.GetHeaderText("LANG_SMREBAL_INVNO",loFormSet.HeaderAlias))
            *N000682,1 11/20/2012 MMT Globlization changes[End]

          CASE UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcSoOrd
            *N000682,1 11/20/2012 MMT Globlization changes[Start]
            *lcExpr = LANG_SMREBAL_ORDNO
            lcExpr = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ORDNO,loFormSet.GetHeaderText("LANG_SMREBAL_ORDNO",loFormSet.HeaderAlias))
            *N000682,1 11/20/2012 MMT Globlization changes[End]
          ...
          CASE UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = lcALPack
            ...
            *lcExpr = LANG_SMREBAL_PACKNO
            lcExpr = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_PACKNO,loFormSet.GetHeaderText("LANG_SMREBAL_PACKNO",loFormSet.HeaderAlias))
            *N000682,1 11/20/2012 MMT Globlization changes[End]
          ...
          OTHERWISE
            ...
            *lcExpr = LANG_SMREBAL_PONO
            lcExpr = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_PONO,loFormSet.GetHeaderText("LANG_SMREBAL_PONO",loFormSet.HeaderAlias))
            *N000682,1 11/20/2012 MMT Globlization changes[End]
        ...
        *laRebMsg[2] = LANG_SMREBAL_NOTUPD + lcExpr + &lcKeyVal + LANG_SMREBAL_DATACHG
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_NOTUPD,loFormSet.GetHeaderText("LANG_SMREBAL_NOTUPD",loFormSet.HeaderAlias)) + lcExpr + &lcKeyVal + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_DATACHG,loFormSet.GetHeaderText("LANG_SMREBAL_DATACHG",loFormSet.HeaderAlias))
        *N000682,1 11/20/2012 MMT Globlization changes[End]
...
*=lfSTRTOFILE(STR(ALEN(laFileStr,1)) )
    IF !USED(laFileStr[lnI,4])
      *B128464,2 WSH 08/21/2005 [End]
*=lfSTRTOFILE(laFileStr[lnI,1]+" Error ")
      *-- Message : XXX Not found in data file.
      *--                   <Ok>
      *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
      IF TYPE('lcXMLFileName') <> 'C'
      ...
      *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
      IF TYPE('lcXMLFileName') <>'C'
        ...
        *WAIT WINDOW LANG_SMREBAL_STARTREB + lcCurrName NOWAIT
        WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_STARTREB,loFormSet.GetHeaderText("LANG_SMREBAL_STARTREB",loFormSet.HeaderAlias)) + lcCurrName NOWAIT
        *N000682,1 11/20/2012 MMT Globlization changes[End]
        *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
      ENDIF
      *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][End]
    ...
    *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
    IF TYPE('lcXMLFileName') <>'C'
      ...
      *E303410,1 TMI 09/01/2013 [Start] don't issue this when called from STYLE screen
      IF EMPTY(lcStyMaj)
        *E303410,1 TMI 09/01/2013 [End  ]
        WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FINISH,loFormSet.GetHeaderText("LANG_SMREBAL_FINISH",loFormSet.HeaderAlias)) + lcCurrName TIMEOUT 1
        *E303410,1 TMI 09/01/2013 [Start]
      ENDIF
      *E303410,1 TMI 09/01/2013 [End  ]
      *N000682,1 11/20/2012 MMT Globlization changes[End]
...
*    =lfSTRTOFILE(lcInExpression)
    SET ORDER TO style IN (lcStyAls)
    =lfSEEK(lcStyAls, "cStyMajor IN "+lcInExpression , '', .T.)
    *B608487,1 WAM 03/19/2008 (End)
* =lfSTRTOFILE("=lfSEEK(lcStyAls,cStyMajor IN +lcInExpression , '', .T.)")
    *B128464,2 WSH 08/21/2005 Use Fox Native SQL Commands insteed of RDAC to Enhance Performance. [Start]
    *= loStyle.GOBOTTOM()
    *B128464,2 WSH 08/21/2005 [End]
...
*    =lfSTRTOFILE("lfSEEK")
    SET ORDER TO style IN (lcStyAls)
*    =lfSTRTOFILE("lcInvType"+lcInvType)
    =lfSEEK(lcStyAls, "CINVTYPE = '"+lcInvType+"'", '', .T.)
*        =lfSTRTOFILE("lcStyAls")
    SELECT (lcStyAls)
    lnUpThermo = RECCOUNT()
    *B608487,1 WAM 03/19/2008 (End)
  ENDIF
  ...
  *-SAB 09-26-2013 [Start]
  IF TYPE('lcXMLFileName') <> 'C'
    ...
    *lcTermoMsg = LANG_SMREBAL_REBCOMP + lcCurrComp_ID + " " + ALLTRIM(gfItemMask('HM', lcFilePath, lcInvType)) + ":  "
    lcTermoMsg = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_REBCOMP,loFormSet.GetHeaderText("LANG_SMREBAL_REBCOMP",loFormSet.HeaderAlias)) + lcCurrComp_ID + " " + ALLTRIM(gfItemMask('HM', lcFilePath, lcInvType)) + ":  "
    *N000682,1 11/20/2012 MMT Globlization changes[End]
...
*  =lfSTRTOFILE("GO TOP IN (lcStyAls)")
  *B610750,1  test code TMI 06/18/2014 14:33 [End  ]
  SET ORDER TO style IN (lcStyAls)
  SET ORDER TO stydye IN   (lcSDyAls)
  GO TOP IN (lcStyAls)
        ...
        IF !llScanInFilt
          ...
          *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
          IF TYPE('lcXMLFileName') <>'C'
            ...
            *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
          ELSE
            IF MOD(lnThermNo ,CEILING(lnUpThermo / 20)) = 0
            ...
            *B128464,2 WSH 08/21/2005 [End]
            ...
            IF llMisMatch
              ...
              *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID + ": " + Style + LANG_SMREBAL_WRONGWO
              laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID + ": " + STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGWO,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGWO",loFormSet.HeaderAlias))
              *N000682,1 11/20/2012 MMT Globlization changes[End]
            ...
            *B128464,2 WSH 08/21/2005 [End]
            ...
            IF llMisMatch
              ...
              *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGWIP
              laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGWIP,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGWIP",loFormSet.HeaderAlias))
              *N000682,1 11/20/2012 MMT Globlization changes[End]
        ...
        IF llViewLog
          ...
          IF llMisMatch
            ...
            *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGUSE
            laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGUSE,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGUSE",loFormSet.HeaderAlias))
            *N000682,1 11/20/2012 MMT Globlization changes[End]
        ...
        *B128464,2 WSH 08/21/2005 [End]
        ...
        IF llMisMatch
          ...
          *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGSHP
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGSHP,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGSHP",loFormSet.HeaderAlias))
          *N000682,1 11/20/2012 MMT Globlization changes[End]
        ...
        *B128464,2 WSH 08/21/2005 [End]
        ...
        IF llMisMatch
          ...
          *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGALO
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGALO,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGALO",loFormSet.HeaderAlias))
          *N000682,1 11/20/2012 MMT Globlization changes[End]
        ...
        *B128464,2 WSH 08/21/2005 [End]
        ...
        IF llMisMatch
          ...
          *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGORD
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGORD,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGORD",loFormSet.HeaderAlias))
          *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    *: B608393,1 MMT 12/27/2007 Add RM to Reblanace Program[Start]
    IF lcOption = 'RA'
      ...
      IF llMisMatch
        ...
        *N000682,1 11/20/2012 MMT Globlization changes[Start]
        *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGRET
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGRET,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGRET",loFormSet.HeaderAlias))
        *N000682,1 11/20/2012 MMT Globlization changes[End]
            ...
            *B128464,2 WSH 08/21/2005 [End]
            ...
            IF llMisMatch
              ...
              *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +": "+ Style + LANG_SMREBAL_WRONGTRN
              laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +": "+ STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGTRN,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGTRN",loFormSet.HeaderAlias))
              *N000682,1 11/20/2012 MMT Globlization changes[End]
...
FUNCTION lfStyStock
  ...
  *B128464,2 WSH 08/21/2005 Use Fox Native SQL Commands insteed of RDAC to Enhance Performance. [Start]
  PRIVATE lcStyCond
  *B128464,2 WSH 08/21/2005 [End]
  *B611463,1 AHH 15/11/2017 modify the rebalance program to set nstkval=0 if totstk=0 in style and stydye tables [T20171106.0028] [Start]
  PRIVATE lnStkVal
  lnStkVal=0
  *B611463,1 AHH 15/11/2017 modify the rebalance program to set nstkval=0 if totstk=0 in style and stydye tables [T20171106.0028] [End]
      ...
      IF llMultWare AND &lcGroupChng.
        ...
        *B611463,1 AHH 15/11/2017 modify the rebalance program to set nstkval=0 if totstk=0 in style and stydye tables [T20171106.0028] [Start]
        IF(laLocStock[9]=0)
          laLocStock[10]=0
        ENDIF
        lnStkVal= lnStkVal + laLocStock[10]
        *B611463,1 AHH 15/11/2017 modify the rebalance program to set nstkval=0 if totstk=0 in style and stydye tables [T20171106.0028] [End]
        *: B608581,1 MMT 06/11/2008 Fix bug of Wrong update of First Material record[Start]
      ...
      IF llViewLog
        ...
        IF llMisMatch
          ...
          *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID + ": " + Style + LANG_SMREBAL_WRONGHND
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID + ": " + STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGHND,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGHND",loFormSet.HeaderAlias))
          *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    IF !llMisMatch
      ...
      IF llViewLog AND llMisMatch
        ...
        *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID + ": " + Style + LANG_SMREBAL_STKVAL_MISMTCH
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID + ": " + STYLE + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_STKVAL_MISMTCH,loFormSet.GetHeaderText("LANG_SMREBAL_STKVAL_MISMTCH",loFormSet.HeaderAlias))
        *N000682,1 11/20/2012 MMT Globlization changes[End]
  ...
  FOR lnI = 1 TO ALEN(laOjects, 1)
    ...
    DO WHILE !llExitLopp
      ...
      *llExitLopp = llExitLopp OR MESSAGEBOX(LANG_SMREBAL_UPDERR + '"' + lcTableName + '"', 5 + 32, _Screen.Caption) = 2
      llExitLopp = llExitLopp OR MESSAGEBOX(IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_UPDERR,loFormSet.GetHeaderText("LANG_SMREBAL_UPDERR",loFormSet.HeaderAlias)) + '"' + lcTableName + '"', 5 + 32, _SCREEN.CAPTION) = 2
      *N000682,1 11/20/2012 MMT Globlization changes[End]
...
FUNCTION lfCREATEOBJECT
  ...
  *!*	           "", "SysTmpCursor", "", oAriaApplication.cAria4SysFiles, 3 , "", SET("Datasession"))
  lnResult = oAriaApplication.RemoteSystemData.Execute("SELECT SydIndex.cFile_Tag, SydIndex.cIndx_Exp FROM SydIndex INNER JOIN SYDFILES on SYDFILES.CFILE_NAM=SydIndex.CFILE_NAM WHERE SydIndex.CFILE_NAM + SydIndex.CFILE_TAG = '" + lcTableName + lcTagName + "' AND SYDFILES.CVER = 'A40'",;
    "", "SysTmpCursor", "", oAriaApplication.cAria4SysFiles, 3 , "", SET("Datasession"))
  *N000682,1 MMT 03/13/2013 Fix issues of Globalization Testing Phase#2[END]
  ...
  ELSE
    LOCAL llRetVal
*    =lfSTRTOFILE("Open File "+lcFilePath + lcTableName)
   * ON ERROR lfSTRTOFILE(MESSAGE())
    *! B612688,1 MMT 09/03/2023 Can not run the Rebalance program from the Request builder[T-ERP-20230821.0001][Start]
    IF TYPE('lcXMLFileName') = 'C'
      USE (lcFilePath + lcTableName) SHARED ALIAS (lcCursorName) AGAIN IN 0 ORDER (lcTagName)
    ...
    *! B612688,1 MMT 09/03/2023 Can not run the Rebalance program from the Request builder[T-ERP-20230821.0001][Start]
    ENDIF
    *! B612688,1 MMT 09/03/2023 Can not run the Rebalance program from the Request builder[T-ERP-20230821.0001][End]
*        =lfSTRTOFILE("After Open File "+lcTableName)
    IF llRetVal
      CURSORSETPROP("Buffering", 5, lcCursorName)
    ELSE
      USE IN SysTmpCursor
  ...
  ELSE
    ...
    IF EOF()
      ...
      ELSE
        ...
        *!*		           "", "SysTmpCursor", "", oAriaApplication.cAria4SysFiles, 3 , "", SET("Datasession"))
        lnResult = oAriaApplication.RemoteSystemData.Execute("SELECT cFile_Tag, cIndx_Exp FROM SydIndex INNER JOIN SYDFILES on SYDFILES.CFILE_NAM=SydIndex.CFILE_NAM  WHERE AND SYDFILES.CVER = 'A40' AND SydIndex.CFILE_NAM + SydIndex.CFILE_TAG = '" + lcFileNam + lcTagNam  + "'",;
          "", "SysTmpCursor", "", oAriaApplication.cAria4SysFiles, 3 , "", SET("Datasession"))
        *N000682,1 MMT 03/13/2013 Fix issues of Globalization Testing Phase#2[END]
        IF lnResult < 1
          oAriaApplication.RemoteSystemData.CheckRetResult("Execute", lnResult, .T.)
...
*    =lfSTRTOFILE(" lnAlias, lnFilePos, llNative, lcTableName, lcTagName, lcSqlStat, lcIndexFlds")
  lnFilePos = ASUBSCRIPT(laFileStr, ASCAN(laFileStr, ALLTRIM(lcCursor)), 1)
*    =lfSTRTOFILE(STR(lnFilePos ))
  llNative  = laFileStr[lnFilePos,6]
 * =lfSTRTOFILE(" llNative  "+IIF(llNative  ,'TRUE','false'))
  IF llNative
  *   =lfSTRTOFILE(" usED lcCursor"+IIF(USED(lcCursor) ,'TRUE','false'))
   *       =lfSTRTOFILE(" ORDER lcCursor"+ORDER(lcCursor) )
*  =lfSTRTOFILE(" lcCursor"+lcCursor)
 *   X=SEEK(lcValue, lcCursor)
  * =lfSTRTOFILE(" lcValue"+IIF(X  ,'TRUE','false'))
  ...
  IF lnConnHnd < 0
    ...
    IF lnConnHnd < 0
      ...
      *=MESSAGEBOX(LANG_SMREBAL_CONNERR, 16, _Screen.Caption)
      =MESSAGEBOX(IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CONNERR,loFormSet.GetHeaderText("LANG_SMREBAL_CONNERR",loFormSet.HeaderAlias)), 16, _SCREEN.CAPTION)
      *N000682,1 11/20/2012 MMT Globlization changes[End]
...
FUNCTION lfvPackNo
  PARAMETERS lcParam
  *! B611091,1 MMT 12/10/2015 Packing List browse in rebalance displayes wrong order CUSTPO#[T20151119.0008][Start]
  IF lcParam = 'S'
    SELECT PACK_HDR
  ...
  ELSE
    ...
    *! B611091,1 MMT 12/10/2015 Packing List browse in rebalance displayes wrong order CUSTPO#[T20151119.0008][End]
...
FUNCTION lfVryItem2
  ...
  *=lfInitThermo(lnTotal, LANG_SMREBAL_RECALC)
  =lfInitThermo(lnTotal, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_RECALC,loFormSet.GetHeaderText("LANG_SMREBAL_RECALC",loFormSet.HeaderAlias)))
  *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][Start]
    IF TYPE('lcXMLFileName') <> 'C'
      ...
      *E303410,1 TMI 09/01/2013 [Start] if called from style screen do not show progress bar
      IF EMPTY(lcStyMaj)
        *E303410,1 TMI 09/01/2013 [End  ]
        oProgress.lblSecondLabel.CAPTION = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_VENDOR,loFormSet.GetHeaderText("LANG_SMREBAL_VENDOR",loFormSet.HeaderAlias))  + ALLTRIM(APVENDOR.cVendCode)
        *N000682,1 11/20/2012 MMT Globlization changes[End]
        oProgress.CurrentProgress(lnCurrent)
        *E303410,1 TMI 09/01/2013 [Start]
      ENDIF
    ...
    IF SEEK(M.cVendCode)
      ...
      IF APVENDOR.nVenOpnPo <> m.nVenOpnPo &&AND !llUpdt
        ...
        *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +LANG_SMREBAL_BARACT + ALLTRIM(M.cVendCode) + LANG_SMREBAL_WRONGOPNPOS
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_BARACT,loFormSet.GetHeaderText("LANG_SMREBAL_BARACT",loFormSet.HeaderAlias)) + ALLTRIM(M.cVendCode) + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGOPNPOS,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGOPNPOS",loFormSet.HeaderAlias))
        *N000682,1 11/20/2012 MMT Globlization changes[End]
        ...
        IF llAutoBal
          *N000682,1 11/20/2012 MMT Globlization changes[Start]
          *laRebMsg[3] = LANG_SMREBAL_ENDPROC +DTOC(DATE())+SPACE(5)+TIME()
          laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
          *N000682,1 11/20/2012 MMT Globlization changes[End]
  ...
  IF SEEK(M.cVendCode)
    ...
    IF APVENDOR.NVENOPNDR <> NVENOPNDR &&AND !llUpdt
      ...
      *laRebMsg[2] = LANG_SMREBAL_COMP + lcCurrComp_ID +LANG_SMREBAL_BARACT + ALLTRIM(M.cVendCode) + LANG_SMREBAL_WRONGDEBIT
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + lcCurrComp_ID +IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_BARACT,loFormSet.GetHeaderText("LANG_SMREBAL_BARACT",loFormSet.HeaderAlias)) + ALLTRIM(M.cVendCode) + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGDEBIT,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGDEBIT",loFormSet.HeaderAlias))
      *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    IF APVENDOR.NVENBAL <> NVENBAL &&AND !llUpdt
      ...
      *laRebMsg[2] = LANG_SMREBAL_COMP  + lcCurrComp_ID +LANG_SMREBAL_BARACT + ALLTRIM(M.cVendCode) + LANG_SMREBAL_CURRENTBAL
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))  + lcCurrComp_ID +IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_BARACT,loFormSet.GetHeaderText("LANG_SMREBAL_BARACT",loFormSet.HeaderAlias)) + ALLTRIM(M.cVendCode) + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CURRENTBAL,loFormSet.GetHeaderText("LANG_SMREBAL_CURRENTBAL",loFormSet.HeaderAlias))
      *N000682,1 11/20/2012 MMT Globlization changes[End]
...
FUNCTION  lfUpdItem2
  ...
  *=lfInitThermo(lnTotal, LANG_SMREBAL_UPDBAL)
  =lfInitThermo(lnTotal, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_UPDBAL,loFormSet.GetHeaderText("LANG_SMREBAL_UPDBAL",loFormSet.HeaderAlias)))
  *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    *-- If The Vendor time stamp is not changed
          ...
          *E303410,1 TMI 09/01/2013 [Start] if called from style screen do not show progress bar
          IF EMPTY(lcStyMaj)
            *E303410,1 TMI 09/01/2013 [End  ]
            ...
            oProgress.lblSecondLabel.CAPTION = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_VENDOR,loFormSet.GetHeaderText("LANG_SMREBAL_VENDOR",loFormSet.HeaderAlias)) + ALLTRIM(&lcAPRBALV..cVendCode)
            *N000682,1 11/20/2012 MMT Globlization changes[End]
    ...
    ELSE    && Else [IF The Vendor time stamp was changed]
      ...
      *laRebMsg[2] = LANG_SMREBAL_COMP    + lcCurrComp_ID +": "+ ALLTRIM(cVendCode) + LANG_SMREBAL_INFORMATION
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))    + lcCurrComp_ID +": "+ ALLTRIM(cVendCode) + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_INFORMATION,loFormSet.GetHeaderText("LANG_SMREBAL_INFORMATION",loFormSet.HeaderAlias))
      *N000682,1 11/20/2012 MMT Globlization changes[End]
...
FUNCTION lfVryItm24
  ...
  *! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033][Start]
  llIncludeInv = .T.
  SELECT (lcRebalHdr)
  *B611096,1 MMT 12/15/2015 Error while rebalancing AP module with update option[T20151210.0043][Start]
  lcOldHeaderKey = EVALUATE(KEY())
  *B611096,1 MMT 12/15/2015 Error while rebalancing AP module with update option[T20151210.0043][End]
  IF SEEK('LCRPINLK')
    llIncludeInv = IIF(ALLTRIM(cUpdFunNam) =='E',.F.,.T.)
  ENDIF
  *B611096,1 MMT 12/15/2015 Error while rebalancing AP module with update option[T20151210.0043][Start]
  IF !EMPTY(lcOldHeaderKey)
    SELECT (lcRebalHdr)
    =SEEK(lcOldHeaderKey)
  ENDIF
  *B611096,1 MMT 12/15/2015 Error while rebalancing AP module with update option[T20151210.0043][End]
  ...
  *! E303349,1 SAB 02/25/2013 Convert Rebalnce to run from Request Builder [T20100422.0001][End]
  SCAN FOR cInvStat <> 'V'
    ...
    *! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033][Start]
    IF !llIncludeInv
      IF gfSEEK(APINVHDR.cFisFYear+APINVHDR.cFspPrdId,'fsprd_A','COMFYRPRDI') AND fsprd_A.lfsplocks
        LOOP
      ENDIF
    ENDIF
    *! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033][End]
    ...
    *IF APINVHDR.NINVPAID <> lnAmtPaid
    IF IIF(APINVHDR.nInvPaid <> lnAmtPaid ,APINVHDR.nInvAdj+APINVHDR.nInvPaid <>lnAmtPaid,.F.)
      *! B609188,3 MMT 12/01/2010 Reblance Give wrong paid amount if ncurrunit <> 1[End]
      DECLARE laRebMsg[3]
      laRebMsg[1] = " "
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))  + lcCurrComp_ID +IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_BARACT,loFormSet.GetHeaderText("LANG_SMREBAL_BARACT",loFormSet.HeaderAlias))+ ALLTRIM(APINVHDR.cVendCode) +;
        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_INVOICE,loFormSet.GetHeaderText("LANG_SMREBAL_INVOICE",loFormSet.HeaderAlias))+ ALLTRIM(APINVHDR.cInvNo) +;
        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_WRONGPAY,loFormSet.GetHeaderText("LANG_SMREBAL_WRONGPAY",loFormSet.HeaderAlias))
      laRebMsg[3] = " "
      ...
      *E303410,1 TMI 09/01/2013 [Start] if called from style screen do not show progress bar
      IF EMPTY(lcStyMaj)
        *E303410,1 TMI 09/01/2013 [End  ]
        ...
        oProgress.lblSecondLabel.CAPTION = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_VENDOR,loFormSet.GetHeaderText("LANG_SMREBAL_VENDOR",loFormSet.HeaderAlias)) + ALLTRIM(APINVHDR.cVendCode)+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_INVOICE2,loFormSet.GetHeaderText("LANG_SMREBAL_INVOICE2",loFormSet.HeaderAlias))+ ALLTRIM(APINVHDR.cInvNo)
        *N000682,1 11/20/2012 MMT Globlization changes[End]
        oProgress.CurrentProgress(lnCurrent)
...
FUNCTION _lfInvInfo
  ...
  * Initialize variables
  ...
  ENDIF
   ...
   IF M.cVendCode ='ARTISAN' AND lcInvNo ='ADJ 2022    '
     SET STEP ON 
   ENDIF
...
*!*	    "laRpCmpCod,laRpCodMod"
  RETURN "lcAria4Modules, lcToReb, lcCompCode, llPurge, llOgForAut, llAutoBal ,LANG_SMREBAL_REBLOG ,laRebMsg ,lcAria4Modules ,lcWinTitl,"+;
    ...
    "lcSelInvNo,lcSelOrdNo,lcSelPONo ,lcSelAccount ,lcSelStyle ,lcSelFabric ,lcSelYear,lcSelCTNo,lcSPO,lcSoOrd,lcArInv,lcMfCt,"+;
    "lcPackSelect , lcALPack ,lnRemResult ,oProgress ,lnPrgSession ,lcComps ,lcModls ,lcPathStat ,lcAutoBal ,llExpr1,llUpdt,laRpSorCmp,"+;
    "laRpCmpCod,laRpCodMod,lcArCsHst"
  *! E303990,1 MMT 05/28/2018 Add AR History rebalance option to AR module[T20180511.0001][End]
...
FUNCTION lfGetLen
  ...
  *! Developer : Mariam Mazhar(MMT)
  *! Date      : 11/25/2015
  *! Purpose   : Update Lock Field in update functiuon name field
  *!*************************************************************
  *! Example   : = lfvUpdLock()
  *!*************************************************************
FUNCTION lfvUpdLock
  ...
  *-- Now change the laInfoTemp.cUpdVryIgn  with the value of (Update/verify/Ignore)
  SELECT (lcRebalHdr)
  IF SEEK('LCRPINLK')
    REPLACE cUpdFunNam WITH LCRPINLK
  ENDIF
  ...
  SELECT (lnAlias)
  RETURN
  *! E303626,1 MMT 11/25/2015 Add option to rebalance to exclude invoices in Locked period[T20151112.0033][Ends]

  *! E303873,1 MMT 09/28/2017 Adding GL Module to Rebalance program[P20170905.0001][Start]
  *!*************************************************************
  *! Name      : lfVryItm25
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Verify GL Account Rebalance
  *!*************************************************************
FUNCTION lfVryItm25
  lcRBalYear = lcFiscYear
  llBalnBfor = .F.
  lcComp_Id =lcCurrComp_ID
  ...
  IF VAL(lcFiscYear) < VAL(lcCurr_yer)
    ...
    * Buttons :           <Proceed> <Cacel>
    IF !llAutoBal
      ...
      ENDIF
    ELSE
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
      IF llViewLog
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
        DECLARE laRebMsg[3]
        laRebMsg[1] = " "
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
          lcCurrComp_ID + ": " +;
          IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLSTARTYEAR1,loFormSet.GetHeaderText("LANG_SMREBAL_GLSTARTYEAR1",loFormSet.HeaderAlias))
        laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLSTARTYEAR2 ,loFormSet.GetHeaderText("LANG_SMREBAL_GLSTARTYEAR2",loFormSet.HeaderAlias))
        laRebMsg[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLSTARTYEAR3 ,loFormSet.GetHeaderText("LANG_SMREBAL_GLSTARTYEAR3",loFormSet.HeaderAlias))
        laRebMsg[5] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLSTARTYEAR4 ,loFormSet.GetHeaderText("LANG_SMREBAL_GLSTARTYEAR4",loFormSet.HeaderAlias))
        laRebMsg[6] = " "
        =lfVryRport()
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
      ENDIF
  ...
  *IF There is any Batches or Transactions to be Reposted
  IF lfSelData()
    SET PROCEDURE TO (oAriaApplication.ApplicationHome+'GL\GL.FXP') ADDITIVE
    loTmpFormSet = CREATEOBJECT('Custom')
    loTmpFormSet.ADDPROPERTY('lcSJ_DEF','GJ')
    loTmpFormSet.ADDPROPERTY('HeaderAlias','')
    loTmpFormSet.ADDPROPERTY('lnCurr_Yer',lnCurr_yer)
    loTmpFormSet.ADDPROPERTY('lcAcsMask',lcAcsMask)
    loTmpFormSet.ADDPROPERTY('lnAcsSegSz',lnAcsSegSz)
    loTmpFormSet.ADDPROPERTY('laEvntTrig[1]','')
    llUesdBefo = .F.        && Check if used before or this the first time
    IF NOT USED("FISHD")
      SELECT 0
      =gfOpenTable(lcFilePath+'FISHD','COMPFYEAR')
      llUesdBefo = .T.
    ENDIF
    llUesdBefSet = .F.        && Check if used before or this the first time
    IF NOT USED("GLSETUP")
      SELECT 0
      =gfOpenTable(lcFilePath+'GLSETUP')
      llUesdBefSet = .T.
    ENDIF
    loProgress = oProgress
    IF RECCOUNT(lcTmpTran) <> 0
      lnTBPosted = 0
      llFrReb = .T.

      =lfTBPost(loTmpFormSet,"Transactions",lcTmpTran ,'CLOSING','GLACBALS',lcComp_Id,lcFilePath,.T.)
      IF RECCOUNT(lcTmpTran) <> lnTBPosted
        * Meesage : "Some of the ð were not succesfully reposted to the "
        *           "balances file , Unable to proceed with rebalancing."
        * Buttons :                           <Ok>
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
        IF llViewLog
          *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
          ...
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            PADC(IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLREBALANCE,loFormSet.GetHeaderText("LANG_SMREBAL_GLREBALANCE",loFormSet.HeaderAlias)) , 68)
          laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBEGINNINGBATCH,loFormSet.GetHeaderText("LANG_SMREBAL_GLBEGINNINGBATCH",loFormSet.HeaderAlias))
          laRebMsg[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBALFILESUCC,loFormSet.GetHeaderText("LANG_SMREBAL_GLBALFILESUCC",loFormSet.HeaderAlias))
          laRebMsg[5] = " "

          IF llAutoBal
            laRebMsg[5] =  IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
          ENDIF
    ...
    *Reposted succesfully
    IF RECCOUNT(lcTmpBBtch) <> 0
      ...
      *DO (gcAppHome+"GL.APP") WITH "LFTBPOST WITH 'Batch',lcTmpBBtch ,'Beginning','GLACBALS',lcComp_Id,lcFilePath,llFrReb",'',"T"
      =lfTBPost(loTmpFormSet,'Batch',lcTmpBBtch ,'Beginning','GLACBALS',lcComp_Id,lcFilePath,.T.)
      IF RECCOUNT(lcTmpBBtch) <> lnTBPosted
        * Meesage : "Some of the ð were not succesfully reposted to the "
        *           "balances file , Unable to proceed with rebalancing."
        * Buttons :           <Ok>
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
        IF llViewLog
          *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
          ...
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            PADC(IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBEGINNINGBATCH ,loFormSet.GetHeaderText("LANG_SMREBAL_GLBEGINNINGBATCH",loFormSet.HeaderAlias)), 68)
          laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLSOMEBATCHSUCC,loFormSet.GetHeaderText("LANG_SMREBAL_GLSOMEBATCHSUCC",loFormSet.HeaderAlias))
          laRebMsg[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBALFILESUCC,loFormSet.GetHeaderText("LANG_SMREBAL_GLBALFILESUCC",loFormSet.HeaderAlias))
          laRebMsg[5] = " "
          IF llAutoBal
            laRebMsg[5] =  IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
    ...
    *Reposted succesfully
    IF RECCOUNT(lcTmpJBtch) <> 0
      ...
      *DO (gcAppHome+"GL.APP") WITH "LFTBPOST WITH 'Batch',lcTmpJBtch ,'Nonbeginning','GLACBALS',lcComp_Id,lcFilePath,llFrReb",'',"T"
      =lfTBPost(loTmpFormSet,'Batch',lcTmpJBtch ,'Nonbeginning','GLACBALS',lcComp_Id,lcFilePath,.T.)
      IF RECCOUNT(lcTmpJBtch) <> lnTBPosted
        * Message : "Some of the ð were not succesfully reposted to the "
        *           "balances file , Unable to proceed with rebalancing."
        * Buttons :           <Ok>
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
        IF llViewLog
          *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
          ...
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            PADC(IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHES,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHES",loFormSet.HeaderAlias)), 68)
          laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLSOMEBATCHSUCC,loFormSet.GetHeaderText("LANG_SMREBAL_GLSOMEBATCHSUCC",loFormSet.HeaderAlias))
          laRebMsg[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBALFILESUCC,loFormSet.GetHeaderText("LANG_SMREBAL_GLBALFILESUCC",loFormSet.HeaderAlias))
          laRebMsg[5] = " "
          IF llAutoBal
    ...
    *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
    IF llViewLog
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
        lcCurrComp_ID + ": " +;
        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLNOBATCHREPOST,loFormSet.GetHeaderText("LANG_SMREBAL_GLNOBATCHREPOST",loFormSet.HeaderAlias))
      laRebMsg[3] = " "
...
FUNCTION lfGetCurYr
  ...
  llOpenComp = .F.
  IF !USED('SYCCOMP_YR')
    llOpenComp = gfOpenTable('SYCCOMP','CCOMP_ID','SH','SYCCOMP_YR')
  ENDIF
  lcCurr_yer = IIF(gfSEEK(oAriaApplication.PrntCompanyID,'SYCCOMP_YR','CCOMP_ID'),SYCCOMP_YR.cCurr_Yer,'')
  lnCurr_yer = INT(VAL(lcCurr_yer))

  IF llOpenComp
    =gfCloseTable('SYCCOMP_YR')
  ENDIF

  =gfOpenTable(lcFilePath+'ACCOD', '' ,'SH')
  SELECT ACCOD
  GO TOP
  lcAcsMask  = ACCOD.cAcsMask
  lcAcsMask  = "X"+SUBSTR(cAcsMask,2)
...
FUNCTION lfFixBatch
  ...
  =lfInitThermo(lnTotRec ,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHFILE,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHFILE",loFormSet.HeaderAlias)) )
  *SCAN Loop to scan the Posted transaction header for the records that
  *has Batch number and this Batch number dose not exist in the GLBATCH
  *file or was deleted
  SCAN FOR cBatchNo + cTranNo <> '000000' ;
...
FUNCTION lfSelData
  ...
  *IF The Temp. Balance file [Opened with the Alias GLACBALS] exist
  ...
  SELECT GLTMPBAL
  lnTotal = RECCOUNT()        && Varible to hold the number of Account Balances file records [Opened with the Alias GLTMPBAL]
  =lfInitThermo(lnTotal , IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLACBALSFILE,loFormSet.GetHeaderText("LANG_SMREBAL_GLACBALSFILE",loFormSet.HeaderAlias)) )
  lnCurrent = 0               && Varible to hold the number of records that was finished
  GO TOP
...
FUNCTION lfPreData
  ...
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Update master file of (lcRpItem25)
  *!*************************************************************
  *! Called from : Screen GLRBAL
  *!*************************************************************
  *! Calls       : None
...
FUNCTION lfUpdItm25
      ...
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
      IF llViewLog
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
        DECLARE laRebMsg[5]
        laRebMsg[1] = " "
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMSGTRAN1,loFormSet.GetHeaderText("LANG_SMREBAL_GLMSGTRAN1",loFormSet.HeaderAlias))
        laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMSGTRAN2,loFormSet.GetHeaderText("LANG_SMREBAL_GLMSGTRAN2",loFormSet.HeaderAlias))
        laRebMsg[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMSGTRAN3,loFormSet.GetHeaderText("LANG_SMREBAL_GLMSGTRAN3",loFormSet.HeaderAlias))
        laRebMsg[5] = " "
        IF llAutoBal
          laRebMsg[5] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
        ENDIF
      ...
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
      IF llViewLog
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
        ...
        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
          lcCurrComp_ID + ": " +;
          IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMSGCHRT1,loFormSet.GetHeaderText("LANG_SMREBAL_GLMSGCHRT1",loFormSet.HeaderAlias))
        laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMSGCHRT2,loFormSet.GetHeaderText("LANG_SMREBAL_GLMSGCHRT2",loFormSet.HeaderAlias))
        laRebMsg[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMSGCHRT3,loFormSet.GetHeaderText("LANG_SMREBAL_GLMSGCHRT3",loFormSet.HeaderAlias))
        laRebMsg[5] = " "
        IF llAutoBal
          laRebMsg[5] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
        ENDIF
  ...
  *IF The user selected to cancel
  ...
  IF !llAutoBal
    IF gfModalGen("TRM02255B00012" , "ALERT" , lcComp_Id + '|'+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLUPDATE,loFormSet.GetHeaderText("LANG_SMREBAL_GLUPDATE",loFormSet.HeaderAlias))) = 2
      RETURN
    ENDIF    && End of IF
  ENDIF
  ...
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Validate Fiscal Year
  *!*************************************************************
FUNCTION lfvFsYear
  DECLARE laRpRetFld(1)
  lcOldBrFld    = lcBrFields
  lcBrFields    = 'CFisFYear:H="'+;
    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
    '",DFisBgDat:H="'+;
    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBEGDATE,loFormSet.GetHeaderText("LANG_SMREBAL_GLBEGDATE",loFormSet.HeaderAlias))+;
    '",DFisEnDat:H="'+;
    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENDDATE,loFormSet.GetHeaderText("LANG_SMREBAL_GLENDDATE",loFormSet.HeaderAlias))+'"'
  laRpRetFld[1] = ''
  lcFicsYr = EVALUATE(SYS(18))
  lcRpCurFld = VARREAD()
  lcOldAlias = SELECT()    && Save the current alias
  ...
  IF EMPTY(lcFicsYr)   OR '?' $ &lcRpCurFld. .OR. !gfSEEK(lcFicsYr)
    =gfBrows( " FOR cFisYStat <> 'H' ",'CFisFyear',"laRpRetFld",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLFISCALYEARBRWTITLE,loFormSet.GetHeaderText("LANG_SMREBAL_GLFISCALYEARBRWTITLE",loFormSet.HeaderAlias)),.F.)
    &lcRpCurFld. = laRpRetFld[1]
    lcFicsYr   = laRpRetFld[1]
...
FUNCTION lfVldItm25
    ...
    *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
    IF llViewLog
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
      ...
      DECLARE laRebMsg[3]
      laRebMsg[1] = " "
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLFISCALYEARINVALID,loFormSet.GetHeaderText("LANG_SMREBAL_GLFISCALYEARINVALID",loFormSet.HeaderAlias))
      laRebMsg[3] = " "
  ...
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Transactions Mover
  ************************************************************
FUNCTION lfvTranMov
  *! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover[Start]
  *= gfMover(@laRPTranSo,@laRPTranTa,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Transaction,loFormSet.GetHeaderText("LANG_Transaction",loFormSet.HeaderAlias)),.T.,.F.,.F.,.T.)
  = lfogMover(@laRPTranSo,@laRPTranTa,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Transaction,loFormSet.GetHeaderText("LANG_Transaction",loFormSet.HeaderAlias)),.T.,.F.,.F.,.T.)
  *! B612401,1 MMT 05/23/2021 Rebalance calls gfMover instead of lfOgMover
  ************************************************************
  *! Name      : RefreshType
...
FUNCTION lfTranType
  ...
  IF !USED('SyGLTran')
    lnSyResult = oAriaApplication.RemoteSystemData.Execute("SELECT * FROM SyGLTran", '', 'SyGLTran', "", oAriaApplication.SystemConnectionString, 3, "", SET("Datasession"))
    IF lnSyResult <> 1
      RETURN .F.
    ENDIF
  ENDIF
  ...
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Copy OG Variables to program Variables
  ************************************************************
FUNCTION lfGLDistVars
  ldStartTran = {}
  ldEndTran ={}
  lnDatePos = ASCAN(laogfxflt,'GLDIST.TRAN_DATE')
  IF lnDatePos > 0
    lnDatePos = ASUBSCRIPT(laogfxflt,lnDatePos,1)
    IF !EMPTY(laogfxflt[lnDatePos,6])
      ldStartTran = CTOD(SUBSTR(laogfxflt[lnDatePos,6],1,ATC('|',laogfxflt[lnDatePos,6])-1))
      ldEndTran = CTOD(SUBSTR(laogfxflt[lnDatePos,6],ATC('|',laogfxflt[lnDatePos,6])+1))
    ENDIF
  ENDIF
  lcTransList =''
  IF !EMPTY(laRPTranTa)
    FOR lnI = 1 TO ALEN(laRPTranTa)
      ...
      lcTransList = lcTransList + "," + lcTranType
    ENDFOR
  ENDIF
  ************************************************************
  *! Name      : lfVryItm26
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Verify GL entries
  ************************************************************
FUNCTION lfVryItm26

  gcUser_Id = oAriaApplication.User_ID
  gdSysDate = oAriaApplication.SystemDate
  lcDataDir = lcFilePath
  lcComp_Id =lcCurrComp_ID
  IF !USED('SyCcomp_A')
    lnSyResult = oAriaApplication.RemoteSystemData.Execute("SELECT * FROM SyCcomp Where cComp_id = '"+lcComp_Id+"'", '', 'SyCcomp_A', "", oAriaApplication.SystemConnectionString, 3, "", SET("Datasession"))
    IF lnSyResult <> 1
      RETURN .F.
    ENDIF
  ENDIF
  gcCom_Name = SyCcomp_A.cCom_Name
  gcBaseCurr =  SyCcomp_A.cCurrCode
  gcContCode = SyCcomp_A.ccont_code
  gcCmpModules = SyCcomp_A.mComp_mdl
  STORE '' TO M_POST_PPRD,M_SYS_DIR,M_GL_VERS,M_GL_CO
  =gfgetmemvar('M_POST_PPRD,M_SYS_DIR,M_GL_VERS,M_GL_CO',lcComp_Id)
  lcSysDir   = ALLTRIM(M_SYS_DIR)
  lcGlVers   = ALLTRIM(M_GL_VERS)
  lcGlComp   = ALLTRIM(M_GL_CO)

  IF lcGlVers = 'S'            &&   <<<... SBT 2.5 ... >>>
    =gfOpenFile(lcSysDir+'SYCCOMP',lcSysDir+'COMPID','SH','SYCCOMP_S')
  ...
  ENDIF
  lcFltExp = ".T."
  IF !EMPTY(ldStartTran) OR !EMPTY(ldEndTran)
    IF EMPTY(ldStartTran) AND !EMPTY(ldEndTran)
      lcFltExp = "DPOSTDATE =< CTOD('"+DTOC(ldEndTran)+"')"
    ELSE
      lcFltExp = "BETWEEN(DPOSTDATE,CTOD('"+DTOC(ldStartTran)+"'),CTOD('"+DTOC(ldEndTran)+"'))"
    ENDIF
  ENDIF
  lcTime     =  gfGetTime()
  ...
  lcTarTrans = lcTransList
  IF !USED('gldist')
    *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
    *!*	  Use (lcDataDir+'gldist.dbf') In 0 Shared Again
  * =gfOpenTable(lcDataDir + 'gldist','SH')
    =gfOpenTable(lcDataDir + 'gldist','','SH')
    *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
    llGlDist = .T.
  ENDIF
  ...
  *!********************************************************************
  IF EMPTY(lcTarTrans) OR "IP" $ lcTarTrans OR "IA" $ lcTarTrans OR ;
    ...
    IF !USED('StyInvJl')
      *E304029,1 SAH CONVERT STYINVJL TO SQL [BEGIN]
      *USE (lcDataDir+'StyInvJl.dbf')   IN 0 SHARED
      =gfOpenTable('StyInvJl','StyInvJl','SH')
      *E304029,1 SAH CONVERT STYINVJL TO SQL [End]
      llStyInvJl = .T.
    ENDIF
    ...
    SELECT StyInvJl
    llUseBins = gfgetmemvar('M_DLUSEBIN',lcComp_Id)
    ...
    *E304029,1 SAH CONVERT STYINVJL TO SQL [BEGIN]
    ...
    SET ORDER TO
    *E304029,1 SAH CONVERT STYINVJL TO SQL [End]
    COUNT FOR &lcScanExp AND &lcStyJorFl TO lnTotStyInv
    =lfInitThermo(lnTotStyInv,IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_GLSTYLEINVJLRECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLSTYLEINVJLRECONC",loFormSet.HeaderAlias)))
    lnProgressSty= 0
    SELECT StyInvJl

    SCAN FOR &lcScanExp AND &lcStyJorFl
      lnProgressSty = lnProgressSty +  1
      oProgress.CurrentProgress(lnProgressSty)
  ...
  *!**********************************
  IF EMPTY(lcTarTrans) OR "IN" $ lcTarTrans OR "VI" $ lcTarTrans
    ...
    COUNT FOR &lcINFulFlt TO lnTotInvhdr
    =lfInitThermo(lnTotInvhdr,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLINVOICERECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLINVOICERECONC",loFormSet.HeaderAlias)) )
    lnProgressInvh= 0
    SELECT InvHdr

    SCAN FOR &lcINFulFlt
      lnProgressInvh = lnProgressInvh + 1
      oProgress.CurrentProgress(lnProgressInvh)
      ...
      ***Taxes
      IF (EMPTY(lcTarTrans) OR "IN" $ lcTarTrans) AND &lcINFltr AND InvHdr.nPstAmt+InvHdr.nHstAmt <> 0
        IF UPPER(ALLTRIM(gcContCode))='CANADA'
          IF InvHdr.nHstAmt > 0
   
*B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [Start]       
          *  DO lpGLDist WITH .F.,'New','030','IN',InvHdr.invoice,;
              - InvHdr.nHstAmt,InvHdr.dPostDate,;
              InvHdr.cTaxAcnt,lcGlPeriod,lcGlYear,;
              lcCurrCode,lnCurrUnit,lnExRate
         DO lpGLDist WITH .F.,'New','030','IN',InvHdr.invoice,;
              -1*InvHdr.nvHstAmt,InvHdr.dPostDate,;
              InvHdr.cTaxAcnt,lcGlPeriod,lcGlYear,;
              lcCurrCode,lnCurrUnit,lnExRate
*B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [End]       

          ELSE
        
*B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [Start]   
           * DO lpGLDist WITH .F.,'New','029','IN',InvHdr.invoice,;
              - InvHdr.nPstAmt,InvHdr.dPostDate,;
              InvHdr.cTaxAcnt,lcGlPeriod,lcGlYear,;
             lcCurrCode,lnCurrUnit,lnExRate

DO lpGLDist WITH .F.,'New','029','IN',InvHdr.invoice,;
              -1*InvHdr.nvPstAmt,InvHdr.dPostDate,;
              InvHdr.cTaxAcnt,lcGlPeriod,lcGlYear,;
              lcCurrCode,lnCurrUnit,lnExRate
*B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [End]    
      ...
      ****
      ...
      IF (EMPTY(lcTarTrans) OR "VI" $ lcTarTrans) AND InvHdr.STATUS='V' AND &lcVIFltr
        *-- Debit Sales tax
        ...
        IF UPPER(ALLTRIM(gcContCode))='CANADA' AND InvHdr.nvPstAmt+InvHdr.nvHstAmt <> 0
          IF InvHdr.nvHstAmt > 0
          
  
*B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [Start]    
     * DO lpGLDist WITH .F.,'New','030','IN',InvHdr.invoice,;
              InvHdr.nvHstAmt,InvHdr.dPostDate,;
              InvHdr.cTaxAcnt,lcGlPeriod,lcGlYear,;
              lcCurrCode,lnCurrUnit,lnExRate
       DO lpGLDist WITH .F.,'New','030','IN',InvHdr.invoice,;
              -1*InvHdr.nvHstAmt,InvHdr.dPostDate,;
...
*B611918,1 Es 10/08/2019 GL Rebalance Inserts incorrect records in GLDIST in case of Canadian companies for PST and HST taxes [Start]      
           * DO lpGLDist WITH .F.,'New','029','IN',InvHdr.invoice,;
              InvHdr.nvPstAmt,InvHdr.dPostDate,;
              InvHdr.cTaxAcnt,lcGlPeriod,lcGlYear,;
              lcCurrCode,lnCurrUnit,lnExRate
  ...
  *- Check if the RM or PS module is installed to open the Rethdr File
  IF 'RM' $ gcCmpModules .OR. 'PS' $ gcCmpModules
    IF EMPTY(lcTarTrans) OR "RM" $ lcTarTrans OR "VR" $ lcTarTrans
      IF !USED('RetHdr')
        gfOpenTable('RetHdr','RetHdr','SH','RetHdr',.F.,lcComp_Id)
        llRetHdr = .T.
      ENDIF
      IF !USED('RetLine')
        gfOpenTable('RetLine','RetLine','SH','RetLine',.F.,lcComp_Id)
        llRetLine = .T.
      ENDIF
      ...
      COUNT FOR  &lcFltExp TO lnTotRethdr
      =lfInitThermo(lnTotRethdr,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLCRMEMORECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLCRMEMORECONC",loFormSet.HeaderAlias)) )
      lnProgressCr= 0
      SELECT RetHdr
      ...
      SCAN FOR &lcFltExp
        ...
        lnProgressCr = lnProgressCr + 1
        oProgress.CurrentProgress(lnProgressCr)
  ...
  *!**********************************
  IF EMPTY(lcTarTrans) OR "MP" $ lcTarTrans OR "MA" $ lcTarTrans OR ;
    ...
    IF EMPTY(lcTarTrans)
      lcScanExp = [INLIST(MatInvJl.CTRTYPE,"1","2","5")]
    ELSE
      lcScanExp = [INLIST(MatInvJl.CTRTYPE]
      IF "MO" $ lcTarTrans
        lcScanExp = lcScanExp + [,"5"]
      ENDIF
      IF "MA" $ lcTarTrans
    ...
    IF !USED('MatInvJl')
      =gfOpenTable('ITEMJRNL','MFGOPR','SH','MatInvJl',.F.,lcComp_Id)
      llMatInvJl = .T.
    ENDIF
    lcMatJorFl = lcFltExp
    SELECT MatInvJl
    ...
    COUNT FOR &lcScanExp AND &lcMatJorFl TO lnTotMatInv
    =lfInitThermo(lnTotMatInv, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMAINVJLRECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLMAINVJLRECONC",loFormSet.HeaderAlias)))
    lnProgressMat = 0
    SELECT MatInvJl
    ...
    SCAN FOR &lcScanExp AND &lcMatJorFl
      lnProgressMat = lnProgressMat + 1
      oProgress.CurrentProgress(lnProgressMat)
      STORE '' TO lcGlYear , lcGlPeriod
      = lfChkPrd(MatInvJl.dTrDate,'lcGlYear','lcGlPeriod')
      ...
      IF (EMPTY(lcTarTrans) OR ;
          ...
          "MP" $ lcTarTrans) ;
          AND MatInvJl.cTrType $ "125"
        DO CASE
          CASE MatInvJl.cTrType= "5"
            lcTranType  = "MO"
            lcOtherCatg = "013"
          CASE MatInvJl.cTrType= "1"
            lcTranType = "MA"
            lcOtherCatg = "016"
          CASE MatInvJl.cTrType= "2"
            lcTranType = "MP"
            lcOtherCatg = "016"
        ENDCASE

        *--Material inventory control GLAccount
        DO lpGLDist WITH .F.,'New','015',lcTranType,MatInvJl.cTrCode,;
          MatInvJl.nStk1*MatInvJl.ncost,MatInvJl.dTrDate,;
          MatInvJl.cIcAcnt,lcGlPeriod,lcGlYear,;
          lcCurrCode,lnCurrUnit,lnExRate

        *--The other side
        DO lpGLDist WITH .F.,'New',lcOtherCatg,lcTranType,MatInvJl.cTrCode,;
          -1*MatInvJl.nStk1*MatInvJl.ncost,MatInvJl.dTrDate,;
          MatInvJl.cAdjAcct,lcGlPeriod,lcGlYear,;
          lcCurrCode,lnCurrUnit,lnExRate
  ...
  *!**********************************
  ...
  IF EMPTY(lcTarTrans) OR "CR" $ lcTarTrans OR "KO" $ lcTarTrans OR;
    ...
    IF (EMPTY(lcTarTrans) OR "IN" $ lcTarTrans OR "DA" $ lcTarTrans)
      ...
      SELECT Debit
      COUNT FOR  &lcFltExp AND &lcDrFlt TO lnTotDeb
      =lfInitThermo(lnTotDeb,  IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLDEBITRECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLDEBITRECONC",loFormSet.HeaderAlias)))
      lnProgressDeb = 0

      SELECT Debit
      SCAN FOR &lcFltExp AND &lcDrFlt
        lnProgressDeb = lnProgressDeb + 1
        oProgress.CurrentProgress(lnProgressDeb)
    ...
    *--------------------------------------------------------------------*
    ...
    IF EMPTY(lcTarTrans) OR "RM" $ lcTarTrans OR "CR" $ lcTarTrans OR "CA" $ lcTarTrans
      ...
      SELECT Credit
      COUNT FOR  &lcFltExp AND &lcCrFlt TO lnTotCre
      =lfInitThermo(lnTotCre, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLCREDITRECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLCREDITRECONC",loFormSet.HeaderAlias)))
      lnProgressCre = 0

      SELECT Credit
      SCAN FOR &lcFltExp AND &lcCrFlt
        lnProgressCre = lnProgressCre + 1
        oProgress.CurrentProgress(lnProgressCre)
    ...
    *--------------------------------------------------------------------*
    ...
    IF EMPTY(lcTarTrans) OR "CR" $ lcTarTrans OR "KO" $ lcTarTrans OR ;
      ...
      SELECT ArHist
      COUNT FOR &lcFltExp AND &lcArFlt TO lnTotHist
      =lfInitThermo(lnTotHist, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLHISTRECONC,loFormSet.GetHeaderText("LANG_SMREBAL_GLHISTRECONC",loFormSet.HeaderAlias)))
      lnProgressHist = 0

      SELECT ArHist
      SCAN FOR &lcFltExp AND &lcArFlt
        lnProgressHist = lnProgressHist + 1
        oProgress.CurrentProgress(lnProgressHist)
  ...
  *!**********************************
  ...
  lcRpExp = '.T.'
  IF !EMPTY(lcFltExp)
    lcFltExp = STRTRAN(lcFltExp,'DPOSTDATE','TRAN_DATE')
    lcRpExp = lcRpExp + " AND "+ lcFltExp
  ENDIF
  SELECT GLDist
  *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Satrt]
  *=gfSEEK('')
  =gfSEEK('','GLDist','GLDISTAC')
  *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
  ...
  ENDIF
  llfromUpdate =(PROGRAM(PROGRAM(-1)-1) =='LFUPDITM26')


  SELECT GLDist
  COUNT FOR IIF(!llfromUpdate ,&lcForExp.,.T.) TO lnTotGLDist
  =lfInitThermo(lnTotGLDist, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_RECONCREPTITL,loFormSet.GetHeaderText("LANG_SMREBAL_RECONCREPTITL",loFormSet.HeaderAlias)))
  lnProgressGL = 0
  SELECT GLDist


  SCAN FOR IIF(!llfromUpdate ,&lcForExp.,.T.)

    IF llfromUpdate
      IF !EMPTY(GLDist.GLBATCH)
        If !Seek(GLDist.GLBATCH+GLDist.CTRNSLEDN+GLDist.TRAN_NO+GLDist.Tran_Type,lcGLBtTrn,lcGLBtTrn)
          INSERT INTO (lcGLBtTrn) (GLBATCH,CTRNSLEDN,TRAN_NO,Tran_Type) VALUES (GLDist.GLBATCH,GLDist.CTRNSLEDN,GLDist.TRAN_NO,GLDist.Tran_Type)
        ENDIF
      ENDIF
      IF !EVALUATE(lcForExp)
        LOOP
      ENDIF
    ENDIF
    lnProgressGL = lnProgressGL + 1
    oProgress.CurrentProgress(lnProgressGL)
  ...
  *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
  IF llViewLog
    *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
    ...
    SELECT(lcGenDist)
    SCAN FOR LIKE("*Y",EVAL(KEY()))
      DECLARE laRebMsg[3]
      laRebMsg[1] = " "
      laRebMsg[3] = " "
      DO CASE
        CASE nGenEqvAmt = 0  &&The GLDIST record should not be there and will be deleted
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANNUM,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANNUM",loFormSet.HeaderAlias)) +;
            TRAN_NO+" ("+ALLTRIM(lfGetTranDesc(Tran_Type,loFormSet))+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLACCNUM,loFormSet.GetHeaderText("LANG_SMREBAL_GLACCNUM",loFormSet.HeaderAlias))+ ;
            ALLTRIM(GLAccount)+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLINCORRECTENTRY,loFormSet.GetHeaderText("LANG_SMREBAL_GLINCORRECTENTRY",loFormSet.HeaderAlias))

        CASE nEqvAmnt = 0  &&The GLDIST record is missing and Should be added
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANNUM,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANNUM",loFormSet.HeaderAlias))+TRAN_NO+;
            " ("+ALLTRIM(lfGetTranDesc(Tran_Type,loFormSet))+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLACCNUM,loFormSet.GetHeaderText("LANG_SMREBAL_GLACCNUM",loFormSet.HeaderAlias))+;
            ALLTRIM(GLAccount)+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMISSINGENTRY,loFormSet.GetHeaderText("LANG_SMREBAL_GLMISSINGENTRY",loFormSet.HeaderAlias))
        OTHERWISE && There is a difference in Amounts between GLDIST and generated record, the difference should be added to GLDIST
          laRebMsg[2] =  IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANNUM,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANNUM",loFormSet.HeaderAlias))+TRAN_NO+;
            " ("+ALLTRIM(lfGetTranDesc(Tran_Type,loFormSet))+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLACCNUM,loFormSet.GetHeaderText("LANG_SMREBAL_GLACCNUM",loFormSet.HeaderAlias))+;
            ALLTRIM(GLAccount)+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLMISMATCH,loFormSet.GetHeaderText("LANG_SMREBAL_GLMISMATCH",loFormSet.HeaderAlias))
      ENDCASE
      =lfVryRport()
      IF llAutoBal
        laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
  ...
  *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
  ...
  LOCAL lnScaleMode, lcWindName, lcIconFile
  lcWindName = gfTempName()
  lcIconFile = _SCREEN.ICON
  lnScaleMode = _SCREEN.SCALEMODE
  DEFINE WINDOW (lcWindName) FROM 0,0 TO SYSMETRIC(2) , SYSMETRIC(1)  ;
    FONT "Courier New",9 TITLE IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_RECONCREPTITL,loFormSet.GetHeaderText("LANG_SMREBAL_RECONCREPTITL",loFormSet.HeaderAlias));
    SYSTEM ICON FILE (lcIconFile)
  _SCREEN.SCALEMODE = 0
  REPORT FORM (oAriaApplication.ReportHome+"\SM\SMGLREC2") TO PRINTER PROMPT PREVIEW  WINDOW (lcWindName) &lcDiffExp.
  ...
  *! Developer : Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Get trans. description
  ************************************************************
FUNCTION lfGetTranDesc
  LPARAMETERS  _TRANTYP ,loFormSet
  XTRANDESC =''
  DO CASE
    CASE _TRANTYP = 'IN'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_INVOICE,loFormSet.GetHeaderText("LANG_GLDIST_INVOICE",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'VI'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_VOIDINVOICE,loFormSet.GetHeaderText("LANG_GLDIST_VOIDINVOICE",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'CR'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_CASHR,loFormSet.GetHeaderText("LANG_GLDIST_CASHR",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'CA'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_CREDITADJ,loFormSet.GetHeaderText("LANG_GLDIST_CREDITADJ",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'DA'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_DEBITADJ,loFormSet.GetHeaderText("LANG_GLDIST_DEBITADJ",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'RM'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_RM,loFormSet.GetHeaderText("LANG_GLDIST_RM",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'VR'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_VOIDRM,loFormSet.GetHeaderText("LANG_GLDIST_VOIDRM",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'IP'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_INVPHY,loFormSet.GetHeaderText("LANG_GLDIST_INVPHY",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'IA'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_INVADJ,loFormSet.GetHeaderText("LANG_GLDIST_INVADJ",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'MP'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_MATINVPHY,loFormSet.GetHeaderText("LANG_GLDIST_MATINVPHY",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'MA'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_MATINVDJ,loFormSet.GetHeaderText("LANG_GLDIST_MATINVDJ",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'PO'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_POREC,loFormSet.GetHeaderText("LANG_GLDIST_POREC",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'MO'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_MAPOREC,loFormSet.GetHeaderText("LANG_GLDIST_MAPOREC",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'CT'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_CTREC,loFormSet.GetHeaderText("LANG_GLDIST_CTREC",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'ZE'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_ZEROOUT,loFormSet.GetHeaderText("LANG_GLDIST_ZEROOUT",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'NL'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_NONMATLIB,loFormSet.GetHeaderText("LANG_GLDIST_NONMATLIB",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'JC'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_POJOBCLOSE,loFormSet.GetHeaderText("LANG_GLDIST_POJOBCLOSE",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'RO'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_MATOPREC,loFormSet.GetHeaderText("LANG_GLDIST_MATOPREC",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'RS'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_STYOPREC,loFormSet.GetHeaderText("LANG_GLDIST_STYOPREC",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'MM'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_RECMFGORDER,loFormSet.GetHeaderText("LANG_GLDIST_RECMFGORDER",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'EX'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_EXRATEDIFF,loFormSet.GetHeaderText("LANG_GLDIST_EXRATEDIFF",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'KO'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_KEYOFF,loFormSet.GetHeaderText("LANG_GLDIST_KEYOFF",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'MC'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_MAJOBCLOSE,loFormSet.GetHeaderText("LANG_GLDIST_MAJOBCLOSE",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'JP'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_CTJOBCLOSE,loFormSet.GetHeaderText("LANG_GLDIST_CTJOBCLOSE",loFormSet.HeaderAlias))
    CASE _TRANTYP = 'LK'
      XTRANDESC = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_GLDIST_INVLOCK,loFormSet.GetHeaderText("LANG_GLDIST_INVLOCK",loFormSet.HeaderAlias))
  ENDCASE
  RETURN XTRANDESC
  ************************************************************
  *! Name      : lpCreatTmp
...
FUNCTION lpCreatTmp
  ...
  lnFileStru = lnFileStru + 1
  laFileStru[lnFileStru ,1] = 'nGenglamnt'
  laFileStru[lnFileStru ,2] = 'N'
  ...
  DIMENSION laTmpStru[4,4]
  laTmpStru[1,1] = 'GLBATCH'
  laTmpStru[1,2] = 'C'
  laTmpStru[1,3] = 6
  laTmpStru[1,4] = 0

  laTmpStru[2,1] = 'CTRNSLEDN'
  laTmpStru[2,2] = 'C'
  laTmpStru[2,3] = 8
  laTmpStru[2,4] = 0

  laTmpStru[3,1] = 'TRAN_NO'
  laTmpStru[3,2] = 'C'
  laTmpStru[3,3] = 6
  laTmpStru[3,4] = 0

  laTmpStru[4,1] = 'TRAN_TYPE'
  laTmpStru[4,2] = 'C'
  laTmpStru[4,3] = 2
  laTmpStru[4,4] = 0

  =gfCrtTmp(lcGLBtTrn,@laTmpStru,'GLBATCH+CTRNSLEDN+TRAN_NO+TRAN_TYPE',lcGLBtTrn)
...
FUNCTION lpGLDist
  ...
  IF EMPTY(ALLTRIM(lcGLAcnt))
    RETURN
  ENDIF
  PRIVATE llEqvAmnt,lcSorcFile,lcCatgKey,lcTranType,lcTranNo,lnAmount,ldTranDate,;
    lcGLAcnt,lcGlPrd,lcGlYr,lcCurrCode,lnCurrUnit,lnExRate,lcAmtFld

  PRIVATE lnCurAlias
  IF lnAmount <> 0
    lnCurAlias = SELECT(0)
    lcTranDesc = ''
    IF ASCAN(laTran,lcTranType) > 0
    ...
    *-- and there is no need to Calculate it.
    ...
    IF SEEK(lcSeekExp,lcGenDist)
      ...
      lnAmount = lnAmount + &lcAmtFld
      lcCatgDesc = &lcGenDist..CATG_DESC
      SELECT (lcGenDist)
    ELSE
      *In Case of Cash receipt that has been reversed in another period other than the period entred in the option grid
      IF lcTranType = 'CR' AND UPPER(lcSorcFile) = "OLD"
        lnOldSel =SELECT()
        SELECT GLDist
        lnCurReco = RECNO()
        lcOldIndex = ORDER()
        *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
        *!*	      Set Order To GLDISTNO   && TRAN_NO+TRAN_TYPE+GLSESSION+CATG_KEY
        *!*	      =Seek(lcTranNo+lcTranType)
        =gfSetOrder('GLDISTNO')   && TRAN_NO+TRAN_TYPE+GLSESSION+CATG_KEY
        =gfSEEK(lcTranNo+lcTranType)
        *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
        SUM nEqvAmnt TO lnEqAcAmt REST WHILE TRAN_NO+Tran_Type+GLSESSION+Catg_Key  = lcTranNo+lcTranType FOR GLAccount =lcGLAcnt AND Catg_Key = lcCatgKey
        IF !EMPTY(lcOldIndex)
          SET ORDER TO (lcOldIndex)
        ELSE
          SET ORDER TO
        ENDIF
        IF BETWEEN(lnCurReco,1,RECCOUNT())
          GO RECORD lnCurReco
        ENDIF
        SELECT(lnOldSel)
        IF lnEqAcAmt = 0
      ...
      ****************************************
      IF !USED('SyGlCatg')
        lnSyResult = oAriaApplication.RemoteSystemData.Execute("SELECT * FROM SyGlCatg", '', 'SyGlCatg', "", oAriaApplication.SystemConnectionString, 3, "", SET("Datasession"))
        IF lnSyResult <> 1
          RETURN .F.
        ENDIF
      ENDIF
    ...
    REPLACE &lcAmtFld WITH lnAmount
    ...
    *- G/L Entry Reconciliation includes Inventory entries even when system flag is set to No [Start]
    IF gfgetmemvar('M_GL_COST',lcComp_Id) ='N'
  ...
  ENDIF
  ************************************************************
  *! Name      : lfGetAccountType
  *! Developer : MMT - Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Get Account-Transaction trype
  ************************************************************
FUNCTION lfGetAccountType
  LPARAMETERS _CATGKEY,_AMOUNT
  LCACNTTYPE = ''
  DO CASE
...
FUNCTION lfGetTranType
  ...
  *! Developer : MMT - Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Update GL Entries
  ************************************************************
FUNCTION lfUpdItm26

  IF EMPTY(lcGenDist) OR (!USED(lcGenDist))
    lfVryItm26()
  ENDIF


  lcDiffExp = [LIKE("*Y",EVAL(KEY()))]
  SELECT (lcGenDist)
  lcRISessn = ''
  LOCATE FOR &lcDiffExp. AND (nGenEqvAmt-nEqvAmnt <> 0) AND (nEqvAmnt = 0 OR (nGenEqvAmt <> 0 AND nEqvAmnt<>0))
  IF FOUND()
    lcRISessn = gfSequence('GLSESSION')
  ENDIF
  llGLDISTAdded = .F.
  SELECT (lcGenDist)
  COUNT FOR &lcDiffExp. TO lnGLDistTot
  =lfInitThermo(lnGLDistTot, IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRIESFILEUPDATE,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRIESFILEUPDATE",loFormSet.HeaderAlias)))
  lnGLDistCnt = 0
  SELECT (lcGenDist)
  SCAN FOR &lcDiffExp.
    lnGLDistCnt = lnGLDistCnt + 1
    oProgress.CurrentProgress(lnGLDistCnt)
    IF nGenEqvAmt-nEqvAmnt <> 0
      IF EMPTY(&lcGenDist..GLFYear) AND EMPTY(&lcGenDist..nGenFYear)
        *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
        IF llViewLog
          *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
          DECLARE laRebMsg[3]
          laRebMsg[1] = " "
          *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
          *!*	        laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRYACCOUNT ,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRYACCOUNT",loFormSet.HeaderAlias))+;
          *!*	                      &lcGenDist..GLACCOUNT+;
          *!*	                      IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANSACTION ,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANSACTION",loFormSet.HeaderAlias))+;
          *!*	                      &lcGenDist..Tran_No+;
          *!*	                      IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLINVALIDPERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_GLINVALIDPERIOD",loFormSet.HeaderAlias))
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
            lcCurrComp_ID + ": " +;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRYACCOUNT ,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRYACCOUNT",loFormSet.HeaderAlias))+;
            &lcGenDist..GLAccount+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANSACTION ,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANSACTION",loFormSet.HeaderAlias))+;
            &lcGenDist..TRAN_NO+;
            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLINVALIDPERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_GLINVALIDPERIOD",loFormSet.HeaderAlias))
      ...
      ENDIF
      llValidPeriod = .F.
      IF SEEK(IIF(!EMPTY(&lcGenDist..GLFYear),&lcGenDist..GLFYear+&lcGenDist..GLPeriod,&lcGenDist..nGenFYear+&lcGenDist..nGenPeriod),'FSPRD_G','COMFYRPRDI')
        IF FSPRD_G.lfsplocks
          *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
          IF llViewLog
            *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
            DECLARE laRebMsg[3]
            ...
            *!*	                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANSACTION ,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANSACTION ",loFormSet.HeaderAlias))+;
            *!*	                        &lcGenDist..Tran_No+;
            *!*	                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLLOCKEDPERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_GLLOCKEDPERIOD",loFormSet.HeaderAlias))+;
            *!*	                        IIF(!EMPTY(&lcGenDist..glPeriod),&lcGenDist..glPeriod,&lcGenDist..nGenPeriod)+" - "+IIF(!EMPTY(&lcGenDist..glFYear),&lcGenDist..glFYear,&lcGenDist..nGenFYear)
            laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
              lcCurrComp_ID + ": " +;
              IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRYACCOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRYACCOUNT",loFormSet.HeaderAlias))+;
              &lcGenDist..GLAccount+;
      ...
      DO CASE
        CASE  nGenEqvAmt = 0 && The GLDIST record should not be there and will be deleted
          ...
          *IF SEEK(&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type,'GLDIST_G','GLDISTNO')
          =gfSetOrder('GLDISTNO')
          IF gfSEEK(&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type,'GLDIST_G','GLDISTNO')
          *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
            SELECT GLDIST_G
            SCAN REST WHILE TRAN_NO+Tran_Type+GLSESSION+Catg_Key =&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type FOR Catg_Key =&lcGenDist..Catg_Key AND GLAccount =&lcGenDist..GLAccount
              *IF FOUND()
              IF Posted =' ' && Not released record
                *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
*!*	                REPLACE cEdit_User WITH 'SMREBAL'
*!*	                DELETE
                =gfREPLACE("cEdit_User WITH 'SMREBAL'")
                =gfDELETE()
				*E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]              
                = gfTableUpdate()
                llGLDISTAdded = .T.
          ...
          ENDIF
          IF !llNotReleased && Release GL record
            IF SEEK(&lcGenDist..GLBATCH,'GLBATCH_G','BATCHNO')
              IF GLBATCH_G.cBatStat <> 'P' && Un-Posted batch -- Can be unreleased
                REPLACE cBatStat WITH 'V' IN 'GLBATCH_G'
                SELECT GLBATCH_G
                = gfTableUpdate()
                SELECT  GLDIST_G
                *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Satrt]
                *!*	        Set Order To GLDISTAC   && GLACCOUNT
                =gfSetOrder('GLDISTAC')
                *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]

                SELECT GLTRNSHD_G
                IF SEEK(&lcGenDist..GLBATCH)
                  SCAN REST WHILE cBatchNo+cTranNo = &lcGenDist..GLBATCH
                    REPLACE cTrnStat WITH 'V'
                    = gfTableUpdate()
                  ENDSCAN
                ...
                *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
                SELECT (lcGLBtTrn)
                IF SEEK(&lcGenDist..GLBATCH)
                  SCAN REST WHILE GLBATCH+CTRNSLEDN+TRAN_NO+Tran_Type = &lcGenDist..GLBATCH
                    SELECT  GLDIST_G
                    *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Satrt]

                    =gfSEEK(&lcGLBtTrn..TRAN_NO+&lcGLBtTrn..Tran_Type)
                    *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[end]

                    SCAN REST WHILE TRAN_NO+Tran_Type+GLSESSION+Catg_Key=&lcGLBtTrn..TRAN_NO+&lcGLBtTrn..Tran_Type  FOR GLBATCH = &lcGLBtTrn..GLBATCH AND CTRNSLEDN =&lcGLBtTrn..CTRNSLEDN
                      REPLACE GLBATCH    WITH  '',;
                        CTRNSLEDN  WITH  '',;
                        Posted     WITH   '',;
                        cEdit_User WITH  'SMREBAL'
                      *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
                      =gfReplace('')
                      *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]  
                      = gfTableUpdate()
                ...
                *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
                IF llViewLog
                  *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
                  ...
                  laRebMsg[1] = " "
                  *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
                  *!*	                laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHNUMBER,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHNUMBER",loFormSet.HeaderAlias))+;
                  *!*	                              &lcGenDist..GLBatch+;
                  *!*	                              IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHUNRELEASED,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHUNRELEASED",loFormSet.HeaderAlias))
                  laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
                    lcCurrComp_ID + ": " +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHNUMBER,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHNUMBER",loFormSet.HeaderAlias))+;
                    &lcGenDist..GLBATCH+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHUNRELEASED,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHUNRELEASED",loFormSet.HeaderAlias))
                  *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
                  laRebMsg[3] = " "
              ...
              ELSE
                *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
                IF llViewLog
                  ...
                  *!*	                            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLTRANSACTION,loFormSet.GetHeaderText("LANG_SMREBAL_GLTRANSACTION",loFormSet.HeaderAlias)) +;
                  *!*	                            &lcGenDist..Tran_No+;
                  *!*	                            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLCANNOTUPDATE,loFormSet.GetHeaderText("LANG_SMREBAL_GLCANNOTUPDATE",loFormSet.HeaderAlias)) +;
                  *!*	                            &lcGenDist..GLBatch+;
                  *!*	                            IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLISPOSTED,loFormSet.GetHeaderText("LANG_SMREBAL_GLISPOSTED",loFormSet.HeaderAlias))
                  laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
                    lcCurrComp_ID + ": " +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRYACCOUNT ,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRYACCOUNT",loFormSet.HeaderAlias)) +;
                    &lcGenDist..GLAccount+;
        ...
        CASE nEqvAmnt = 0 && The GLDIST record is missing and Should be added
          SELECT (lcGenDist)
          SCATTER MEMO MEMVAR
          m.nEqvAmnt = m.nGenEqvAmt
          m.GLFYear = m.nGenFYear
          m.GLPeriod = m.nGenPeriod
          m.nglamount = m.nGenglamnt
          m.GLSESSION = lcRISessn
          m.glacnttype = lfGetAccountType(m.Catg_Key,m.nglamount)
          m.Posted =''
          m.CTRNSLEDN =''
          m.GLBATCH = ''
          m.cAdd_User = 'SMREBAL'
          ...
          m.CADD_TIME = TIME()
          INSERT INTO GLDIST_G FROM MEMVAR
          SELECT GLDIST_G
          *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
          =gfReplace('')
          *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
          ...
          llGLDISTAdded = .T.
        OTHERWISE && There is a difference in Amounts between GLDIST and generated record, the difference should be added to GLDIST
          lnDiffAmt = &lcGenDist..nEqvAmnt - &lcGenDist..nGenEqvAmt
          SELECT  GLDIST_G
          *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
          *!*	  Set Order To GLDISTNO
          =gfSetOrder('GLDISTNO')
          ...
          *!*	      If Seek(&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type,'GLDIST_G','GLDISTNO')
          IF gfSEEK(&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type,'GLDIST_G','GLDISTNO')
            ...
            *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[end]
            ...
            SELECT GLDIST_G
            LOCATE REST WHILE TRAN_NO+Tran_Type+GLSESSION+Catg_Key =&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type FOR Catg_Key =&lcGenDist..Catg_Key AND GLAccount =&lcGenDist..GLAccount
            IF FOUND()
              IF Posted =' ' && Not release GLDIST record, can be updated with the correct amount.
                REPLACE nglamount  WITH &lcGenDist..nGenglamnt,;
                  nEqvAmnt   WITH &lcGenDist..nGenEqvAmt,;
                  cEdit_User WITH 'SMREBAL'
                *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[Start]
                =gfReplace('')
                *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]  
                = gfTableUpdate()
                llGLDISTAdded = .T.
              ELSE  && Release Entry, check if the related batch is posted or not
                IF SEEK(GLDIST_G.GLBATCH,'GLBATCH_G','BATCHNO')
                  **********************************
                  IF GLBATCH_G.cBatStat <> 'P' && Un-Posted batch -- Can be unreleased
                    REPLACE cBatStat WITH 'V' IN 'GLBATCH_G'
                    SELECT  'GLBATCH_G'
                    ...
                    *E303964,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[end]
                    ...
                    IF SEEK(&lcGenDist..GLBATCH)
                      SCAN REST WHILE GLBATCH+CTRNSLEDN+TRAN_NO+Tran_Type = &lcGenDist..GLBATCH
                        ...
                        *!*	                    =Seek(&lcGLBtTrn..TRAN_NO+&lcGLBtTrn..Tran_Type)
                        =gfSEEK(&lcGLBtTrn..TRAN_NO+&lcGLBtTrn..Tran_Type)
                    ...
                    *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
                    IF llViewLog
                      ...
                      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
                      *!*	                  laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHNUMBER,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHNUMBER",loFormSet.HeaderAlias))+;
                      *!*	                                  &lcGenDist..GLBatch+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHUNRELEASED,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHUNRELEASED",loFormSet.HeaderAlias))
                      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
                        lcCurrComp_ID + ": " +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHNUMBER,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHNUMBER",loFormSet.HeaderAlias))+;
                        &lcGenDist..GLBATCH+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLBATCHUNRELEASED,loFormSet.GetHeaderText("LANG_SMREBAL_GLBATCHUNRELEASED",loFormSet.HeaderAlias))
                    ...
                    *!*	                If Seek(&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type,'GLDIST_G','GLDISTNO')
                    ...
                    IF gfSEEK(&lcGenDist..TRAN_NO+&lcGenDist..Tran_Type,'GLDIST_G','GLDISTNO')
                      ...
                      *ENDIF
                    ENDIF
                  ELSE && if the related GL Batch is Posted, New GLDIST record will be added with the difference
                    SELECT (lcGenDist)
                    SCATTER MEMO MEMVAR
                    m.nEqvAmnt = m.nGenEqvAmt - m.nEqvAmnt
                    m.GLFYear = m.nGenFYear
                    m.GLPeriod = m.nGenPeriod
                    m.nglamount = m.nGenglamnt - m.nglamount
                    m.GLSESSION = lcRISessn
                    m.glacnttype = lfGetAccountType(m.Catg_Key,m.nglamount)
                    m.Posted =''
                    m.CTRNSLEDN =''
  ...
  IF llViewLog
    *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
    IF llGLDISTAdded
      ...
      laRebMsg[1] = " "
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][Start]
      *laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRYUPDATED,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRYUPDATED",loFormSet.HeaderAlias))
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN", LANG_SMREBAL_COMP     ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP"    , loFormSet.HeaderAlias))  +;
        lcCurrComp_ID + ": " +;
        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLENTRYUPDATED,loFormSet.GetHeaderText("LANG_SMREBAL_GLENTRYUPDATED",loFormSet.HeaderAlias))
      *! E303873,2 MMT 10/31/2017 Fix issue of displaying log even the option of show log is NO[P20170905.0001][End]
      laRebMsg[3] = " "
      IF llAutoBal
        laRebMsg[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ENDPROC,loFormSet.GetHeaderText("LANG_SMREBAL_ENDPROC",loFormSet.HeaderAlias)) +DTOC(DATE())+SPACE(5)+TIME()
...
FUNCTION lfHistory
  ...
  SELECT HISTORY,cArGlAcc,SUM(IIF(TranType$'89',Amount,-Amount)) AS Sum_Amt ;
    FROM ArHist ;
    INTO DBF (oAriaApplication.WorkDir+lcHistory) ;
    GROUP BY HISTORY,cArGlAcc;
    HAVING Sum_Amt <> 0 ;
    ORDER BY HISTORY
  INDEX ON HISTORY+cArGlAcc TAG (lcHistory) OF (oAriaApplication.WorkDir+lcHistory+'.CDX')
  ************************************************************
  *! Name      : lfVldItm26
  *! Developer : MMT - Mariam Mazhar
  *! Date      : 09/28/2017
  *! Purpose   : Validate Item#26
  ************************************************************
FUNCTION lfVldItm26
  *! E303873,1 MMT 09/28/2017 Adding GL Module to Rebalance program[P20170905.0001][End]
  ...
  *! Developer : MMT - Mariam Mazhar
  *! Date      : 10/31/2017
  *! Purpose   : Refresh Modules option
  ************************************************************
FUNCTION RefreshModule
  LOCAL lcStatusStr, lnTarget
  lcStatusStr = ""
  IF !EMPTY(laRpTarMod)
    FOR lnTarget = 1 TO ALEN(laRpTarMod,1)
      lcStatusStr = lcStatusStr + ", " + laRpTarMod[lnTarget]
    ENDFOR
    lcStatusStr = SUBSTR(lcStatusStr,3)
  ENDIF
  RETURN lcStatusStr
  ...
  *! Developer : MMT - Mariam Mazhar
  *! Date      : 10/31/2017
  *! Purpose   : Refresh Companies option
  ************************************************************
...
ENDIF
lcCurrArea = SELECT(0)
IF FILE(lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)+'.DBF') 
  USE (lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)) IN 0 EXCL
  SELECT ALLTRIM(&lcRebalHdr..cTempName)
  SET ORDER TO TAG (&lcRebalHdr..cTempName)
ELSE
  lcTag = &lcRebalHdr..cTempName
  SELECT ArCusHst 
  =AFIELDS(laFields)
  CREATE TABLE (lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)) FROM ARRAY laFields  
ENDIF

IF FILE(lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)+'.CDX')               
  SET ORDER TO TAG ALLTRIM(&lcRebalHdr..cTempName)
ELSE
  INDEX ON Account+cFisFYear TAG &lcTag OF (lcFilePath +ALLTRIM(&lcRebalHdr..cTempName)+'.CDX')
ENDIF
SELECT (lcCurrArea)
*-- End of lfStpCsHst.
...
IF FILE(oAriaApplication.WorkDir + lcSelAccount+ '.DBF') AND !USED(lcSelAccount)
  USE (oAriaApplication.WorkDir + lcSelAccount+ '.DBF') IN 0 ALIAS (lcSelAccount)
ENDIF
lcFilter = ''
IF USED(lcSelAccount) AND RECCOUNT(lcSelAccount) > 0 
  SELECT(lcSelAccount)
  SCAN
    lcFilter = lcFilter +  IIF(EMPTY(lcFilter),'',' OR ') + " Account = '"+&lcSelAccount..Account +"'" 
  ENDSCAN
...
IF FILE(oAriaApplication.WorkDir + lcSelYear+ '.DBF') AND !USED(lcSelYear)
  USE (oAriaApplication.WorkDir + lcSelYear+ '.DBF') IN 0 ALIAS (lcSelYear)
ENDIF
lcDateExpr = ''
IF USED(lcSelYear) AND RECCOUNT(lcSelYear) > 0 
  SELECT(lcSelYear)
  lcDateExpr = [STR(YEAR(dPostDate),4) $ "]
  llFirstY = .T.
  SCAN
    lcDateExpr = lcDateExpr + IIF(!llFirstY,"|",'')+"'"+&lcSelYear..cFisfYear+"'" 
    llFirstY = .F.
  ENDSCAN
  lcDateExpr = lcDateExpr + '"'
ENDIF
IF EMPTY(lcDateExpr )
  lcDateExpr = '.T.'
ENDIF  
...
SCAN FOR &lcExpr
  IF &lcDateExpr AND Status <> 'V'
    WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_INV,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_INV",loFormSet.HeaderAlias)) + Invoice NOWAIT
    STORE '' TO lcGlYear,lcGlPeriod,lcUntSin
    STORE 0 TO lnCOGSAmt,lnShipAmnt,lnDiscAmnt
    ...
    IF EMPTY(lcGLYear) OR EMPTY(lcGLPeriod)
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+;
                    lcCurrComp_ID + ": "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_INVNO ,loFormSet.GetHeaderText("LANG_SMREBAL_INVNO",loFormSet.HeaderAlias))+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_TRAN_DATE,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_TRAN_DATE",loFormSet.HeaderAlias)) +;
                    DTOC(InvHdr.dPostDate) +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FALLIN_ANY_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_FALLIN_ANY_PERIOD",loFormSet.HeaderAlias))
      laRebMsg[3] = " "
      =lfVryRport()
    ELSE
      lcGlPeriod = PADL(ALLTRIM(lcGlPeriod),2,"0")
...
SCAN FOR &lcExpr
  IF &lcDateExpr AND !(Status $ 'BX') AND !Direct_Inv
    WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_ORD,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_ORD",loFormSet.HeaderAlias)) + Order NOWAIT
    ...
    IF EMPTY(lcGLYear) OR EMPTY(lcGLPeriod)
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))  +;
                    lcCurrComp_ID + ": "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ORDNO,loFormSet.GetHeaderText("LANG_SMREBAL_ORDNO",loFormSet.HeaderAlias)) + ;
                    OrdHdr.Order +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_TRAN_DATE,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_TRAN_DATE",loFormSet.HeaderAlias)) +;
                    DTOC(OrdHdr.Entered) +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FALLIN_ANY_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_FALLIN_ANY_PERIOD",loFormSet.HeaderAlias)) 
...
SCAN FOR &lcExpr
  IF &lcDateExpr
    WAIT WINDOW LANG_SMREBAL_CUSTOMER_HISTORY_CR + CrMemo NOWAIT
    ...
    IF EMPTY(lcGLYear) OR EMPTY(lcGLPeriod)
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))   + ;
                    lcCurrComp_ID +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_RETURN ,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_RETURN",loFormSet.HeaderAlias))+;
                    RetHdr.CrMemo +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_TRAN_DATE ,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_TRAN_DATE",loFormSet.HeaderAlias))+;
                    DTOC(RetHdr.CrDate) +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FALLIN_ANY_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_FALLIN_ANY_PERIOD",loFormSet.HeaderAlias)) 
...
IF USED(lcSelYear) AND RECCOUNT(lcSelYear) > 0 
  SELECT(lcSelYear)
  lcDateExpr = [STR(YEAR(TranDate),4) $ "]
  llFirstY = .T.
  SCAN
    lcDateExpr = lcDateExpr + IIF(!llFirstY,"|",'')+"'"+&lcSelYear..cFisfYear+"'" 
    llFirstY = .F.
...
SCAN FOR &lcExpr
  IF &lcDateExpr AND TranType <> '1'
    WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_DEBIT,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_DEBIT",loFormSet.HeaderAlias)) + Tran NOWAIT
    ...
    IF EMPTY(lcGLYear) OR EMPTY(lcGLPeriod)
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))    +;
                    lcCurrComp_ID + ": "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_DEBITTR ,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_DEBITTR",loFormSet.HeaderAlias))+;
                    Debit.Tran + ;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_TRAN_DATE,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_TRAN_DATE",loFormSet.HeaderAlias))+ ;
                    DTOC(Debit.TranDate) +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FALLIN_ANY_PERIOD ,loFormSet.GetHeaderText("LANG_SMREBAL_FALLIN_ANY_PERIOD",loFormSet.HeaderAlias))
...
SCAN FOR &lcExpr
  IF &lcDateExpr AND TranType <> '0'
    WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_CREDIT,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_CREDIT",loFormSet.HeaderAlias)) + Tran NOWAIT
    ...
    IF EMPTY(lcGLYear) OR EMPTY(lcGLPeriod)
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))     +;
                    lcCurrComp_ID + ": "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_CREDITTR,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_CREDITTR",loFormSet.HeaderAlias)) +;
                    Credit.Tran +;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_TRAN_DATE,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_TRAN_DATE",loFormSet.HeaderAlias))  + ;
                    DTOC(Credit.TranDate) + ;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FALLIN_ANY_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_FALLIN_ANY_PERIOD",loFormSet.HeaderAlias)) 
      laRebMsg[3] = " "
      =lfVryRport()
    ELSE
...
SCAN FOR &lcExpr
  IF &lcDateExpr AND !(TranType $ '89')
    WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_ARHIST,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_ARHIST",loFormSet.HeaderAlias))+ Tran NOWAIT
    ...
    IF EMPTY(lcGLYear) OR EMPTY(lcGLPeriod)
      ...
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))    +;
                    lcCurrComp_ID + ": "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_HISTORY_HISTTRAN,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_HISTORY_HISTTRAN",loFormSet.HeaderAlias)) +;
                    ArHist.Tran + ;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_TRAN_DATE,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_TRAN_DATE",loFormSet.HeaderAlias)) +;
                    DTOC(Arhist.TranDate) + ;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_FALLIN_ANY_PERIOD ,loFormSet.GetHeaderText("LANG_SMREBAL_FALLIN_ANY_PERIOD",loFormSet.HeaderAlias))
      laRebMsg[3] = " "
      =lfVryRport()
    ELSE
...
IF .T.&&!llUpdt
  ...
  DECLARE laRebMsg[3]
  SCAN
    WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CUSTOMER_REPORT,loFormSet.GetHeaderText("LANG_SMREBAL_CUSTOMER_REPORT",loFormSet.HeaderAlias)) +;
                Account +", "+;
                IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                " : " + cFisFYear NOWAIT
    IF SEEK(Account+cFisFYear,'ArCusHst')
      FOR lnLoop = 1 TO 13
        ...
        *-- Checking Order Amount.
        IF nOrdAmt&lcLoop <> ArCusHst.nOrdAmt&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ORDER_AMOUNT_ACCOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_ORDER_AMOUNT_ACCOUNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD ,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias))+;
                        lcLoop + ;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_AMNT_DIFF,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_AMNT_DIFF",loFormSet.HeaderAlias))
          laRebMsg[3] = " "
          =lfVryRport()
        ENDIF
        ...
        *-- Checking Order Qty.
        IF nOrdQty&lcLoop <> ArCusHst.nOrdQty&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) +;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ORDER_QTY_ACCOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_ORDER_QTY_ACCOUNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias))+;
                        lcLoop +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_QTY_DIFF,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_QTY_DIFF",loFormSet.HeaderAlias))
          laRebMsg[3] = " "
          =lfVryRport()
        ENDIF
        ...
        *-- Checking Sales Amount.
        IF nSlsAmt&lcLoop <> ArCusHst.nSlsAmt&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+ ;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_SALES_AMOUNT_ACCOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_SALES_AMOUNT_ACCOUNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD ,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias))+ ;
        ...
        *-- Checking Sales Qty.
        IF nSlsQty&lcLoop <> ArCusHst.nSlsQty&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_SALES_QTY_AMOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_SALES_QTY_AMOUNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) + ;
        ...
        *-- Checking Discount Amount.
        IF nDisAmt&lcLoop <> ArCusHst.nDisAmt&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) +;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_DISCOUNT_AMOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_DISCOUNT_AMOUNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) +;
        ...
        *-- Checking Cost of goods sold Amount.
        IF nCogsAmt&lcLoop <> ArCusHst.nCogsAmt&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) +;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COST_OF_GOODS,loFormSet.GetHeaderText("LANG_SMREBAL_COST_OF_GOODS",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) + ;
        ...
        *-- Checking Return Amount.
        IF nRetAmt&lcLoop <> ArCusHst.nRetAmt&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))     +;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_RETURNED_AMOUNT,loFormSet.GetHeaderText("LANG_SMREBAL_RETURNED_AMOUNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) + ;
        ...
        *-- Checking Return Amount.
        IF nRetQty&lcLoop <> ArCusHst.nRetQty&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+ ;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_RETURNED_QTY,loFormSet.GetHeaderText("LANG_SMREBAL_RETURNED_QTY",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear + ;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) +;
        ...
        *-- Checking Allowance.
        IF nAllow&lcLoop <> ArCusHst.nAllow&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+ ;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_ALLOWANCE,loFormSet.GetHeaderText("LANG_SMREBAL_ALLOWANCE",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) + ;
        ...
        *-- Checking Payment.
        IF nPayment&lcLoop <> ArCusHst.nPayment&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP ,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))+ ;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_PAY_ACCNT,loFormSet.GetHeaderText("LANG_SMREBAL_PAY_ACCNT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear + ;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias))+;
        ...
        *-- Checking Debit Adjustments.
        IF nDrAdj&lcLoop <> ArCusHst.nDrAdj&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) + ;
                        lcCurrComp_ID +  ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_DEBIT_AMT_ACT,loFormSet.GetHeaderText("LANG_SMREBAL_DEBIT_AMT_ACT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear + ;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) +;
        ...
        *-- Checking Credit Adjustments.
        IF nCrAdj&lcLoop <> ArCusHst.nCrAdj&lcLoop
          laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias))  +;
                        lcCurrComp_ID + ": "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_CREDIT_AMT_ACT,loFormSet.GetHeaderText("LANG_SMREBAL_CREDIT_AMT_ACT",loFormSet.HeaderAlias))+;
                        " : " + Account + ", "+;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLYEAR,loFormSet.GetHeaderText("LANG_SMREBAL_GLYEAR",loFormSet.HeaderAlias))+;
                        " : " + cFisFYear +;
                        IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_AND_PERIOD,loFormSet.GetHeaderText("LANG_SMREBAL_AND_PERIOD",loFormSet.HeaderAlias)) + ;
    ...
    ELSE
      laRebMsg[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_COMP,loFormSet.GetHeaderText("LANG_SMREBAL_COMP",loFormSet.HeaderAlias)) +;
                    lcCurrComp_ID + ": "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_HAS_NO_RECORD_ACT ,loFormSet.GetHeaderText("LANG_SMREBAL_HAS_NO_RECORD_ACT",loFormSet.HeaderAlias))+" "+;
                    Account + ", "+;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_GLFISCALYEARBRWTITLE,loFormSet.GetHeaderText("LANG_SMREBAL_GLFISCALYEARBRWTITLE",loFormSet.HeaderAlias))+;
                    " " + cFisFYear + ;
                    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SMREBAL_IN_CUSTOMER_HIST,loFormSet.GetHeaderText("LANG_SMREBAL_IN_CUSTOMER_HIST",loFormSet.HeaderAlias))
      laRebMsg[3] = " "
      =lfVryRport()        
    ENDIF
  ENDSCAN
...
IF FILE(lcFilePath + ALLTRIM(&lcRebalHdr..cTempName) + '.DBF')
  IF !USED(ALLTRIM(&lcRebalHdr..cTempName))
    USE (lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)) IN 0 
  ENDIF
  IF !USED('ArCusHst_UP')
    gfOpenTable('ArCusHst','ACTHST','SH','ArCusHst_UP')
  ENDIF
  SELECT (ALLTRIM(&lcRebalHdr..cTempName))
  lcFile = ALLTRIM(&lcRebalHdr..cTempName)
  SCAN
    SCATTER MEMVAR MEMO
    SELECT ArCusHst_UP
    IF !gfSEEK(&lcFile..Account+&lcFile..cFisFYear,'ArCusHst_UP')
      APPEND BLANK
  ...
  SELECT ALLTRIM(&lcRebalHdr..cTempName)
  USE
  ERASE(lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)+'.DBF')
  ERASE(lcFilePath + ALLTRIM(&lcRebalHdr..cTempName)+'.CDX')  
ELSE
  llUpdt = .T.
  =lfStpCsHst()
  =lfVryCsHst()
