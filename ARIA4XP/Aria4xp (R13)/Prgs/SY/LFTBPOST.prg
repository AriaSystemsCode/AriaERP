...
**********************************************************************************************
*! Modifications
*! B610229,1 MMT 02/07/2013 Error while posting batch [T20130207.0003]
*! B610233,1 HIA, 02/10/2013 Aria4xp - GL - Posting Journal BATCH - ERROR ON posting specific BATCH [T20130207.0003]
*B610277,1 TMI 03/21/2013 fix a problem that Single Transaction won't post [T20130312.0020]
*C201563,E303372,1 TMI 03/22/2013 call the trigger lfGLPTRNDT that updates the CINVNO,CVENDCODE in the GLPTRNDT file [T20130301.0001] 
*B610300,1 TMI 04/15/2013 Hide the progress bar when showing the messagebox [T20130322.0008]
*N000682,1 TMI 12/12/2012 TMI Globlization changes[Start] Define the language related
*B610484,1 TMI 08/27/2013 [Start] put the correct variable value for gcAct_Appl( = oAriaApplication.User_ID)
*E303873,1 MMT 09/26/2017 Adding GL Module to Rebalance program[P20170905.0001]
*B611970,1 MMT 11/18/2019 Total Debit <> Total Credit while comparing while its equal in table (Rounding issue)[T20191111.0005]
**********************************************************************************************
...
*! B610233,1 HIA, 02/10/2013 Aria4xp - GL - Posting Journal BATCH - ERROR ON posting specific BATCH [T20130207.0003][Start]
lcSJ_DEF = loFormSet.lcSJ_DEF
*! B610233,1 HIA, 02/10/2013 Aria4xp - GL - Posting Journal BATCH - ERROR ON posting specific BATCH [T20130207.0003][End]

gcWorkDir = oAriaApplication.WorkDir
gcUser_ID = oAriaApplication.User_ID
*B610484,1 TMI 08/27/2013 [Start] put the correct variable
  ...
  IF !SEEK(lcRetActMj,'GLACCHAR')
    ...
    *Don't display message on line and it to the log file if called from rebalance.
    IF llFromReb
      ...
      =lfVryRport()
    ELSE
     *B610300,1 TMI 04/15/2013 [Start]      
     =IIF(TYPE('oProgress')='O',lfHideProgress() , '' )
     *B610300,1 TMI 04/15/2013 [End  ]     
    ...
    CASE lcTrOrBat = 'B' .AND. !llOneRec
      ...
      *** are valid 
      IF llContinue .AND. lcBatType = 'B'
        ...
        *B611970,1 MMT 11/18/2019 Total Debit <> Total Credit while comparing while its equal in table (Rounding issue)[T20191111.0005][Start]
*!*	        IF lnTotalDr <> lnTotalCr .OR. ;
*!*	           (lnTotalDr = 0 .AND. lnTotalCr = 0)
        IF ROUND(lnTotalDr,2) <> ROUND(lnTotalCr,2) .OR. ;
           (lnTotalDr = 0 .AND. lnTotalCr = 0)
        *B611970,1 MMT 11/18/2019 Total Debit <> Total Credit while comparing while its equal in table (Rounding issue)[T20191111.0005][End]   
  ...
  *B610229,1 MMT 02/07/2013 Error while posting batch [T20130207.0003][Start]
  *oProgress = NULL
  *B610229,1 MMT 02/07/2013 Error while posting batch [T20130207.0003][End]
  ...
  *B610277,1 TMI 03/21/2013 [Start] release the oProgress before the screen calling as this caused that the screen releases before it starts
  oProgress = NULL
  *B610277,1 TMI 03/21/2013 [End  ] 
...
*!*	    (lnTotalDr = 0 .AND. lnTotalCr = 0)) 
IF llBatVld .AND. ((ROUND(lnTotalDr,2) <> ROUND(lnTotalCr,2)) .OR.;
    (lnTotalDr = 0 .AND. lnTotalCr = 0)) 
*B611970,1 MMT 11/18/2019 Total Debit <> Total Credit while comparing while its equal in table (Rounding issue)[T20191111.0005][End]
  IF lcBatType='B'
    ...
    *** since 
    ...
    (lcTrOrBat='T' .AND. GLTRNSHD.cStandard='Y'))  
      ...
      *IF lnTotalDr <> lnTotalCr       &&       
      IF ROUND(lnTotalDr,2) <> ROUND(lnTotalCr,2)       && 
      *B611970,1 MMT 11/18/2019 Total Debit <> Total Credit while comparing while its equal in table (Rounding issue)[T20191111.0005][End]
      ...
      SCAN REST WHILE &lcTrnsDtEx. = lcBatchNo+lcTranNo
        ...
        *C200958,1 NNA 03/05/2008 (BEGIN) Trigger to get Apdist.cinvno and Apdist.cvendcode and save then in GLPTRNDT table
        IF ASCAN(loFormSet.laEvntTrig,PADR('GLPTRNDT',10))<>0
          *C201563,E303372,1 TMI 03/22/2013 [Start] write the correct trigger name
          *=gfDoTriger(lcProgName,PADR('GLPTRNDT',10))
          loFormSet.mDoTrigger(PADR('GLPTRNDT',10))
...
*! Developer : TMI - Tarek Mohamed Ibrahim
*! Date      : 04/15/2013
*! Purpose   : lfHideProgress
************************************************************
*B610300,1
FUNCTION lfHideProgress

oProgress.Visible = .F.

*- End of lfHideProgress.
