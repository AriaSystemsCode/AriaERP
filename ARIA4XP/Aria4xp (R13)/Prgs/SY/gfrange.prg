...
#INCLUDE R:\ARIA4XP\Classes\optiongrid.H
...
IF oAriaApplication.oActivelang.cLang_ID <> "EN"
  lcHeaderFile = ADDBS(UPPER(ALLTRIM(oAriaApplication.LangPath))) + "CLASSES\" + ALLTRIM("OPTIONGRID")+"_"+"H" +".XML"
  lccHeaderAlias = oAriaApplication.GetClassHeaderFile(lcHeaderFile)
ENDIF
...
IF (VARTYPE(lcWinTitl) = "C") AND !EMPTY(lcWinTitl)
	...
	*lcWinTitl = "Select " + ALLTRIM(lcWinTitl) + " Range"
    lcWinTitl = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SELECT,oAriaApplication.GetHeaderText("LANG_SELECT",lccHeaderAlias))+" " + ALLTRIM(lcWinTitl) + " "+;
    IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_RANGE,oAriaApplication.GetHeaderText("LANG_RANGE",lccHeaderAlias))
    *N000682,1 MMT 12/09/2012 Globalization changes[END]
  ENDIF
ELSE
   *N000682,1 MMT 12/09/2012 Globalization changes[Start]
   *lcWinTitl = "Select Range"
   lcWinTitl =  IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_SELECT,oAriaApplication.GetHeaderText("LANG_SELECT",lccHeaderAlias))+" " +;
                IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_RANGE,oAriaApplication.GetHeaderText("LANG_RANGE",lccHeaderAlias))
   *N000682,1 MMT 12/09/2012 Globalization changes[END]
ENDIF
