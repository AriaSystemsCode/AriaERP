  ...
  *USE (LCSQLDICPATH +"SYUSTATC") IN 0 SHARED
  ...
  CATCH
    ...
    ELSE
      llFPTDel = .T.    
    ENDIF
    IF (llCDXDel AND llFPTDel AND llDBFDel) AND FILE(ADDBS(oAriaApplication.DefaultPath) +"XXUSTATC.FPT") AND ;
      FILE(ADDBS(oAriaApplication.DefaultPath) +"XXUSTATC.CDX")  AND FILE(ADDBS(oAriaApplication.DefaultPath) +"XXUSTATC.DBF")  
      COPY FILE (ADDBS(oAriaApplication.DefaultPath) +"XXUSTATC.FPT") TO (LCSQLDICPATH +"SYUSTATC.FPT")
      COPY FILE (ADDBS(oAriaApplication.DefaultPath) +"XXUSTATC.DBF") TO (LCSQLDICPATH +"SYUSTATC.DBF")
      COPY FILE (ADDBS(oAriaApplication.DefaultPath) +"XXUSTATC.CDX") TO (LCSQLDICPATH +"SYUSTATC.CDX")
      USE (LCSQLDICPATH +"SYUSTATC") IN 0 SHARED
    ENDIF   
  ENDTRY
