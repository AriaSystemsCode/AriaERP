...
FUNCTION lfFormInit
  ...
  *- initializations
  WITH loFormSet
    ...
    .AriaBrFields.edtBrowseFields.VALUE = "CMACHINEID :H='Machine ID',CDESC :H='Machine Desc'"
  ENDWITH
...
select (loFormSet.datafile)
locate 
llLock = EVALUATE(loFormSet.datafile+'.LLOCK')
WITH loFormSet.Ariaform1.Ariapageframe1.Ariapage2.Calendar1
  IF llLock 
    .tsTimeFrm.Olecontrol1.OBJECT.ENABLED = .F.
    .tsTimeTo.Olecontrol1.OBJECT.ENABLED = .F.
    .tsTimeTo.ENABLED = .F.
    .dtpDateTo.ENABLED = .F.
    .dtpDatFrom.ENABLED = .F.
    .cboShftTo.ENABLED = .F.
    .cboShftFrm.ENABLED = .F.
    .txtReson.ENABLED = .F.
    .BtnRemove.ENABLED = .F.
  ENDIF
ENDWITH    
  ...
  *- save the calender data
  ...
  IF FOUND()
    ...
    IF lnSeq = 0
      ...
      * HES Fix the issue of calendar sequence = 0 [BEGIN]
      DO WHILE lnseq = 0
      * HES Fix the issue of calendar sequence = 0 [END  ]
    ...
    SCAN FOR !LLOCK
      ...
      if deleted()
        SELECT SCCALDTL
        if seek(STR(m.NCALID,10)+DTOS(m.DDATFRM)+m.CTIMFRM)
          =gfdelete()
        endif 
      else 
...
loFormSet.Ariaform1.Ariapageframe1.Ariapage2.Calendar1.cboClndTyp.valid()
