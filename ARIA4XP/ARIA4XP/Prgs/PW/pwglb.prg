...
*B609978,1 TMI 12/06/2012 [T20121203.0028] fix a problem that the screen crashes when the shift id is empty
*!*	************************************************************
...
*! Developer : TMI - Tarek Mohamed Ibrahim
*! Date      : 11/04/2012
*! Purpose   : Compare two time fields
************************************************************
FUNCTION lfTimeCompare
PARAMETERS lcFr,lcTo
IF RIGHT(&lcFr,2)==RIGHT(&lcTo,2)
  RETURN STRTRAN(LEFT(&lcFr,2),'12','00')+SUBSTR(&lcFr,3)>=&lcTo
ELSE
  RETURN .F.
ENDIF 

*- End of lfTimeCompare.
...
lnRemResult = oAriaApplication.RemoteSystemData.Execute;
   ("Select SYDFLFLD.CFILE_NAM,SYDFILES.CFILE_TTL from SYDFLFLD,SYDFILES WHERE CFLD_NAME='&lcFld' AND SYDFLFLD.CFILE_NAM=SYDFILES.CFILE_NAM",'',"TMPSYDFLFLD","",oAriaApplication.cAria4SysFiles,3,"",SET("Datasession"))
SELECT TMPSYDFLFLD
...
*! Developer : TMI - Tarek Mohamed Ibrahim
*! Date      : 12/06/2012
*! Purpose   : adjust value to not be out of the selected shift limits
************************************************************DODEFAULT()
*E303321,1 TMI 12/06/2012 
FUNCTION lfAdjustVal
PARAMETERS loFld
LOCAL lcShiftID,lcVal,lnPos,lcStart,lcEnd,lnHrs

lnHrs = SET("Hours")
*SET HOURS TO 24

lcTimeNm = UPPER(SUBSTR(loFld.Name,7))
lcSftNm  = 'cboShft'+lcTimeNm
lcShiftID = loFld.Parent.&lcSftNm..Value
=SEEK(lcShiftID ,'PESHIFT')

lcVal = TTOC(loFld.olecontrol1.oBJECT.Value)
lnPos = AT(':',lcVal)-2
lcVal = SUBSTR(lcVal,lnPos)
lcVal   = IIF(LEFT(lcVal,2)='12','00'+SUBSTR(lcVal,3),lcVal)
lcStart = IIF(LEFT(PESHIFT.cShift_str,2)='12','00'+SUBSTR(PESHIFT.cShift_str,3),PESHIFT.cShift_str)
lcEnd   = IIF(LEFT(PESHIFT.cShift_fns,2)='12','00'+SUBSTR(PESHIFT.cShift_fns,3),PESHIFT.cShift_fns)
IF lcTimeNm = 'FRM'
  *B609978,1 TMI 12/10/2012 [Start] check if lcStart is not empty
  IF !EMPTY(lcStart)
    *B609978,1 TMI 12/10/2012 [End  ]  
    IF RIGHT(lcVal,2)<>RIGHT(lcStart,2) OR lcVal<lcStart
      loFld.olecontrol1.oBJECT.Value = CTOT(lcStart)
    ENDIF 
    *B609978,1 TMI 12/10/2012 [Start] 
  ENDIF
  *B609978,1 TMI 12/10/2012 [End  ] 
ELSE
  *B609978,1 TMI 12/10/2012 [Start] 
  IF !EMPTY(lcEnd)
    *B609978,1 TMI 12/10/2012 [End  ] 
    IF RIGHT(lcVal,2)<>RIGHT(lcEnd,2) OR lcVal>lcEnd
      loFld.olecontrol1.oBJECT.Value = CTOT(lcEnd)
    ENDIF 
    *B609978,1 TMI 12/10/2012 [Start] 
  ENDIF 
  *B609978,1 TMI 12/10/2012 [End  ] 
