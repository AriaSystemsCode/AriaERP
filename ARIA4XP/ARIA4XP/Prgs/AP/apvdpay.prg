...
*E303442,1 TMI 02/19/2014 Add a new setup to APSETUP table to allow/prevent voiding payments in different periods [T20140214.0007] 
...
WITH loFormSet.ariaform1.grdAPPAYMNT
  ...
  *=lfAddColumn(@lnI,oGrid,'lfGetPayMeth(Thisformset)',LANG_APVDPAY_Pay_Meth      ,100 )  &&" Pay.  Meth  "  
  =lfAddColumn(@lnI,oGrid,'Thisformset.lfGetVdPayMeth()',LANG_APVDPAY_Pay_Meth      ,100 )  &&" Pay.  Meth  "
...
IF APSETUP.CVDPAYMNT $ 'SW'
  IF lfGetPeriod(ldVoidDate) <> lfGetPeriod(APPAYMNT.DPAYDATE)
    ...
    CASE APSETUP.CVDPAYMNT = 'S'
      =gfModalGen('INM00000B00000',.F.,.F.,.F.,LANG_DIFFERENT_PERIODS_DONT_CONTINUE)
      RETURN .F.
    CASE APSETUP.CVDPAYMNT = 'W'
      IF gfModalGen('INM00000B00006',.F.,.F.,.F.,LANG_DIFFERENT_PERIODS_DONT_WARNING) = 2
        RETURN .F.
      ENDIF     
    ENDCASE 
  ENDIF 
