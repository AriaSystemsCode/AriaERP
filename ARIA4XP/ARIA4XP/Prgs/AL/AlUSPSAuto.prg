...
*!*	  STORE .F. TO llRpBolAsi, llRpUseExs, llRpUsePre,llRpMltSku, llRp1BoxPk,llEdiSys 
*!*	   gcRequestXMLFileName = oAriaApplication.WorkDir+gfTempaName()+'.Xml'
*!*	  lcExpr = gfOpGrid('ALAUTPLS',.T.,"R",.T.,.T.,.T.)
*!*	ENDFUNC
...
*!*	  looGScroll.laOgFxFlt[lnInd,6] = lcTempOrdFile

*!*	  =loOGScroll.RefreshScroll()
*!*	  IF llShowOpG
*!*	    loOGScroll.SetFilterExpr()
*!*	      lcXMLFileName = loogscroll.ConvertVariablestoXML()
*!*	      STRTOFILE(lcXMLFileName,oAriaApplication.workdir+lcautplistfile +'.xml')
*!*	    RETURN .F.
*!*	  ENDIF
*!*	  *wizard
...
*!*	  SELECT (lcTempOrdFile)
*!*	  INDEX ON KeyExp TAG (lcTempOrdFile)
*!*	  *MOH: Change code below to add record for each order will create piktkt for
*!*	  *APPEND BLANK
*!*	  *REPLACE ORDER WITH '031759',;
*!*	  *  keyExp WITH '031759'
*!*	  
*!*	  loOGScroll.laOgFxFlt[lnInd,6] = lcTempOrdFile
...
*!*	    loOGScroll.SetFilterExpr()
*!*	   lcXmlStr = loogScroll.convertvariablestoxml()
*!*	   STRTOFILE(lcXmlStr,oAriaApplication.workdir+lcalselgrdfile+'.xml')
*!*	    RETURN .F.
*!*	  ENDIF
...
*!*	  gcRequestXMLFileName = oAriaApplication.workdir+lcalselgrdfile+'.xml'
*!*	  llnoShow =.T.
*!*	  IF llSelect
...
*!*	  ENDIF
*!*	  =gfSeek('O'+'031759','OrdHdr')
*!*	  lcRpPkFWrh = OrdHdr.Cwarecode

*!*	  IF llShowOpG
*!*	    loOGScroll.SetFilterExpr()
*!*	    lcXmlFl = loogScroll.ConvertVariablestoXMl()
*!*	    STRTOFILE(lcXmlFl ,oAriaApplication.workdir+lcallogicgrdfile+'.xml')    
*!*	    RETURN .F.
*!*	  ENDIF
...
*!*	  IF llShowOpG
*!*	    loOGScroll.SetFilterExpr()
*!*	    lcXmlFile = loogscroll.ConvertVariablestoXml()
*!*	    STRTOFILE(lcXmlFile,oAriaApplication.WorkDir+lcpkTkgrdfile +'.xml')
*!*	* gcRequestXMLFileName = oAriaApplication.WorkDir+lcpkTkgrdfile +'.xml'    
*!*	    RETURN .F.
*!*	  ENDIF
...
*!*	  lcPKOrd = ORDER()
*!*	  SET ORDER TO ORDPIK
*!*	  gfSEEK('031759','PikTkt')
*!*	  SCAN REST WHILE ORDER+piktkt = '031759' FOR Status ='O'
*!*	    INSERT INTO (lcTempOrdFile) (PikTkt,keyExp) VALUES (PikTkt.PikTkt,PikTkt.PikTkt)
*!*	  ENDSCAN
...
FUNCTION lfwOGWhen
  ...
  lcPKOrd = ORDER()
  SET ORDER TO ORDPIK
  *gfSEEK(lcOrder,'PikTkt')
  *SCAN REST WHILE ORDER+piktkt = lcOrder FOR Status ='O'
  *  INSERT INTO (lcTempOrdFile) (PikTkt,keyExp) VALUES (PikTkt.PikTkt,PikTkt.PikTkt)
  *ENDSCAN
  
  *Hassan [Begin]
  IF !USED(loFormSet_Tmpoutput)
    SELECT 0
    USE (oAriaApplication.WORKDIR+loFormSet_Tmpoutput+'.dbf' ) IN 0 SHARED 
  ENDIF 
  SELECT (loFormSet_Tmpoutput)
  *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[Start]
  IF TYPE('PIKTKT') ='U' OR (TYPE('PIKTKT') ='C' AND EMPTY(PIKTKT))
  *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[END]
    *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[Start]
    *Scan FOR chk = .T. AND !EMPTY(CCARTTCKNO)  
    Scan FOR chk = .T. &&AND !EMPTY(CCARTTCKNO)
    *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[ENd]
      SELECT PikTkt
      gfSEEK(&loFormSet_Tmpoutput..Order,'PikTkt')
      SCAN REST WHILE ORDER+piktkt = &loFormSet_Tmpoutput..Order FOR Status ='O'
        INSERT INTO (lcTempOrdFile) (PikTkt,keyExp) VALUES (PikTkt.PikTkt,PikTkt.PikTkt)
      ENDSCAN
      SELECT (loFormSet_Tmpoutput)
    ENDSCAN 
  *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[Start]
  ELSE
    Scan FOR chk = .T. 
      INSERT INTO (lcTempOrdFile) (PikTkt,keyExp) VALUES (&loFormSet_Tmpoutput..PikTkt,&loFormSet_Tmpoutput..PikTkt)
    ENDSCAN   
  ENDIF
  *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[END]
  *Hassan [End]
...
*elect (lcOrdhdr)
  ...
  *E302954,1 MMT 08/09/2011  Interface to send shipping information, receive tracking# and label[End]
    SELECT (lcTempOrdFile)  
    APPEND BLANK
    REPLACE ORDER WITH &loFormSet_Tmpoutput..Order, keyExp WITH &loFormSet_Tmpoutput..Order    
    *REPLACE ORDER WITH lcOrder, keyExp WITH lcOrder   
