...
IF (EMPTY(lcPickTicketNo) AND This.loFormSet.ariaform1.kbpickNo.selectedfrombrowse) OR ;
  ...
  lcBrFields = [PIKTKT :R :H= LANG_Pick_Ticket_No , ACCOUNT :R :H= LANG_Account, STORE :R :H= LANG_Store ,] +;
                   [ORDER :R :H= LANG_Order , DATE :R :H= LANG_Date, ] +;
                   [CWARECODE :R :H= LANG_Location , CUSTPO :R :H= LANG_CustPo , ;
                   STATUS = IIF(STATUS = 'O' , LANG_Open , IIF(STATUS = 'H' ,LANG_Hold,;
                            IIF(STATUS = 'C',LANG_Complete,IIF(STATUS = 'K',LANG_Packed	,;
                            IIF(STATUS = 'X',LANG_Cancel,LANG_Pulled))))) :R :H= LANG_Status]
...
SELECT ORDHDR
LOCATE 
lcBrFields = [Order:H=LANG_Order_No,status:1:H=LANG_Order_Status,lcSesDesc=gfCodDes(Season,'SEASON'):H=LANG_Season,lcDivDesc=gfCodDes(cDivision,'CDIVISION'):H=LANG_Division,]+;
             [CustPo=IIF(multipo,LANG_MultiPo,custpo):H=LANG_CustPo,]+;
             [ACCOUNT:H=LANG_Account,store=IIF(MULTI='Y',LANG_MultiStore,STORE):H=LANG_Store,Customer.stname]+;
             [:15:H=LANG_CustName,Open:H=LANG_OpenQty,OpenAmt:H=LANG_OpenAmt,Ship:H=LANG_ShipQty,Shipamt:H=LANG_ShipAmt,]+;
             [start:H=LANG_Start,Complete:H=LANG_Complete,]+;
             [Note1:6:H=LANG_Notes]
DO CASE
...
IF !EMPTY(EVALUATE(This.loFormSet.lc_TmpOrdL+'.Pack_ID'))
  ...
  *-- <YES>  <NO>
  IF gfModalGen('QRM00000B40000','ALERT',.F.,.F.,;
                LANG_Pack_Style + ALLTRIM(EVALUATE(This.loFormSet.lc_TmpOrdL+'.Pack_ID'))+'-'+ALLTRIM(EVALUATE(This.loFormSet.lc_TmpOrdL+'.cPkColor'))+;
...
IF !EMPTY(EVALUATE(lcPackSize)) AND This.Scale.SEEK('S'+LEFT(EVALUATE(lcPackSize),1))
