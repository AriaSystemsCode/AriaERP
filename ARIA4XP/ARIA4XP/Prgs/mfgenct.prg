...
IF lcChoice $ 'CPLO'
  ...
  IF lcGenProj $ 'AI'
    ...
    *lcPoType   = IIF(lcChoice = 'P',LANG_MFGENCT_PO,LANG_MFGENCT_CT)
    lcPoType   = IIF(lcChoice $ 'PO',LANG_MFGENCT_PO,LANG_MFGENCT_CT)
    *! E302650,1 MMT 12/02/2009 Give user ability to create project per PO/CT Line[End]
  ENDIF
ENDIF
...
CASE lcModeToChange='A'
  WITH loFormSet.AriaForm1
    ...
    *N000606,1 NNA (End)
    ...
    .cmdNotes.CAPTION    = IIF(loFormSet.llRpBOPFab,IIF(loFormSet.llAllocate,LANG_MFGENCT_CLEARALO,;
      LANG_MFGENCT_ALLOCATE),LANG_MFGENCT_NOTES)
    .cmdNotes.ENABLED    = .F.
    .cmdPO.ENABLED       = .F.
    *N000606,1 NNA 10/06/2007 (Begin) Add the OTS option to lcChoice
    *.cmdPO.Caption       = IIF(loFormSet.llRpBOPFab  ,LANG_MFGENCT_ALORDERS,;
    ...
    .cmdPO.CAPTION       = IIF(loFormSet.llRpBOPFab  ,LANG_MFGENCT_ALORDERS,;
      IIF(loFormSet.lcChoice $ 'PO',LANG_MFGENCT_POCMD,"\<"+LANG_MFGENCT_CT))
    *N000606,1 NNA (End)
...
IF EVALUATE(lcQtyFld) < EVALUATE(lcOrdFld)
  ...
  *IF loParentForm.lcChoice='O' THEN
  ...
  ELSE
    *N000606,1 NNA (END)
    =gfModalGen('INM38133B38018','DIALOG',IIF(loParentForm.lcChoice='P',LANG_MFGENCT_PO,LANG_MFGENCT_CT)+'|'+;
      IIF(loParentForm.lcChoice='P',LANG_MFGENCT_PO,LANG_MFGENCT_CT))
    *N000606,1 NNA (END)
  ENDIF
  *N000606,1 NNA (END)
...
lcPrompt  = IIF(EVALUATE(loFormSet.lcFabrics+'.cWareCode') = EVALUATE(loFormSet.lcTmpOrd+'.cFabWare') AND;
  ...
  EVALUATE(loFormSet.lcTmpOrd+'.lSelect'),LANG_MFGENCT_DEALLOCATE,LANG_MFGENCT_ALLOCATE1)
...
lcPrompt = IIF(EVALUATE(loParentForm.lcFabrics+'.cWareCode')=EVALUATE(loParentForm.lcTmpOrd+'.cFabWare') AND;
  ...
  EVALUATE(loParentForm.lcTmpOrd+'.lSelect'),LANG_MFGENCT_DEALLOCATE,LANG_MFGENCT_ALLOCATE1)
...
WITH loFormSet.AriaForm1
  IF loParentForm.lcChoice <> 'L'
    ...
    WITH .cntDetails
      ...
      .lblPoLine.CAPTION             = IIF(loParentForm.lcChoice='P',LANG_MFGENCT_PO+" "+LANG_MFGENCT_LINE,;
        LANG_MFGENCT_CT+" "+LANG_MFGENCT_LINE)
...
IF !SEEK(lcInvType+PADR(lcStyMajr,19)+lcCstShtType+EVALUATE(lcTmpPoLn+'.cCstSht_ID'),lcTmpBomHd)
  ...
  IF gfCstShtBrow('',IIF(loParentForm.lcChoice = 'P',LANG_MFGENCT_IMPSTYCOST,LANG_MFGENCT_MFGSTYCOST),;
...
ELSE
  ...
  IF !loBomHead.SEEK('0001'+PADR(lcStyMajor,19)+lcCstShtType+PADR(lcCstShtID,6))
    ...
    IF gfCstShtBrow(['0001'+lcMaj],'Style Cost Sheets',@lcCstShtID,lcTempBomHdr,1)
      ...
      REPLACE ALL cCstSht_Id WITH  EVALUATE(loFormsetPar.lcHdrFile+'.cCstSht_ID') IN (loFormsetPar.lcPOLine) FOR PO+cDivision+SEASON+cWeek+cYear+STYLE+Dyelot = lcPo+lcDivision+lcSeason+lcWeek+lcYear+lcStyle
      RETURN 1
    ELSE
      PRIVATE loBomHeader
      loBomHeader = CREATEOBJECT('RemoteTable','BomHeadr','BomHeadr','BomHeadr',loFormsetPar.DATASESSIONID)
