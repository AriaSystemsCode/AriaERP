...
*B610957,1 MMT 03/03/2015 Stop inserting repeated records in syueror table[T20150215.0015]
...
*B610957,1 MMT 03/03/2015 Stop inserting repeated records in syueror table[T20150215.0015][Start]
*DO gfsaveErr
llAddErrorLine = .T.
IF TYPE('gnError') <> 'U' AND TYPE('gcmsg') <> 'U' AND TYPE('gckode') <> 'U' AND TYPE('gcmodul') <> 'U' AND TYPE('gnline') <> 'U' AND ;
  gnError = lnError AND gcmsg=lcmsg AND gckode=lckode AND gcmodul =lcmodul AND gnline = lnline
  llAddErrorLine = .F.
ENDIF
IF TYPE('gnError') = 'U'
  PUBLIC gnError
...
IF llAddErrorLine 
  DO gfsaveErr
ENDIF
*B610957,1 MMT 03/03/2015 Stop inserting repeated records in syueror table[T20150215.0015][End]
WAIT CLEAR
