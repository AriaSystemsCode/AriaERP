...
DO CASE
  CASE lcItemType = '0001'
    ...
    *--Displayed Screen Message
    lcStyCst  = LANG_UPDATE_TOTCST + ALLTRIM(lcStyHdr) + ': "' + ;
...
*!*      lcItemDye = IIF(TYPE('loParForm') = 'O',loParForm.lcItemloc ,'ITEMLOC_U')
    ...
    *--Displayed Screen Message
    ...
    IF SUBSTR(ALLTRIM(lcItemCode),LEN(lcItemCode)-1,1) = '-'
      lcStyCst  = LANG_UPDATE_TOTCST + lcMatHdr + ':"' + ;
                  SUBSTR(ALLTRIM(lcItemCode),1,LEN(lcItemCode)-1)+ LANG_UPDATE_CHNG     
    ELSE
      lcStyCst  = LANG_UPDATE_TOTCST + lcMatHdr + ':"' + ;
                 lcItemCode+ LANG_UPDATE_CHNG   
    ENDIF
