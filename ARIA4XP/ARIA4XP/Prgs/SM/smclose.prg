      ...
      *E303557,1 MMT 03/09/2015 Don't close period has unreleased transactions[T20150227.0001][Start]
      IF lcClose = 'P'
        ...
        DO WHILE llCheckAgain
          ...
          ENDIF
          IF llUnReleaseFound 
            IF gfModalGen('QRM00000B00012',.F.,.F.,.F.,LANG_UNRELEASEDTRANSACTIONS) = 2
              llCheckAgain = .F.
              RETURN 
            ELSE
    ...
    *B610674,1 TMI 02/07/2014 18:46 [Start] show this message only when the company is linked to GL
    IF gfGetMemVar('M_LINK_GL') == 'Y'
      *B610674,1 TMI 02/07/2014 18:46 [End  ] 
      ...
      loFormSet.laCompany[loFormSet.puCompany,9] = 'F'
      RETURN .F.
      *B610674,1 TMI 02/07/2014 18:49 [Start] 
    ENDIF 
    *B610674,1 TMI 02/07/2014 18:49 [End  ] 
  ENDIF
ELSE
  ...
  *B610674,1 TMI 02/07/2014 18:46 [Start] show this message only when the company is linked to GL
  IF gfGetMemVar('M_LINK_GL') == 'Y'
    *B610674,1 TMI 02/07/2014 18:46 [End  ] 
    ...
    loFormSet.laCompany[loFormSet.puCompany,9] = 'F'
    RETURN .F.
    *B610674,1 TMI 02/07/2014 18:47 [Start] 
  ENDIF
  *B610674,1 TMI 02/07/2014 18:47 [End  ] 
ENDIF
  ...
  *B610674,1 TMI 02/07/2014 18:46 [Start] show this message only when the company is linked to GL
  IF gfGetMemVar('M_LINK_GL') == 'Y'
    *B610674,1 TMI 02/07/2014 18:46 [End  ] 
    ...
    loFormSet.laCompany[loFormSet.puCompany,9] = 'F'
    RETURN .F.
    *B610674,1 TMI 02/07/2014 18:48 [Start] 
  ENDIF 
  *B610674,1 TMI 02/07/2014 18:49 [End  ] 
ENDIF
...
SCAN FOR &lcBatch..cBatStat $"AEHOU";
  ...
  ** Choices : "   ® Resume ¯  < Display >  < Print >  ® Cancel ¯  "
  lnChoice   = gfModalGen('QRM00101B00021','Dialog',;
                          LOWER(LANG_SMCLOSE_lctUnpostb)+'|'+LOWER(LANG_SMCLOSE_lctUnpostb),"lfvUnpBatch()")
...
SCAN FOR  cTrnStat $ "AEHOU";
  ...
  ** Choices : "   ® Resume ¯  < Display >  < Print >  ® Cancel ¯  "
  lnChoice = gfModalGen('QRM00101B00021','Dialog',;
                        LOWER(LANG_SMCLOSE_lctUnpostd)+'|'+LOWER(LANG_SMCLOSE_lctUnpostd),"lfvUnpSTrn()")
  lnDlgOpTion = lnChoice 
...
IF SEEK('A')
  SCAN REST WHILE cAutType  = 'A';
    ...
    ** Choices : "   ® Resume ¯  < Display >  < Print >  ® Cancel ¯  "
    lnChoice    = gfModalGen('QRM00101B00021','Dialog',;
                             LOWER(LANG_SMCLOSE_lctUngenAl)+'|'+LOWER(LANG_SMCLOSE_lctUngenAl),"lfvUngnAllo()")
    lnDlgOpTion = lnChoice 
...
IF SEEK('R')
  ********* Ungenerated Recurring Transactions *********
  SCAN REST WHILE cAutType = 'R';
    ...
    ** Choices : "   ® Resume ¯  < Display >  < Print >  ® Cancel ¯  "
    lnChoice = gfModalGen('QRM00101B00021','Dialog',;
                          LOWER(LANG_SMCLOSE_lctUngenRc)+'|'+LOWER(LANG_SMCLOSE_lctUngenRc),"lfvUngnRec()")
    lnDlgOpTion = lnChoice 
    ...
    IF FOUND()               
      ...
      ** of the nest year last period.
      SELECT &lcAcChar..CACCTCODE,;
             ...
             lfThermo(LANG_SMCLOSE_lcTAdjBal);
  ...
  IF lcClosStat = 'I'
    ...
    ** batch file.
    ...
    ELSE
      ...
      REPLACE CBATCHNO  WITH lcBefor_Bat_No,; 
              CTRANNO   WITH ' ',;
              CDESCRIP  WITH LANG_SMCLOSE_lcTRclasfi,;
  ...
  IF lnTotTrnDr <> 0
    ...
    ** This is then padded with zeroes from the left.
    ...
    REPLACE cBatchNo  WITH PADL(lnBatchNo,FSIZE('CBATCHNO'),'0'),;
            ...
            cTrnRefer WITH LANG_SMCLOSE_lcTYECNo+' '+PADL(lnClosNo,FSIZE('CTRANNO'),'0'),;
            dTrnPDate WITH loFormSet.laCompany[loFormSet.puCompany,10],;
            cTrnPYr   WITH loFormSet.laCompany[loFormSet.puCompany,4],;
            cTrnPPrd  WITH loFormSet.laCompany[loFormSet.puCompany,3],;
            cTrnType  WITH 'Y',;
    ...
    REPLACE ALL FOR EMPTY(cBatchNo);
                cBatchNo WITH PADL(lnBatchNo,FSIZE('CBATCHNO'),'0'),;
                cTranNo  WITH PADL(lnTranNo,FSIZE('CTRANNO'),'0'),;
                cTrDtExp WITH LANG_SMCLOSE_lcTClsEntN+' '+;
...
IF lnTotBatDr <> 0
  ...
  REPLACE cBatchNo  WITH PADL(lnBatchNo,FSIZE('CBATCHNO'),'0'),;
          ...
          cBatRefer WITH LANG_SMCLOSE_lcTYECNo+' '+PADL(lnClosNo,FSIZE('CTRANNO'),'0'),;
          cBatDesc  WITH LANG_SMCLOSE_lcTPrior+' '+lcUserName,;
  ...
  SCAN
    ...
    ** we are going to creat a new transaction no.
    ...
    REPLACE nBatCnTot WITH lnTotBatDr,;
            nBatotDr  WITH lnTotBatDr,;
            nBatotCr  WITH lnTotBatCr,;
            cBatRefer WITH LANG_SMCLOSE_lcTYECNo+' '+lcClosNo
  ...
  ELSE
    ...
    ** Choices : "                    ® Ok ¯                     "
    IF loFormSet.puCompany <> 0
      =gfModalGen("TRM00110B00000","ALERT",'File '+loFormSet.laSFileOpn[lnCount,1]+'|'+ALLTRIM(loFormSet.laCompany[loFormSet.puCompany,1])+'|'+LANG_SMCLOSE_lctCloseL)
      loFormSet.laCompany[loFormSet.puCompany,9] = 'F'
    ENDIF  
    llOpenFile = .F.
    EXIT
