      ...
      lcAccount=Account
        ...
        SCAN REST WHILE account+invoice = lcAccount FOR &lcSeekHdr
            ...
            SCAN REST WHILE invoice+STR(lineno,6) = lcInvoice FOR &lcSeekLn
              WAIT 'Selecting records for report ,Account '+PADR(lcAccount,6) + ' for Invoice # '+lcInvoice       WINDOW NOWAIT
    ...
    IF lcMainFile = 'INVHDR'
      *--if the user didn't select any Style(s) or  Customer(s) .
      ...
      SCAN FOR Invoice = '' AND &lcSeekHdr
        ...
        =GFSEEK(lcInvoice)
        SCAN REST WHILE invoice+STR(lineno,6) = lcInvoice FOR &lcSeekLn
           WAIT 'Selecting records for report , Invoice # '+lcInvoice WINDOW NOWAIT
            =lfGetData()
        ENDSCAN
      ENDSCAN   
    ELSE
      *--if the user select specific Style(s)
      ...
      SCAN
        ...
        SCAN REST WHILE style+invoice+STR(lineno,6) = lcStyle FOR &lcSeekLn
          WAIT 'Selecting records for report ,Style '+lcStyle+ ' for Invoice # '+invoice WINDOW NOWAIT
...
loOgScroll.laCRParams[1,2]= 'SHIPMENTS BY SALESREP'
