...
*!B609952,1 MMT 06/04/2012 Print Check report does not update the next check# when user prints from preview[T20120530.0027]
*!B610010,1 HIA 07/18/2012 Add default value to Payment method [T20120709.0013]
*!B610036,1 MMT 08/07/2012 Print check report include Factored and Non-Factored invoices in One check[T20120711.0038]
*!B610036,2 MMT 08/15/2012 Error Connection level error when does not select vendor or division [T20120814.0038]
*!B610396,1 TMI 06/25/2013 lock some tables while printing checks [T20130610.0032] 
*!B610588,1 MMT 11/14/2013 Print check report gives error if selected vendor has '[T20131112.0027]
*!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[T20140813.0014]   
*!B610958,1 MMT 03/05/2015 Print check updates exchange differnce with wrong GL account[T20150303.0011]
*:***************************************************************************
...
*!B609952,1 MMT 06/04/2012 Print Check report does not update the next check# when user prints from preview[Start]
*!*	IF TYPE('lcXMLFileName') <> 'C'
...
*!*	ENDIF 
llPrintChk =.F.
*!B609952,1 MMT 06/04/2012 Print Check report does not update the next check# when user prints from preview[End]
*!*  IF TYPE('lcXMLFileName') <> 'C'
...
*!N000635,2 MMT 01/04/2009 Convert AP Print Check report to Aria4 [End]
*!B610036,1 MMT 08/07/2012 Print check report include Factored and Non-Factored invoices in One check[T20120711.0038][Start]
IF !USED('APINVHDR_B')
  =gfOpenTable('APINVHDR','INVVEND','SH','APINVHDR_B')
ENDIF
IF !USED('APVENDOR_B')
  =gfOpenTable('APVENDOR','VENCODE','SH','APVENDOR_B')
ENDIF
*!B610036,1 MMT 08/07/2012 Print check report include Factored and Non-Factored invoices in One check[T20120711.0038][END]
DO CASE
  ...
  CASE lcPrintMod = 'V'     && Print approved checks
    ...
    *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[Start]     
    *DIMENSION laFileStru[lnFileStru+4,18]
    DIMENSION laFileStru[lnFileStru+5,18]
    *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[End]     
    *!N000635,2 MMT 01/04/2009 Convert AP Print Check report to Aria4 [End]
	...
	*!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[Start]     
    *FOR lnCount = 1 TO 4
    laFileStru[lnFileStru+5,1] = 'llokbyme'
...
*!*	              LOOP
*!*	            ENDIF
		  *!B610588,1 MMT 11/14/2013 Print check report gives error if selected vendor has '[T20131112.0027][Start]
*!*	          =gfSqlRun("Select "+lcRpFields+",apvendor.cvencomp From APINVHDR Inner join apvendor on apvendor.Cvendcode  = APINVHDR.Cvendcode  Where "+;
*!*	          			 " APINVHDR.CINVSTAT <> 'V' ANd  APINVHDR.CVENPRIOR<>'0' AND  APINVHDR.CVENDCODE = '"+;
*!*	                     ALLTRIM(&lcCursorVend..KeyExp)+"'"+;
*!*	                     IIF(!EMPTY(lcRpVenPay),[ AND APINVHDR.CVENPMETH $ "]+lcRpVenPay+["],'')+;
*!*	                     " AND APINVHDR.DINVDATE <= CTOD('"+DTOC(ldChkDat)+"') AND APINVHDR.DPOSTDATE <= CTOD('"+DTOC(ldChkDat)+"')"+;
*!*	                     IIF(llPrioSelected," AND BETWEEN(APINVHDR.CVENPRIOR,'"+lcPStrt+"','"+lcPFnsh+"')",'')+;
*!*	                     IIF(llDueSelected ," AND BETWEEN(APINVHDR.DINVDUDAT,CTOD('"+DTOC(ldDueStrtDate)+"'),CTOD('"+DTOC(ldDueFnshDate)+"'))",'')+;
*!*	                     " AND APINVHDR.CBNKCODE+APINVHDR.CCHKACCT = '"+lcRpBnkCod+lcRpChkAct+"'",'APINVHDR_B',.F.,'APINVHDR_A')
          lcVendorCode = ALLTRIM(&lcCursorVend..KeyExp)
          =gfSqlRun("Select "+lcRpFields+",apvendor.cvencomp From APINVHDR Inner join apvendor on apvendor.Cvendcode  = APINVHDR.Cvendcode  Where "+;
          			 " APINVHDR.CINVSTAT <> 'V' ANd  APINVHDR.CVENPRIOR<>'0' AND  APINVHDR.CVENDCODE = ?m.lcVendorCode "+;
                     IIF(!EMPTY(lcRpVenPay),[ AND APINVHDR.CVENPMETH $ "]+lcRpVenPay+["],'')+;
                     " AND APINVHDR.DINVDATE <= CTOD('"+DTOC(ldChkDat)+"') AND APINVHDR.DPOSTDATE <= CTOD('"+DTOC(ldChkDat)+"')"+;
                     IIF(llPrioSelected," AND BETWEEN(APINVHDR.CVENPRIOR,'"+lcPStrt+"','"+lcPFnsh+"')",'')+;
                     IIF(llDueSelected ," AND BETWEEN(APINVHDR.DINVDUDAT,CTOD('"+DTOC(ldDueStrtDate)+"'),CTOD('"+DTOC(ldDueFnshDate)+"'))",'')+;
                     " AND APINVHDR.CBNKCODE+APINVHDR.CCHKACCT = '"+lcRpBnkCod+lcRpChkAct+"'",'APINVHDR_B',.F.,'APINVHDR_A')
		  *!B610588,1 MMT 11/14/2013 Print check report gives error if selected vendor has '[T20131112.0027][End]                     
          SELECT APINVHDR_A
          SCAN 
            IF llDiviSelected AND !SEEK(APINVHDR_A.CDIVISION,lcDivCursor)
              LOOP
            ENDIF
...
*!*	            ENDIF  
*!*	          ENDIF 
        *!B610036,2 MMT 08/15/2012 Error Connection level error when does not select vendor or division [T20120814.0038][Start]
*!*	        =GFSQLRUN("SELECT "+LCRPFIELDS+",APVENDOR.CVENCOMP FROM APINVHDR INNER JOIN APVENDOR ON APVENDOR.CVENDCODE  = APINVHDR.CVENDCODE WHERE "+;
*!*	                    "APINVHDR.CINVSTAT <> 'V' AND  APINVHDR.CVENPRIOR<>'0' AND APINVHDR.DINVDATE <= CTOD('"+DTOC(LDCHKDAT)+"') AND APINVHDR.DPOSTDATE <= CTOD('"+DTOC(LDCHKDAT)+"')"+;
*!*	                    IIF(LLPRIOSELECTED," AND  BETWEEN (APINVHDR.CVENPRIOR,'"+LCPSTRT+"','"+LCPFNSH+"')",'')+;
*!*	                    IIF(!EMPTY(LCRPVENPAY),[ AND APINVHDR.CVENPMETH $ "]+LCRPVENPAY+["],'')+;
*!*	                    IIF(LLDUESELECTED ," AND BETWEEN(APINVHDR.DINVDUDAT,CTOD('"+DTOC(LDDUESTRTDATE)+"'),CTOD('"+DTOC(LDDUEFNSHDATE)+"')",'')+;
*!*	                    " AND APINVHDR.CBNKCODE+APINVHDR.CCHKACCT = '"+LCRPBNKCOD+LCRPCHKACT+"'",'APINVHDR_B',.F.,'APINVHDR_A')
        =gfSqlRun("Select "+lcRpFields+",apvendor.cvencomp From APINVHDR Inner join apvendor on apvendor.Cvendcode  = APINVHDR.Cvendcode Where "+;
                    ...
                    IIF(llDueSelected ," AND BETWEEN(APINVHDR.DINVDUDAT,CTOD('"+DTOC(ldDueStrtDate)+"'),CTOD('"+DTOC(ldDueFnshDate)+"'))",'')+;
                    " AND APINVHDR.CBNKCODE+APINVHDR.CCHKACCT = '"+lcRpBnkCod+lcRpChkAct+"'",'APINVHDR_B',.F.,'APINVHDR_A')
       *!B610036,2 MMT 08/15/2012 Error Connection level error when does not select vendor or division [T20120814.0038][End]                    
        SELECT APINVHDR_A   
        lnPerCent = RECNO()/RECCOUNT('APINVHDR_A')      
        SCAN 
          IF TYPE('lcXMLFileName') = 'C'
            IF MOD(RECNO('APINVHDR_A'),CEILING(RECCOUNT('APINVHDR_A') / 10)) = 0
              loProgress.Percent = lnPerCent * 0.9
              loProgress.Description = "Collecting Data For Invoice:"+ALLTRIM(APINVHDR_A.CinvNo)
              loAgent.UpdateObjectProgress(lcRequestID, loProgress)
            ENDIF  
          ENDIF           
*!B610036,1 MMT 08/07/2012 Print check report include Factored and Non-Factored invoices in One check[T20120711.0038][End]
...
*!*	          ENDIF
          ...
          *B610396,1 lock some tables while printing checks TMI 06/26/2013 [Start] 
          =IIF(llPrintChk , lfLockUpInv() , '' )
          *B610396,1 lock some tables while printing checks TMI 06/26/2013 [End  ] 
...
IF TYPE('lcXMLFileName') <> 'C'
  ...
  *IF (oAriaApplication.gcDevice = "FILE" .AND. INLIST(loOGScroll.cTextRepType ,"EXCEL",'XML')) AND llPrintChk  AND lcPrintMod = 'V'
  IF llPrintChk  AND lcPrintMod = 'V'
  *!B609952,1 MMT 06/04/2012 Print Check report does not update the next check# when user prints from preview[End]
...
*! Date      : 06/26/2013
*! Purpose   : Lock files for use by lfUpInv function
************************************************************
*B610396,1 Locking table before updates TMI 06/26/2013 [Start] 
FUNCTION lfLockUpInv

LOCAL llContinue
llContinue = .T.
SELECT APCHECKS
llContinue = llContinue AND gfObj_Lock(.T.)
SET STEP ON 

*B610396,2 TMI 07/03/2013 [Start] 
LOCAL lcTempVnd
lcTempVnd = gfTempName()
CREATE CURSOR (lcTempVnd) (CVENDCODE c(8) , CFISFYEAR c(4))
INDEX on CVENDCODE+CFISFYEAR TAG &lcTempVnd
*B610396,2 TMI 07/03/2013 [End  ] 

SELECT (lcRpTargt)
LOCATE 
SCAN
  SELECT APINVHDR
  IF gfSeek(&lcRpTargt..cinvno+&lcRpTargt..cvendcode)
    llContinue = llContinue AND gfObj_Lock(.T.)
    *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[Start]     
    IF llContinue
      REPLACE &lcRpTargt..llokbyme WITH .T.
    ENDIF    
    *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[End]     
  ENDIF 
  
  *B610396,2 TMI 07/03/2013 [Start] if the vendor is locked , do not lock it again 
  IF !SEEK(&lcRpTargt..cvendcode,lcTempVnd)
    *B610396,2 TMI 07/03/2013 [End  ] 
  SELECT APVENDOR        
  IF gfSeek(&lcRpTargt..cvendcode)
    llContinue = llContinue AND gfObj_Lock(.T.)
  ENDIF 
    *B610396,2 TMI 07/03/2013 [Start] 
    INSERT INTO (lcTempVnd) (CVENDCODE) VALUES (APVENDOR.CVENDCODE)
  ENDIF 
  IF !SEEK(&lcRpTargt..cvendcode+&lcRpTargt..cfisfyear,lcTempVnd)  && if the vendor-year is locked , do not lock it again 
    *B610396,2 TMI 07/03/2013 [End  ]   
  SELECT APVENHST
  IF gfSeek(&lcRpTargt..cvendcode+&lcRpTargt..cfisfyear)
    llContinue = llContinue AND gfObj_Lock(.T.)
  ENDIF    
    *B610396,2 TMI 07/03/2013 [Start]     
    INSERT INTO (lcTempVnd) VALUES (APVENHST.CVENDCODE,APVENHST.CFISFYEAR)  &&- update the temp vendor 
  ENDIF   
  IF !llContinue
    EXIT
  ENDIF 
...
*B610396,2 TMI 07/03/2013 [Start] release lcTempVnd
USE IN (lcTempVnd)
*B610396,2 TMI 07/03/2013 [End  ] 
...
lPrntCompl = IIF(lPrntCompl,llContinue,lPrntCompl)
llPrintChk = IIF(llPrintChk,llContinue,llPrintChk)
RETURN llContinue
*- End of lfLockUpInv.
...
*! Developer : TMI - Tarek Mohamed Ibrahim
*! Date      : 06/26/2013
*! Purpose   : Unlock files used by lfUpInv function
************************************************************
*B610396,1 lock some tables while printing checks TMI 06/26/2013  
FUNCTION lfUnLockUpInv
...
SCAN
  *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[Start]     
  IF &lcRpTargt..llokbyme
  *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[End]     
    SELECT APINVHDR
    gfSeek(&lcRpTargt..cinvno+&lcRpTargt..cvendcode)
    =gfObj_Lock(.F.)
  *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[Start]     
  ENDIF
  *!B610855,1 MMT 09/14/2014 Printed check report doesn't update invoice paid amount[End] 
  ...
  SELECT APVENDOR        
  gfSeek(&lcRpTargt..cvendcode)
  =gfObj_Lock(.F.)
    
  SELECT APVENHST
  gfSeek(&lcRpTargt..cvendcode+&lcRpTargt..cfisfyear)
  =gfObj_Lock(.F.)  
ENDSCAN

*- End of lfUnLockUpInv.
  ...
  *N000635,5 TMI 07/28/2011 [End  ] 
  ...
  SCAN REST 
    ...
    *N000635,5 TMI 07/28/2011 [End  ] 
    ...
    IF lnExchDiff <> 0
      *APPEND BLANK
      *!B610958,1 MMT 03/03/2015 Print check updates exchange differnce with wrong GL account[T20150303.0011][Start]
      llExApGlLink = .F.
	  SELECT APSETUP
	  =gfSeek('')
  	  GO TOP
	  IF !EOF()
	    GO 1
  	    llExApGlLink = (CAPSGLLINK = 'Y')
   	  ENDIF
	  lcApsAcMas = ""
	  lnApsAcLen = 0
	  lcApsAcMas = lfGetGLPIC()
	  llGlLinkEX   = "GL" $ oAriaApplication.CompanyInstalledModules 
	  IF llGlLinkEX
	    llUsd_GLSTP = gfOpenTable('GLSETUP' ,'GLSETUP','SH')
		SELECT GLSETUP
		=gfSeek('')
		GO TOP
	  ENDIF
	  lcEmptyAcc = REPLICATE('0',lnApsAcLen )
	  lcExDifAcc  = IIF(llExApGlLink , IIF(EMPTY(GLSETUP.CSETEXMJ), lcEmptyAcc,;
	  ...
	  SELECT APDIST                              
      *!B610958,1 MMT 03/03/2015 Print check updates exchange differnce with wrong GL account[T20150303.0011][End]
...
*! Date      : 06/04/2012
*! Purpose   : update advanced payment
*!*************************************************************
FUNCTION lfUpAdvInv
SELECT (lcDebtCurs)
SCAN 
...
*!B610010,1 HIA 07/18/2012 Add default value to Payment method [T20120709.0013][Begin]
*STORE ''               TO laRpTarget[1]
STORE 'Printed Checks' TO laRpTarget[1]
*!B610010,1 HIA 07/18/2012 Add default value to Payment method [T20120709.0013][End]
...
= lfOGMover(@laRpSource,@laRpTarget,'Select Payment Method',.T.,'')  && call mover function.
...
*! Developer : Mariam MAzhar
*! Date      : 06/04/2012
*! Purpose   : update Print check flag
*!*************************************************************
FUNCTION lfPntChk
