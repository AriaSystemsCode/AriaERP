...
*! B610194,1 SAB 02/04/2013 Fix error when running the report from request builder []
...
IF !llRpCurrnt AND !llRp30 AND !llRp60 AND !llRp90 AND !llRp120
  ...
  *-- Buttons <                     OK                      >
  *! B610194,1 SAB 02/04/2013 Fix error when running the report from request builder [Start]
  *=gfModalGen('TRM40136B40011','ALERT')
  IF TYPE('lcXMLFileName') <> 'C'    
    =gfModalGen('TRM40136B40011','ALERT')
  ENDIF
  *! B610194,1 SAB 02/04/2013 Fix error when running the report from request builder [End]
  RETURN
ENDIF
...
IF !FILE('&lcPath.ARSTMSG.MEM')
  ...
  *IF gfModalGen('QRM40157B40003','DIALOG') = 2
  IF TYPE('lcXMLFileName') <> 'C' .AND. gfModalGen('QRM40157B40003','DIALOG') = 2
  *! N000683,1 SAB 11/12/2012 Append Request Builder Changes [End]
    RETURN
  ELSE
...
IF TYPE('lcXMLFileName') <> 'C' .AND. RIGHT(lcFormName,1)= "B" AND lcRepMode = "Text"
*! B610194,1 SAB 02/04/2013 Fix error when running the report from request builder [End]
  lcAliasB = SELECT(0)
  IF USED(lcTmpTrn_A)
    USE IN (lcTmpTrn_A)
...
IF !EMPTY(lcRpFactor) AND !SEEK(lcRpFactor,'SycFact')
  ...
  *!*    lcBrFields  = [cFacCode:H='Factor',cFacComp:H='Name',cFacCont:H='Contact',cPhoneNo :P= gfPhoneTem() :H='Phone']
  lcBrFields  = [cFacCode:H=LANG_Arcstm_Factr,cFacComp:H=LANG_Arcstm_Name,cFacCont:H=LANG_Arcstm_Cntct,cPhoneNo :P= gfPhoneTem() :H=LANG_Arcstm_Phone]
  SELECT SycFact
