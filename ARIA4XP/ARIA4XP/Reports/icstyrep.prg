...
*: B608166,1 SSH            b) you should not be checkign the totstk figure anyway -but each size - so that if there is a positve/negative figure in any size then the style shoudl be included in the report - even if the total stock is zero (for example in this case there are postive 3 in one location and negative 3 in another location for different sizes) - so if I had selected both locations and print zero inventory was set to No - both lines should have printed
