...
*!* E302688,1 MMT 08/15/2010 Fix bug of Error when Program run from RB[T20100512.0026]
*:***************************************************************************
...
STORE .F. TO  lcToReb, lcCompCode, llPurge, llOgForAut, llAutoBal
PRIVATE loAgent
...
*!* E302688,1 MMT 08/15/2010 Fix bug of Error when Program run from RB[Start]
*loAgent.UpdateObjectProgress(lcRequestID, loProgress, ClientID)
*!* E302688,1 MMT 08/15/2010 Fix bug of Error when Program run from RB[End]
*T20100512.0026 Hassan 2010 05 23 [END]
...
SELECT (lcRebalHdr)
  ...
  SCAN
	 IF TYPE(&lcRebalHdr..cItemName)<> 'U'
	  x = "m."+ &lcRebalHdr..cItemName
	  replace &lcRebalHdr..cUpdVryIgn WITH EVALUATE(x) 
	 ENDIF 
  ENDSCAN
 
*=lfConvertToArray(lcRpCmpExp ,@laRpCmpCod1)
...
*!*	CREATE TABLE compcode(compcod c (20))
*!*	APPEND FROM ARRAY laRpCmpCod1
*!*	SELECT compcode 
*!*	i = 0
*!*	 SCAN 
*!*	 i = i+1
*!*	   SELECT SYCCOMP 
*!*	    SCAN FOR SYCCOMP.CCOMP_ID = ALLTRIM(compcode.compcod)
*!*	        laRpCmpCod [i,1]= ccomp_id+" - "+cCom_Name        
*!*	        laRpCmpCod [i,2]= cCom_dDir
*!*	        laRpCmpCod [i,3]= mModlSet
*!*	    ENDSCAN 
*!*	ENDSCAN  
*=lfConvertToArray(lcRpModExp ,@laRpCodMod)
...
*!*	  
  ...
  IF  .T. &&!EMPTY(llExpr1) AND IIF(TYPE('llExpr1') = 'C', llExpr1 # '.F.', .T.)
...
INDEX ON  cItemName TAG ItemHdr  
...
*B608487,1 WAM 03/19/2008 (End)
 ...
 IF MOD(lnCurrent ,CEILING(lnUpThermo / 10)) = 0
    ...
    loProgress.Description = "Company "+ lcCurrComp_ID +' '+IIF(UPPER(ALLTRIM(&lcRebalHdr..cTempName)) = LCMFCT,LANG_SMREBAL_CTNO,lcKeyVal)+ ' : ' + ALLTRIM(&lcKeyVal)
    *T20100512.0026 Hassan 2010 05 23 [BEGIN]
    *loAgent.UpdateObjectProgress(lcRequestID, loProgress)
    loAgent.UpdateObjectProgress(lcRequestID, loProgress, CLIENTID)    
    *T20100512.0026 Hassan 2010 05 23 [END]
 ...
 ***MOS** 
 ** oProgress.CurrentProgress(lnCurrent)
   *loProgress.Percent     = lnCurrent / RECCOUNT()
   * loProgress.Description = lcKeyVal + ' : ' + ALLTRIM(&lcKeyVal)
   * loAgent.UpdateObjectProgress(lcRequestID, loProgress)
  ***MOS**
  *B608487,1 WAM 03/19/2008 Enhance performance of rebalance (Commented out)
...
IF FILE(oAriaApplication.WorkDir + ALLTRIM(&lcRebalHdr..cTempName) + '.DBF')
  ...
  SCAN
    ...
    ***MOS**
    IF MOD(lnCurrent ,CEILING(lnUpThermo / 10)) = 0
      loProgress.Percent     = lnCurrent / lnUpThermo
      loProgress.Description = "Company "+ lcCurrComp_ID +' '+lcKeyVal + ' : ' + ALLTRIM(&lcKeyVal)
      *T20100512.0026 Hassan 2010 05 23 [BEGIN]
      *loAgent.UpdateObjectProgress(lcRequestID, loProgress)
      loAgent.UpdateObjectProgress(lcRequestID, loProgress, CLIENTID)      
      *T20100512.0026 Hassan 2010 05 23 [END]
...
oAriaEnvironment.setups.getSetting(@laSetups,lcCurrComp_ID)
...
*!*	  *B128464,2 WSH 08/21/2005 [End]
  ...
  *B608487,1 WAM 03/19/2008 (End)
ENDIF
lcTermoMsg = 'Company ' + lcCurrComp_ID + " " + ALLTRIM(gfItemMask('HM', lcFilePath, lcInvType)) + ":  "
    ...
    IF !llScanInFilt
      ...
      ***MOS**
      IF MOD(lnThermNo ,CEILING(lnUpThermo / 10)) = 0
        ...
        loProgress.Description = "Company "+ lcCurrComp_ID +IIF(lcInvType = '0001',' Style:',' Fabric:')+Style
    *T20100512.0026 Hassan 2010 05 23 [BEGIN]
    *loAgent.UpdateObjectProgress(lcRequestID, loProgress)
    loAgent.UpdateObjectProgress(lcRequestID, loProgress, CLIENTID)    
    *T20100512.0026 Hassan 2010 05 23 [END]
...
INDEX ON Style+cWareCode+Dyelot TAG (lcTmpDyeFl) 
*B609356,1 SMA 07/29/2010 remove of clause to prevent empty *.cdx files from creation.....[END]
...
*!** DEBUG*
IF TYPE("lcTableName")="C".AND.lctablename="ORDHDR"
...
lcTagName    = IIF(TYPE('lcTagName') = 'C', PADR(ALLTRIM(UPPER(lcTagName)), oAriaApplication.FieldW), '')
*E303030,1 END
  ...
  ELSE
    ...
    *lcFileNam = PADR(ALLTRIM(UPPER(lcFileNam)), 8)
    ...
    lcFileNam = PADR(ALLTRIM(UPPER(lcFileNam)), oAriaApplication.FileW)
  	lcTagNam     = IIF(TYPE('lcTagNam ') = 'C', PADR(ALLTRIM(UPPER(lcTagNam)), oAriaApplication.FieldW), '')
    *E303030,1 END
...
*!*************************************************************
*! Name      : lfConvertToArray
*: Developer : Mostafa Eid(MOS)
*: Date      : 07/02/2008
*! Purpose   : Convert a list of values into a array
*!*************************************************************
*!*	FUNCTION lfConvertToArray
*!*	PARAMETERS lcStrToConv,laArrayName 

*!*	DECLARE aaaa[1,1]
*!*	=gfSubStr(lcRpCmpExp, @aaaa, ',')

*!*	lcValuesToConvert = lcStrToConv

*!*	IF AT(',',lcValuesToConvert )> 0
*!*	lcValuesToConvert = STRTRAN(lcValuesToConvert,',','|')
*!*	ENDIF 
...
*!*	    IF lnEnd <> 0
*!*	          DO WHILE lnEnd <> 0
*!*		        FOR n =1 TO ALEN(laArrayName,2)
*!*		             laArrayName [1,n]= SUBSTR(lcValuesToConvert,lnStart,lnEnd-1)
*!*		             lcValuesToConvert = STUFF(lcValuesToConvert ,lnStart,lnEnd,"") 
*!*		             lnEnd=AT('|',lcValuesToConvert )
*!*		               IF lnEnd = 0
*!*		                 laArrayName [1,n+1]= lcValuesToConvert 
*!*		               EXIT 
*!*		               ENDIF  
*!*		   	    ENDFOR 
*!*	        ENDDO 
*!*	    ELSE 
*!*	        laArrayName [1,1]= lcValuesToConvert
*!*	    ENDIF 
*!*	ENDIF 
*!*	RETURN .T.