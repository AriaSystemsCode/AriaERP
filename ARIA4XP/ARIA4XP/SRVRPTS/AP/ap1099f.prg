 ...
 * =gfOpenTABLE('APVENDOR','VENCODE','SH')
  oAriaEnvironment.remotetableaccess.OpenTable(oAriaEnvironment.DataDir + 'APVENDOR','VENCODE','SH',.F.,.F.)
  SELECT APVENDOR
 * =gfSeek('')
...
*  =gfOpenTABLE('APSETUP','APSETUP','SH')
  oAriaEnvironment.remotetableaccess.OpenTable(oAriaEnvironment.DataDir + 'APSETUP','APSETUP','SH',.F.,.F.)
  SELECT APSETUP
 * =gfSeek('')
  oAriaEnvironment.remotetableaccess.SeekRecord('')
ENDIF
...
oAriaEnvironment.Cursors.createcursor(lcTempFile,@laTempStru,"CVENDCODE",lcTempFile ,.F.)
*--End of function
*!******************************************************************************
*! Name         : lfCler1009
...
*!*	IF lnClear = 1
  ...
  SCAN FOR EMPTY(APVENDOR.cven1099t)= .F. AND APVENDOR.nven1099B >= 0.01 = .T.
    ...
    *=gfTableUpdate(.T.,'APVENDOR') 
    oAriaEnvironment.remotetableaccess.TableUpdate(.T.,'APVENDOR')  
  ENDSCAN   
*!*	ELSE   
*!*	 * SCAN FOR EMPTY(APVENDOR.cven1099t)= .F. AND APVENDOR.nven1099B >= 0.01 = .T.
*!*	    SELECT APVENDOR
...
oAriaEnvironment.remotetableaccess.SeekRecord(oAriaApplication.ActiveCompanyID)
*AHS
