// Decompiled with JetBrains decompiler
// Type: UPS.UPSShip.TransportationChargeType
// Assembly: UPS, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: FE2C527D-E7B1-4D14-9A59-DD5F5EB261A9
// Assembly location: D:\Aria4xp\DLLS\UPS.dll

using System;
using System.CodeDom.Compiler;
using System.ComponentModel;
using System.Diagnostics;
using System.Xml.Serialization;


namespace UPS.UPSShip
{
    [GeneratedCode("System.Xml", "4.6.1087.0")]
    [DebuggerStepThrough]
    [XmlType(Namespace = "http://www.ups.com/XMLSchema/XOLTWS/Ship/v1.0")]
    [DesignerCategory("code")]
    [Serializable]
    public class TransportationChargeType
    {
      private ShipChargeType grossChargeField;
      private ShipChargeType discountAmountField;
      private string discountPercentageField;
      private ShipChargeType netChargeField;

      public ShipChargeType GrossCharge
      {
        get => this.grossChargeField;
        set => this.grossChargeField = value;
      }

      public ShipChargeType DiscountAmount
      {
        get => this.discountAmountField;
        set => this.discountAmountField = value;
      }

      public string DiscountPercentage
      {
        get => this.discountPercentageField;
        set => this.discountPercentageField = value;
      }

      public ShipChargeType NetCharge
      {
        get => this.netChargeField;
        set => this.netChargeField = value;
      }
    }
}
