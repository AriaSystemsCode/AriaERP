// Decompiled with JetBrains decompiler
// Type: UPS.UPSTrack.ActivityLocationType
// Assembly: UPS, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: FE2C527D-E7B1-4D14-9A59-DD5F5EB261A9
// Assembly location: D:\Aria4xp\DLLS\UPS.dll

using System;
using System.CodeDom.Compiler;
using System.ComponentModel;
using System.Diagnostics;
using System.Xml.Serialization;


namespace UPS.UPSTrack
{
    [XmlType(Namespace = "http://www.ups.com/XMLSchema/XOLTWS/Track/v1.1")]
    [DesignerCategory("code")]
    [GeneratedCode("System.Xml", "4.6.1087.0")]
    [DebuggerStepThrough]
    [Serializable]
    public class ActivityLocationType
    {
      private AddressType addressField;
      private string codeField;
      private string descriptionField;
      private string signedForByNameField;
      private SignatureImageType signatureImageField;
      private PODLetterType pODLetterField;

      public AddressType Address
      {
        get => this.addressField;
        set => this.addressField = value;
      }

      public string Code
      {
        get => this.codeField;
        set => this.codeField = value;
      }

      public string Description
      {
        get => this.descriptionField;
        set => this.descriptionField = value;
      }

      public string SignedForByName
      {
        get => this.signedForByNameField;
        set => this.signedForByNameField = value;
      }

      public SignatureImageType SignatureImage
      {
        get => this.signatureImageField;
        set => this.signatureImageField = value;
      }

      public PODLetterType PODLetter
      {
        get => this.pODLetterField;
        set => this.pODLetterField = value;
      }
    }
}
