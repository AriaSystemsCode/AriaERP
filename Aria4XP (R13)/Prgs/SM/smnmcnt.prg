...
#INCLUDE R:\ARIA4XP\PRGS\SM\SMNMCNT.H
...
*lcBrowTitl  = LANG_Sequences
lcBrowTitl  = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Sequences,loFormSet.GetHeaderText("LANG_Sequences",loFormSet.HeaderAlias))
...
*N000682,1 11/20/2012 MMT Globlization changes[Start]
*lfSetColumnsProp('1',"&lcTmpSeq..cfld_head",LANG_Transaction ,380 ,oGrd)
lfSetColumnsProp('1',"&lcTmpSeq..cfld_head",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Transaction,loFormSet.GetHeaderText("LANG_Transaction",loFormSet.HeaderAlias)) ,380 ,oGrd)
*N000682,1 11/20/2012 MMT Globlization changes[End]

*N000682,1 11/20/2012 MMT Globlization changes[Start]
*lfSetColumnsProp('2',"&lcTmpSeq..nSeq_no",LANG_Sequence_Num  ,100 ,oGrd)
lfSetColumnsProp('2',"&lcTmpSeq..nSeq_no",IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_Sequence_Num,loFormSet.GetHeaderText("LANG_Sequence_Num",loFormSet.HeaderAlias))  ,100 ,oGrd)
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
IF loFormSet.ActiveMode = 'S'
  ...
  *: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][Start]
  IF TYPE('loFormSet.llDivBased')  ='L' AND loFormSet.llDivBased 
    loFormSet.oToolbar.cmdSelect.Enabled = .F.
    loFormSet.oToolbar.cmdAdd.Enabled = .T.
    loFormSet.oToolbar.cmdAdd.ControlEnable = 1
...
DO CASE
  CASE loFormSet.ActiveMode = 'S'  && Select Mode
    ...
    *: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][Start]
  	ELSE    
      loFormSet.Ariaform1.laCompany.Value = oAriaApplication.ActiveCompanyID
	  =lfvCompany(loFormset,loFormSet.Ariaform1.laCompany)
      IF TYPE('loFormSet.llDivBased')  ='L' AND loFormSet.llDivBased 
        loFormSet.oToolbar.cmdSelect.Enabled = .F.
        loFormSet.oToolbar.cmdAdd.Enabled = .T.
        loFormSet.oToolbar.cmdAdd.ControlEnable = 1
 ...
 *: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][Start]
 CASE loFormSet.ActiveMode = 'A' && Add Mode
    ...
    *STORE 'DISABLE' TO lcCompStat,lcDivStat
    STORE 'DISABLE' TO lcCompStat,loFormSet.lcDivStat
    loFormSet.Ariaform1.laCompany.Enabled = .F.
    loFormSet.Ariaform1.laDivArray.Enabled = .F.
    IF ALEN(loFormSet.laDivArray,1) = 2
      lcCurrGrp = '001'
    ELSE
      lcCurrGrp = PADL(ALLTRIM(STR(VAL(loFormSet.laDivArray[ALEN(loFormSet.laDivArray,1),1])+1)),3,'0')
    ENDIF
   
    DECLARE loFormSet.laDivArray[ALEN(loFormSet.laDivArray,1) + 1 , 1] 
    loFormSet.laDivArray[ALEN(loFormSet.laDivArray,1),1] = lcCurrGrp

    loFormSet.lnGroup   = ALEN(loFormSet.laDivArray,1)
    loFormSet.Ariaform1.laDivArray.ReQuery()
    loFormSet.Ariaform1.laDivArray.ListIndex = loFormSet.lnGroup
    *SHOW GET lnCompany &lcCompStat
    *SHOW GET lnGroup &lcDivStat
    
    = lfFillData(loFormSet,.T.)
    lfSetGridDataSource(loFormSet)
    loFormSet.llNewGrp= loFormSet.llDivBased

    loFormSet.llNewMode = loFormSet.llNewGrp
    loFormSet.Ariaform1.grdSMNMCNT.refresh()
    loFormSet.Ariaform1.grdSMNMCNT.Readonly = .F.
    loFormSet.Ariaform1.grdSMNMCNT.Column1.Readonly = .T.
    loFormSet.Ariaform1.grdSMNMCNT.Column2.Readonly = .F.
  *: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][End]
...
IF !loFormSet.llDivBased OR (loFld.Value <> loFld.OldValue)
  *: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][Start]
  loFormSet.lnGroup = loFormSet.Ariaform1.laDivArray.ListIndex  
  *: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][End]
   = lfFillData(loFormSet)
  loFormSet.llFromEdit = .F.
...
ENDIF
*: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][Start]
loFormSet.oToolbar.cmdDelete.Enabled = loFormSet.llDivBased 
*: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][End]
...
SELECT (loFormSet.lcTmpSeq)
*: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][Start]
lcFltrExp = "'"+loFormSet.lcCompCode+loFormSet.laDivArray[loFormSet.lnGroup,1]+"'"
SET FILTER TO cComp_Id + cSeq_Group + cFld_Name = &lcFltrExp.
*: B611277,1 MMT 03/08/2017 Sequence screen does not work properly with division groups[T20170206.0016][End]
LOCATE
