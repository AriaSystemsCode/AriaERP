...
*!*	    USE &gcSysHome.sydfield ORDER TAG 'Cfld_name' IN 0 SHARED ALIAS Aydfield
  ...
  *= SEEK(PADR(lcPropFld,10),'sydfield')
  = SEEK(PADR(lcPropFld,oAriaApplication.FieldW),'Aydfield')
  SELECT Aydfield
  LOCATE REST WHILE CFLD_NAME = PADR(lcPropFld,oAriaApplication.FieldW) FOR (CVER = 'A27' OR EMPTY(CVER))
...
*!*	  LOCATE REST WHILE cFld_Name=PADR(lcPropFld,10) FOR lEnumerate
  = SEEK(PADR(lcPropFld,oAriaApplication.FieldW))
...
*!*	      USE &gcSysHome.sydfiles ORDER TAG 'Cfile_nam' IN 0 SHARED
    ...
    *N000682,1 MMT 03/06/2013 Merge SysFiles and SqlDictionary folder[Start]
    SELECT sydfiles
    LOCATE REST WHILE CFILE_NAM = sydflfld.cFile_Nam FOR CVER = 'A27'
    *N000682,1 MMT 03/06/2013 Merge SysFiles and SqlDictionary folder[END]
    SELECT SEQUENCE
