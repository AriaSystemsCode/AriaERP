...
IF SEEK(lcDbfName)
  ...
  IF !EMPTY(lcBrFields)
    ...
    *N000682,1 MMT 12/09/2012 Globalization changes[ENDIF]
    ...
    *N000682,1 MMT 12/09/2012 Globalization changes[Start]
    ELSE
      FOR lnCount=1 TO ALEN(laHead,1)
        laHead[lnCount] = UPPER(ALLTRIM(laHead[lnCount]))
        IF SEEK (laHead[lnCount],"sydfield")
          lcFldHead = oAriaApplication.geta27fldheader (ALLTRIM(laHead[lnCount]))&&,SET("Datasession"),"sydfield")
          lcBrFields=lcBrFields+laHead[lnCount]+;
                     IIF(lvldEntry,'=LOOKUP(CODES.cDiscrep,'+['N'+]+lcDbfName;
                     +'.'+laHead[lnCount]+',CODES.cCode_No,"CODES")';
                     ,'')+IIF(EMPTY(sydfield.cFld_Head),' ,'," :H='"+;
                     IIF(EMPTY(lcFldHead),ALLTRIM(sydfield.cFld_Head),lcFldHead)+"',")
        ENDIF
      endfor     
    ENDIF
...
*                ALLTRIM(sydField.cFld_Head)+"',")
        ...
        *N000682,1 MMT 12/09/2012 Globalization changes[Start]
        ELSE
          lcFldHead = oAriaApplication.geta27fldheader (ALLTRIM(sydFlFld.cFld_Name))&&,SET("Datasession"),'sydFlFld')        
          lcBrFields=lcBrFields+ALLTRIM(sydFlFld.cFld_Name)+;
               IIF(sydField.lvldEntry,'=LOOKUP(CODES.cDiscrep,'+['N'+]+lcDbfName;
               +'.'+ALLTRIM(sydFlFld.cFld_Name)+',CODES.cCode_No,"CODES")';
               ,'')+IIF(EMPTY(sydField.cFld_Head),' ,'," :H='"+;
...
lcAriaHFile = ''
IF oAriaApplication.oActivelang.cLang_ID <> "EN"
  lcAriaHFile =oAriaApplication.GetClassHeaderFile(ADDBS(UPPER(ALLTRIM(oAriaApplication.LangPath))) + "PRGS\AP\" + ALLTRIM("APPYINV")+"_"+"H" +".XML")
ENDIF
lcFile_Ttl = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_VENDORS,oAriaApplication.GetHeaderText("LANG_APPYINV_VENDORS",lcAriaHFile))
lcBrFields = "CVENDCODE :H= '"+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_VENDORCODE,oAriaApplication.GetHeaderText("LANG_APPYINV_VENDORCODE",lcAriaHFile))+"',;
              CVENCOMP :H= '"+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_COMPANY,oAriaApplication.GetHeaderText("LANG_APPYINV_COMPANY",lcAriaHFile))+"',;
              CPHONENO :H= '"+IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_PHONE,oAriaApplication.GetHeaderText("LANG_APPYINV_PHONE",lcAriaHFile))+"'"
*N000682,1 MMT 12/09/2012 Globalization changes[END]
  ...
  IF llBrowse .OR. !SEEK(loFld.KeyTextBox.VALUE) .OR. ATC("?",loFld.KeyTextBox.VALUE) > 0  &&lower
    ...
    IF llBrowse .OR. ATC("?",loFld.KeyTextBox.VALUE) > 0
      ...
      *=gfBrows(' ','CVENDCODE,CPHONENO,CVENCOMP','laTemp','Vendors')      
      =gfBrows(' ','CVENDCODE,CPHONENO,CVENCOMP','laTemp',IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_VENDORS,oAriaApplication.GetHeaderText("LANG_APPYINV_VENDORS",lcAriaHFile)))
      *N000682,1 MMT 12/09/2012 Globalization changes[END]
      IF !EMPTY(laTemp[1])
    ...
    ELSE
      ...
      *lcTVenKey = IIF(lcType='V','Vendor Code',IIF(lcType='P','Phone','Vendor/Company name'))      
      lcTVenKey = IIF(lcType='V',;
       IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_VENDORCODE,oAriaApplication.GetHeaderText("LANG_APPYINV_VENDORCODE",lcAriaHFile)),IIF(lcType='P',;
       IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_PHONE,oAriaApplication.GetHeaderText("LANG_APPYINV_PHONE",lcAriaHFile)),;
       IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_APPYINV_VENDDCOMPNAME,oAriaApplication.GetHeaderText("LANG_APPYINV_VENDDCOMPNAME",lcAriaHFile))))
      *N000682,1 MMT 12/09/2012 Globalization changes[END]
  ...
  *Old: SHOW GET (lcFactObj) &&Revise
  ...
  IF llBrowse .OR. !SEEK(lcPrFactor,'SYCFACT')
    ...
    *=gfBrows(.F.,'cFacCode','laTemp')
    lcFile_Ttl = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",lcFile_Ttl ,oAriaApplication.GetHeaderText("LANG_APPYINV_FACTORS",lcAriaHFile))
