...
*! B609740,1 MMT 11/23/2011 Material conversion factor does not allow a value between 1.000 and 2.000[T20111025.0005]
...
* E303079,1 MMT 03/21/2012 Media changes[T20120304.0004]
* E303855,1 MMT 07/31/2017 Applying Aria5 new Framework - Decorator[T20170706.0015]
* B612144,1 MMT 05/18/2020 Show loading icon while opening Aria4XP till login screen is opened and just when aria4xp shortcut clicked[T20200429.0002]
*:************************************************************************
...
*_SCREEN.Caption = "Aria 4 XP"
*N000682,1 11/20/2012 MMT Globlization changes[Start]
*_SCREEN.Caption =LANG_ARIA_ARIA_4_XP
*N000682,1 MMT 03/06/2013 Merge SysFiles and SqlDictionary folder[Start]
*_SCREEN.Caption =IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARIA_ARIA_4_XP,loFormSet.GetHeaderText("LANG_ARIA_ARIA_4_XP",loFormSet.HeaderAlias))
_SCREEN.Caption = "Aria 4 XP"
...
*B612144,1 MMT 05/18/2020 Show loading icon while opening Aria4XP till login screen is opened and just when aria4xp shortcut clicked[T20200429.0002][Start]
IF FILE(FULLPATH('')+"Aria5ERP\Screens\SY\SYLoading.scx")
  PUBLIC goLoadingForm 
  DO FORM (FULLPATH('')+"Aria5ERP\Screens\SY\SYLoading.scx") NAME goLoadingForm 
ENDIF  
*B612144,1 MMT 05/18/2020 Show loading icon while opening Aria4XP till login screen is opened and just when aria4xp shortcut clicked[T20200429.0002][End]
...
lcAriaMainPath = "SET PROCEDURE TO " + lcCurPath + "\PRGS\SY\ariaglb.fxp,"+lcCurPath + "\Aria5ERP\PRGS\SY\ariaglb5.fxp ADDITIVE "
...
* E303855,1 MMT 07/31/2017 Applying Aria5 new Framework - Decorator[T20170706.0015][Start]
lcClasPath  = lcCurPath + "\Aria5ERP\CLASSES\"
lnClasses   = ADIR(laClasses, lcClasPath + "*.*", "D")
FOR lnClass = 1 TO lnClasses
  IF "D" $ laClasses[lnClass,5] AND !INLIST(laClasses[lnClass,1], ".", "..")
    = lpSetLibs (lcClasPath + laClasses[lnClass,1])
...
oAriaApplication = CREATEOBJECT("AriaApplication54",IIF(TYPE('lcSysFiles')='C',lcSysFiles,.F.))
* E303855,1 MMT 07/31/2017 Applying Aria5 new Framework - Decorator[T20170706.0015][End]
...
*!*	* E303079,1 MMT 03/21/2012 Media changes[T20120304.0004][Start]
*!*	LOCAL lcPath
...
*! B128052,1 MAH 06/05/2005 [END]
*!*	*E303079,1 MMT 03/21/2012 Media changes[T20120304.0004][END]
...
*!*	  *E302692,1 [T20100418.0003] Split the Aria.exe TMI 5/10/2010 [ END ] comment out this part as all classes will be excluded ( note, building aria.exe used the commented version, pls comment when
...
gcPerfLogFileName = lcPath + "\SysFiles\SYPREFLG.dbf"
*N000682,1 MMT 03/06/2013 Merge SysFiles and SqlDictionary folder[END]
