...
CASE m.g_grph2char
   ...
   IF FOUND()
      ...
      m.wndstyle = fontstyle
   ELSE
      m.wndface  = m.g_ctrlfface    && Courier New for Windows
...
CASE m.g_char2grph OR m.g_grph2grph
   ...
   CASE objtype = c_otheader
		...
		CASE commonfont(fontface)
			* Original measurement font was Arial, Courier New, etc.  Leave it
			* alone.
		OTHERWISE
			* Use the defaults
	...
	* Set fonts based on 2D/3D choice--Mac only
	...
	ELSE
		...
		* The cxChar for Geneva, 10 nonbold is 6 pixels, just like
 		* Courier New,8 bold.  This is a good mapping for screens coming
		* over from Windows.
...
OTHERWISE
   * Font for push buttons
   m.g_ctrlfface        = "Courier New"
   m.g_ctrlfsize        = 10
   m.g_ctrlfstyle       = ""

	* Window measurement font
   m.g_windfface        = "Courier New"
   m.g_windfsize        = 10
   m.g_windfstyle       = ""
...
CASE m.g_grph2char AND UPPER(m.ofontface) == "Courier New" AND ;
      UPPER(m.bfontface) == "Courier New" AND ;
      m.ofontsize = m.bfontsize AND ;
      !("B" $ m.ofontstyle) AND ;
      "B" $ m.bfontstyle
   * We can't use FONTMETRIC on DOS, so we use heuristics instead.  Most
   * of the time we will be converting between Courier New 8 Bold and
   * Courier New Regular.  If that is the case here, use the 5/6 conversion
...
* Courier New      5x13   6x13	 5x13   6x13   7x16   8x16    8x20
