...
***            Requesthandler.exe "config"
***            Requesthandler.exe "config" "IIS://localhost/w3svc/21/root"
************************************************************************
...
IF VARTYPE(lcAction) = "C" AND StartsWith(LOWER(lcAction),"config")
         do Requesthandler_ServerConfig.prg with lvParm1
      RETURN 
ENDIF

*** Load the server - wc3DemoServer class below
goWCServer = CREATE("RequesthandlerServer")
...
IF llAutoRestart
   LiveReloadShutdown("DO RequesthandlerMain.prg")
ELSE
...
**************************************************************
DEFINE CLASS RequesthandlerServer AS WWC_SERVER OLEPUBLIC
*************************************************************
...
*** THIS.cAppIniFile = addbs(THIS.cAppStartPath) + "Requesthandler.ini"

THIS.cAppName = "Requesthandler"
THIS.cAppIniFile = addbs(THIS.cAppStartPath) + "Requesthandler.ini"
...
THIS.oConfig = CREATEOBJECT("RequesthandlerConfig")
...
************************************************************************
*  RequesthandlerServer :: OnInitCompleted
****************************************
...
DO CASE
     ...
     CASE lcParameter == "REQUESTHANDLERCLASS"
         DO RequestHandlerClass with THIS
...
***************************************************************
DEFINE CLASS RequesthandlerConfig AS wwServerConfig
***************************************************************
*: Author: Rick Strahl
...
THIS.oRequestHandlerClass = CREATEOBJECT("RequestHandlerClassConfig")
...
ENDDEFINE
*EOC RequesthandlerConfig


DEFINE CLASS wwMaintConfig as RELATION

cHTMLPagePath = "c:\WebConnectionProjects\RequestHandler\Web\"
cDATAPath = ".\"
cVirtualPath = "/RequestHandler/"

ENDDEFINE



*** Configuration class for the RequestHandlerClass Process class
DEFINE CLASS RequestHandlerClassConfig as wwConfig
