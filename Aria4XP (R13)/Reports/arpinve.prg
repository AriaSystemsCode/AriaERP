...
*:         Author: Tarek Noaman (TNA)
*:  Documented   : 05/16/2006
*:  Modifications:
*:************************************************************************
...
STORE TRIM(laCompAdd[3])+' '+laCompAdd[4]+' '+laCompAdd[5]+' '+laCompAdd[6] TO HLINE4
...
DECLARE loOGScroll.laCRParams[15,2]
...
loOGScroll.laCRParams[14,1] = 'MSG1'
loOGScroll.laCRParams[14,2] = MSG1
loOGScroll.laCRParams[15,1] = 'MSG2'
loOGScroll.laCRParams[15,2] = MSG2
...
*WAIT WINDOW LANG_ARPINV_NORECSEL
WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ARPINV_NORECSEL,oAriaApplication.GetHeaderText("LANG_ARPINV_NORECSEL",AHEADERFILE))
...
SCAN FOR &lcASExp   WHILE INKEY() <> 32
   ...
   m.CARTONS  = INVHDR->CARTONS
   m.WEIGHT   = INVHDR->WEIGHT
   m.COD      = INVHDR->COD
   m.INSUR    = INVHDR->INSUR
   IF llNote
     STORE "" TO m.MnotHd
     STORE "F" TO m.hasnot
     SELECT NOTEPAD
...
SELECT(lcHeader)
*COPY TO oAriaApplication.WorkDir +  "Header.DBF"
COPY TO oAriaApplication.WorkDir +  lcHeader1  + ".DBF"
SELECT(lcLines)
*COPY TO oAriaApplication.WorkDir +  "Lines.DBF"
