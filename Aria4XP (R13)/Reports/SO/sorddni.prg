...
#include r:\aria4xp\reports\so\sorddni.h
...
*  WAIT WINDOW 'Selected ' + ALLTRIM(STR(RECCOUNT(lcMastFile))) + ' Records in ' + ALLTRIM(STR(lnInterval,6,2)) + ' Seconds...' NOWAIT
  WAIT WINDOW LANG_Selected + ALLTRIM(STR(RECCOUNT(lcMastFile))) + LANG_Records_in + ALLTRIM(STR(lnInterval,6,2)) + lANG_Seconds NOWAIT
...
*  WAIT WINDOW 'Selected ' + ALLTRIM(STR(RECCOUNT(lcMastFile))) + ' Records in ' + ALLTRIM(STR(lnInterval,6,2)) + ' Seconds...' TIMEOUT 1
  WAIT WINDOW LANG_Selected + ALLTRIM(STR(RECCOUNT(lcMastFile))) + LANG_Records_in + ALLTRIM(STR(lnInterval,6,2)) + lANG_Seconds TIMEOUT 1
  * N000682 ,1 Thabet Handle globalization issues [END]
ENDIF  
...
IF !FILE(gcDataDir+'PACKINDX.IDX') .OR. llRpUpPInd
  ...
  IF lnResp = 1
    ...
    *WAIT WINDOW NOWAIT 'Creating/Updating temp. index for Packs in OrdLine file..'
    WAIT WINDOW NOWAIT LANG_Creating_Updating_temp
    * N000682 ,1 Thabet Handle globalization issues [END]
    SELECT ORDLINE
    lcErrHndl = ON("ERROR")
    ON ERROR llErr = .T.
