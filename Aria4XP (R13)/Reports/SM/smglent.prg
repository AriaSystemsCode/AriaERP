...
*:************************************************************************
*: Modifications:
*: E303960,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL
*: B611728,1 Es 03/04/2019 Modify issue "Gl Entry don't filter data according to selected Batch" [T20190215.0004]
*: B611861,1 Es 08/04/2019  GL Entries report displays incorrect data when user filters by Transaction [T20190722.0002]
*:************************************************************************
...
=AFIELDS(laFileStru)
=gfCrtTmp(GLTEMP,@laFileStru)

lcWhereCond = ''
ldDateStart = {}
ldDateEnd = {}
lnDatePos = ASCAN(loogscroll.laOGFXFlt,"GLDIST.TRAN_DATE")
IF lnDatePos > 0
...
IF !EMPTY(ldDateStart) AND !EMPTY(ldDateEnd)
  lcWhereCond = "GLDIST.TRAN_DATE BETWEEN '"+DTOS(ldDateStart)+"' AND '"+DTOS(ldDateEnd)+"'"
ENDIF  
lnStTrn = ''
lnEndTrn = ''
lnTranPos = ASCAN(loogscroll.laOGFXFlt,"GLDIST.TRAN_NO")
IF lnTranPos > 0
...
IF !EMPTY(lnStTrn) AND !EMPTY(lnEndTrn)
  lcWhereCond = lcWhereCond +IIF(!EMPTY(lcWhereCond),' AND ','')+"GLDIST.TRAN_NO BETWEEN '"+lnStTrn+"' AND '"+lnEndTrn+"'"
ENDIF 
lcTranType = ''
lnTranTyPos = ASCAN(loogscroll.laOGFXFlt,"GLDIST.TRAN_TYPE")
IF lnTranPos > 0
  lnTranTyPos= ASUBSCRIPT(loogscroll.laOGFXFlt,lnTranTyPos,1)
  IF !EMPTY(loogscroll.laOGFXFlt[lnTranTyPos,6])
    lcTranType = "'"+STRTRAN(loogscroll.laOGFXFlt[lnTranTyPos,6],"|","','")+"'"
  ENDIF
ENDIF
IF !EMPTY(lcTranType)
  lcWhereCond = lcWhereCond +IIF(!EMPTY(lcWhereCond),' AND ','')+"GLDIST.TRAN_TYPE IN  ("+lcTranType+")"
ENDIF 
...
*B611728,1 Es 03/04/2019 Modify issue "Gl Entry don't filter data according to selected Batch" [Start]
*IF lcRPBatch $'UB'

*B611861,1 Es 08/04/2019  GL Entries report displays incorrect data when user filters by Transaction [Start]
 *IF lcRPBatch $'PB'
*B611728,1 Es 03/04/2019 Modify issue "Gl Entry don't filter data according to selected Batch" [End]  
*lcWhereCond = lcWhereCond +IIF(!EMPTY(lcWhereCond),' AND ','')+ IIF(lcRPBatch='P',"GLDIST.Posted='X'","GLDIST.Posted=' '")
IF lcRPBatch $'UP'
  lcWhereCond = lcWhereCond +IIF(!EMPTY(lcWhereCond),' AND ','')+ IIF(lcRPBatch='P',"GLDIST.Posted='X'","GLDIST.Posted<>'X'")
ENDIF 
IF lcRPBatch $'PB'
*B611861,1 Es 08/04/2019  GL Entries report displays incorrect data when user filters by Transaction [End]

  lnGLTranTyPos = ASCAN(loogscroll.laOGFXFlt,"GLDIST.CTRNSLEDN")
  IF lnGLTranTyPos > 0
  ...
  ENDIF
  IF !EMPTY(lcGLTran)
    lcWhereCond = lcWhereCond +IIF(!EMPTY(lcWhereCond),' AND ','')+"GLDIST.CTRNSLEDN='"+lcGLTran+"'"
  ENDIF 

  
  
  lnGLBatPos = ASCAN(loogscroll.laOGFXFlt,"GLDIST.GLBATCH")
  IF lnGLBatPos> 0
  ...
  ENDIF
  IF !EMPTY(lcGLBat)
    lcWhereCond = lcWhereCond +IIF(!EMPTY(lcWhereCond),' AND ','')+"GLDIST.GLBATCH ='"+lcGLBat +"'"
  ENDIF 
ENDIF

llSelGLAcc = .F.
lcGLAccCur = ''
lnGLAccPos = ASCAN(loogscroll.laOGVRFlt,"GLDIST.GLACCOUNT")
IF lnGLAccPos > 0
  lnGLAccPos = ASUBSCRIPT(loogscroll.laOGVRFlt,lnGLAccPos ,1)
  IF !EMPTY(loogscroll.laOGVRFlt[lnGLAccPos ,6])
    IF USED(loogscroll.laOGVRFlt[lnGLAccPos ,6]) AND RECCOUNT(loogscroll.laOGVRFlt[lnGLAccPos ,6])> 0
      SELECT (loogscroll.laOGVRFlt[lnGLAccPos ,6])
      LOCATE 
      IF !EOF()
        llSelGLAcc  = .T.
        lcGLAccCur = loogscroll.laOGVRFlt[lnGLAccPos ,6]
      ENDIF
    ENDIF  
  ENDIF
ENDIF
...
IF llSelGLAcc 
  ...
  LOCATE
  SCAN
    IF gfSeek(&lcGLAccCur..cAcctCode,'GLDIST')
      SELECT GLDIST
      SCAN REST WHILE GLACCOUNT = &lcGLAccCur..cAcctCode FOR IIF(lcRPBatch='P',Posted='X',IIF(lcRPBatch='U',Posted=' ',.T.)) AND;
           IIF(!EMPTY(ldDateStart) AND !EMPTY(ldDateEnd),BETWEEN(GLDIST.TRAN_DATE,ldDateStart,ldDateEnd),.T.) AND ;
           IIF(lcRPBatch $'UB' AND !EMPTY(lcGLTran),GLDIST.CTRNSLEDN = lcGLTran,.T.) AND ;
           IIF(lcRPBatch $'UB' AND  !EMPTY(lcGLBat),GLDIST.GLBATCH = lcGLBat,.T.) AND ;
           IIF(!EMPTY(lcTranType), GLDIST.TRAN_TYPE $ lcTranType,.T.) AND ;
           IIF(!EMPTY(lnStTrn) AND !EMPTY(lnEndTrn),BETWEEN(GLDIST.TRAN_NO,lnStTrn,lnEndTrn),.T.)
        SCATTER MEMO MEMVAR 
        INSERT INTO (GLTEMP) FROM MEMVAR
      ENDSCAN
    ENDIF 
  ENDSCAN
ELSE
  IF !EMPTY(lcWhereCond) 
    =gfSQLRUN("Select * from GLDIST WHERE "+lcWhereCond,'GLDIST') 
  ELSE
    gfSeek('')
  ENDIF 
  SELECT GLDIST 
...
IF !EMPTY(lcTranVal) AND !gfSEEK(lcTranVal,'GLDIST')
*: E303960,1 AHH 1/04/2018 use the GLDIST table remotely not native, because of conversion to SQL[End]
  llNothing  = lfTranBrw()
