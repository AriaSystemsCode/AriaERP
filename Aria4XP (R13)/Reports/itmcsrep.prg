...
*: B610826,1 MMT 08/28/2014 Fix bug of priniting style component for all sizes in case of cost sheet per size[BTB10 issue]
*: B611517,1 MMT 01/24/2018 Fix error while browsing styles in Style cost sheet report
*:**********************************************************************
 ...
 *   lcStatmnt = lcStatmnt + " INNER JOIN " + lcSqlCost + " TmpCost ON BomHeadr.cItmMajor+'-'+BomHeadr.cCstSht_id = TmpCost.cCostSht "
  ...
  IF lnResult > 0
    ...
    IF oAriaApplication.ActiveModuleID <> 'MA'
      ...
      ENDIF
      *: B611517,1 MMT 01/24/2018 Fix error while browsing styles in Style cost sheet report[Start]
      IF USED('STYLE_A')
        USE IN 'STYLE_A'
	  ENDIF
      *: B611517,1 MMT 01/24/2018 Fix error while browsing styles in Style cost sheet report[End]
    ELSE
...
lcRptName = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_FabCstSht,oAriaApplication.GetHeaderText("LANG_ITMCSREP_FabCstSht",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]

        OTHERWISE
          *N000682,1 11/20/2012 MMT Globlization changes[Start]
*lcRptName = LANG_ITMCSREP_StyCstSht
lcRptName = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_StyCstSht,oAriaApplication.GetHeaderText("LANG_ITMCSREP_StyCstSht",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
loOGScroll.laCRParams[2,2] = IIF(oAriaApplication.ActiveModuleID = 'MA',IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_FabClr,oAriaApplication.GetHeaderText("LANG_ITMCSREP_FabClr",AHEADERFILE)),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_StyClr,oAriaApplication.GetHeaderText("LANG_ITMCSREP_StyClr",AHEADERFILE)))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
loOGScroll.laCRParams[2,2] = IIF(oAriaApplication.ActiveModuleID = 'MA',IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_FabOnly,oAriaApplication.GetHeaderText("LANG_ITMCSREP_FabOnly",AHEADERFILE)),IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_StyOnly,oAriaApplication.GetHeaderText("LANG_ITMCSREP_StyOnly",AHEADERFILE)))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
SCAN
  ...
  *: E303189,1 MMT 07/03/2012 Add option to Style cost sheet report to show non-costing or not[T20120425.0002][END]
      *B610826,1 MMT 08/28/2014 Fix bug of priniting style component for all sizes in case of cost sheet per size[Start]
      IF !LIKE(STRTRAN(CITMMASK,'*','?'),STYLE_A.Style) .OR. (!EMPTY(MSIZES) .AND. ATCLINE(STYLE_A.SCALE+'~',MSIZES)=0) .OR. ;
        (!EMPTY(MSZCROSREF) .AND. ATCLINE(STYLE_A.SCALE+',',MSZCROSREF)=0)
        LOOP
      ENDIF     
      *B610826,1 MMT 08/28/2014 Fix bug of priniting style component for all sizes in case of cost sheet per size[End]
      DO CASE
...
WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_WaitSty,oAriaApplication.GetHeaderText("LANG_ITMCSREP_WaitSty",AHEADERFILE)) + cSeekStyle NOWAIT
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
lcColorTlt = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_OnlyThs,oAriaApplication.GetHeaderText("LANG_ITMCSREP_OnlyThs",AHEADERFILE)) + ALLTRIM(lcNonMajTl) + 's'
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
laFormDesc[2,1] = lcStyMajor + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Only,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Only",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
laFormDesc[1,1] = lcStyMajor + IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Only,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Only",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
*!*	                    LIKE(STRTRAN(&WORKFILE..cItmMask , "*" , "?") , STYLE_A.Style)
    LOCATE REST FOR STYLE_A.cStyMajor = &WORKFILE..cItmMajor .AND. ;
                    LIKE(STRTRAN(&WORKFILE..cItmMask , "*" , "?") , STYLE_A.Style)
    *B610826,1 MMT 08/28/2014 Fix bug of priniting style component for all sizes in case of cost sheet per size[END]                    
    ...
    *- sizes between the parent style & its child.
    FOR lnMln=1 TO MEMLINES(MSzCrosref)
      ...
      *IF STYLE.Scale=SUBSTR( MLINE(MSzCrosref,lnMln) ,1 , 3)
      IF STYLE_A.Scale=SUBSTR( MLINE(MSzCrosref,lnMln) ,1 , 3)
      *B610826,1 MMT 08/28/2014 Fix bug of priniting style component for all sizes in case of cost sheet per size[End]
...
laRpSource[1] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Active,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Active",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]

*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laRpSource[2] = LANG_ITMCSREP_Hold
laRpSource[2] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Hold,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Hold",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]

*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laRpSource[3] = LANG_ITMCSREP_Cncld
laRpSource[3] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Cncld,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Cncld",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]

*N000682,1 11/20/2012 MMT Globlization changes[Start]
*laRpSource[4] = LANG_ITMCSREP_InWork
laRpSource[4] = IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_InWork,oAriaApplication.GetHeaderText("LANG_ITMCSREP_InWork",AHEADERFILE))
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
= lfOGMover(@laRpSource,@laRpTarget,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_CstStat,oAriaApplication.GetHeaderText("LANG_ITMCSREP_CstStat",AHEADERFILE)),.T.,'')
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
IF ASCAN(laRpTarget,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Active,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Active",AHEADERFILE))) > 0
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
IF ASCAN(laRpTarget,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Hold,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Hold",AHEADERFILE))) > 0
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
IF ASCAN(laRpTarget,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_Cncld,oAriaApplication.GetHeaderText("LANG_ITMCSREP_Cncld",AHEADERFILE))) > 0
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
IF ASCAN(laRpTarget,IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_InWork,oAriaApplication.GetHeaderText("LANG_ITMCSREP_InWork",AHEADERFILE))) > 0
*N000682,1 11/20/2012 MMT Globlization changes[End]
...
WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",LANG_ITMCSREP_WaitFab,oAriaApplication.GetHeaderText("LANG_ITMCSREP_WaitFab",AHEADERFILE)) + Fabric NOWAIT
*N000682,1 11/20/2012 MMT Globlization changes[End]
