...
*!B611176,1 MMT 08/03/2016 Sales Rep. Booking report does not export customer name to EXCEL[P20160601.0001 - Issue#24]
*:************************************************************************
#Include r:\aria4xp\reports\SR\srbokng.h
lcTime = TIME()
...
*WAIT WINDOW Lang_SELECTED + ALLTRIM(STR(RECCOUNT(lcOrdhTmp))) +  Lang_RECORDS_FOR_REPORT TIMEOUT 1
WAIT WINDOW IIF(oAriaApplication.oActivelang.cLang_ID = "EN",Lang_SELECTED,oAriaApplication.GetHeaderText("Lang_SELECTED",AHEADERFILE)) + ALLTRIM(STR(RECCOUNT(lcOrdhTmp))) +  IIF(oAriaApplication.oActivelang.cLang_ID = "EN",Lang_RECORDS_FOR_REPORT,oAriaApplication.GetHeaderText("Lang_RECORDS_FOR_REPORT",AHEADERFILE)) TIMEOUT 1
...
ENDIF
*B608169,1 TMI [End  ]
*B611176,1 MMT 08/03/2016 Sales Rep. Booking report does not export customer name to EXCEL[P20160601.0001 - Issue#24][Start]
IF oAriaApplication.gcDevice = "FILE" .AND. loOGScroll.cTextRepType = "EXCEL"
  SELECT &lcOrdhTmp
  LOCATE
  SCAN
    REPLACE CSTNAME WITH Customer.StName
  ENDSCAN
  LOCATE
ENDIF
*B611176,1 MMT 08/03/2016 Sales Rep. Booking report does not export customer name to EXCEL[P20160601.0001 - Issue#24][End]
...
*B611176,1 MMT 08/03/2016 Sales Rep. Booking report does not export customer name to EXCEL[P20160601.0001 - Issue#24][Start]
SELECT (lcOrdhTmp)
ALTER Table (lcOrdhTmp) ADD COLUMN CSTNAME C(30)
*B611176,1 MMT 08/03/2016 Sales Rep. Booking report does not export customer name to EXCEL[P20160601.0001 - Issue#24][End]
