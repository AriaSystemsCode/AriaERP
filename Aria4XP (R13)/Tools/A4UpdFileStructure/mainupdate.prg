...
ELSE
  MESSAGEBOX("Install log does not exist under Aria4XP installation folder")    
ENDIF
...
*!*	    MESSAGEBOX("Invalid Aria4XP SQL Dictionary")
*!*	    RETURN .F.
*!*	  ENDIF
*!*	ENDIF  
...
*!*	    MESSAGEBOX("Invalid Aria4XP Sysfiles Directory")
*!*	    RETURN .F.
*!*	  ENDIF
*!*	ENDIF  
...
lcAria4xpSqlDicPath = UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+'SYSFILES\'
...
*MT
IF FILE(UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"InstallLog.xml")
  use (lcAria4xpSqlDicPath+'SYCCOMP.DBF') in 0 Shared 
  Sele SYCCOMP
  SET ORDER TO CCOMP_ID   && CCOMP_ID 
  *LOCATE 
  IF !SEEK("99")
    APPEND BLANK 
    REPLACE ccomp_id WITH  '99' ,;
           ccom_name WITH 'Demo company                  ',;
           ccurrcode WITH 'USD',;
           ccom_ddir WITH  UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"DBFS\99\",;
           ccurr_yer WITH ALLTRIM(STR(YEAR(DATE()))),;
           ccurr_prd WITH "01",;
           mcomp_mdl WITH "AP|AR|AS|EB|GL|IC|MF|PO|PS|RM|SO|SR|UP|AL|MA|SP|HR|PW",;
           ccont_code WITH 'USA',;
           mmodlSet WITH "|AL|AP|AR|AS|EB|GL|IC|MF|PO|PS|RM|SO|SR|UP|MA",;
           ccondbname WITH lcDB99,;
...
*Merge Start
LOCAL lcSQLDICPATH
lcSQLDICPATH = lcSharedFolderSqlDict&& In all cases you are multi-inst
IF DIRECTORY(lcSharedFolderSqlDict)
...
*********************-- BADRAN 2011/04/07 Move ARIA4XP.txt under the SQLDictionary ************ END
  ...
  *MT
  ...
  FOR I = 1 TO LNSQLDFILESCOUNT
    ...
    IF FILE(lcSQLDICPATH + SYSFILE) AND FILE(lcShared27Sys + SYSFILE)
        USE  (lcShared27Sys  + SYSFILE) ALIAS SYSTABLE IN 0 EXCLUSIVE
        USE  (lcSQLDICPATH+ SYSFILE) ALIAS CUSTTABLE IN 0 EXCLUSIVE        
        SELECT SYSTABLE
*!*	        DELETE FROM SYSTABLE WHERE CUPGRDLVL = 'A' .OR. CUPGRDLVL = 'S'
*!*	        PACK  IN SYSTABLE
        SELECT CUSTTABLE
        LOCATE 
        SCAN FOR CUPGRDLVL = 'U' AND !DELETED()
          SCATTER MEMO MEMVAR 
          m.CVER = 'A40' 
          INSERT INTO SYSTABLE FROM MEMVAR 
        ENDSCAN   
      USE IN SYSTABLE 
      use IN CUSTTABLE
    ENDIF     
  ENDFOR
IF !DIRECTORY(UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"R12_SQLDICTIONARY\")
  MD (UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"R12_SQLDICTIONARY\")
ENDIF
IF DIRECTORY((UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"SYSFILES\"))
  COPY FILE (lcSQLDICPATH +'*.BIN') TO (UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"SYSFILES\*.*")
ENDIF  
COPY FILE (lcSQLDICPATH +'*.*') TO (UPPER(ADDBS(ALLTRIM(FULLPATH(''))))+"R12_SQLDICTIONARY\"+'*.*')
TRY 
ERASE (lcSQLDICPATH +'*.*')
RD (lcSQLDICPATH)
CATCH
...
SELECT SYDFIELD
SET FILTER TO CVER = 'A40' OR EMPTY(CVER)
 ...
 DO lpupdatetablestructure IN modistruc.prg WITH lcAria4xpSqlDicPath, .F., 'SYDFILES', 'SYDFLFLD', 'SYDFIELD', 'SYDINDEX', 'UpdFiles'
