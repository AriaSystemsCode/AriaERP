...
namespace Aria5SystemAdmin.Module
{
    ...
    public class CustomEditorPropertySettings : ASPxPropertyEditor
        
    {
        ...
        public CustomEditorPropertySettings(Type objectType, IModelMemberViewItem info)
        ...
        protected override WebControl CreateEditModeControlCore()
        {
            ...
            try
            {
                ...
                { 
                    ColumnType = ((AriaObjectPropertySetting)this.CurrentObject).DataType.ToString().ToUpper().TrimEnd();
                    PropertyValueOver = this.PropertyValue.ToString();

                    width = int.Parse(((AriaObjectPropertySetting)this.CurrentObject).Width.ToString());
                    decimalPlaces = int.Parse(((AriaObjectPropertySetting)this.CurrentObject).DecimalPlaces.ToString());

                    if (((AriaObjectPropertySetting)this.CurrentObject).Validvalues != null)
                    { ValidValues = ((AriaObjectPropertySetting)this.CurrentObject).Validvalues.ToString().Split(','); };

                    if (((AriaObjectPropertySetting)this.CurrentObject).Format != null)
                    { Format = ((AriaObjectPropertySetting)this.CurrentObject).Format.ToString().TrimEnd(); };
                }
