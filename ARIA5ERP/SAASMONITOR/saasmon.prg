...
lcLogFolder = 'D:\Shared\SAAS Monitoring Log\'
IF !DIRECTORY(lcLogFolder)
  MKDIR (lcLogFolder)
ENDIF
lcDef = lcLogFolder
IF FILE(lcDef +'Log.DBF')
  ERASE (lcDef +'Log.DBF')  
ENDIF
IF !FILE(lcDef +'SAAS_Monitoring_Running_Log.txt')
  =STRTOFILE('SAAS Monitoring Running Log'+CHR(13)+CHR(10),lcDef +'SAAS_Monitoring_Running_Log.txt',1)       
ENDIF
...
*!*	   	   CATCH
*!*	         INSERT INTO LOG(Client_ID ,Company_ID ,File,issue ) VALUES (lcClient,"",lcMapPath,' Could not be mapped:'+MESSAGE())
*!*		   ENDTRY 
    ELSE
    *HMS
...
ENDIF
lcFileName = 'SAAS_Monitoring_Tool_'+DTOS(DATE())+'_'+ALLTRIM(STR(HOUR(DATETIME())))+'_'+ALLTRIM(STR(MINUTE(DATETIME())))+".XLS"
   SELECT LOg
   IF RECCOUNT()>0
     COPY TO (lcDef+lcFileName) TYPE XLS
   ENDIF
   SET DATE LONG 
 =STRTOFILE('The SAAS Monitoring Tool executed on '+TTOC(DATETIME())+' and '+;
 IIF(FILE(lcDef +lcFileName),"found problems","found no problems")+CHR(13)+CHR(10),lcDef +'SAAS_Monitoring_Running_Log.txt',1)       
   
   
IF FILE(lcDef +lcFileName)
  Do SYMAILER With 'Aria.Apps.Notifications@ariany.com','Aria@notifications',lcDef +lcFileName,;
  'Hesham.e@ariasystems.biz,heba.m@ariany.com,hytham.m@ariany.com,it@ariasystems.biz,Eslam.m@Ariany.com,Distribution@Ariasystems.biz','SAAS Monitoring Tool Result',"Hi All,"+CHR(13)+CHR(10)+" Please find attached the result of monitoring SAAS" 
*ELSE
...
SCAN FOR !DELETED()
  ...
  lcDataDir = STRTRAN(UPPER(lcDataDir) ,'X:\',ADDBS(lcMapPath))
  *lcMapPath 
