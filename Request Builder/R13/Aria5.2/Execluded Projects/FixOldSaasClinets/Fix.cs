       ...
       AriaLogManage LogM = new AriaLogManage();
        ...
        private void cmdfixxml_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= dt.Rows.Count ; i++)
            {
                ...
                if (ClientXML(ClientSharedFolder + @"\ARIA4XP\SysFiles\Client_Setting.XML") == true )
                {
                    System.IO.File.Delete(ClientSharedFolder + @"\ARIA4XP\SysFiles\Client_Setting.XML");
                }
        ...
        {
            string ClientInsert = @"UPDATE CLIENTS SET [ARIA27SYS]=RTRIM(CDATAPATH)+'ARIA27\SYSFILES\', [ARIA40SYS]=RTRIM(CDATAPATH)+'ARIA4XP\SysFiles\', [CCONSERVER]= '" + CCONSERVER + "'" + ",[CCONDBNAME]= '" + CCONDBNAME + "'" + ",[CCONUSERID]= '" + CCONUSERID + "'" + ",[CCONPASWRD] =  '" + CCONPASWRD + "' WHERE [CCLIENTID] = '" + ClientID + "'";
            SqlConnection sqlConnection;
            if (chkwindowsauth.Checked == true)
            {
                sqlConnection = new SqlConnection(@"Data Source=" + txtsqlserver.Text.ToString() + @";Initial Catalog=SYSTEM.Master;Trusted_Connection=yes");
