            ...
            try
            {
                        ...
                        if (isEmail)
                        {
                            ...
                            for (int index = 0; index < users.Length; index++)
                            {
                                ...
                                if (isInternal)
                                {
                                    ...
                                    string commandText = "SELECT Cemail_add FROM Aria4XPSecurityUser() WHERE UPPER(Cuser_id) = UPPER('@UserId@')";
                ...
                else
                {
                    ...
                    if (ParameterName.Equals("AriaUser"))
                    {
                        ...
                        string commandText = "SELECT Cemail_add FROM Aria4XPSecurityUser() WHERE UPPER(Cusr_name) = UPPER('@UserName@')";
                        AriaDbCommand command = new AriaDbCommand(commandText, Connection, AriaDatabaseTypes.Aria50ClientSystemFiles, clientId);
                        command.Parameters.Add(new AriaDbParameter("UserName", ParameterDataPath.Trim()));
                        //SAB 04/02/2014 Fix RB to use Aria5 user mail [T20140121.0008] [End]
                    ...
                    else if (ParameterName.Equals("Request"))
                    {
                        if (ParameterDataPath.Equals("UserName"))
                        {
                            ...
                            string commandText = "SELECT cusr_name FROM Aria4XPSecurityUser() WHERE UPPER(Cuser_Id) = UPPER('@UserName@')";
                            AriaDbCommand command = new AriaDbCommand(commandText, Connection, AriaDatabaseTypes.Aria50ClientSystemFiles, clientId);
                            command.Parameters.Add(new AriaDbParameter("UserName", Connection.Context.UserName));
                            //SAB 04/02/2014 Fix RB to use Aria5 user mail [T20140121.0008] [End]
                            if (command.GetDataTable().Rows.Count == 0)
                        ...
                        else if (ParameterDataPath.Equals("UserEmail"))
                        {
                            ...
                            //AriaDbCommand command = new AriaDbCommand(commandText, Connection, AriaDatabaseTypes.Aria27SystemFiles, clientId);
                            //command.Parameters.Add(new AriaDbParameter("UserName", Connection.Context.UserName));
                            string commandText = "SELECT Cemail_add FROM Aria4XPSecurityUser() WHERE UPPER(Cuser_Id) = UPPER('@UserName@')";
                            AriaDbCommand command = new AriaDbCommand(commandText, Connection, AriaDatabaseTypes.Aria50ClientSystemFiles, clientId);
                            command.Parameters.Add(new AriaDbParameter("UserName", Connection.Context.UserName));
                            //SAB 04/02/2014 Fix RB to use Aria5 user mail [T20140121.0008] [End]
