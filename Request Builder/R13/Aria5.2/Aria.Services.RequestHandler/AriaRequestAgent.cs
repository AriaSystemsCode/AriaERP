...
namespace Aria.EnterpriseServices.RequestHandler
{
    ...
    {
        //HIA  T20140226.0012 - How to edit or adjust the current requests on RB (Nina McLemore, Inc) [Begin]
        private Dictionary<int, int> _failedRequest = new Dictionary<int, int>();
        ...
        public void GenerateScheduleRequests(string clientId)
        {
            ...
            for (int requestIndex = 0; requestIndex < requests.Count; requestIndex++)
            {
                ...
                {
                    if (_failedRequest[request.RequestNumber] >= env.FailedRequestRetryNumber)
                    {
        ...
        public void InvokeRequestMethod(object requestObject, string clientId)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_failedRequest.ContainsKey(request.RequestNumber))
                {
                    _failedRequest.Add(request.RequestNumber, _failedRequest[request.RequestNumber] + 1);
                }
