    ...
    public partial class FormAddFieldToDictionary : Form
    {
        ...
        private long GetFileObjectID()
        {
            if (comboBoxObjectName.SelectedIndex > -1)
            {
                ...
                SqlCommand revCom = new SqlCommand("Select * FROM AriaObjectRevision WHERE ObjectID IN (SELECT ObjectID FROM AriaObject WHERE ObjectType = 'Data')");
                //SAB 09-08-2013 [End]
                revCom.Connection = new SqlConnection(EnviromentVariables.Aria50SystemFilesConnectionString);
                revCom.Connection.Open();
        ...
        private void buttonAddToAria4_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (checkBoxAddAll.Checked && comboBoxFields.SelectedIndex < comboBoxFields.Items.Count-1)
                {
                    comboBoxFields.SelectedIndex += 1;
                    if (MessageBox.Show("Do you want to continue", "Aria 5.0", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        buttonAddToAria4_Click(sender, e);
                    }
                }
                else
                {
                    MessageBox.Show("Property Sucessfully added to Aria 5.0", "Aria 5.0", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    DialogResult = DialogResult.OK;
                }
                //SAB 09-08-2013 [End]
