...
namespace Aria.EnterpriseServices.RequestHandler
{
    ...
    public class AriaRequestAgent : MarshalByRefObject
    {
        ...
        public void InvokeRequestMethod(object requestObject, string clientId)
        {
            ...
            try
            {
                ...
                if (request.RecurrenceType == AriaRequestRecurrenceTypes.Immediate && RequestCaseHandled == false)
                {
                    ...
                    object[] argumentsArray = new object[request.MethodArgumentList.Count + 3];
                    argumentsArray[0] = request.RequestID.ToString();
                ...
                if (request.EventName.Trim().Length == 0 && request.EventObjectName.Length != 0 && RequestCaseHandled == false)
                {
                    ...
                    for (int index = 0; index < list.Count; index++)
                    {
                        ...
                        if (request.MethodObjectName != null && request.MethodObjectName.Trim() != "")
                        {
                            ...
                            // MAH 21/5/2104 [T20140415.0001]start, if request failed , try again
                            int numberOfTrial = 0;
                            ...
                            try
                            {
                                // MAH 21/5/2104[T20140415.0001]end
                                ...
                                // MAH 21/5/2104 [T20140415.0001]start, if request failed add to the log
                            }
                            catch (Exception ex)
                            {
                                numberOfTrial++;

                                AriaLogElement TrailNumberLog = new AriaLogElement("Trail Number", numberOfTrial.ToString());
                                AriaDatabaseLogManager.AriaAddToLog(clientId, connection, new Guid(request.RequestID), "Retry Reques", new AriaLogElement[] { TrailNumberLog });

                                if(numberOfTrial < 10) goto Retry;
            ...
            {
                EventLog.WriteEntry("Request Error", ex.Message);
                EventLog.WriteEntry("Request Error", ex.GetBaseException().Message);
        ...
        // tmi [T20140415.0001][start] create a new method to update the process id in the ariarequest table
        public void UpdateRequestProcessID(string requestId, string clientId, int procID)
        {
            ...
            if (request.Status == AriaRequestStatusTypes.Running)
            {
                ...
                connection = new AriaDbConnection("Aria", "");
                dataProvider.UpdateObject(connection, AriaDatabaseTypes.Aria50ClientSystemFiles, "AriaRequest", request,
                                            new string[] { "ProcessID" }, @"RequestId = @RequestId", clientId);
            }
        }
        // tmi [T20140415.0001][end] create a new method to update the process id in the ariarequest table
        ...
        public ArrayList GetAgentRequests(AriaDbConnection connection, string objctName, AriaConditionList conditions, string clientId, string requestID)
        {
            ...
            for (int index = 0; index < rootObjectProperties.Count; index++)
            {
                ...
                //Test This part will
                //SAB Add check on the calculated field 01-02-2014 [Start]
                //if (((AriaFieldSettings)rootObjectProperty.PropertySettings).IsPrimaryKey ||
                //    conditionsString.ToUpper().Replace(" ", "").Contains(((AriaFieldSettings)rootObjectProperty.PropertySettings).FieldName.Trim().ToUpper()))
                //{
                string fldName = ((AriaFieldSettings)rootObjectProperty.PropertySettings).FieldName.Trim().ToUpper();
                bool isCalculatedField = (fldName.IndexOf("(") > 0 || fldName.IndexOf("+") > 0 || fldName.IndexOf("-") > 0 || fldName.IndexOf("/") > 0 || fldName.IndexOf("*") > 0);

                if ((((AriaFieldSettings)rootObjectProperty.PropertySettings).IsPrimaryKey ||
                    (conditionsString.ToUpper().Replace(" ", "").Contains(fldName)) && !isCalculatedField))
                {
                    //SAB Add check on the calculated field 01-02-2014 [End]
