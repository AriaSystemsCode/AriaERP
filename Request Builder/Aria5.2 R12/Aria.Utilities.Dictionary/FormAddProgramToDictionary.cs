...
{
    public partial class FormAddProgramToDictionary : Form
    {
        public FormAddProgramToDictionary()
        {
            InitializeComponent();
        }
        ...
        private bool IsReportExist(out long objectID, out long parentObjectID)
        {
            ...
            if (comboBoxReports.SelectedIndex > -1)
            {
                SqlCommand revCom = new SqlCommand("Select *, (SELECT ParentObjectID FROM AriaObject WHERE AriaObject.ObjectID = AriaObjectRevision.ObjectID) AS ParentObjectID FROM AriaObjectRevision WHERE ObjectID IN (SELECT ObjectID FROM AriaObject WHERE ObjectType = 'OptionGrid')");
        ...
        public void AddReportToAria5()
        {
            ...
            command.Parameters.AddWithValue("@ObjectType", AriaObjectTypes.Server.ToString());
            ...
            command.CommandText = "INSERT INTO AriaObjectMethod(ObjectID, ObjectRevision, MethodName, ModificationType, BusinessObjectParameterName) Values (@ObjectID, @ObjectRevision, @MethodName, @ModificationType, @BusinessObjectParameterName)";
            command.Parameters.AddWithValue("@ObjectID", reportObjectID);
            command.Parameters.AddWithValue("@ObjectRevision", ActiveRevision);
            command.Parameters.AddWithValue("@MethodName", "Execute");
            ...
            command.CommandText = "INSERT INTO AriaObjectMethodParameter(ObjectID, ObjectRevision, MethodName, ParameterNo, ParameterName, ParameterType, ParameterSettings) Values (@ObjectID, @ObjectRevision, @MethodName, @ParameterNo, @ParameterName, @ParameterType, @ParameterSettings)";
            command.Parameters.AddWithValue("@ObjectID", reportObjectID);
            command.Parameters.AddWithValue("@ObjectRevision", ActiveRevision);
            command.Parameters.AddWithValue("@MethodName", "Execute");
