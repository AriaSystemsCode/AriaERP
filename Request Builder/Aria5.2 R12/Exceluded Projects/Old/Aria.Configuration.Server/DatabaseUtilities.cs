...
namespace Aria.Configuration.Server
{
    public class DatabaseUtilities
    {
        ...
        public static int GetTargetDatabaseVersion(string serverName, DatabaseServerLoginTypes auth, string userName, string pwd, string database)
        {
            DirectoryInfo root = new DirectoryInfo(Application.StartupPath);

            string[] dirs = Directory.GetDirectories(Path.Combine(root.Parent.FullName, "Databases"));
                ...
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    for (int col = 0; col < table.Columns.Count; col++)
                    {
                        if (table.Columns[col].ColumnName.EndsWith("$#"))
                        {
                            ...
                            keyValues.Add(table.Rows[i][table.Columns[col].ColumnName]);
        ...
        public static void UpdateDatabase(string serverName, DatabaseServerLoginTypes auth, string userName, string pwd, string database, string template, ToolStripProgressBar progress)
        {
            SqlConnection.ClearAllPools(); 
