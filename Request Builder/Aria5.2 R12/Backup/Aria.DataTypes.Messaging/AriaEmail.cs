...
namespace Aria.DataTypes.Messaging
{
    ...
    public class AriaEmail
    {
        ...
        {
            string[] array = attachment.Split(new char[] { ',', ';' });
        ...
        public bool IsValid()
        {
            ...
            {
                To = To.Trim();
                string[] emailTo = To.Split(new char[] { ',', ';'} );
            ...
            {
                Cc = Cc.Trim();
                string[] emailCc = Cc.Split(new char[] { ',', ';' });
            ...
            {
                Bcc = Bcc.Trim();
                string[] emailBcc = Bcc.Split(new char[] { ',', ';' });
        ...
        public bool IsValidMail(string mailList)
        {
            ...
            if (mailList != null && mailList.Trim() != "")
            {
                mailList = mailList.Trim();
                string[] emailAddresses = mailList.Split(new char[] { ',', ';' });
                for (int index = 0; index < emailAddresses.Length; index++)
                {
                    Match match = Regex.Match(emailAddresses[index].Trim(), pattern, RegexOptions.IgnoreCase);
                    if (match.Success)
                    {
                        continue;
                    }

                    if (emailAddresses[index].Trim().StartsWith("<") && emailAddresses[index].Trim().EndsWith(">"))
                    {
                        continue;
                    }
