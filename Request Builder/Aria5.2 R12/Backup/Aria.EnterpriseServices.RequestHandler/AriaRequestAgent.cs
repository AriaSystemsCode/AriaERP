...
namespace Aria.EnterpriseServices.RequestHandler
{
    ...
    public class AriaRequestAgent : MarshalByRefObject
    {
        ...
        {
            EventLog.WriteEntry("Get One Time Only Requests", "Before Select Query", EventLogEntryType.Information);
            ...
            //EventLog.WriteEntry("Get One Time Only Requests", "After Assigning Parameters", EventLogEntryType.Information);
            ...
            //paramsList.Add(new AriaLogElement("SelectCommand", AriaDbCommandText));
            //for (int i = 0; i < command.Parameters.Count; i++)
            //{
            //    paramsList.Add(new AriaLogElement(command.Parameters[i].ParameterName, command.Parameters[i].ParameterValue.ToString()));
            //}
            //AriaDatabaseLogManager.AriaAddToLog(clientId, connection, new Guid(""), "Get One Time Only Requests", paramsList.ToArray());
            //AriaWindowsLogManager.AriaAddToLog("Get One Time Only Requests", paramsList.ToArray().ToString(), EventLogEntryType.Information);
            //EventLog.WriteEntry("Get One Time Only Requests", AriaDbCommandText, EventLogEntryType.Information);
            //for (int i = 0; i < command.Parameters.Count; i++)
            //{
            //    EventLog.WriteEntry("Get One Time Only Requests", command.Parameters[i].ParameterValue.ToString(), EventLogEntryType.Information);
            //}
            // SAB 01/28/2013 [End]
            ...
            for (int index = 0; index < requests.Count; index++)
            {
                ...
                EventLog.WriteEntry("Get One Time Only Requests", "After Getting Similar Requests", EventLogEntryType.Information);
                ...
                else
                {
                    ...
                    {
                        EventLog.WriteEntry("Get One Time Only Requests", "Before Updating Request", EventLogEntryType.Information);
                        ...
                        EventLog.WriteEntry("Get One Time Only Requests", "Before Executing Request", EventLogEntryType.Information);
                        try
                        {
                            AriaThreadManager thread = new AriaThreadManager();
        ...
        public void InvokeRequestMethod(object requestObject, string clientId)
        {
            ...
            {
                EventLog.WriteEntry("RequestError", ex.Message, EventLogEntryType.Error);
                EventLog.WriteEntry("RequestError", ex.GetBaseException().Message, EventLogEntryType.Error);
        ...
        private string GetCondition(AriaDbCommand command, int conditionNo, string objectName, AriaCondition condition, string clientId)
        {
            ...
            if (condition.Operator == AriaConditionOperators.Between)
            {
                ...
                {
                    command.Parameters.Add(new AriaDbParameter("Param" + conditionNo.ToString().TrimEnd() + "_1", from.Value));
                    command.Parameters.Add(new AriaDbParameter("Param" + conditionNo.ToString().TrimEnd() + "_2", to.Value));
            ...
            else
            {
                ...
                {
                    command.Parameters.Add(new AriaDbParameter("Param" + conditionNo.ToString().TrimEnd(),
