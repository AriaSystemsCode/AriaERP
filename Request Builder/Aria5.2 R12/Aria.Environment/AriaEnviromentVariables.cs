...
namespace Aria.Environment
{
    public class AriaEnviromentVariables : MarshalByRefObject
    {
        ...
        //mah 20/5/2014 [T20140415.0001]
        private string _vfpConnectionStringTemplate = @"Driver={Microsoft Visual FoxPro Driver};sourcedb=<DataBaseLocation>;sourcetype=DBF;exclusive=No;Backgroundfetch=NO;collate=Machine;null=No;deleted=Yes;conntimeout=30;";
        //private string _vfpConnectionStringTemplate = @"Driver={Microsoft Visual FoxPro Driver};sourcedb=<DataBaseLocation>;sourcetype=DBF;exclusive=No;Backgroundfetch=NO;collate=Machine;null=No;deleted=Yes;conntimeout=30;BACKGROUNDFETCH=NO;";
        //mah 20/5/2014
        private string _SQLAuthenticatedConnectionStringTemplate = @"Data Source=<Server>;Initial Catalog=<DBName>;User ID=<UserID>;Password=<Password>;Trusted_Connection=no;Connection Timeout=300;";
        ...
        //_debugMode
        private DebugMode _debugMode = DebugMode.Disabled;
        ...
        public string Aria50ClientSystemFilesConnectionStringOdbc
        {
            get 
            {
                return _aria50ClientSystemFilesConnectionStringOdbc;
            }
        ...
        public string AriaSMTPUserName
        {
            get 
            {
                if (_ariaSMTPUserName == null || _ariaSMTPUserName.Trim() == "")
                {
                    return _ariaSenderMail;
                }
                else
                {
                    return _ariaSMTPUserName;
                }
            }
        ...
        public string AriaSenderMail
        {
            get 
            {
                if (_ariaSenderMail == null || _ariaSenderMail.Trim() == "")
                {
                    return _ariaSMTPUserName;
                }
                else
        ...
        private int _maxRecordsPerReport;
        public int MaxRecordsPerReport
        {
            get { return _maxRecordsPerReport; }
            set { _maxRecordsPerReport = value; }
        }

        private int _maxExecutionTimePerRequest;
        public int MaxExecutionTimePerRequest
        {
            get { return _maxExecutionTimePerRequest; }
            set { _maxExecutionTimePerRequest = value; }
        }

        private string _exceedLimitNotificationEmail;
        public string ExceedLimitNotificationEmail
        {
            get { return _exceedLimitNotificationEmail; }
            set { _exceedLimitNotificationEmail = value; }
        }

        private int _clientMaxRecordsPerReport;
        public int ClientMaxRecordsPerReport
        {
            get { return _clientMaxRecordsPerReport; }
            set { _clientMaxRecordsPerReport = value; }
        }

        private int _clientMaxExecutionTimePerRequest;
        public int ClientMaxExecutionTimePerRequest
        {
            get { return _clientMaxExecutionTimePerRequest; }
            set { _clientMaxExecutionTimePerRequest = value; }
        }

        private string _clientExceedLimitNotificationEmail;
        public string ClientExceedLimitNotificationEmail
        {
            get { return _clientExceedLimitNotificationEmail; }
            set { _clientExceedLimitNotificationEmail = value; }
        }
            ...
            for (int index = 0; index < xmlDocument.DocumentElement.ChildNodes.Count; index++)
            {
                ...
                else if (documentElement.ChildNodes[index].Name == "FileServer")
                {
                    ...
                    for (int childIndex = 0; childIndex < documentElement.ChildNodes[index].ChildNodes.Count; childIndex++)
                    {
                        ...
                        //SAB 01/21/2013 [Start]                        
                        else if (xmlNode.Name == "DebugMode")
                        {
                            _debugMode = (DebugMode)Enum.Parse(typeof(DebugMode), xmlNode.InnerText);
                        }
                        //SAB 01/21/2013 [End]
                    }
                }
                else if (documentElement.ChildNodes[index].Name == "RequestServer")
                {
                    ...
                    for (int childIndex = 0; childIndex < documentElement.ChildNodes[index].ChildNodes.Count; childIndex++)
                    {
                        ...
                        {
                            _maxRecordsPerReport = Convert.ToInt32(xmlNode.InnerText);
                        }

                        if (xmlNode.Name == "MaxExecutionTimePerRequest")
                        {
                            _maxExecutionTimePerRequest = Convert.ToInt32(xmlNode.InnerText);
                        }

                        if (xmlNode.Name == "ExceedLimitNotificationEmail")
                        {
                            _exceedLimitNotificationEmail = Convert.ToString(xmlNode.InnerText);
                        }
                        //SAB 04-03-2014 Add validation on the Execution Time [End]
                    }
                    //SAB 04-03-2014 Add validation on the Execution Time [Start]
                    // Set Default Values in case of not founding these settings in the XML file
                    _maxRecordsPerReport = (_maxRecordsPerReport == null || _maxRecordsPerReport == 0) ? 10000 : _maxRecordsPerReport;
                    _maxExecutionTimePerRequest = (_maxExecutionTimePerRequest == null || _maxExecutionTimePerRequest == 0) ? 600 : _maxExecutionTimePerRequest;
                    _exceedLimitNotificationEmail = string.IsNullOrEmpty(_exceedLimitNotificationEmail) ? "" : _exceedLimitNotificationEmail;
                    //SAB 04-03-2014 Add validation on the Execution Time [End]
                }
        ...
        private void ReadClientSettings(string clientId)
        {
            ...
            //SAB 04-03-2014 Add validation on the Execution Time [Start]
            if (File.Exists(Path.Combine(_aria40SystemFilesPath, "Client_Setting.xml")))
            {
                string xmlFileName = Path.Combine(_aria40SystemFilesPath, "Client_Setting.xml");
                ...
                for (int index = 0; index < xmlDocument.DocumentElement.ChildNodes.Count; index++)
                {
                    if (documentElement.ChildNodes[index].Name == "ClientConfig")
                    {
                        ...
                        for (int childIndex = 0; childIndex < documentElement.ChildNodes[index].ChildNodes.Count; childIndex++)
                        {
                            ...
                            {
                                _clientMaxRecordsPerReport = Convert.ToInt32(xmlNode.InnerText);
                            }

                            if (xmlNode.Name == "ClientMaxExecutionTimePerRequest")
                            {
                                _clientMaxExecutionTimePerRequest = Convert.ToInt32(xmlNode.InnerText);
                            }

                            if (xmlNode.Name == "ClientExceedLimitNotificationEmail")
                            {
                                _clientExceedLimitNotificationEmail = Convert.ToString(xmlNode.InnerText);
                            }
                        }
                    }
                }
            ...
            // Set Default Values in case of not founding these settings in the XML file
            _clientMaxRecordsPerReport = (_clientMaxRecordsPerReport == null || _clientMaxRecordsPerReport == 0) ? 0 : _clientMaxRecordsPerReport;
            _clientMaxExecutionTimePerRequest = (_clientMaxExecutionTimePerRequest == null || _clientMaxExecutionTimePerRequest == 0) ? 0 : _clientMaxExecutionTimePerRequest;
            _clientExceedLimitNotificationEmail = string.IsNullOrEmpty(_clientExceedLimitNotificationEmail) ? "" : _clientExceedLimitNotificationEmail;
            //SAB 04-03-2014 Add validation on the Execution Time [End]
        ...
        private void ReadCompanySettings(string companyId)
        {
            ...
            //OleDbConnection aria27Conn = new OleDbConnection();
            //aria27Conn.ConnectionString = "Provider=VFPOLEDB;" + "Data Source=" + Aria27SystemFilesPath + ";";
            //Tarek 20/5/2014

            // Mah 20/5/2014
            //aria27Conn.Open();
            aria27Conn.Open();
            //while (aria27Conn.State != ConnectionState.Open)
            //{
            //    _conn = aria27Conn;

            //    Thread thread = new Thread(new ThreadStart(OpenOleDbConnection));
            //    thread.Start();

            //    System.Threading.Thread.Sleep(5 * 1000);

            //    thread.Abort();
            //    if (aria27Conn.State != ConnectionState.Open)
            //    {
            //        EventLog.WriteEntry("Request", "Timeout", EventLogEntryType.Information);
            //        aria27Conn = new OleDbConnection("Provider=VFPOLEDB;" + "Data Source=" + Aria27SystemFilesPath + ";");
            //    }
            //}
            // Mah 20/5/2014
            ...
            //OleDbCommand setupCmd = new OleDbCommand("SELECT Cfld_name, mdata_def FROM Setups WHERE cFld_Name " +
            //                                       "in('M_SMTPSRVR', 'M_SMTPPORT', 'M_SMTPUSER', 'M_SMTPPSS ', " +
            //                                       "'M_SNDRNME ' ,'M_SSL     ','M_SMTPUSR2')", aria27Conn);
            //Tarek 20/5/2014
            ...
            {
                //SAB 03/07/2013 Modify Aria5 to use same email settings like Aria4 [Start]
                //_ariaSMTPUserName = setupDt.Select("cFld_Name = 'M_SMTPUSER'")[0]["mdata_def"].ToString();
                _ariaSenderMail = setupDt.Select("cFld_Name = 'M_SMTPUSER'")[0]["mdata_def"].ToString();
                //SAB 03/07/2013 Modify Aria5 to use same email settings like Aria4 [End]
            }
            ...
            if (setupDt.Select("cFld_Name = 'M_SMTPUSR2'").Length > 0)
            {
                ...
                _ariaSMTPUserName = setupDt.Select("cFld_Name = 'M_SMTPUSR2'")[0]["mdata_def"].ToString();
                //SAB 03/07/2013 Modify Aria5 to use same email settings like Aria4 [End]
            }
        ...
        public string GetAria27CompanyDataConnectionString(string companyID)
        {
            try
            {
                ...
                aria27Conn.Open();
                //EventLog.WriteEntry("Request3", "Provider=VFPOLEDB;.1;" + "Data Source=" + Aria27SystemFilesPath + ";", EventLogEntryType.Information);

                //OleDbConnection conn = new OleDbConnection("Provider=VFPOLEDB;" + "Data Source=" + Aria27SystemFilesPath + ";");
                //conn.ConnectionString = "Provider=VFPOLEDB;" + "Data Source=" + Aria27SystemFilesPath + ";";

                //EventLog.WriteEntry("Request3", conn.ConnectionString, EventLogEntryType.Information);
                ...
                //    {
                //        EventLog.WriteEntry("Request", "Timeout", EventLogEntryType.Information);
                //        conn = new OleDbConnection("Provider=VFPOLEDB;" + "Data Source=" + Aria27SystemFilesPath + ";");
                //    }
                //}
                //mah 20/5/2014
                ...
                //OleDbCommand command = new OleDbCommand("SELECT Ccom_ddir FROM Syccomp WHERE Ccomp_id = '" + companyID + "'", conn);
            ...
            {
                EventLog.WriteEntry("Request4", ex.Message, EventLogEntryType.Information);
                return null;
            }
        }
        ...
        public string GetAria04CompanyDataConnectionString(string companyID)
        {
            ...
            //OleDbCommand command = new OleDbCommand("SELECT CconServer, CconDbname, Cconuserid, Cconpaswrd " +
            //                                      "FROM Syccomp WHERE Ccomp_id = '" + companyID + "'", aria27Conn);
            //Tarek 20/5/2014
            DataTable row = new DataTable();
            row.Load(command.ExecuteReader());
        ...
        public string ResolveMappedDrive(string path)
        {
            ...
            //OleDbCommand sycinstCommand = new OleDbCommand("SELECT cinsysfdr from sycinst", aria27Conn);
            //Tarek 20/5/2014
            ...
            for (int index = 0; index < documentElement.ChildNodes.Count; index++)
            {
                ...
                if (documentElement.ChildNodes[index].Name == "RequestServer")
                {
                    ...
                    for (int childIndex = 0; childIndex < documentElement.ChildNodes[index].ChildNodes.Count; childIndex++)
                    {
                        ...
                        if (xmlNode.Name == "MaxRecordsPerReport")
                        {
                            xmlNode.InnerText = _clientMaxRecordsPerReport.ToString();
                        }

                        if (xmlNode.Name == "MaxExecutionTimePerRequest")
                        {
                            xmlNode.InnerText = _clientMaxExecutionTimePerRequest.ToString();
                        }

                        if (xmlNode.Name == "ExceedLimitNotificationEmail")
                        {
                            xmlNode.InnerText = _clientExceedLimitNotificationEmail.ToString();
                        }
                        //SAB 04-03-2014 Add validation on the Execution Time [End]
                    }
                }
        ...
        public bool IsDatabaseAdmin(string connection)
        {
            ...
            try
            {
                ...
                if (uid.Trim() == "")
                {
                    constring = @"Data Source=<Server>;Initial Catalog=<DBName>;Trusted_Connection=Yes".Replace("<Server>", serverName)
                                                          .Replace("<DBName>", "master");
                }
                else
                {
                    constring = @"Data Source=<Server>;Initial Catalog=<DBName>;User ID=<UserID>;Password=<Password>;Trusted_Connection=No".Replace("<Server>", serverName)
                                                          .Replace("<DBName>", "master")
                                                          .Replace("<UserID>", uid)
                ...
                if (uid.Trim() == "")
                {
                    com = new SqlCommand("SELECT IS_SRVROLEMEMBER('sysadmin') AS Result", con);
                }
                else
                {
                    com = new SqlCommand("SELECT IS_SRVROLEMEMBER('sysadmin', '" + uid.Trim() + "') AS Result", con);
                }

                DataTable table = new DataTable();
                table.Load(com.ExecuteReader());

                return table.Rows.Count > 0 && table.Rows[0]["Result"].ToString().Trim() == "1";
            }
            catch (Exception)
            {
                return false;
        ...
        public void CreateClientSettings(string ClientId)
        {
            ...
            if(table.Rows.Count > 0)
            {
                ...
                string xml = String.Format("<?xml version=\"1.0\" standalone=\"yes\"?>" +
                                            ...
                                            "<RemoteSrv>{1}</RemoteSrv>" +
                                            "<RemotePort>{2}</RemotePort>" + 
                                            "<ClientMaxRecordsPerReport>{3}</ClientMaxRecordsPerReport>" + 
                                            "<ClientMaxExecutionTimePerRequest>{4}</ClientMaxExecutionTimePerRequest>" +
                                            "<ClientExceedLimitNotificationEmail>{5}</ClientExceedLimitNotificationEmail>" +
                                            "</ClientConfig>" +
                                            "</DocumentElement>", ClientId.Trim(), table.Rows[0]["REQSERVER"].ToString().Trim(), "1500", "1000", "600", "");
                //SAB 04-03-2014 Add validation on the Execution Time [End]
        ...
        private bool _ariaIsSAAS = false;
        public bool AriaIsSAAS
        {
            get
            {
                return _ariaIsSAAS;
