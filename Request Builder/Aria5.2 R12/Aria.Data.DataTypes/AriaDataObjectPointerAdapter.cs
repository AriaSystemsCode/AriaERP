    ...
    public class AriaDataObjectPointerAdapter : AriaDataTypeAdapter
    {
        ...
        private void LoadRelatedData(AriaObject ariaObject, string clientId)
        {
            ...
            for (int childIndex = 0; childIndex < childrenObjects.Count; childIndex++)
            {
                try
                {
                    ...
                    EventLog.WriteEntry("Load Relared Data", sqlCommand, EventLogEntryType.Information);
                    ...
                    EventLog.WriteEntry("Load Relared loaded", ((AriaObject)childrenObjects[childIndex]).ObjectName, EventLogEntryType.Information);
                    

                    LoadedRecords[((AriaObject)childrenObjects[childIndex]).ObjectName] = dataProvider.GetDataTable(command);

                    EventLog.WriteEntry("Load Relared loaded1", ((DataTable)LoadedRecords[((AriaObject)childrenObjects[childIndex]).ObjectName]).Rows.Count.ToString(), EventLogEntryType.Information);
                ...
                {
                    EventLog.WriteEntry("Load Relared error", EX.Message, EventLogEntryType.Information);
                    EventLog.WriteEntry("Load Relared error", EX.GetBaseException().Message, EventLogEntryType.Information);
        ...
        public override object GetData(string dataPath, string clientId)
        {
            ...
            try
            {
                ...
                EventLog.WriteEntry("Get Data ", dataPath, EventLogEntryType.Information);
                EventLog.WriteEntry("Get Data ", objectName, EventLogEntryType.Information);
                EventLog.WriteEntry("Get Data ", LoadedRecords.Count.ToString(), EventLogEntryType.Information);

                foreach (var item in LoadedRecords)
                {
                    EventLog.WriteEntry("Get Data ", item.Key, EventLogEntryType.Information);
                }

                if (LoadedRecords[objectName].Rows.Count == 0) 
                {
                    EventLog.WriteEntry("Request Data Error2", "1", EventLogEntryType.Information);
                    return "";

                
                }

                EventLog.WriteEntry("Get Data ", "End", EventLogEntryType.Information);
                ...
                if (LoadedRecords.ContainsKey(objectName))
                {
                    if (isFeild)
                    {
                        ...
                        {
                            EventLog.WriteEntry("Request Data Error2", "3", EventLogEntryType.Information);
                            return GetCalculatedData(dataPath, fieldName, clientId);
                        }
                        // T20110803.0001 MAH 8/2/2011 End

                        EventLog.WriteEntry("Request Data Error2", "2" + objectName, EventLogEntryType.Information);
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (fieldName.Contains("(") ||
                                fieldName.Contains("+") || fieldName.Contains("-") || fieldName.Contains("*") || fieldName.Contains("/"))
                            {
                                EventLog.WriteEntry("Request Data Error2", "5", EventLogEntryType.Information);
                                return GetCalculatedData(dataPath, fieldName, clientId);
                            }
                            // T20110803.0001 MAH 8/2/2011 End

                            EventLog.WriteEntry("Request Data Error2", "6", EventLogEntryType.Information);
