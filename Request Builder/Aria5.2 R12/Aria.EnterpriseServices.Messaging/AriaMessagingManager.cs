...
namespace Aria.EnterpriseServices.Messaging
{
    ...
    public class AriaMessagingManager : MarshalByRefObject
    {
        ...
        private AriaRequest GetRequest(string requestId, string clientId)
        {
            ...
            EventLog.WriteEntry("Test1", "Client ID : " + clientId + "Request id : " + requestId, EventLogEntryType.Information);
        ...
        {
            EventLog.WriteEntry("Test1yyy", "start", EventLogEntryType.Information);

            //AriaParameterSubstituter parameterSubstituter = new AriaParameterSubstituter(connection, argumentList, clientId);
            EventLog.WriteEntry("step1yyy", "start", EventLogEntryType.Information);
            MailMessage mailMessage = new MailMessage();
            EventLog.WriteEntry("step2", "start", EventLogEntryType.Information);

            //T20100512.0026 Hassan 2010 05 23 [Begin]
            _enviromentVariables.ClientID = clientId;
            EventLog.WriteEntry("step3", "start", EventLogEntryType.Information);
            _enviromentVariables.ConnectionsRefresh();
            EventLog.WriteEntry("step4", "start", EventLogEntryType.Information);
            ...
            EventLog.WriteEntry("step5", "start", EventLogEntryType.Information);
            //SAB 03/07/2013 overwrite the company email settings with user email settings if exists [Start]            
            ...
            {
                EventLog.WriteEntry("step6", "start", EventLogEntryType.Information);
            ...
            {
                EventLog.WriteEntry("step7", "", EventLogEntryType.Information);
                string[] emailTo = message.To.Trim().Split(new char[] { ',', ';' });

                for (int index = 0; index < emailTo.Length; index++)
                {
                    EventLog.WriteEntry("Test8", "", EventLogEntryType.Information);
                    string mailAdd = emailTo[index]; // parameterSubstituter.GetSubstitutedText(emailTo[index], clientId).Trim();
                    if (mailAdd.Equals(""))
                        continue;
                    ...
                    {   EventLog.WriteEntry("Test5", "", EventLogEntryType.Information);
                        //SAB 05-22-2013 Send failure notification mail to Sender Mail [Start]
            ...
            EventLog.WriteEntry("Test6", "ssss", EventLogEntryType.Information);
            if (message.Cc != null && message.Cc.Length > 0)
            {
                ...
                {
                    string mailAdd = emailCc[index]; //parameterSubstituter.GetSubstitutedText(emailCc[index], clientId).Trim();
                    if (mailAdd.Equals(""))
                        continue;
                    //[ADD]Ahmed Maher -Date: 09/04/2010 -Error for Group Notify
                    if (mailAdd.Contains(",") || mailAdd.Contains(";"))
                    {
                        ...
                        for (int i = 0; i < emailCcSubstituter.Length; i++)
                        {
                            ...
                            //SAB 05-22-2013 Send failure notification mail to Sender Mail [End]
                            EventLog.WriteEntry("Test7", "", EventLogEntryType.Information);
                        }
                    }
                    else
                    {
            ...
            if (message.Bcc != null && message.Bcc.Length > 0)
            {
                ...
                {
                    string mailAdd = emailBcc[index]; // parameterSubstituter.GetSubstitutedText(emailBcc[index], clientId).Trim();
                    if (mailAdd.Equals(""))
                        continue;
                    //[ADD]Ahmed Maher -Date: 09/04/2010 -Error for Group Notify
                    if (mailAdd.Contains(",") || mailAdd.Contains(";"))
            ...
            {
                mailMessage.Subject = message.Subject; // parameterSubstituter.GetSubstitutedText(message.Subject, clientId);
            ...
            {
                mailMessage.Body = message.Body; // parameterSubstituter.GetSubstitutedText(message.Body, clientId).Replace("\n", "<br>");
            ...
            foreach (DictionaryEntry entry in message.Attachment)
            {
                ...
                //mailMessage.Attachments.Add(new Attachment(entry.Key.ToString()));
        ...
        public void SendEmail(AriaDbConnection connection, AriaArgumentList argumentList, AriaEmail mailMessage, string clientId)
        {
            ...
            EventLog.WriteEntry("Derby1", "", EventLogEntryType.Information);
            



            log.Clear();
            EventLog.WriteEntry("Test10", "", EventLogEntryType.Information);
            SmtpClient smtp = new SmtpClient();
            EventLog.WriteEntry("Test11", "", EventLogEntryType.Information);
            //T20100512.0026 Hassan 2010 05 23 [Begin]
            _enviromentVariables.ClientID = clientId;
            ...
            AriaRequest request = GetRequest(((AriaRequestContext)connection.Context).RequestId, clientId);
            DataTable rowUser = GetUserSettings(request.UserName);
            EventLog.WriteEntry("Test777", "user :"+_enviromentVariables.AriaSMTPUserName+ "   Password  :" +_enviromentVariables.AriaSMTPPassword , EventLogEntryType.Information);
            EventLog.WriteEntry("Test13", "SMTPUSER: " + rowUser.Rows[0]["cSMTPUser"].ToString() + "  Password:" + rowUser.Rows[0]["cEmail_Pas"].ToString(), EventLogEntryType.Information);
            if (!string.IsNullOrEmpty(rowUser.Rows[0]["cSMTPUser"].ToString().Trim()) && !string.IsNullOrEmpty(rowUser.Rows[0]["cEmail_Pas"].ToString().Trim()) &&
                !string.IsNullOrEmpty(rowUser.Rows[0]["cEmail_Add"].ToString().Trim()) && !string.IsNullOrEmpty(rowUser.Rows[0]["cUsr_Name"].ToString().Trim()))
            {
            ...
            //try
            {
                ...
                {
                    EventLog.WriteEntry("Test1", "convertedMessage", EventLogEntryType.Information);
                    ...
                    {
                        EventLog.WriteEntry("Test1final", "convertedMessage1", EventLogEntryType.Information);
                        ...
                        if (lnToMails + lnCCMails + lnBCCMails > 0)
                        {
                            ...
                            EventLog.WriteEntry("Test1final", "convertedMessage2", EventLogEntryType.Information);
                            smtp.Send(failurMessage);
                            EventLog.WriteEntry("Test1final", "convertedMessage3", EventLogEntryType.Information);
                        }
                        //SAB 05-22-2013 Send failure notification mail to Sender Mail [End]

                        //SAB 05-28-2013 Send failure notification mail to Sender Mail [Start]
                        //smtp.Send(convertedMessage);

                        try
                        {
                            EventLog.WriteEntry("Test1xxxx", "before send", EventLogEntryType.Information);
                            smtp.Send(convertedMessage);
                            EventLog.WriteEntry("Test1xxxx", "after send", EventLogEntryType.Information);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            EventLog.WriteEntry("Test1", "ERROr :  " + ex.ToString(), EventLogEntryType.Information);
                            MailMessage failurMessage = new MailMessage();
                            failurMessage.Subject = convertedMessage.Subject + "_Faild to Send Emails";
                            failurMessage.SubjectEncoding = Encoding.UTF8;
                            failurMessage.Body = ex.Message + "\n"; ;
                            failurMessage.Body += ex.InnerException;
                            failurMessage.BodyEncoding = Encoding.UTF8;
                            ...
                            EventLog.WriteEntry("Test1finalFail", "convertedMessage1", EventLogEntryType.Information);
                            smtp.Send(failurMessage);
                            EventLog.WriteEntry("Test1finalFail", "convertedMessage2", EventLogEntryType.Information);

                        }
                        //SAB 05-28-2013 Send failure notification mail to Sender Mail [End]
                    ...
                    {
                        EventLog.WriteEntry("Test No To, CC, Bcc", "No tooo", EventLogEntryType.Information);
                    }
                    //SAB 02-03-2014 Fix attachmets locking issue[Start]
                }
                //SAB 02-03-2014 Fix attachmets locking issue[End]
        ...
        {
            EventLog.WriteEntry("Derby2", "", EventLogEntryType.Information);
            //SAB 02-03-2014 Fix attachmets locking issue[Start]
            //MailMessage mailMessage = new MailMessage();
        ...
        public void Aria4SendEmail(string To, string Cc, string Bcc, string Attach)
        {
            ...
            {
                EventLog.WriteEntry("Test1", "exeption", EventLogEntryType.Information);
                _error = true;
        ...
        {
            EventLog.WriteEntry("Derby3", "", EventLogEntryType.Information);
