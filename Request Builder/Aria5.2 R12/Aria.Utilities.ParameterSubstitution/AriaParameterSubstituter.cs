...
namespace Aria.Utilities.ParameterSubstitution
{
    ...
    public class AriaParameterSubstituter : MarshalByRefObject
    {
        ...
        {
            //sara
            EventLog.WriteEntry("AriaParameterSubstituterclientId", clientId, EventLogEntryType.Information);
            //sara
        ...
        {
            EventLog.WriteEntry("AriaParameterSubstituter", "start", EventLogEntryType.Information);
        }
        public string ClientID { get; set; }
        public void BuildArgumentsDataProvider1()
        {
            EventLog.WriteEntry("BuildArgumentsDataProvider1", "BuildArgumentsDataProvider1", EventLogEntryType.Information);
            BuildArgumentsDataProvider(Connection, ArgumentList, ClientID);
        }

        public void BuildArgumentsDataProvider()
        {
            EventLog.WriteEntry("BuildArgumentsDataProvider", "BuildArgumentsDataProvider", EventLogEntryType.Information);
            BuildArgumentsDataProvider(Connection, ArgumentList, ClientID);
        }
        ...
        private void BuildArgumentsDataProvider(AriaDbConnection connection, AriaArgumentList sourceArguments, string  clientId)
        {
            ...
            EventLog.WriteEntry("BuildArgumentsDataProvider", "sourceArguments", EventLogEntryType.Information);
            ...
            {
                EventLog.WriteEntry("BuildArgumentsDataProviderError1", Ex.Message, EventLogEntryType.Information);
                EventLog.WriteEntry("BuildArgumentsDataProvider Error1", Ex.GetBaseException().Message, EventLogEntryType.Information);
                //SAB 04-09-2013 Remove Write to Event Log Test Code [Start]
        ...
        private object GetValue(string paramterizeValue,string clientId)
        {
            ...
            EventLog.WriteEntry("GetValue", paramterizeValue, EventLogEntryType.Information);
            //Trace
            AriaLogManage M = new AriaLogManage();
            try
            {
                if (paramterizeValue.IndexOf(':') < 0)
                {
                    if (_isData)
                    {
                        ...
                        //sara
                        EventLog.WriteEntry("Full Path", fullPath, EventLogEntryType.Information);
                        //sara
                        ...
                        //sara
                        EventLog.WriteEntry("propertyName", propertyName, EventLogEntryType.Information);
                        //sara
                        ...
                        EventLog.WriteEntry("beforeisemail", paramterizeValue, EventLogEntryType.Information);
                        if (isEmail)
                        {
                            string result = "";
                            //SAB 01-16-2014 Add the ability to use multiple separator for emails [Start]
                            ...
                            EventLog.WriteEntry("2Afterusersarray", paramterizeValue, EventLogEntryType.Information);
                            //SARA
                            if (users!= null)
                            {
                                for (int x = 0; x < users.Length; x++)
                                {
                                    EventLog.WriteEntry("3ALLusersarray", users[x].ToString(), EventLogEntryType.Information);
                                }
                            }
                            //SAB 01-16-2014 Add the ability to use multiple separator for emails [End]
                            ...
                            for (int index = 0; index < users.Length; index++)
                            {
                                ...
                                if (isInternal)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        //sara
                                        EventLog.WriteEntry("EmailParameter", Parameters, EventLogEntryType.Information);
                                        //sara
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                EventLog.WriteEntry("Mahmoud1", paramterizeValue, EventLogEntryType.Information);
                                object returnedResult = _argumentsDataProvider.GetData(paramterizeValue, clientId);
                                EventLog.WriteEntry("Mahmoud2", returnedResult.ToString(), EventLogEntryType.Information);
                                //SAB 01-20-2014 Remove the time part from the Date fields [Start]
                ...
                else
                {
                    ...
                    //sara
                    EventLog.WriteEntry("ParameterDataPath", ParameterDataPath, EventLogEntryType.Information);
                    //sara
            ...
            catch (Exception Ex)
            {
                ...
                EventLog.WriteEntry("GetValueError3", Ex.Message, EventLogEntryType.Information);
                EventLog.WriteEntry("GetValue Error3", Ex.GetBaseException().Message, EventLogEntryType.Information);
        ...
        {
            EventLog.WriteEntry("GetSubstitutedText", "GetSubstitutedText");
            ...
            for (int tagIndex = 0; tagIndex < tags.Length; tagIndex++)
            {
                ...
                {
                    EventLog.WriteEntry("GetSubstitutedText Error2", ex.Message, EventLogEntryType.Information);
                    EventLog.WriteEntry("GetSubstitutedText Error2", ex.GetBaseException().Message, EventLogEntryType.Information);
                }
            }
        ...
        {
            EventLog.WriteEntry("DeepSubstitute", "ariaDataType", EventLogEntryType.Information);
        ...
        {
            EventLog.WriteEntry("GetSubstitutedValue", "GetSubstitutedValue", EventLogEntryType.Information);
        ...
        public string GetTimeStamp(string  clientId)
        {
            ...
            EventLog.WriteEntry("GetTimeStamp", "GetTimeStamp", EventLogEntryType.Information);
            AriaDataProvider provider = new AriaDataProvider();
            AriaDbCommand command = new AriaDbCommand("SELECT * FROM AriaRequest WHERE RequestId = @RequestID", new AriaDbConnection("Aria", ""), AriaDatabaseTypes.Aria50ClientSystemFiles, clientId);
            command.Parameters.Add("RequestID", Connection.Context.RequestId);
            AriaRequest request = (AriaRequest)provider.GetObjectList(command, typeof(AriaRequest))[0];
