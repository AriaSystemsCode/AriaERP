...
Private Const SystemSetting = &H80000001
...
Sub Main()
  ...
  Dim frmZip As POSendByEMail.ZipProgress
  ...
  If UBound(astrArguments) < 4 Then
    MsgBox "Wrong number of parameters, cannot proceed."
  Else
    Set frmZip = New POSendByEMail.ZipProgress
    
    frmZip.ZipFile = astrArguments(3)
    astrFiles = Split(astrArguments(4), ";", -1, vbTextCompare)
    For lngCounter = LBound(astrFiles) To UBound(astrFiles)
      frmZip.Files.Add (astrFiles(lngCounter))
    Next lngCounter
    
    frmZip.Send
    
    For lngCounter = LBound(astrFiles) To UBound(astrFiles)
      Kill astrFiles(lngCounter)
    Next lngCounter
    
    Dim sendEm As POSendByEMail.SendEmail
    Set sendEm = New POSendByEMail.SendEmail
    Dim x As Variant
    
    x = sendEm.emailfile(astrArguments(0), astrArguments(1), astrArguments(3))
    Kill astrArguments(3)
  End If
End Sub
