****************************************************************************
* PROG : ALPKTKSI.PRG CONVERTED FROM 26 TO 27  (Optional program)
* DATE : 05/19/98 .
* DESC : Custom Pick Ticket form CUSTOMIZED FOR SIGNUM.
...
*-- Get the scales.
lcScTemp = gfTempName()
CREATE TABLE (gcWorkDir+lcScTemp) (Piktkt C(6),;
...
SCAN WHILE .T.
  ...
  *-- Now re-initial the used variables.
  ...
  lcPikTkt = PIKTKT
  lcOrder  = ORDER
  INSERT INTO (lcScTemp) (PIKTKT) VALUES (lcPikTkt)  
  SCAN REST WHILE PikTkt+Order+cGrupDetal+STR(LineNo,6)=lcPikTkt+lcOrder+'D'
    ...
    ENDIF
  ENDSCAN
  *-- Now the cGrupDetal is 'H' for the same piktkt.
  SELECT (lcScTemp)
  REPLACE ALL FOR PIKTKT = lcPikTkt nCount WITH lnCount
...
ELSE
  SELECT (lcTmpOrdL)
  SET RELATION TO "S"+ORDHDR.ACCOUNT+STYLE  INTO SPCK_LIN ADDI
ENDIF  
SELECT (lcTmpOrdL)
SET RELATION TO PIKTKT INTO (lcScTemp) ADDITIVE
*--Delete the header lines for be fit with FRX.
